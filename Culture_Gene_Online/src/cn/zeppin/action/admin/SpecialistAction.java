...
public class SpecialistAction extends BaseAction {
	...
	public void List() {
		...
		searchMap.put("role", Dictionary.USER_ROLE_SPECIALIST + "");
	...
	public void Load() {
		...
		if (user != null && user.getRole().getId() == Dictionary.USER_ROLE_SPECIALIST) {
			Map<String, Object> data = SerializeEntity.user2Map(user);
	...
	public void Add() {
		...
		if(name == null || name.equals("")){
			...
			if(count > 0 ){
				...
				Role role = new Role();
				role.setId(Dictionary.USER_ROLE_SPECIALIST);
	...
	public void Delete() {
		...
		if (user != null && user.getRole().getId() == Dictionary.USER_ROLE_SPECIALIST) {
			...
			result.init(FAIL_STATUS, "专家不存在！", null);
		}
		String dataType = request.getParameter("datatype");
		Utlity.ResponseWrite(result, dataType, response);
	}
