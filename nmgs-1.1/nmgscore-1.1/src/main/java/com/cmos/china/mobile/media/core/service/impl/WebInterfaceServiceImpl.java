...
public class WebInterfaceServiceImpl extends BaseServiceImpl implements IWebInterfaceService {
	 ...
	 */
	@Override
	public void categoryList(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
...
//			throw new ExceptionUtil("所属地区异常");
		}
		else{
			...
			if(prov!=null && "normal".equals(prov.getStatus())){
				...
				throw new ExceptionUtil("所属地区不存在或已停用");
			}
		}
		if(component==null||"".equals(component)){
//			throw new ExceptionUtil("接入组件异常");
		}
		else{
			...
			if(count>0){
				...
				throw new ExceptionUtil("组件不存在或已停用");
			}
		}
		...
		for(WebCategoryVO wcat : list){
			...
			Map<String, String> childParamMap = new HashMap<>();
	 ...
	 */
	@Override
	public void publishList(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
	 ...
	 */
	@Override
	public void videoInfo(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
	 ...
	 */
	@Override
	public void commodityInfo(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
	...
	@Override
	public void provinceTemplateInfo(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
			...
			if (moudleComponent != null && !"".equals(moudleComponent.getId())) {
				if (province != null && !"".equals(province.getId())) {
					if(!moudleComponent.getStatus().equals(Entity.GerneralStatusType.NORMAL) || !province.getStatus().equals(Entity.GerneralStatusType.NORMAL))
							{
								throw new ExceptionUtil("组件或地区已停用");
							}
						...
						if (!moduleList.isEmpty()) {
							...
							for (Module module : moduleList) {
								...
								for (ProvinceModuleVO pm : pmList) {
									...
									data.put("index", String.valueOf(moduleDataList.size()));
				...
				throw new ExceptionUtil("接入组件信息不存在");
			}
		}
	}

	@Override
	public void leaveMessageInfo(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
	...
	@Override
	public void addLeaveMessage(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
		String province = inputObject.getValue("province");
		...
		if (phone != null && !"".equals(phone)) {
			...
			if(ssoUser!=null && !"".equals(ssoUser)){
			...
			if (videoPublish != null && !"".equals(videoPublish)) {
				leavemessage.setVideoPublish(videoPublish);
				if (content != null && !"".equals(content)) {
					leavemessage.setContent(content);
				} else {
					throw new ExceptionUtil("发布内容异常");
				}
			} else {
				throw new ExceptionUtil("发布视频异常");
			}
		} else {
			throw new ExceptionUtil("发布人异常");
		}
	...
	@Override
	public void checkComponent(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
	...
	@Override
	public void totalPublishList(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
		String provinceId = inputObject.getValue("province");
		String componentId = inputObject.getValue("component");
		String except = inputObject.getValue("except");
		Integer pagenum = Utlity.getIntValue(inputObject.getValue("pagenum"), 1);
