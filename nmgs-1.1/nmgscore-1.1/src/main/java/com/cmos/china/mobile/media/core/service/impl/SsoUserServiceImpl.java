...
public class SsoUserServiceImpl extends BaseServiceImpl implements ISsoUserService {
	...
	public void login(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
		...
		if(ipErrorList.size() == 10){
			...
			if(userErrorMark){
				throw new ExceptionUtil("ip已被冻结");
			}
		}
		
		if(loginname==null||"".equals(loginname)){
			throw new ExceptionUtil("账号不能为空");
		}
		...
		if(ssoUser!=null && ssoUser.getFreezeTime()!=null && new Timestamp(new Date().getTime()).before(ssoUser.getFreezeTime())){
			throw new ExceptionUtil("账号已被冻结");
		}
		...
		if(password==null||"".equals(password)){
			...
			if(esbresult!=null && !"".equals(esbresult)){
				if(esbresult.getBean()!=null && !"".equals(esbresult.getBean())){
...
//		System.out.println(inputObject.toJson());
		...
		if(token==null||"".equals(token)){
			throw new ExceptionUtil("token不能为空");
		}
	...
	public void editInfomation(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
		...
		if(name==null||"".equals(name)){
			throw new ExceptionUtil("昵称不能为空");
		}
	...
	public void editPassword(InputObject inputObject, OutputObject outputObject) throws ExceptionUtil {
		...
		if(password==null||"".equals(password)){
			throw new ExceptionUtil("未填写密码");
		}
		if(newPassword==null||"".equals(newPassword)){
			throw new ExceptionUtil("新密码不能为空");
		}
		...
		MsgCode msgCode = this.getBaseDao().queryForObject("msgCode_getByPhone", user, MsgCode.class);
			if(msgCode!=null && !"".equals(msgCode)){
				if(!new Timestamp(new Date().getTime()).after(msgCode.getNextTime())){
					throw new ExceptionUtil("请等待60s后重新发送");
				}
			}
//				code = String.valueOf((int) ((Math.random()*9+1)*100000));
				...
				Map<String, String> params = new HashMap<>();
				 ...
				 params.put("infoMap", "{checkCode:"+code+"}");
					 ...
					 String result = esbresult.getRtnCode();
						if(result!=null && !"".equals(result)){
							if ("0".equals(result)) {
								...
								throw new ExceptionUtil("获取验证码错误");
							}
						}else {
								throw new ExceptionUtil("获取验证码错误");
						}
...
//				throw new Exception("ip已被冻结");
		...
		if(user==null||"".equals(user)){
			throw new ExceptionUtil("手机号不能为空");
		}
		if(code==null||"".equals(code)){
			throw new ExceptionUtil("验证码不能为空");
		}
		//验证账号是否被冻结
