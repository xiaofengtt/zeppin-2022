$(document).ready(function() {
	...
	var navarr = []; 
	var provinceName = "";
	$("#createBtn").attr("href","../views/categoryAdd.html?parent="+parent+"&component="+component+"&province="+province);
	...
	if(parent!=''){
		$('.cui-menu-left').css("display","none");
	}
		...
		$.get('../front/admin/category!execute?uid=a0006&parent='+parent,function(r) {
			if(r.status =='success') {
				for ( var i = 0, l = r.data.length; i < l; i++ ) {
				    var navhtml = ' &gt; <a href="../views/categoryList.html?province=' + province + '&parent='+ r.data[i].id +'&level='+r.data[i].level +'">'+ r.data[i].name +'</a>';
					navarr.push(navhtml);
				}
			}
		}).done(function(){
			$('#TableContainer').jtable({
				title : '<a href="../views/categoryList.html?province='+province+'">栏目管理</a> ' + provinceName + navarr.join(''),
				...
				actions : {
					listAction : '../front/admin/category!execute?uid=a0001&parent='+ parent +'&level='+level +'&component='+component +'&province='+province
				},
				fields : {
					id : {
						title : '',
					...
					addviewchild: {
	                    ...
	                    display: function (data) {
	                    	var html ='';
	                    	if(parent==''){
	                    		html = html + '<a href="../views/categoryList.html?province=' + province + '&parent=' + data.record.id+'&level='+data.record.level+'&component='+data.record.component +'" style="color:#3ebef3">管理下级栏目</a>';
	                    	}
	                    	html = html + '<a class="btn-edit" style="color:#3ebef3" data-fancybox-type="iframe" href="../views/categoryEdit.html?parent=' + parent+'&id=' + data.record.id + '">编辑</a>'
...
function changeComponent(t){
	var province = (url('?province') != null) ? url('?province') : '';
	var component = $(t).val();
	window.location.href="../views/categoryList.html?province="+province+"&component="+component;
}
