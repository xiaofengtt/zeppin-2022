...
var publish = (url('publish') != null) ? url('?publish') : '';
$(function(){		
	getList();
...
function related(e,p){
	...
	$.get('../front/web/webInterface!execute?uid=i0002&publish='+p+'&category='+e+'&pagesize=3&pagenum=&sort=',function(r){
		if(r.status=="success"){
			var str ="";
			for ( var i = 0, l = r.data.length; i < l; i++ ) {
				...
				if(index==0){
					str+="<a href='video.html?publish=" + r.data[i].publishId +"&id="+r.data[i].id+"&category="+e+"&pagetype=0'><div class='video'><div class='img'><div class='top top5'><div class='cover-img' style='background-image:url(../"+urlimg+");'></div></div></div><p>"+titles+"</p><div class='clear'></div></div></a>";	
				}else if(index==1){
					str+="<a href='video.html?publish=" + r.data[i].publishId +"&id="+r.data[i].id+"&category="+e+"&pagetype=1'><div class='video'><div class='top top1'><div class='cover-img cover-img1' style='background-image:url(../"+urlimg+");'></div></div><p>"+titles+"</p></div></a>";
				}else if(index==3){
					str+="<a href='video.html?publish=" + r.data[i].publishId +"&id="+r.data[i].id+"&category="+e+"&pagetype=3'><div class='video'><div class='top top1'><div class='cover-img cover-img1' style='background-image:url(../"+urlimg+");'></div></div><p>"+titles+"</p></div></a>";
				}else if(index==2){
					str+="<a href='video.html?publish=" + r.data[i].publishId +"&id="+r.data[i].id+"&category="+e+"&pagetype=2'><div class='video'><div class='top top3'><div class='cover-img cover-img1' style='background-image:url(../"+urlimg+");'></div></div></div></a>";
				}
			}	
	...
	for(var i=0;i<arr.length;i++){		
		if(a["time"]>=Math.round(arr[i])&&a["time"]<=Number(Math.round(arr[i])+20)){			
			if($(".videoAnimate"+i).css("display")=="none"){
				...
				$(".videoAnimate"+i).css("display","block").animate({"right":"20px","bottom":80*i+80+"px"});
			}
		}else{
			textBoxClose(i);
