...
function leftside(dataid,htmlid) {
	///CETV_TEST/navLoadRoleFunction 
	$.getJSON('../navLoadRoleFunction?name='+url('filename'), function(r) {
...
$(function(){
	...
	$(window).resize(function () {$('.sidebar,.main').height($(window).height()-110);$('.qtree-bd').height($(window).height()-203);})
	...
	$('.question_meta_bar').length && $( ".main" ).scroll(function() {
		var th = $(this),top = th.scrollTop(),wd = th.width() - 15;
		if(top > 0) {
			$('.question_meta_bar').addClass('meta_bar_fixed').css('width',wd);
		}else {
			$('.question_meta_bar').removeClass('meta_bar_fixed').removeProp('style');
		}
