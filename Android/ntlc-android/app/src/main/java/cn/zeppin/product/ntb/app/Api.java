...
/*
   1. noCache 不使用缓存，全部走网络
     ...
     * @return
     */
    public static Observable<ResultData2<String>> getTime() {
        return Api.getDefault().getTime().map(new Func1<ResultData2<String>, ResultData2<String>>() {
            @Override
    ...
    public static String getServiceTime() {
        try {
            ResultData2<String> time = Api.getDefault().getTime1().execute().body();
            if (time.success()) {
                return time.getData();
            }
        } catch (Exception e) {
