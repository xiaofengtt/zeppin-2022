		...
		Ext.onReady(function(){
			 var courseExamRoomDataStore = new Ext.data.Store({
				...
				<s:if test='#session.flag.equals("a")'>
		            url: "/test/myList.action?sql=select distinct peTchCourse.Id||'-'||peExamRoom.Id id,peTchCourse.Name||'__'||peSite.Name||'__'||peExamNo.Name||'__'||peExamRoom.Classroom name from pe_site peSite,pe_tch_course peTchCourse, pe_exam_no peExamNo, pe_exam_room peExamRoom, pr_exam_booking prExamBooking, pe_semester peSemester, pr_tch_stu_elective prTchStuElective, pr_tch_opencourse prTchOpencourse where peSemester.Flag_Active = '1' and peExamNo.Fk_Semester_Id = peSemester.Id and prExamBooking.Fk_Tch_Stu_Elective_Id = prTchStuElective.Id and prTchStuElective.Fk_Tch_Opencourse_Id = prTchOpencourse.Id and prTchOpencourse.Fk_Course_Id = peTchCourse.Id and prExamBooking.Fk_Exam_Room_Id = peExamRoom.Id and peExamRoom.Fk_Site_Id = peSite.Id and peExamRoom.Fk_Exam_No = peExamNo.Id and peTchCourse.Fk_Exam_Score_Input_Usera_Id='<s:property value="#session.user.id"/>' order by name"
		        </s:if><s:else>
		        	url: "/test/myList.action?sql=select distinct peTchCourse.Id||'-'||peExamRoom.Id id,peTchCourse.Name||'__'||peSite.Name||'__'||peExamNo.Name||'__'||peExamRoom.Classroom name from pe_site peSite,pe_tch_course peTchCourse, pe_exam_no peExamNo, pe_exam_room peExamRoom, pr_exam_booking prExamBooking, pe_semester peSemester, pr_tch_stu_elective prTchStuElective, pr_tch_opencourse prTchOpencourse where peSemester.Flag_Active = '1' and peExamNo.Fk_Semester_Id = peSemester.Id and prExamBooking.Fk_Tch_Stu_Elective_Id = prTchStuElective.Id and prTchStuElective.Fk_Tch_Opencourse_Id = prTchOpencourse.Id and prTchOpencourse.Fk_Course_Id = peTchCourse.Id and prExamBooking.Fk_Exam_Room_Id = peExamRoom.Id and peExamRoom.Fk_Site_Id = peSite.Id and peExamRoom.Fk_Exam_No = peExamNo.Id and peTchCourse.Fk_Exam_Score_Input_Userb_Id='<s:property value="#session.user.id"/>' order by name"
		        </s:else>
		        }),
