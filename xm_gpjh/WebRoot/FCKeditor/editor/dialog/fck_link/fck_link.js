...
function Ok()
{
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
		...
		// decode link first     modify by lwx 2008-10-15
		//oLink.innerHTML = sInnerHtml ;		// Set (or restore) the innerHTML
		
		tempInner = decodeURI(sInnerHtml);
		
		// only show link file name,  this showed more url in <a>   modify by lwx 2008-12-10;
		var temp = tempInner;
		var c = tempInner.lastIndexOf("/");
		if(c!=-1){
			tempInner = tempInner.substring(c+1);
			if(tempInner=="")
				tempInner = temp;
		}
				
		oLink.innerHTML = tempInner;
		//-----------------------------------------------//
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	switch ( errorNumber )
	{
		...
		case 101 :	// Custom warning
			...
			break ;
		case 201 :
			alert( '服务器存在同名的文件，新文件被更名为 "' + fileName + '"' ) ;
			break ;
		case 202 :
			alert( '无效的文件类型' ) ;
			return ;
