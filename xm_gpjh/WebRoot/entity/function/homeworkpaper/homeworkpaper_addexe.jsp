...
<%
	...
	try {
		...
		String types = request.getParameter("types");//1是作业策略，0是手工添加。
		String creatUser = user.getName();
		
		int paperId = testManage.addHomeworkPaper(title,creatUser,status,note,null,startDate,endDate,"homework",teachclass_id);
...
<script type="text/javascript">
	alert("添加成功");
	location.href = "homeworkpaper_add_bypolicy.jsp?id=0&paperId=<%=paperId%>&type1=<%=types%>";
</script>
<%
		}else {
%>
