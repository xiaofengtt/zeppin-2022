...
%>
  ...
  <body>
    <form  name="forum" action="<%=basePath%>login.whaty?action=external" method="post">
		...
		<input type="hidden" name="username" value="<%=session_teacher.getLoginId()%>">
		<input type="hidden" name="passwd" value="<%=session_teacher.getPassword()%>">
		<input type="hidden" name="applicationType" value="<%=SsoConstant.INTERACTION_PAPER_FORUM_APPLICTION_TYPE %>">
		<input type="hidden" name="applicationId" value="<%=session_teacher.getLoginId()%>">
		<input type="hidden" name="boardName" value="<%=session_teacher.getName()%>">
    </form>
  </body>
