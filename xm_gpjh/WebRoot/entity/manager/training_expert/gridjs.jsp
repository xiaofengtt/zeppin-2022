...
Ext.onReady(function(){
	...
	var typeData = new Ext.data.SimpleStore({
        ...
        data : [
        	['XLS', 'Excel11 <s:text name="test.currentpage" />', ''],['XLSALL','Excel <s:text name="test.allpage" />','']
        ]
    });
...
<s:property value="gridConfig.getExtSearchVars()"/>
   ...
   var s_formPanel =new Ext.FormPanel({
        ...
        labelWidth: 100,
        frame:true,
        title: '<s:property value="gridConfig.getTitle()"/>',
...
<s:else>
	...
	window.onresize=function(){
		...
		if(document.getElementById('searchtool').innerHTML==""){ 
			return ;//濡傛灉椤甸潰姝ｅ浜庨鎼滅储鐘舵�侊紝灏变笉鏇存柊s_formpanel
		}
