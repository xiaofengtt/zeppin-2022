...
<%
	   ...
	   dbpool db = new dbpool();
		...
		sql="select a.assess as assess, count(a.assess) as num from pe_bzz_assess a ,pe_bzz_course_feedback f "
			 +"where f.px='9' and a.fk_feedback_id=f.id and a.fk_course_id='"+opencourseId+"'"
			 +"group by a.assess";
		...
		while(rs!=null && rs.next())
		{
			...
			else if(assess.equals("偏深"))
			{
				results[1]=rs.getInt("num");
				total+=results[1];
			}
			else if(assess.equals("偏浅"))
			{
       ...
       String str ="<chart palette='4' caption='总人数："+total+"' showPercentageValues='1'>"+
                    ...
                    "<set label='偏深' value='"+results[1]+"' />"+
                    "<set label='偏浅' value='"+results[2]+"' />"+
                   "</chart>"; 
