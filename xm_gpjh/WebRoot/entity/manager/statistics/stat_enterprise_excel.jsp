	...
	if(us.getRoleId().equals("2") || us.getRoleId().equals("402880a92137be1c012137db62100006"))  //表示为一级主管和一级辅导员
	{
	 ...
	 String sql_con="";
				...
				String sql = "select a.id,a.name,a.total_num,nvl(b.completed_num,0) as completed_num,(a.total_num-nvl(b.completed_num,0)) as incompleted_num from "
							+"	(select e.id,e.name,count(s.id) as total_num from pe_bzz_batch b,pe_bzz_student s,pe_enterprise e where s.fk_batch_id=b.id and s.fk_enterprise_id=e.id and b.id='"+id+"' "+sql_con1+" group by e.id,e.name) a, "
							...
							+"		where b.id=s.fk_batch_id and b.id=o.fk_batch_id and s.fk_sso_user_id=u.id and u.id=cs.student_id and s.fk_enterprise_id=e.id and b.id='"+id+"' and cs.course_id=o.id and o.fk_course_id=c.id and cs.learn_status='COMPLETED' "+sql_con+" group by b.id, e.id,e.name,s.id,s.reg_no,s.name) "
...
<%              rs = db.executeQuery(sql);
 ...
 <tr height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td class=xl25 style='border-top:none;border-left:none'><%=total_num%></td>
  <td class=xl25 style='border-top:none;border-left:none'><%=completed_num %></td>
  <td class=xl25 style='border-top:none;border-left:none'>0</td>
  <td class=xl25 style='border-top:none;border-left:none'>0</td>
  <td class=xl25 style='border-top:none;border-left:none'><%=incompleted_num%></td>
 </tr>
 <%
 	}
 	db.close(rs);%>
