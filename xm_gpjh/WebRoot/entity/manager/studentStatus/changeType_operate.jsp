	...
	<head>
		...
		//生成专业下拉列表
			...
			var siteCombo = new Ext.form.ComboBox({
				...
				valueField: 'id' ,
				typeAhead: true,
				name: 'bean.peEdutypeByFkNewEdutypeId.name',
				id:'peEdutypeByFkNewEdutypeId_id',
				mode: 'local',
				triggerAction: 'all',
				emptyText:'请选择新层次...',
				applyTo: 'combo-box-edutype',
				editable: true,
				forceSelection: true
			});
			Ext.get("main_content").addListener("mouseover",function(){
				if(siteCombo.getValue()!=siteCombo.getRawValue()){
					document.getElementById("combo-box-edutype-id").value=siteCombo.getValue();
				}
			});
...
<script >
  ...
  return true;
	...
	<body>
		<form name="print" method="post" action="/entity/studentStatus/peChangeType_executechange.action"   onsubmit="return check();">
			<div id="main_content">
			   ...
			   <table width="100%">
			   		...
			   		<tr>
			   			...
			   			<td width="10%">
			   				<div align="center" class="form_table" >现所属层次</div>
			   			</td>
			   			<td width="20%">
			   		...
			   		<tr>
			   			...
			   			<td>
			   				<div align="center" class="postFormBox" ><u><s:property value="bean.peStudent.peEdutype.name"/></u></div>
			   		...
			   		<tr>
			   			<td colspan="1">
			   				<div align="left" class="postFormBox"><span class="name"><label>转到新层次*</label></span></div>
			   			</td>
			   			<td colspan="6">
			   				<div align="left" class="postFormBox">
			   					<input type="hidden" name="bean.peEdutypeByFkOldEdutypeId.id" value="<s:property value='bean.peEdutypeByFkOldEdutypeId.id'/>" />
			   					<input type="text" name="bean.peEdutypeByFkNewEdutypeId.name" value="<s:property value='bean.peEdutypeByFkNewEdutypeId.name'/>"  size="40"  id="combo-box-edutype" />
			   					<input type="hidden" name="bean.peEdutypeByFkNewEdutypeId.id" value="<s:property value='bean.peEdutypeByFkNewEdutypeId.id'/>" id="combo-box-edutype-id"/>
			   				</div>
			   			</td>
			   		</tr>
		              ...
		              <td valign="middle" colspan="6"><div align="left" class="postFormBox" id = "showinputDate"></div></td>
			   		...
			   		<tr>
			   			<td colspan="7">
			   				<div align="center" class="postFormBox"><input type="submit" name="Submit" value="提交"/>
			   				&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="back" onclick="window.location='/entity/studentStatus/studentStatus.action'" value="返回"/></div>
			   			</td>
			   		</tr>
			   </table>
			   </div>
