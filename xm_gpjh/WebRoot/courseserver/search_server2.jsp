...
<%!
	...
	public String getSeverFromType(String type,String path)
	{
		...
		if(type==null || type.equals(""))
		{
			Map map_cnc = getInfoByType("cnc",path);
			Map map_tel = getInfoByType("tel",path);
			if(map_cnc != null && map_tel != null){
				int cnc_value = 0;
				int tel_value = 0;
				cnc_value = Integer.parseInt(map_cnc.get("minValue").toString());
				tel_value = Integer.parseInt(map_tel.get("minValue").toString());
				if(cnc_value > tel_value){
					result = map_tel.get("result").toString();
				}else{
					result = map_cnc.get("result").toString();
				}
			}
			if(result == null || "".equals(result)){
				return "2.cnc.webtrn.cn";
			}
			return result;
		}
		else
	...
	//获取给定类型下的信息
	public Map getInfoByType(String type,String path){
		Map map = new HashMap();
		String result="";
		String filepath=path+type+".txt";
			//System.out.println("filepath:"+filepath);
...
<% 
	...
	//System.out.println("courseServer:"+courseSever);
	
	//System.out.println("filepath:"+filepath);
	if(courseSever==null || courseSever.equals(""))
	{
		courseSever="2.cnc.webtrn.cn";
	...
	//String url="mms://"+courseSever+"/whaty/zhibo/"+filepath+".wmv";
	String url="http://"+courseSever+":8080/rks/"+filepath+".wmv";
	//String url="mms://210.76.109.66/zhibo";
	//url+=filepath+".wmv";
	//String http_url=basePath1+"CourseImports/"+filepath+"/contents.files/1/video.wmv";
	//if(client_ip.equals("61.155.140.202"))
	//	out.println(http_url);
//	else{
		out.println(url);
	//}
