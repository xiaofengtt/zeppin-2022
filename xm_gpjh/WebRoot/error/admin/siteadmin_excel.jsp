...
<%	response.setHeader("Content-Disposition", "attachment;filename=siteadmin.xls"); %>
...
List manager=rightManage.getSiteManagerList(null,null,null,null,"0");
%>
...
<tr class=xl24 height=19 style='height:14.25pt'>
  	<td height=19 class=xl24 width=109 style='height:14.25pt;width:82pt'>登录ID</td>
	<td class=xl24 width=109 style='width:82pt'>管理员姓名</td>
	<td class=xl24 width=109 style='width:82pt'>管理员类型</td>
	<td class=xl24 width=109 style='width:82pt'>分站站点名称</td>
		<td class=xl24 width=109 style='width:82pt'>移动电话号码</td>
			<td class=xl24 width=109 style='width:82pt'>固定电话号码</td>
				<td class=xl24 width=109 style='width:82pt'>电子信箱</td>
...
<%
  if(manager!=null){
     for(int i=0;i<manager.size();i++){
       ...
       SiteManager mana=(SiteManager) manager.get(i); 
         mana = rightManage.getSiteAdmin(mana.getId());
   ...
   String siteName = fixnull1(mana.getSite_name());
   String mobilePhone = fixnull1(mana.getMobilePhone());
   String phone = fixnull1(mana.getRedundace().getRe2());
   String email = fixnull1(mana.getEmail());
   
  
%>
 <tr height=19 style='height:14.25pt'>
  <td height=19 style='height:14.25pt'><%=mana.getLoginId()%></td>
  <td ><%=mana.getName()%></td>
  <td ><%=type%></td>
  <td ><%=siteName%></td>
  <td ><%=mobilePhone%></td>
  <td ><%=phone%></td>
  <td ><%=email%></td>
  <td ><% if(fixnull(mana.getGroup_name()).equals("")) {out.println("<font color='red'>未指定权限组</font>");}else{out.println(fixnull(mana.getGroup_name()));};%></td>
  <td ><%=status%></td>
 </tr>
<%
	 List managerList =rightManage.getSiteManagerList(null,null,null,mana.getSite_id(),"1");
   for(int j=0;j<managerList.size();j++){
   mana = (SiteManager)managerList.get(j);
    longId= java.net.URLEncoder.encode(fixnull(mana.getLoginId()),"UTF-8");
    maname= java.net.URLEncoder.encode(fixnull(mana.getName()),"UTF-8");
    mastatus=java.net.URLEncoder.encode(fixnull(mana.getStatus()),"UTF-8");
    mobile=java.net.URLEncoder.encode(fixnull(mana.getMobilePhone()),"UTF-8");
    siteName = fixnull1(mana.getSite_name());
    mobilePhone = fixnull1(mana.getMobilePhone());
    phone = fixnull1(mana.getRedundace().getRe2());
    email = fixnull1(mana.getEmail());
   %>
   <tr height=19 style='height:14.25pt'>
  <td height=19 style='height:14.25pt'><%=mana.getLoginId()%></td>
  <td ><%=mana.getName()%></td>
