...
public class PrPriEdutypeManageAction extends MyBaseAction {

	private PrPriManagerEdutype bean;
	
	private PrPriManagerEdutypeService prPriManagerEdutypeService;
	
	public PrPriManagerEdutype getBean() {
		return bean;
	}

	public void setBean(PrPriManagerEdutype bean) {
		this.bean = bean;
	}
	...
	@Override
	public Map add() {
		this.setBean((PrPriManagerEdutype)super.setSubIds(this.getBean()));
		
		PrPriManagerEdutype instance = null;
		Map map = new HashMap();
		try {
			instance = this.getPrPriManagerEdutypeService().save(this.getBean());
	...
	public Map delete() {
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			if (str != null && str.length() > 0) {
				...
				try{
					int count = getPrPriManagerEdutypeService().deleteByIds(idList);
					if (count>0) {
						map.put("success", "true");
	...
	@Override
	public Page list() {
		DetachedCriteria dc = DetachedCriteria.forClass(PrPriManagerEdutype.class);
		dc.createCriteria("ssoUser", "ssoUser");
		dc.createCriteria("peEdutype", "peEdutype");
		
		dc = super.setDetachedCriteria(dc, this.getBean());
		return getPrPriManagerEdutypeService().getByPage(dc, Integer.parseInt(this.getLimit()), Integer.parseInt(this.getStart()));
	}

	@Override
	public Map update() {
		PrPriManagerEdutype dbInstance = this.getPrPriManagerEdutypeService().getById(this.getBean().getId());
		
		this.setBean((PrPriManagerEdutype)super.setSubIds(dbInstance, this.getBean()));
