...
public class PeBulletinAction extends MyBaseAction {
    ...
    public void initGrid() {
		...
		this.getGridConfig().addColumn(this.getText("通知标题"), "title");
...
//		this.getGridConfig().addColumn(this.getText("发布人"), "peManager.name");
		...
		this.getGridConfig().addRenderScript("发布到项目执行办", 
			...
			this.getGridConfig().addRenderScript(
					this.getText("详情内容"), 
					"{if( record.data['enumConstByFlagIsvalid.code']=='1') return '<a href=\"peBulletinView_viewDetail.action?bean.id=' +record.data['id']+ '\" target=\"_blank\">查看详细信息</a>'; else return '<a href=peBulletin_edit.action?bean.id=' +record.data['id']+ ' >修改</a>';}", "");
    ...
    public String addNotice() throws AddressException {
		...
		if(this.getSend().equals("1")){
			...
			this.setMsg("您已经成功添加通告，系统已把通告内容发至相关单位联系人的邮箱");
		}else{
			this.setMsg("您已经成功添加通告!");
		}
		
		bulletin.setScopeString(this.getPerson()+","+scopeString.toString());
		...
		if (!managers.isEmpty()) {
			bulletin.setPeManager((PeManager) managers.get(0));
		}
		bulletin.setEnumConstByFlagIstop(this.getMyListService().getEnumConstByNamespaceCode("FlagIstop", this.getBean().getEnumConstByFlagIstop().getCode()));
		bulletin.setEnumConstByFlagIsvalid(this.getMyListService().getEnumConstByNamespaceCode("FlagIsvalid", this.getBean().getEnumConstByFlagIsvalid().getCode()));
		bulletin.setTitle(this.getBean().getTitle());
		bulletin.setNote(this.getBean().getNote());
		try {
			this.getGeneralService().save(bulletin);
    ...
    public String edit() {
		try {
		    this.setBean((PeBulletin) this.getGeneralService().getById(PeBulletin.class,
			    this.getBean().getId()));
    ...
    public String editexe() throws AddressException {
	    PeBulletin peBulletin = null;
	    try {
//	    	this.getGeneralService().getGeneralDao().setEntityClass(PeBulletin.class);
	    	peBulletin = (PeBulletin)this.getGeneralService().getById(PeBulletin.class,this.getBean().getId());
		} catch (EntityException e) {
			e.printStackTrace();
		}
		StringBuffer scopeString = new StringBuffer();
		...
		if(this.getSend().equals("1")){
			...
			this.setMsg("通告修改成功，系统已把通告内容发至相关单位联系人的邮箱");
			
		}else{
			this.setMsg("通告修改成功!");
		}
		
		peBulletin.setScopeString(this.getPerson()+","+scopeString.toString());
		...
		if (!managers.isEmpty()) {
			peBulletin.setPeManager((PeManager) managers.get(0));
		}
			peBulletin.setEnumConstByFlagIstop(this.getMyListService().getEnumConstByNamespaceCode("FlagIstop", this.getBean().getEnumConstByFlagIstop().getCode()));
			peBulletin.setEnumConstByFlagIsvalid(this.getMyListService().getEnumConstByNamespaceCode("FlagIsvalid", this.getBean().getEnumConstByFlagIsvalid().getCode()));
			peBulletin.setTitle(this.getBean().getTitle());
			peBulletin.setNote(this.getBean().getNote());
			peBulletin.setUpdateDate(new Date());
		try {
			this.getGeneralService().save(peBulletin);
		    this.setTogo("/entity/information/peBulletin.action");
		} catch (EntityException e) {
		    e.printStackTrace();
		    this.setMsg("操作失败");
  ...
  //设置中西部
    ...
    public void checkBeforeDelete(List idList) throws EntityException {
		try {
			...
			if (((PeBulletin) this.getGeneralService().getById(PeBulletin.class,
