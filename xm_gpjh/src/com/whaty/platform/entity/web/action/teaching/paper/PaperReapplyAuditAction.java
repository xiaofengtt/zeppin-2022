...
public class PaperReapplyAuditAction extends SystemApplayAction {
	...
	public void initGrid() {
		this.getGridConfig()
				.addMenuFunction(this.getText("审核通过"), "reapply", "");
		this.getGridConfig().addMenuFunction(this.getText("审核不通过"),
				"rejection", "");
		
		this.getGridConfig().setCapability(false, false, false);
		this.getGridConfig().setTitle(this.getText("申请毕业论文重修列表"));
	...
	public void setServletPath() {
		this.servletPath = "/entity/teaching/paperReapplyAudit";
	}
	...
	public DetachedCriteria initDetachedCriteria() {
		...
		dc.add(Restrictions.eq("enumConstByApplyType.code", "10"));
		...
		if (this.getIds() != null && this.getIds().length() > 0){
			...
			try {
				if(this.getColumn().equals("reapply")){
					...
					map.put("info", this.getText(String.valueOf(count)+"个学生通过毕业论文重修申请。"));
				}else if(this.getColumn().equals("rejection")){
					count = this.getSystemApplyService().updateForPaperReapply(idList,false);
					map.put("success", "true");
					map.put("info", this.getText(String.valueOf(count)+"个学生驳回毕业论文重修申请。"));					
				}else{
				}
