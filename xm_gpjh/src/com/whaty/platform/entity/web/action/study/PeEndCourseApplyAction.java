...
public class PeEndCourseApplyAction extends MyBaseAction {
...
//	private String msg;//提示信息
	...
	public DetachedCriteria initDetachedCriteria() {
		...
		dc.add(Restrictions.or(Restrictions.or(Restrictions.eq("enumConstByApplyType.code","21"),Restrictions.eq("enumConstByApplyType.code","25")),Restrictions.eq("enumConstByApplyType.code","26")));//结业申请
		return dc;
	}
	...
	public void initGrid() {
		this.getGridConfig().setTitle(this.getText("结业申请审核"));
		...
		this.getGridConfig().addColumn(this.getText("ID"), "id", false);
			...
			this.getGridConfig().addColumn(this.getText("结业类型"),"enumConstByApplyType.name");
...
//		script.append("        roomformPanel.form.load({url:'"+this.getDetailAction()+"?bean.id='+ids,waitMsg:'Loading'});  ");
		...
		script.append(" 		 		        	url:'/entity/study/peEndCourseApplyAction_refuse.action?' ,");
...
//		script1.append("        roomformPanel.form.load({url:'"+this.getDetailAction()+"?bean.id='+ids,waitMsg:'Loading'});  ");
		...
		script1.append(" 		 		        	url:'/entity/study/peEndCourseApplyAction_pass.action?' ,");
	...
	public String pass(){
		...
		boolean flag=true;
			...
			if(!apply.getEnumConstByFlagApplyStatus().getCode().equals("0")){//判断该申请是否已经被允许或驳回
				...
				PeTrainee ee=apply.getPeTrainee();
				EnumConst enu1=ee.getEnumConstByStatus();
				EnumConst enu2=null;
				if(enu1!=null&&Integer.parseInt(enu1.getCode())>0&&Integer.parseInt(enu1.getCode())<4){
					enu2=this.getMyListService().getEnumConstByNamespaceCode("Status", (Integer.parseInt(enu1.getCode())+3)+"");
					ee.setEnumConstByStatus(enu2);
				}
