...
public class PeTrainingTypeApplyAction extends MyBaseAction {

//	private List applyList;//所有申请列表
//	private boolean forbidApply;//是否到达最高级别培训类型，是的话就不能再申请了
//	private String msg;//提示信息
//	private boolean noUpTrain = true;//不能申请升级培训
//	private boolean noCertificate = true;//不能申请证书
//	private boolean noCompletion = true;//不能申请结业
...
//	}
//
//	public boolean getNoCertificate() {
...
//	}
//
//	public boolean getNoCompletion() {
//		return noCompletion;
//	}
//
...
//	}
	...
	public DetachedCriteria initDetachedCriteria() {
		...
		dc.add(Restrictions.or(Restrictions.eq("enumConstByApplyType.code","20"),Restrictions.eq("enumConstByApplyType.code","27")));
		return dc;
	}
	...
	public void initGrid() {
		this.getGridConfig().setTitle(this.getText("申请更高级别的培训"));
...
//		script.append("        roomformPanel.form.load({url:'"+this.getDetailAction()+"?bean.id='+ids,waitMsg:'Loading'});  ");
		...
		script.append(" 		 		        	url:'/entity/study/peTrainingTypeApplyAction_refuse.action?' ,");
...
//		script1.append("        roomformPanel.form.load({url:'"+this.getDetailAction()+"?bean.id='+ids,waitMsg:'Loading'});  ");
		...
		script1.append(" 		 		        	url:'/entity/study/peTrainingTypeApplyAction_pass.action?' ,");
...
//		this.getGridConfig().addColumn(this.getText("层次"),"prRecPlanMajorEdutype.peEdutype.name",false,false,true,"");
	...
	public String pass(){
		...
		boolean flag=true;
			this.getGeneralService().getGeneralDao().setEntityClass(SystemApply.class);
			DetachedCriteria dcs=DetachedCriteria.forClass(SystemApply.class);
			dcs.createCriteria("peTrainee", "peTrainee");
			dcs.add(Restrictions.eq("id", this.getBean().getId()));
			SystemApply apply=null;
			try {
				List list = this.getGeneralService().getList(dcs);
				if(list != null && list.size() > 0) {
					apply=(SystemApply)list.get(0);
				}
//				apply=(SystemApply) this.getGeneralService().getById(this.getBean().getId());
		...
		if(flag){
			...
			map.put("info", "操作成功");
			this.setTraineeStatus(apply.getPeTrainee());
		}
		this.setJsonString(JsonUtil.toJSONString(map));
		return this.json();
	}
	
	private void setTraineeStatus(PeTrainee peTrainee) {
		this.getGeneralService().getGeneralDao().setEntityClass(PeTrainee.class);
		DetachedCriteria dc=DetachedCriteria.forClass(PeTrainee.class);
		dc.createCriteria("enumConstByStatus", "enumConstByStatus");
		dc.add(Restrictions.eq("id", peTrainee.getId()));
		try {
			List list = this.getGeneralService().getList(dc);
			if(list != null && list.size() > 0) {
				PeTrainee pt = (PeTrainee)list.get(0);
				String code = pt.getEnumConstByStatus().getCode();
				if("4".equals(code)) {
					EnumConst ec = this.getMyListService().getEnumConstByNamespaceCode("Status", "7");
					pt.setEnumConstByStatus(ec);
					EnumConst ec2 = this.getMyListService().getEnumConstByNamespaceCode("TrainingType", "002");
					pt.setEnumConstByTrainingType(ec2);
					this.getGeneralService().getGeneralDao().setEntityClass(PeTrainingPlan.class);
					DetachedCriteria dcp=DetachedCriteria.forClass(PeTrainingPlan.class);
					dcp.createCriteria("enumConstByTrainingType", "enumConstByTrainingType");
					dcp.createCriteria("enumConstByVersion", "enumConstByVersion");
					dcp.add(Restrictions.eq("enumConstByTrainingType.code", "002"));
					dcp.addOrder(Order.desc("enumConstByVersion.code"));
					List l = this.getGeneralService().getList(dcp);
					if(l != null && l.size() > 0) {
						PeTrainingPlan p = (PeTrainingPlan)l.get(0);
						pt.setPeTrainingPlan(p);
					}
				} else if("5".equals(code)) {
					EnumConst ec = this.getMyListService().getEnumConstByNamespaceCode("Status", "8");
					pt.setEnumConstByStatus(ec);
					EnumConst ec2 = this.getMyListService().getEnumConstByNamespaceCode("TrainingType", "003");
					pt.setEnumConstByTrainingType(ec2);
					this.getGeneralService().getGeneralDao().setEntityClass(PeTrainingPlan.class);
					DetachedCriteria dcp=DetachedCriteria.forClass(PeTrainingPlan.class);
					dcp.createCriteria("enumConstByTrainingType", "enumConstByTrainingType");
					dcp.createCriteria("enumConstByVersion", "enumConstByVersion");
					dcp.add(Restrictions.eq("enumConstByTrainingType.code", "003"));
					dcp.addOrder(Order.desc("enumConstByVersion.code"));
					List l = this.getGeneralService().getList(dcp);
					if(l != null && l.size() > 0) {
						PeTrainingPlan p = (PeTrainingPlan)l.get(0);
	...
	public String refuse(){
		...
		System.out.println(apply.getEnumConstByApplyType());
		System.out.println(apply.getEnumConstByApplyType().getCode());
		System.out.println(apply.getEnumConstByApplyType());
		if(!apply.getEnumConstByFlagApplyStatus().getCode().equals("0")){//判断该申请是否已经被允许或驳回
			flag=false;
			map.put("success", "false");
	...
	public void setServletPath() {
		this.servletPath = "/entity/study/peTrainingTypeApplyAction";
	}
...
//		dc.createCriteria("enumConstByFlagApplyStatus","enumConstByFlagApplyStatus");
//		dc.createCriteria("enumConstByApplyType","enumConstByApplyType");
//		dc.add(Restrictions.eq("enumConstByApplyType.code","20"));//类型为 培训级别申请
...
//				this.setForbidApply(true);
//				this.setMsg("您当前级别已经是最高的，不能再申请了");
//			}
//		}
//		return "newApply";
//	}
...
//		List<String> resultList=new LinkedList<String>();
//		try {
//			resultList =this.getGeneralService().canApplyHigherTrainingType(us.getSsoUser().getLoginId());//查看当前学员是否可以提交申请更高培训级别的申请
...
//		dcc.add(Restrictions.eq("code", "20"));//培训级别申请
//		List enuListt=null;
//		this.getGeneralService().getGeneralDao().setEntityClass(EnumConst.class);
//		try {
...
//		}else{
//			System.out.println("查询未申请培训级别的类别出错");
//		}
