 ...
 * 
 */
public class RecruitStatusAction extends MyBaseAction {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle(this.getText("录取状态考生人数查询统计"));
		this.getGridConfig().addColumn(this.getText("id"), "id", false);
		this.getGridConfig().addColumn(this.getText("招生批次"),
				"combobox_peRecruitplan.peRecruitplanName");
		this.getGridConfig().addColumn(this.getText("peRecruitplan.id"),
				"peRecruitplan.id", false);
		...
		if (this.getCheckBox() != null && !"".equals(this.getCheckBox())) {
			if (this.getCheckBox().contains("site")) {
				...
				this.getGridConfig().addColumn(this.getText("peSite.id"),
						"peSite.id", false);
			...
			if (this.getCheckBox().contains("edutype")) {
				...
				this.getGridConfig().addColumn(this.getText("peEdutype.id"),
						"peEdutype.id", false);
			...
			if (this.getCheckBox().contains("major")) {
				...
				this.getGridConfig().addColumn(this.getText("peMajor.id"),
						"peMajor.id", false);
			...
			if (this.getCheckBox().contains("noexam")) {
				...
				this.getGridConfig().addColumn(this.getText("id"),
						"enumConstByFlagNoexam.id", false);
				temp += "&bean.enumConstByFlagNoexam.id=\"+record.data['enumConstByFlagNoexam.id']+\"";
			}
		}

		this.getGridConfig().addColumn(this.getText("录取状态"),
				"combobox_enumConstByFlagMatriculate.matriculate");
		this.getGridConfig().addColumn(
				this.getText("enumConstByFlagMatriculate.id"),
				"enumConstByFlagMatriculate.id",false);
		temp += "&bean.enumConstByFlagMatriculate.id=\"+record.data['enumConstByFlagMatriculate.id']+\"";
	...
	public Page list() {
		...
		String sql = "    select rownum as id, one.* from ( select peRecruitplan.Name as peRecruitplanName                                  "
				+ "        ,peRecruitplan.id      as peRecruitplan_id 						"
				+ displayItems
				+ ",enumConstByFlagMatriculate.Name as matriculate							"
				+ ",enumConstByFlagMatriculate.id as matriculateId							"
				...
				+ temp_where
				+ "    group by peRecruitplan.Name ,enumConstByFlagMatriculate.Name "
				+ ", peRecruitplan.id ,enumConstByFlagMatriculate.id "
				+ temp_groupBy +" ) one";
	...
	public void setServletPath() {
		this.servletPath = "/entity/recruit/recruitStatus";
	}
