 ...
 *
 */
public class EnglishBAvoidApplyAction extends EnglishAAvoidApplyAction {
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			try {
				if (this.getColumn().equals("CheckForPass")) {
					count = this.getSystemApplyService().updateUniteExamPass(idList,"B");
				} else if (this.getColumn().equals("CancelForPass")) {
					count = this.getSystemApplyService().updateUniteExamCancelPass(idList,"B");
	...
	public void setServletPath() {
		this.servletPath="/entity/exam/englishBAvoidApply";
	}
	@Override
	public DetachedCriteria initDetachedCriteria() {
		...
		dc.createAlias("enumConstByFlagApplyStatus", "enumConstByFlagApplyStatus")
		  ...
		  .add(Restrictions.and(Restrictions.eq("enumConstByApplyType.namespace", "ApplyType"), Restrictions.eq("enumConstByApplyType.code", "5")))
