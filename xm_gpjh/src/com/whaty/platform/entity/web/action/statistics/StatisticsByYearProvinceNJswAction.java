 ...
 *
 */
public class StatisticsByYearProvinceNJswAction extends MyBaseAction {
...
//		this.getGridConfig().addColumn(this.getText("省份"),"province");
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/statistics/statisticsByYearProvinceNJswAction";
	}
	...
	public Page list() {
		...
		strb.append(" where ee.fk_training_class=cls.id and ee.fk_province=area.id and ee.flag_jsw_job=enu.id and enu.code='0' and area.fk_parent_id is null                                                                           ");
		...
		strb.append(" and ee.fk_province=area.id and ee.flag_jsw_job=enu1.id and enu1.code='0' and area.fk_parent_id is null                                                                                                             ");
		...
		strb.append(" (select 'zzz' as id,'累计' as year,'累计' as province,count(1) as pxrs from pe_trainee ee,enum_const enu,pe_area area where ee.flag_jsw_job=enu.id and enu.code='0' and ee.fk_province=area.id and area.fk_parent_id is null )a,                                  ");
		strb.append(" (select count(1) as jyrs from pe_trainee ee,enum_const enu,enum_const enu1,pe_area area where ee.status=enu.id and (enu.code='4' or enu.code='5' and enu.code='6') and ee.flag_jsw_job=enu1.id and enu1.code='0' and ee.fk_province=area.id and area.fk_parent_id is null)b ");
		strb.append(" ) order by id");
//		strb.append(" select id,year,province,pxrs,jyrs from ( ");
//		strb.append(" select a.id||a.province as id,a.year as year,a.province as province,a.pxrs as pxrs,b.jyrs as jyrs from                                      ");
//		strb.append(" (select  to_char(cls.start_date,'yyyy') as id,area.name as province, to_char(cls.start_date,'yyyy') as year,count(1) as pxrs            ");
//		strb.append(" from pe_trainee ee,pe_training_class cls ,pe_area area ,enum_const enu                                                                                  ");
//		strb.append(" where ee.fk_training_class=cls.id and ee.fk_province=area.id and ee.flag_jsw_job=enu.id and enu.code='0'                                                                           ");
//		strb.append(" group by to_char(cls.start_date,'yyyy'),area.name) a                                                                                    ");
//		strb.append(" full outer join                                                                                                                         ");
//		strb.append(" (select  to_char(cls.start_date,'yyyy') as id,area.name as province,count(1) as jyrs                                                    ");
//		strb.append(" from pe_trainee ee,enum_const enu,pe_training_class cls ,pe_area area ,enum_const enu1                                                                  ");
//		strb.append(" where ee.status=enu.id and (enu.code='4' or enu.code='5' and enu.code='6') and ee.fk_training_class=cls.id                              ");
//		strb.append(" and ee.fk_province=area.id and ee.flag_jsw_job=enu1.id and enu1.code='0'                                                                                                             ");
//		strb.append(" group by to_char(cls.start_date,'yyyy'),area.name) b                                                                                    ");
//		strb.append(" on a.id=b.id and a.province =b.province                                                                                                 ");
//		strb.append(" union                                                                                                                                   ");
//		strb.append(" select a.id as id,a.year as year,a.province as province,a.pxrs as pxrs,b.jyrs as jyrs  from                                     ");
//		strb.append(" (select 'zzz' as id,'累计' as year,'累计' as province,count(1) as pxrs from pe_trainee ee,enum_const enu where ee.flag_jsw_job=enu.id and enu.code='0' )a,                                  ");
