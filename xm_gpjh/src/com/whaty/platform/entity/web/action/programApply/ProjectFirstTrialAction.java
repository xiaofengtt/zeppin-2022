 ...
 *
 */
public class ProjectFirstTrialAction extends MyBaseAction{
...
//		this.getGridConfig().addColumn(this.getText("项目执行办审核结果"), "enumConstByFkCheckNational.name");
		...
		this.getGridConfig().addRenderFunction(this.getText("评审打分"), "<a href='/gpjh/judgment.action?expid=${value}' target='_blank'>打分</a>", "id");
		...
		script1.append(" 		 		        	url:'/entity/programApply/projectFirstTrial_distributeProject.action?' ,");
		...
		script1.append("  }");
		this.getGridConfig().addMenuScript(this.getText("项目初审"), script1.toString());
	...
	public void setServletPath() {
		this.servletPath = "/entity/programApply/projectFirstTrial";
	}
	@Override
...
//			.add(Restrictions.eq("peProApplyno.year", Const.getYear()))
			...
			.createAlias("enumConstByFkCheckFinal", "enumConstByFkCheckFinal",DetachedCriteria.LEFT_JOIN)
		  .add(Restrictions.not(Restrictions.eq("enumConstByFkCheckFirst.code", "1011")))
		.add(Restrictions.eq("enumConstByFkCheckFinal.code", "1000"));
	...
	public String distributeProject(){
		...
		if(!prProExpert.getPeProApply().getEnumConstByFkCheckFirst().getCode().equals("1010")){
			map.put("success", "false");
			map.put("info", "修改失败,该项目已经初审完毕！");
			this.setJsonString(JsonUtil.toJSONString(map));
			return this.json();
		}
		prProExpert.setResultFirst(Double.parseDouble(this.getScore()));
		prProExpert.setNoteFirst(this.getOpinion());
		try {
...
//		try {
//			String sql="update pr_pro_expert t set t.result_first=:score , t.note_first = :opinion where t.id=:ids";
