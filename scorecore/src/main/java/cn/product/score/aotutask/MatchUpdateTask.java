...
//	@Scheduled(cron="20 2 *  * * * ")
	...
	private void saveMatchData(Map<String, Object> result, Calendar starttime,Calendar endtime) throws ParseException{
		...
		if(flag && result.containsKey("result")){
			...
			List<Team> teamList = this.teamDao.getListByParams(searchParams);
			...
			List<Category> categoryList = this.categoryDao.getListByParams(searchParams);
...
//				}
			...
			List<InfoMatch> matchs = this.infoMatchDao.getListByParams(searchParams);
			...
			for(InfoMatch match : matchList){
				if(matchsMap.containsKey(match.getInterfaceid())){
					InfoMatch dbMatch = this.infoMatchDao.get(matchsMap.get(match.getInterfaceid()));
