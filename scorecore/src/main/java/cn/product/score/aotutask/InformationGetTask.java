...
import cn.product.score.entity.CategoryStanding.CategoryStandingStatus;
...
import cn.product.score.util.GetInfoUtil;
import cn.product.score.util.ImageUtils;
		...
		try {
			...
			if(flag && result.containsKey("result")){
				...
				if(listDatas != null && listDatas.size() > 0){
					//获取顶级菜单项
					Category parent = categoryDao.get(CategoryMainUuid.FOOTBALL);
					if(parent == null){
						parent = new Category();
					...
					Integer count = categoryDao.getCountByParams(params);
					int i = 0;
					for(Map<String, String> data : listDatas){
						...
						int countnew = categoryDao.getCountByParams(params);
						if(countnew > 0){
							params.clear();
							params.put("interfaceid", country_key);
							List<Category> list = categoryDao.getListByParams(params);
							cat = list.get(0);
...
//					categoryDao.insertList(listResult);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
				...
				if(listDatas != null && listDatas.size() > 0){
					...
					for(Map<String, String> data : listDatas){
						...
						if(mapCountry.containsKey(country_key)){
							...
							Map<String, Object> params = new HashMap<String, Object>();
							params.put("interfaceid", country_key);
							List<Category> parents = categoryDao.getListByParams(params);
							if(parents != null && parents.size() > 0){
								...
								params.clear();
								params.put("parent", parent.getUuid());
								int count = categoryDao.getCountByParams(params);
...
//	@Scheduled(cron="10 8 0/8  * * * ")
	...
	public void GetInforTeamTask() {
		...
		for(String uuid : listLea){
			Category cat = this.categoryDao.get(uuid);
			try {
				...
				if(flag && result.containsKey("result")){
					...
					if(listDatas != null && listDatas.size() > 0){
						...
						for(Map<String, Object> data : listDatas){
							this.teamDao.batchProcess(data, cat);
						}
					}
				}
				cat.setFlagGet(true);
...
//	@Scheduled(cron="30 11 11  * * * ")
	public void GetInforTeamIconTash() {
		...
		for(String uuid : listLea){
			...
			List<Team> listTeam = this.teamDao.getListByParams(params);
	...
	public void GetInforStandingsTask() {
		try {
			...
			for(String leaUuid: listLea){
				Category cat = this.categoryDao.get(leaUuid);
					...
					if(mapDatas != null){
						...
						if(listDatas != null && listDatas.size() > 0){
							...
							for(Map<String, Object> datas : listDatas){
								...
								List<Team> teams = this.teamDao.getListByParams(params);
							...
							this.categoryStandingDao.updateByCategory(leaUuid, season, csList);
						}
					}
				}
			}
				...
				if(flag && result.containsKey("result")){
					...
					if(listDatas != null && listDatas.size() > 0){
						...
						for(Map<String, Object> datas : listDatas){
							...
							List<InfoPlayers> players = this.infoPlayersDao.getListByParams(params);
						...
						this.categoryTopscoreDao.updateByCategory(leaUuid, ctList);
					}
				}
			}
		} catch (Exception e) {
