...
//		sql.append(" GROUP BY suti.ITEM ) aliasname");
	...
	public SsoUserTestItem getPreviousSsoUserTestItem(SsoUserTestItem ssoUserTestItem) {
		// TODO Auto-generated method stub
		StringBuilder sb = new StringBuilder();
		sb.append(" select max(suti.id) from SsoUserTestItem suti where 1=1 ");
...
//		sb.append(" order by suti.ssoUserTest.createtime desc");
		//取倒序符合条件不是自己本身的第一条记录
//		sb.append(" limit 0,1");
		Long ssoUserTestItemId = (Long) this.getResultByHQL(sb.toString());
		if (ssoUserTestItemId != null){
			return this.get(ssoUserTestItemId);
		}
