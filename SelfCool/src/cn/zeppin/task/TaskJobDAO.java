...
public class TaskJobDAO extends HibernateTemplateDAO<Object, Integer> {
	...
	public void calculateUserSubjectData() {
		...
		sql.append(" , now())");
		sql.append("where user_subject.subject in (select subjectid from subject_countdown where exam_time > now())");
	...
	public void calculateAllKnowledgeItemData() {
		...
		sql.append(" (select count(*) from knowledge a, knowledge b, item_type it , item i");
		sql.append(" left outer join (test_paper_item tpi  join paper p on tpi.test_paper = p.id and p.type <> 2) on tpi.item = i.id");
		sql.append(" where b.id = i.knowladge ");
