...
public class SubjectItemTypeAction extends BaseAction {
	...
	@ActionParam(key = "proportion", type = ValueType.NUMBER)
	public void Update() {
		...
		if (subject != null) {
			...
			for(String proportion:proportions){
				ff += Float.valueOf(proportion);
			}
			if(ff>100){
				result.init(FAIL_STATUS, "占总分比超出比例!", null);
				String dataType = request.getParameter("datatype");
				Utlity.ResponseWrite(result, dataType, response);
				return;
			}else if(ff>0 && ff != 100){
				result.init(FAIL_STATUS, "占总分比之和不足100%!", null);
				String dataType = request.getParameter("datatype");
				Utlity.ResponseWrite(result, dataType, response);
				return;
			}
			List<SubjectItemType> subjectItemTypes = this.getSubjectItemTypeService().updateSubjectItemType(subject, proportions, itemTypeList);
