...
//	@SuppressWarnings("unchecked")
	...
	public void Add() {
		...
		Integer price = this.getIntValue(itemMap.get("price").toString(), null);
		Boolean isFree = Short.valueOf(itemMap.get("isFree").toString()) == 1 ? true : false;
		...
		if (subject != null && !this.getSysUserService().isCanOpt(currentUser, subject)) {
			...
			paper.setIsFree(isFree);
			paper.setPrice(price);
			
//			this.paperService.addPaper(paper,null);//先将试卷入库
			this.paperService.addPaper(paper, itemMap);
	...
	public void Update() {
		...
		if (paper == null) {
			...
			if (subject != null && !this.getSysUserService().isCanOpt(currentUser, subject)) {
				...
				this.paperService.updatePapers(paper, itemMap);
//				if (itemMap.get("data") != null) {
//
//					List<Map<String, Object>> listMapData = (List<Map<String, Object>>) itemMap.get("data");
//					List<TestPaperSection> lstSections = new LinkedList<TestPaperSection>();
...
//			List<Item> childItems = this.getItemService().getItemsByItem(id);
	...
	public String LoadPaper() throws IOException {
		...
		if (paper == null) {
			...
			paperEx.setPrice(paper.getPrice());
			paperEx.setIsFree(Utlity.getIsFree(paper.getIsFree()));
			
			Map<String, Object> sectionsMap = new HashMap<String, Object>();
			sectionsMap.put("paper.id", id);

			List<TestPaperSection> lstSections = this.getTestPaperSectionService().searchTestPaperSections(sectionsMap, "inx", -1, -1);//查询level==1的目录
			...
			for (TestPaperSection tmpSection : lstSections) {
				...
				if(tmpSection.getItemType() != null){
				...
				paperSectionEx.setNumString(Utlity.numInx2Str(pIndex));
				paperSectionEx.setLevel(tmpSection.getLevel());
				paperSectionEx.setIsparent(false);
				pIndex++;
				sectionsMap.put("testPaperSection.id", tmpSection.getId());
				List<TestPaperSection> lstSection = this.getTestPaperSectionService().searchTestPaperSections(sectionsMap, "inx", -1, -1);//查询level == 1的目录的子目录
				if(lstSection != null && !lstSection.isEmpty()){
					paperSectionEx.setIsparent(true);
					List<PaperSectionEx> childSec = paperSectionEx.getChildSec();
					short cIndex = 1;
					for(TestPaperSection section: lstSection){
						PaperSectionEx paperSectionEx0 = new PaperSectionEx();
						paperSectionEx0.setId(section.getId());
						paperSectionEx0.setInx(section.getInx());
						paperSectionEx0.setItemTypeId(section.getItemType().getId());
						paperSectionEx0.setItemTypeName(section.getItemType().getName());
						paperSectionEx0.setModeType(section.getItemType().getModelType());
						paperSectionEx0.setName(section.getName());
						paperSectionEx0.setNumString(Utlity.numInx2Str(cIndex));
						paperSectionEx0.setLevel(section.getLevel());
						cIndex++;
						Map<String, Object> searchPaperItem = new HashMap<String, Object>();
						searchPaperItem.put("paper.id", id);
						searchPaperItem.put("testPaperSection.id", section.getId());
...
//								if (tmpSection.getItemType().getModelType() == 4) {//根据题目的modeltype和isgroup来判断是否是组合题
								...
								paperSectionEx0.getLstItem().add(item);

							}
						}
						childSec.add(paperSectionEx0);
					}
					paperSectionEx.setChildSec(childSec);
				}else{
					Map<String, Object> searchPaperItem = new HashMap<String, Object>();
					searchPaperItem.put("paper.id", id);
...
//								if (tmpSection.getItemType().getModelType() == 4) {//根据题目的modeltype和isgroup来判断是否是组合题
								if (tmpItem.getItem().getIsGroup() && tmpItem.getItem().getModelType() == 4){
									...
									sb.append("<br><dd><b>父题题号：</b>").append(tmpItem.getItem().getId());
									String pstatus = "";
									if(tmpItem.getItem().getStatus() == 1){
										pstatus = "已审核";
									}else if(tmpItem.getItem().getStatus() == 2){
										pstatus = "草稿";
									}else if(tmpItem.getItem().getStatus() == 3){
										pstatus = "已发布";
									}else {
										pstatus = "已停用（删除）";
									}
									sb.append("&nbsp&nbsp&nbsp<b>试题状态：</b>").append(pstatus).append("</dd><br>");
									sb.append("<div class=\"question-title\">").append(mapContent.get("stem")).append("</div>");
									int serNum = 1;
									for (Item itemchild : childItemList) {
										sb.append("<br><dd><b>子题题号：</b>").append(itemchild.getId());
										String status = "";
										if(itemchild.getStatus() == 1){
											status = "已审核";
										}else if(itemchild.getStatus() == 2){
											status = "草稿";
										}else if(itemchild.getStatus() == 3){
											status = "已发布";
										}else {
											status = "已停用（删除）";
										}
										sb.append("&nbsp&nbsp&nbsp<b>试题状态：</b>").append(status).append("</dd><br>");
										sb.append(ItemToHtml.itemGroup2Html(itemchild, serNum));
									...
									// 不是组合题
									StringBuilder sb = new StringBuilder();
									sb.append("<br><dd><b>题号：</b>").append(tmpItem.getItem().getId());
									String status = "";
									if(tmpItem.getItem().getStatus() == 1){
										status = "已审核";
									}else if(tmpItem.getItem().getStatus() == 2){
...
//			}
	...
	@ActionParam(key = "isFree", type = ValueType.NUMBER)
	public void List() {
		...
		searchMap.put("isFree", request.getParameter("isFree"));
