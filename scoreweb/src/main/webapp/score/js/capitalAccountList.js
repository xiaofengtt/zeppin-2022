        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				'<td>'+r.data[i].name+'</td>'+
    				'<td>'+r.data[i].accountNum+'</td>'+
    				'<td>'+r.data[i].poundageRate+'</td>'+
    				'<td>'+r.data[i].sort+'</td>'+
    				'<td>'+status+'</td>'+
    				'<td>'+formatDate(r.data[i].createtime)+'</td>'+
    				'<td data-id="'+r.data[i].uuid+'">'+
    				'<a href="capitalAccountDetail.html?uuid='+r.data[i].uuid+'">修改</a> <a class="disableBtn mr-5">禁用</a></td><td class="align-right"> <input class="check" type="checkbox" name="" value="'+r.data[i].uuid+'">'+
    				'</td></tr>'; 
    			}
