...
@Validated
@Api(value = "优惠券",description = "优惠券接口")
public class WxCouponController {
     ...
     */
    @GetMapping("list")
    @ApiOperation(value="优惠券列表", notes="优惠券列表(登入验证)")
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType="query", name="status" ,value = "0则未使用；如果是1则已使用；如果是2则已过期", required = true, dataType = "Int"),
            @ApiImplicitParam(paramType="query", name="page" ,value = "", required = true, dataType = "String"),
            @ApiImplicitParam(paramType="query", name="limit" ,value = "", required = true, dataType = "String")
    })
    @ApiOperation(value="个人优惠券列表", notes="个人优惠券列表(登入验证)")
    @AuthInterceptor(needAuthTokenVerify = true)
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType="query", name="userId" ,value = "用户id", required = true, dataType = "Int"),
            @ApiImplicitParam(paramType="query", name="cartId" ,value = "购物车id,0查全部", required = true, dataType = "Int")
    })
    @ApiOperation(value="当前购物车下单商品订单可用优惠券", notes="当前购物车下单商品订单可用优惠券(登入验证)")
    @AuthInterceptor(needAuthTokenVerify = true)
     ...
     *
     * @param userId 用户ID
     * @param couponId 请求内容， { couponId: xxx }
     * @return 操作结果
     */
    @PostMapping("receive")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType="query", name="userId" ,value = "用户id", required = true, dataType = "Int"),
            @ApiImplicitParam(paramType="query", name="couponId" ,value = "优惠卷id", required = true, dataType = "Int")
    })
    @ApiOperation(value="优惠券领取", notes="优惠券领取(登入验证)")
    @AuthInterceptor(needAuthTokenVerify = true)
    public Object receive(@LoginUser Integer userId,  Integer couponId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
...
//        Integer couponId = JacksonUtil.parseInteger(body, "couponId");
    ...
    @ApiOperation(value="优惠券领取", hidden = true)
