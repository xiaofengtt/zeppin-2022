...
import org.linlinjava.litemall.wx.web.ApyController;
...
public class WxOrderService {
    ...
    @Autowired
    private ApyController apyController;
     ...
     */
    @Transactional
    public Object submit(Integer userId, Integer cartId,Integer addressId ,Integer couponId,String message,Integer grouponRulesId,Integer grouponLinkId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
...
//        Integer grouponLinkId = JacksonUtil.parseInteger(body, "grouponLinkId");
        ...
        if (checkedAddress == null) {
            return ResponseUtil.fail(-5,"请添加收货地址");
        }
...
//                LitemallCart cart = cartService.findById(Integer.parseInt (str));
//
//                checkedGoodsList.add(cart);
//            }
//
//        } else if(cartlist.size ()==1){
//            if(cartlist.get (0).equals ("0")){
//                checkedGoodsList = cartService.queryByUidAndChecked(userId);
//            }else{
//                LitemallCart cart = cartService.findById(Integer.parseInt (cartlist.get (0)));
//                checkedGoodsList = new ArrayList<>(1);
//                checkedGoodsList.add(cart);
//            }
//        }
        ...
        order.setConsignee(checkedAddress.getName()); //收货地址
...
//        Integer orderId = orderId1.intValue();
        if (orderId == null) {
            return ResponseUtil.badArgument();
        }
     ...
     */
    @Transactional
    public Object alipay(Integer userId, Integer orderId, HttpServletRequest request,String returnUrl) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
//        Integer orderId = orderId1.intValue();
...
//                body= apyController.alipaipri(orderRequest.getOutTradeNo(), order.getOrderSn(), returnUrl, actualPrice.toString(), orderRequest.getBody());
                body= apyController.apiwaypay (orderRequest.getOutTradeNo(), order.getOrderSn(), returnUrl, actualPrice.toString(), orderRequest.getBody(),"");
            }catch (Exception e){
                return ResponseUtil.fail(ORDER_PAY_FAIL, "支付异常");
            }
...
//            formIdService.addUserFormid(userFormid);
    ...
    @Transactional
    public Object pay201(Integer userId, Integer orderId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (orderId == null) {
     ...
     */
    @Transactional
    public Object alipayNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        //获取支付宝POST过来反馈信息
...
//                notifyService.notifyWxTemplate(result.getOpenid(), NotifyType.PAY_SUCCEED, parms, "pages/index/index?orderId=" + order.getId());
     ...
     * @return 订单退款操作结果
     */
    public Object refund(Integer userId, Integer orderId,String amount,String reason) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
//        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        ...
        model.setOutRequestNo(order.getOrderSn()+order.getUserId()+order.getId());
        model.setTradeNo(order.getPayId());
...
//        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
     ...
     * @return 订单操作结果
     */
    public Object comment(Integer userId, Integer orderGoodsId,String content,Boolean hasPicture,String picUrls1,Integer star) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
...
//        Boolean hasPicture = JacksonUtil.parseBoolean(body, "hasPicture");
        List<String> picUrls =Arrays.asList(picUrls1.split(","));
