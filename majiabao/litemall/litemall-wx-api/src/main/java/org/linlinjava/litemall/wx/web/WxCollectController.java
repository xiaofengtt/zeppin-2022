...
public class WxCollectController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType="query", name="userId" ,value = "用户id", required = true, dataType = "Int"),
            @ApiImplicitParam(paramType="query", name="valueId" ,value = "收藏id", required = false, dataType = "Int")
    })
    @ApiOperation(value="订单列表", notes="订单列表(登入验证)")
    public Object addordelete(@LoginUser Integer userId,  Integer valueId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
