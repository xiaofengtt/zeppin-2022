...
//				$(".options").css("display","none");
			...
			//获取项目
			function getProject(){
				$.get('../mw_gpjh/trainee_headsearchLv.action',function(r){
					var str="";
					var strs="";
...
//	
//	
	$.post("trainee_adminTraineeNumForMW.action",{subject:subjects,parent:parents}, function(r) {
		$("#layerDiv").css("display","none");
		var trs = "<tr class='tr1'><th>省份</th><th>计划人数</th><th>参训人数</th><th>结业人数</th></tr>";
		var sum1 = 0;
		var sum2 = 0;
		var sum3 = 0;
		for(var i = 0; i < r.rows.length; i++){
			trs+="<tr><td>"+r.rows[i].cell[0]+"</td><td>"+r.rows[i].cell[1]+"</td><td>"+r.rows[i].cell[2]+"</td><td>"+r.rows[i].cell[3]+"</td></tr>";
			sum1+=Number(r.rows[i].cell[1]);
			sum2+=Number(r.rows[i].cell[2]);
			sum3+=Number(r.rows[i].cell[3]);
		}
		trs+=("<tr><td>合计</td><td>"+sum1+"</td><td>"+sum2+"</td><td>"+sum3+"</td></tr>");
		$('table').html(trs);
...
function importTrainee() {	
	$("#layerDiv").css("display","block");
	$.post("trainee_importTraineeNumForMW.action",{subject:subjects,parent:parents}, function(r) {
		$("#layerDiv").css("display","none");
