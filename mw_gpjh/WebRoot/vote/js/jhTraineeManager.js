...
$(function() {
	...
	function initGrid() {
		...
		$("#list2").jqGrid(
				{
					url : "trainee_listjh.action?userid=" + userid + "&role="
							+ role,
					datatype : 'json',
					mtype : 'POST',
					...
					colNames : [ '所属地区（省）', '所属地区（市）', '所属地区（县）', '所属项目', '子项目', '培训单位', '学科', '姓名','身份证号', '手机',
							'电子邮件', '工作单位', '学校所在区域', '学校类别', '民族', '职称', '主要任教学段', '主要任教学科', '最高学历', '毕业院校','所学专业','状态' ],
		...
		$('#list2').navButtonAdd('#pager2', {
			...
			onClickButton : function() {
				...
				$('#downloadcsv').attr('src', 'trainee_importListJH.action');
				...
				onClickButton : function() {
					if (ids.length == 0) {
						...
						if (confirm('是否确定删除项目?')) {
							...
							$.post('trainee_trainJHDelete.action', {
								'id' : pIds
							}, function(r) {
...
//		var hiredate = $('#addHiredate').val();
		...
		$.post('trainee_trainJHEdit.action', {
