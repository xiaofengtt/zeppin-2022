...
//				+ "/WEB-INF/config/sms.xml";
		...
		try
		{
			...
			uid = document.getElementsByTagName("uid").item(0).getFirstChild()
					.getNodeValue();
			pwd = root.getElementsByTagName("pwd").item(0).getFirstChild()
					.getNodeValue();
			url = document.getElementsByTagName("url").item(0).getFirstChild()
					.getNodeValue();
