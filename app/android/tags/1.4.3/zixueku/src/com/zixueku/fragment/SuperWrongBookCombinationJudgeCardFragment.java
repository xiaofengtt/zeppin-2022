 ...
 * 
 */
public class SuperWrongBookCombinationJudgeCardFragment extends BaseFragment implements OnClickListener {
	...
	public static SuperWrongBookCombinationJudgeCardFragment newInstance(ViewPager viewPager, ViewPager subViewPager) {
		SuperWrongBookCombinationJudgeCardFragment f = new SuperWrongBookCombinationJudgeCardFragment(viewPager, subViewPager);
		return f;
	}
	...
	public SuperWrongBookCombinationJudgeCardFragment(ViewPager viewPager, ViewPager subViewPager) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		try {
			options = wrongBookInfo.getWrongItemList().get(position).getChildren().get(subPosition).getData().getOptions();
		...
		if (options == null || options.size() == 0) {
			...
			options.add(option2);
			try {
				wrongBookInfo.getWrongItemList().get(position).getChildren().get(subPosition).getData().setOptions(options);
			} catch (Exception e) {
			}
		}
	...
	Runnable runnable = new Runnable() {
		@Override
		public void run() {
			floatButton.setOnClickListener(SuperWrongBookCombinationJudgeCardFragment.this);
			deleteFloatButton.setOnClickListener(SuperWrongBookCombinationJudgeCardFragment.this);
