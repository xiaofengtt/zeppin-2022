...
public class ZoomableImageView extends View {
	...
	// Statics
	static final float sPanRate = 15;
	...
	private void setBaseMatrix(Bitmap bitmap, Matrix matrix) {
		...
		matrix.postTranslate((viewWidth - ((float) bitmap.getWidth() * scale)) / 2F, (viewHeight - ((float) bitmap.getHeight() * scale)) / 2F);
	}
	...
	private class MyGestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			// If the zoom is default, zoom into 2x
				...
				zoomTo(zoomDefault() * 2, e.getX(), e.getY(), 200);
