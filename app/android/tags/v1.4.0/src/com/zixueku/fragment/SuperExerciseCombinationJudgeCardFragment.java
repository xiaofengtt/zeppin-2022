 ...
 */
public class SuperExerciseCombinationJudgeCardFragment extends BaseFragment implements OnItemClickListener {
	...
	public static SuperExerciseCombinationJudgeCardFragment newInstance(int parentPosition, int subPosition, ViewPager parentViewPager, ViewPager subViewPager, PagerSlidingTabStrip2 tab) {
		SuperExerciseCombinationJudgeCardFragment f = new SuperExerciseCombinationJudgeCardFragment(parentViewPager, subViewPager, tab);
		Bundle b = new Bundle();
	...
	public SuperExerciseCombinationJudgeCardFragment(ViewPager parentViewPager, ViewPager subViewPager, PagerSlidingTabStrip2 tab) {
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			List<Option> options = exercise.getItems().get(parentPosition).getChildren().get(subPosition).getData().getOptions();
			if (options == null || options.size() == 0) {
				Option option1 = new Option("1", "正确", false);
				Option option2 = new Option("2", "错误", false);
				options.add(option1);
				options.add(option2);
				exercise.getItems().get(parentPosition).getChildren().get(subPosition).getData().setOptions(options);
			}
			...
			listView.setOnItemClickListener(SuperExerciseCombinationJudgeCardFragment.this);
		}
	};
