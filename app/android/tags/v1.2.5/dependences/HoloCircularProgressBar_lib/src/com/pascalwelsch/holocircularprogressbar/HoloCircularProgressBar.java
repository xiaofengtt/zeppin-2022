...
public class HoloCircularProgressBar extends View {
	...
	protected void onDraw(final Canvas canvas) {
		...
		if (mIsMarkerEnabled) {
			...
			canvas.drawLine((float) (mThumbPosX + mThumbRadius / 2 * 1.4), mThumbPosY, (float) (mThumbPosX - mThumbRadius / 2 * 1.4), mThumbPosY,
					mMarkerColorPaint);
			canvas.restore();
		}
		...
		if (isThumbEnabled()) {
			...
			canvas.drawOval(mSquareRect, mThumbColorPaint);
			canvas.restore();
		}
	}
