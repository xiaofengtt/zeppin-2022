...
public class ItemTypeAction extends BaseAction {
	...
	@ActionParam(key = "isGroup", type = ValueType.NUMBER, nullable = false, emptyable = false)
	...
	public void Add(){
		...
		Boolean isGroup = this.getBoolValue(request.getParameter("isGroup"));
		...
		if (itemTypes == null){
			...
			itemType = this.getItemTypeService().addItemType(itemType);
	...
	public void Update() {
		...
		if (itemType == null){
		...
		else if (duplicateItemType != null && duplicateItemType.getId() != typeID && 
			...
			itemType.setIsStandard(isStandard);
			itemType.setModelType(modelType);
			itemType = this.getItemTypeService().updateItemType(itemType);
			Map<String, Object> data = SerializeEntity.itemType2Map(itemType);
	...
	public void Delete() {
		...
		if (itemType == null){
			...
			//判断该题型下是否存在试题，如果存在则不允许删除
			Map<String, Object> searchMap = new HashMap<>();
			searchMap.put("itemType.id", itemType.getId());
			int itemCount = this.getItemService().searchItemCount(searchMap);
			if (itemCount > 0) {
				result.init(FAIL_STATUS, "存在该题型下的试题，无法删除！", null);
			}
			else{
				itemType = this.getItemTypeService().deleteItemType(itemType);
				Map<String, Object> data = SerializeEntity.itemType2Map(itemType);
