			...
			$.post('../itemAddJson',{'questionItem':QuestionGOpt.G_createjson('add')},function(r){	
				...
				if(r.Status == 'success') {
					if(e.target.id == 'savenextbutton') {
						...
						location.href = url('host')+'/ItemDatabase/admin/paperLoadPaper?id='+_this.Pid;
					}else {
						location.href = url('host')+'/ItemDatabase/admin/itemList.html';
					}
				}
			})
		})
	...
	addsetting : function() {//保存
		$('#saveSetting').click(function(){
			$.post('../paperAdd',{paper:paperQuestion.settingjson()},function(r){
				if(r.Status == 'success') {
					location.href = url('host')+'/ItemDatabase/admin/paperLoadPaper?id='+r.Records.id;
				}else {
					alert('服务端出错，稍后请重试');
				}
			}).fail(function(){
