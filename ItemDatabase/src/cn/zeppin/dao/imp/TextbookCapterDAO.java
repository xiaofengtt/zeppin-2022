...
public class TextbookCapterDAO extends HibernateTemplateDAO<TextbookCapter, Integer> implements ITextbookCapterDAO
{
	 ...
	 * @return
	 */
	public List<TextbookCapter> searchTextbookCapter(Map<String, Object> searchMap){
		StringBuilder sb = new StringBuilder();
		sb.append("from TextbookCapter tc where 1=1");
		// 出版社外部
		if (searchMap.get("publisherFcode") != null && !searchMap.get("publisherFcode").equals("")) {
			sb.append(" and tc.textbook.publisher.fcode='").append(searchMap.get("publisherFcode")).append("'");
		}
		
		// 学科外部
		if (searchMap.get("subjectFcode") != null && !searchMap.get("subjectFcode").equals("")) {
			sb.append(" and tc.textbook.subject.fcode='").append(searchMap.get("subjectFcode")).append("'");
		}
		
		// 年级外部
		if (searchMap.get("gradeFcode") != null && !searchMap.get("gradeFcode").equals("")) {
			sb.append(" and tc.textbook.grade.fcode like '%").append(searchMap.get("gradeFcode")).append("%'");
		}
		sb.append(" order by tc.level , tc.number");
