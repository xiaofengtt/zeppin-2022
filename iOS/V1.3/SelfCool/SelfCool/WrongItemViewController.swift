...
class WrongItemViewController: UIViewController , UIScrollViewDelegate , UIAlertViewDelegate , HttpDataProtocol {
                ...
                if itemArray.count != 0{
                    ...
                    for index in 0 ..< itemArray.count{
                        ...
                        if itemModel.isGroup{
                            if itemModel.isGroup{
                                var childList = item.objectForKey("children") as! NSArray
                                for i in 0 ..< itemModel.children!.count{
                                    var childItem = childList[i] as! NSDictionary
                                    var childItemModel = itemModel.children![i]
                                    childItemModel.parentIndex = index
                                    childItemModel.blankInx = childItem.objectForKey("blankInx") as! Int
                                    var isWrongbookItemTested = childItem.objectForKey("isWrongbookItemTested") as! Bool
                                    if isWrongbookItemTested{
                                        var lastTestCompleteType = childItem.objectForKey("lastTestCompleteType") as! Int
    ...
    @IBAction func deleteButton(sender: UIButton) {
        ...
        if item.isGroup && item.children!.count != 1 {
            ...
            var childPage = Int(floor((childrenScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
            ...
            var deleteScrollView = childrenScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + page + ItemViewTag.groupViewTagBase() * (childPage + 1)) as! UIScrollView
            deleteScrollView.removeFromSuperview()
            for index in 0 ..< globalItemList.count{
                var thisChildScrollView = itemScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + index) as! UIScrollView?
                var thisItem = globalItemList[index]
                if let thisChildScrollView = thisChildScrollView{
                    if thisItem.isGroup{
                        var thisChildrenScrollView = thisChildScrollView.viewWithTag(ItemViewTag.groupChildrenScrollViewTag()) as! ChildrenScrollView
                        for i in 0 ..< thisItem.children!.count{
                            if index == page && i == childPage{
                                itemIndex++
                            }else{
                                var thisGroupChildScrollView = thisChildrenScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + index + ItemViewTag.groupViewTagBase() * (i + 1)) as! UIScrollView
                                var pageLabel = thisGroupChildScrollView.viewWithTag(ItemViewTag.countLabelTag()) as! UILabel
                                if index == page && i > childPage{
                        ...
                        var pageLabel = thisChildScrollView.viewWithTag(ItemViewTag.countLabelTag()) as! UILabel
                        if index >= page{
                            pageLabel.text = "\(itemIndex)/\(itemCount - 1)"
                        }else{
                            pageLabel.text = "\(itemIndex + 1)/\(itemCount - 1)"
            ...
            for index in 0 ..< globalItemList.count{
                if index != page{
                    ...
                    if let childScrollView = childScrollView{
                        if thisItem.isGroup{
                            var thisChildrenScrollView = childScrollView.viewWithTag(ItemViewTag.groupChildrenScrollViewTag()) as! ChildrenScrollView
                            for i in 0 ..< thisItem.children!.count{
                                var thisGroupChildScrollView = childScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + index + ItemViewTag.groupViewTagBase() * (i + 1)) as! UIScrollView
                                var pageLabel = thisGroupChildScrollView.viewWithTag(ItemViewTag.countLabelTag()) as! UILabel
                                if index > page {
