...
extension StandardItemAnswerViewController{
    func groupAnswer(index: Int){
        ...
        var contentScrollView = UIScrollView(frame: CGRectMake(0, 0, screenWidth, itemScrollViewHeight / 3 - scrollButtonHeight))
        ...
        var childrenScrollView = ChildrenScrollView(frame: CGRectMake(0, itemScrollViewHeight / 3 , screenWidth, itemScrollViewHeight / 3 * 2))
        childrenScrollView.delegate = self
        childrenScrollView.canResignFirstResponder()
        childScrollView.addSubview(childrenScrollView)
        if item.children!.count > 0{
            for i in 0 ..< item.children!.count{
                if item.children![i].modelType == 1{
                    childSingleChoiceAnswer(item.children![i] , parent: childrenScrollView)
                }else if item.children![i].modelType == 3{
                    childJudgeAnswer(item.children![i], parent: childrenScrollView)
                }else if item.children![i].modelType == 5{
                    childMultipleChoiceAnswer(item.children![i], parent: childrenScrollView)
                }
            }
        }
        ...
        var panGesture = UIPanGestureRecognizer(target: self, action: "scrollButton:")
    ...
    func scrollButton(sender: UIPanGestureRecognizer){
        if sender.numberOfTouches() > 0{
            ...
            if translation.y > 0 && translation.y < itemScrollViewHeight - (scrollButton.frame.height + submitButtonHeight){
                ...
                for i in 0 ..< globalItemList[page].children!.count{
                    ...
                    if globalItemList[page].children![i].modelType == 5{
                        ...
                        var submitButton = groupChildScrollView.viewWithTag(ItemViewTag.submitButtonTag()) as! UIButton
                        submitButton.frame = CGRectMake(0, groupChildScrollView.frame.height - submitButtonHeight, screenWidth, submitButtonHeight)
                        var groupMultipleScrollView = groupChildScrollView.viewWithTag(ItemViewTag.groupMultipleScrollViewTag()) as! UIScrollView
