...
extension StandardItemAnalysisViewController{
    func groupItemAnalysis(index: Int ,itemList: [ItemModel], scrollView: UIScrollView){
        ...
        var childScrollView = UIScrollView(frame: CGRectMake(screenWidth * CGFloat(index), 0, screenWidth , scrollView.frame.height))
        ...
        var contentScrollView = UIScrollView(frame: CGRectMake(0, 0, screenWidth, scrollView.frame.height / 3 - scrollButtonHeight))
        ...
        var childrenScrollView = ChildrenScrollView(frame: CGRectMake(0, scrollView.frame.height / 3, screenWidth, scrollView.frame.height / 3 * 2))
        ...
        if scrollView == analysisScrollView{
            var panGesture = UIPanGestureRecognizer(target: self, action: "allScrollButton:")
            scrollButton.addGestureRecognizer(panGesture)
        }else{
            var panGesture = UIPanGestureRecognizer(target: self, action: "failScrollButton:")
            scrollButton.addGestureRecognizer(panGesture)
        }
    ...
    func allScrollButton(sender: UIPanGestureRecognizer){
        if sender.numberOfTouches() > 0{
            var translation : CGPoint = sender.locationOfTouch(0, inView: analysisScrollView)
            let page = Int(floor((analysisScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
            var childScrollView = analysisScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + page) as! UIScrollView
    ...
    func failScrollButton(sender: UIPanGestureRecognizer){
        if sender.numberOfTouches() > 0{
            var translation : CGPoint = sender.locationOfTouch(0, inView: analysisFailScrollView)
            let page = Int(floor((analysisFailScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
            var childScrollView = analysisFailScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + page) as! UIScrollView
            var scrollButton = childScrollView.viewWithTag(ItemViewTag.groupScrollButtonTag()) as! UIButton
