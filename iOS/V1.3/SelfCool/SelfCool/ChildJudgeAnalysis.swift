...
extension StandardItemAnalysisViewController{
    func childJudgeAnalysis(index : Int , item: ItemModel , parent: UIScrollView){
        paragraphStyle.lineSpacing = lineSpacing
        
        var childScrollView = UIScrollView(frame: CGRectMake(screenWidth * CGFloat(index), 0, screenWidth , parent.frame.height))
        ...
        for optionIndex in 0 ..< item.answer.options.count{
            ...
            if item.answer.userResults.count != 0 && item.answer.userResults[0].inx.toInt()! - 1 == optionIndex{
                ...
                optionIcon.setImage(UIImage(named: "judge_\(optionIndex + 1)_selected"), forState: UIControlState.Normal)
            }
