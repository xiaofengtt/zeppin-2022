...
extension StandardItemAnswerViewController {
    ...
    func multipleChioceAnswer(index: Int){
        ...
        for optionIndex in 0 ..< item.answer.options.count{
            ...
            optionButton.addTarget(self, action: "multipleChioceButton:", forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func multipleChioceButton(sender: UIButton) {
        let index = Int(floor((itemScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
        ...
        if allOptionButtons[index][optionIndex].selected{
            ...
            allOptionButtons[index][optionIndex].setTitleColor(UIColor.blackColor(), forState: UIControlState.Normal)
        }else{
        ...
        globalItemList[index].answer.userResults.removeAll(keepCapacity: false)
        for i in 0 ..< allOptionButtons[index].count{
            if allOptionButtons[index][i].selected{
                globalItemList[index].answer.userResults.append(globalItemList[index].answer.options[i])
            }
        }
        
        if globalItemList[index].answer.userResults.count == 0{
