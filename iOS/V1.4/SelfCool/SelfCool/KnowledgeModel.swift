...
class KnowledgeModel : NSObject{
    ...
    init(dictionary: NSDictionary){
        ...
        self.isopen = false
        let haschild = dictionary.objectForKey("hasChild") as! Bool
        if haschild {
            var childKnowledgeList : [KnowledgeModel] = []
            self.haschild = true
            let childKnowledgeArray = dictionary.objectForKey("data") as! NSArray
            for index in 0 ..< childKnowledgeArray.count{
                let childKnowledge = childKnowledgeArray[index] as! NSDictionary
                let childKnowledgeModel = KnowledgeModel(dictionary: childKnowledge)
