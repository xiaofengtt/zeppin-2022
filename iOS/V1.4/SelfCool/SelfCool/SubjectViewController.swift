...
class SubjectViewController: UIViewController , UITableViewDataSource , UITableViewDelegate , HttpDataProtocol{
    ...
    override func viewDidLoad() {
        ...
        //进度
        let progressBackgroundView = UIView(frame: CGRectMake(progressBorderPadding, progressBorderPadding, progressViewWidth - 2 * progressBorderPadding, progressViewWidth - 2 * progressBorderPadding))
        ...
        let endAngle :CGFloat = CGFloat(selectSubject!.preparingProgress / 100) * 360
        ...
        let progressWordLabel = UILabel()
        progressWordLabel.text = "备考进度"
        progressWordLabel.font = UIFont.systemFontOfSize(12)
    ...
    override func viewDidAppear(animated: Bool) {
        MobClick.beginLogPageView("Subject\(selectSubject!.id)")
    }
    
    override func viewDidDisappear(animated: Bool) {
        MobClick.endLogPageView("Subject\(selectSubject!.id)")
    }
    ...
    func recieveDataResults(recieveType: String,dataDictionary: NSDictionary , inputParam : NSDictionary) {
        if recieveType == "getKnowledges" {
            let knowledgeArray = dataDictionary.objectForKey("Records") as! NSArray
            var knowledgeList : [KnowledgeModel] = []
            for index in 0 ..< knowledgeArray.count{
                let knowledge = knowledgeArray[index] as! NSDictionary
                let knowledgeModel = KnowledgeModel(dictionary: knowledge)
            ...
            let itemTpyeArray = dataDictionary.objectForKey("Records") as! NSArray
            var itemTypeList : [ItemTypeModel] = []
            for index in 0 ..< itemTpyeArray.count{
                let itemType = itemTpyeArray[index] as! NSDictionary
                let itemTypeModel = ItemTypeModel(dictionary: itemType)
    ...
    @IBAction func beginAnswer(sender: UIButton) {
        MobClick.event("KnowledgeAnswer")
        ...
        let indexPath = knowledgeTableView.indexPathForCell(tableViewCell)
        selectKnowledge = tableKnowledgeList[indexPath!.row]
    }
    
    @IBAction func randomAnswer(sender: UIButton) {
        MobClick.event("RandomAnswer")
