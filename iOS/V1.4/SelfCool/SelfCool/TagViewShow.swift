...
func TagViewShow(item : ItemModel,hasPage : Bool) -> UIView{
    ...
    let tagView = UIView(frame: CGRectMake(0, 0, screenWidth,tagViewPaddingTop +  taglViewHeight))
    let itemTypeLabel = UILabel()
    let itemTypeString = " \(item.itemType)   |"
    ...
    let sourceTypeLabel = UILabel()
    let sourceTypeString = "   \(item.sourceType) "
    ...
    if hasPage{
        ...
        let indexLabel = UILabel(frame: CGRectMake(screenWidth - tagViewPaddingLeft - indexLabelWidth, tagViewPaddingTop +  (taglViewHeight - itemTypeSize.height) / 2, indexLabelWidth, itemTypeSize.height))
