...
class ItemModel : NSObject {
    ...
    init(dictionary: NSDictionary) {
        ...
        if modelType == 4{
            let answer = dictionary.objectForKey("data") as! NSDictionary
            let answerModel = AnswerModel()
            answerModel.stem = answer.objectForKey("stem") as! String
            self.answer = answerModel
            self.analysis = ""
            var children : [ItemModel] = []
            let childArray = dictionary.objectForKey("children") as! NSArray
            for i in 0 ..< childArray.count{
                let childItem = childArray[i] as! NSDictionary
                let childItemModel = ItemModel(dictionary: childItem)
            ...
            let answer = dictionary.objectForKey("data") as! NSDictionary
            let answerModel = AnswerModel(dictionary: answer , modelType: modelType)
            if modelType == 6{
