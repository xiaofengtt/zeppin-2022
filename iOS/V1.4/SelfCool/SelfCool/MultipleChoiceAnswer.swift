...
extension StandardItemAnswerViewController {
    ...
    func multipleChoiceButton(sender: UIButton) {
        ...
        if globalItemList[page].isGroup{
            let childrenScrollView = childScrollView.viewWithTag(ItemViewTag.groupChildrenScrollViewTag()) as! ChildrenScrollView
            let childPage = Int(floor((childrenScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
            let groupChildScrollView = childrenScrollView.viewWithTag(ItemViewTag.childScrollViewTagBase() + page + ItemViewTag.groupViewTagBase() * (childPage + 1)) as! UIScrollView
            let nextButton = groupChildScrollView.viewWithTag(ItemViewTag.submitButtonTag()) as! UIButton
            ...
            let nextButton = childScrollView.viewWithTag(ItemViewTag.submitButtonTag()) as! UIButton
    ...
    func multipleChoiceAnswer(index: Int){
        ...
        //下一题按钮
        let nextButton = UIButton(frame: CGRectMake(0, itemScrollViewHeight - submitButtonHeight, screenWidth, submitButtonHeight))
