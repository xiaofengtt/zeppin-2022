import Foundation
class SportsCommentViewController: UIViewController, UITextViewDelegate, UIScrollViewDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    func createFooter(){
        ...
        commentLabel.text = "       我来文明的吐个槽..."
        commentLabel.textColor = UIColor.ColorPlaceholder()
        commentLabel.font = UIFont.fontNormal(size: UIFont.FontSizeLesser() * screenScale)
        ...
        commentLabelIcon.image = UIImage(named: "image_comment_label")
    ...
    func createCommentView(){
        ...
        commentView.tag = SportsNumberController.CommentNumbers.commentView
        commentView.backgroundColor = UIColor.white
        let cancleButton = UIButton(frame: CGRect(x: 0, y: 0, width: UIFont.FontSizeBigger() * screenScale * 2 + paddingLeft * 2, height: 40 * screenScale))
        cancleButton.setTitle("取消", for: UIControl.State.normal)
        cancleButton.setTitleColor(UIColor.colorMainColor(), for: UIControl.State.normal)
        cancleButton.titleLabel?.font = UIFont.fontNormal(size: UIFont.FontSizeBigger() * screenScale)
        ...
        commentText.tag = SportsNumberController.CommentNumbers.commentTextView
        ...
        placeholderLabel.tag = SportsNumberController.CommentNumbers.placeholderLabel
        placeholderLabel.text = "我来说两句..."
        placeholderLabel.textColor = UIColor.ColorPlaceholder()
        placeholderLabel.font = UIFont.fontNormal(size: UIFont.FontSizeMiddle() * screenScale)
    ...
    @objc func sendComment(){
        let commentTextView = self.view.viewWithTag(SportsNumberController.CommentNumbers.commentTextView) as! UITextView
        let placeholderLabel = commentTextView.viewWithTag(SportsNumberController.CommentNumbers.placeholderLabel) as! UILabel
        let loadingView: SportsLoadingView = SportsHttpWorker.showLoading(viewController: self)
    ...
    func textViewDidChange(_ textView: UITextView) {
        if(textView.tag == SportsNumberController.CommentNumbers.commentTextView){
            let placeholderLabel = textView.viewWithTag(SportsNumberController.CommentNumbers.placeholderLabel) as! UILabel
            if(textView.text == ""){
                placeholderLabel.isHidden = false
            }else{
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        let commentView = self.view.viewWithTag(SportsNumberController.CommentNumbers.commentView)!
