...
class SportsSpecialViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    func createTopView(){
        ...
        let bgHideView = UIImageView(frame: CGRect(origin: CGPoint(x: 0, y: 10 * screenScale), size: CGSize(width: topView.frame.width, height: topView.frame.height - 30 * screenScale)))
        bgHideView.image = UIImage(named: "phoenix_special_hide")
        topView.addSubview(bgHideView)
