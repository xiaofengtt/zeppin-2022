...
class SportsNavigationBackground: UIView {
    ...
    init(navigationController: UINavigationController) {
        ...
        super.init(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: navigationFrame.width, height: (navigationFrame.height + UIApplication.shared.statusBarFrame.height))))
        ...
        titleLabel.frame = CGRect(x: 70 * screenScale, y: (navigationFrame.height + UIApplication.shared.statusBarFrame.height) - navigationFrame.height + 5 * screenScale, width: self.frame.width - 140 * screenScale, height: 30 * screenScale)
        ...
        backIconView.image = UIImage(named: "phoenix_back")
