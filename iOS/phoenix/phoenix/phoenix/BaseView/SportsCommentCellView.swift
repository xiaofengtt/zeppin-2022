...
class SportsCommentCellView: UIView {
    init(frame: CGRect, comment: SportsNewsCommentModel) {
        ...
        iconView.image = UIImage(named: "phoenix_user_login")
        ...
        nameLabel.text = "热心球迷"
        nameLabel.textColor = UIColor.colorFontDarkGray()
        nameLabel.font = UIFont.fontNormal(size: UIFont.FontSizeMiddle() * screenScale)
        self.addSubview(nameLabel)
        let timeLabel = UILabel(frame: CGRect(x: nameLabel.frame.origin.x, y: nameLabel.frame.origin.y + nameLabel.frame.height, width: 100 * screenScale, height: nameLabel.frame.height))
