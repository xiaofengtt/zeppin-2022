...
extension StandardItemAnswerViewController {
    ...
    func multipleChoiceButton(sender: UIButton) {
        ...
        if globalItemList[page].isGroup{
            ...
            let childPage = Int(floor((itemScrollView.contentOffset.x * 2.0 + screenWidth) / (screenWidth * 2.0)))
            index += (childPage + 1)
        }else{
            index++
        }
        ...
        var nextButton = childView.viewWithTag(ItemViewTag.submitButtonTag()) as! UIButton
