...
class AnswerResultViewController: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        var recordsDictionary = dictionary.objectForKey("Records") as! NSDictionary
        ...
        for index in 0 ..< knoledgesData.count{
            ...
            if knowledgeChange.objectForKey("changeFlag") as! Bool == true{
                var knowledgeChangeModel = DictionaryToKnowledgeChange(knowledgeChange)
        ...
        var buttomLabel = UILabel(frame: CGRectMake(paddingLeft, correctRateViewHeight - correctRateViewPaddingTop - correctRateViewFontSize, screenWidth - 2 * paddingLeft, correctRateViewFontSize))
        buttomLabel.font = UIFont.systemFontOfSize(correctRateViewFontSize)
        buttomLabel.textColor = UIColor.whiteColor()
        buttomLabel.textAlignment = NSTextAlignment.Right
        buttomLabel.text = "/\(globalItemList.count)"
        correctRateView.addSubview(buttomLabel)
    ...
    func answerCardButton(sender:UIButton) {
        ...
        var vc = sb.instantiateViewControllerWithIdentifier("analysisViewController") as!  AnalysisViewController
