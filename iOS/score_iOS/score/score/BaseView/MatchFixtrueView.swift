...
class MatchFixtrueView: UIView {
    ...
    init(frame: CGRect, matchList: Array<MatchModel>, hometeam: String, hometeamName: String, hometeamIconUrl: String) {
        ...
        SDWebImageManager.shared().loadImage(with: URL(string: UrlBase + hometeamIconUrl), options: SDWebImageOptions.allowInvalidSSLCertificates, progress: nil) { (SDImage, data, error, cacheType, result, url) in
            if(SDImage != nil){
        ...
        if(matchList.count > 0){
            let matchListView = MatchListView(frame: CGRect(x: paddingLeft, y: titleView.frame.origin.y + titleView.frame.height + 5 * screenScale, width: frame.width - paddingLeft * 2, height: 0), matchList: matchList, hometeam: hometeam)
