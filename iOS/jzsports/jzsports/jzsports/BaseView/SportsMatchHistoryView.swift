...
class SportsMatchHistoryView: UIView {
    ...
    init(frame: CGRect, historyList: Array<SportsMatchModel>, match: SportsMatchDetailModel) {
        ...
        let titleLabelView = SportsMatchTitleView(frame: CGRect(x: 0, y: 0, width: frame.width, height: 60 * screenScale), title: "历史交锋")
        ...
        resultLabel.frame = CGRect(x: paddingLeft, y: titleLabelView.frame.origin.y + titleLabelView.frame.height + 20 * screenScale, width: resultLabel.frame.width, height: 20 * screenScale)
        ...
        winLineLabel.font = UIFont.fontNormal(size: UIFont.FontSizeMiddle() * screenScale)
        ...
        if(historyList.count > 0){
            let matchListView = SportsMatchListView(frame: CGRect(x: paddingLeft, y: resultLineView.frame.origin.y + resultLineView.frame.height + 5 * screenScale, width: frame.width - paddingLeft * 2, height: 0), matchList: historyList, hometeam: match.hometeam)
