...
class SportsStandingFooterView: UIView {
    ...
    init(category: String) {
        ...
        switch category {
        case CategoryUuid.YAGUAN:
            title.text = "亚冠小组赛出现规则："
        case CategoryUuid.OUGUAN:
            title.text = "欧冠淘汰赛抽签和晋级规则："
        case CategoryUuid.YINGCHAO:
            title.text = "英超决定名次规则："
        case CategoryUuid.XIJIA:
            title.text = "西甲决定名次规则："
        case CategoryUuid.YIJIA:
            title.text = "意甲决定名次规则："
        case CategoryUuid.DEJIA:
            title.text = "德甲决定名次规则："
        case CategoryUuid.ZHONGCHAO:
            title.text = "中超决定名次规则："
        default:
            title.text = ""
        }
        title.textColor = UIColor.colorFontDarkGray()
        title.font = UIFont.fontNormal(size: UIFont.FontSizeMiddle() * screenScale)
        self.addSubview(title)
        let textView = UILabel(frame: CGRect(x: 0, y: 0, width: screenWidth - paddingLeft * 2, height: CGFloat(Int.max)))
        textView.numberOfLines = 0
        switch category {
        case CategoryUuid.YAGUAN:
            textView.text = YAGUANText
        case CategoryUuid.OUGUAN:
            textView.text = OUGUANText
        case CategoryUuid.YINGCHAO:
            textView.text = YINGCHAOText
        case CategoryUuid.XIJIA:
            textView.text = XIJIAText
        case CategoryUuid.YIJIA:
            textView.text = YIJIAText
        case CategoryUuid.DEJIA:
            textView.text = DEJIAText
        case CategoryUuid.ZHONGCHAO:
            textView.text = ZHONGCHAOText
        default:
            textView.text = ""
        }
        textView.toHtmlContent()
        textView.textAlignment = NSTextAlignment.justified
        textView.textColor = UIColor.colorFontDarkGray()
        textView.font = UIFont.fontNormal(size: UIFont.FontSizeSmaller() * screenScale)
        textView.sizeToFit()
