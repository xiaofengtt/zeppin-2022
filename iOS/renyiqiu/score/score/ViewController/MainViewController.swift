...
//  ryqiu
//
//  Created by worker on 2019/5/22.
...
class MainViewController: UIViewController, CategoryScrollViewDelegate, UITableViewDataSource, UITableViewDelegate{
    ...
    let tableHeaderHeight: CGFloat = 300 * screenScale
    ...
    override func viewDidLoad() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: headView.frame.origin.y + headView.frame.height + 10 * screenScale, width: screenWidth, height:  self.tabBarController!.tabBar.frame.origin.y - (categoryView.frame.origin.y + categoryView.frame.height + 10 * screenScale)))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == 0){
            ...
            let headImageView = UIImageView(frame: CGRect(x: paddingLeft, y: 0, width: screenWidth - paddingLeft * 2, height: 180 * screenScale))
            ...
            let cellView = NoMoreView(frame: CGRect(x: 0, y: 0, width: tableView.frame.width, height: 50 * screenScale), title: "没有更多比赛了", flagLine: true)
