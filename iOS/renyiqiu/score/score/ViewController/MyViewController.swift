...
class MyViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let headUnloginView = headView.viewWithTag(TagController.MyTags.headUnloginView)!
        
        if(user == nil){
            logoutView.isHidden = true
            
            headTextView.isHidden = true
            headIconView.isHidden = true
            headUnloginView.isHidden = false
        }else{
            logoutView.isHidden = false
            
            headTextView.text = user!.mobile
            headTextView.isHidden = false
            headIconView.isHidden = false
            headUnloginView.isHidden = true
        }
    }
    ...
    func createHead(){
        ...
        let headUnloginView = UIView(frame: CGRect(x: headView.frame.width * 0.3, y: abs(navigationFrame.origin.y) + 25 * screenScale, width: headView.frame.width * 0.4, height: 40 * screenScale))
        headUnloginView.tag = TagController.MyTags.headUnloginView
        headUnloginView.backgroundColor = UIColor.mainRed()
        headUnloginView.layer.cornerRadius = 2 * screenScale
        headUnloginView.layer.masksToBounds = true
        
        let headUnloginIcon = UIImageView(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: 18 * screenScale, height: 16 * screenScale)))
        headUnloginIcon.image = UIImage(named: "my_head_unlogin")
        let headUnloginLabel = UILabel(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: 80 * screenScale, height: headUnloginIcon.frame.height)))
        headUnloginLabel.text = "立即登录"
        headUnloginLabel.textColor = UIColor.white
        headUnloginLabel.font = UIFont.mainFont(size: UIFont.bigSize() * screenScale)
        headUnloginLabel.textAlignment = NSTextAlignment.center
        
        headUnloginIcon.frame.origin = CGPoint(x: (headUnloginView.frame.width - (headUnloginIcon.frame.width + headUnloginLabel.frame.width)) / 2, y: (headUnloginView.frame.height - headUnloginIcon.frame.height) / 2)
        headUnloginLabel.frame.origin = CGPoint(x: headUnloginIcon.frame.origin.x + headUnloginIcon.frame.width, y: headUnloginIcon.frame.origin.y)
        headUnloginView.addSubview(headUnloginIcon)
        headUnloginView.addSubview(headUnloginLabel)
        headView.addSubview(headUnloginView)
        
        let iconView = UIImageView(frame: CGRect(origin: CGPoint(x: screenWidth * 0.4, y: abs(navigationFrame.origin.y) - 15 * screenScale), size: CGSize(width: screenWidth/5, height: screenWidth/5)))
    ...
    func createLogout(){
        ...
        logoutButton.setTitleColor(UIColor.mainRed(), for: UIControl.State.normal)
