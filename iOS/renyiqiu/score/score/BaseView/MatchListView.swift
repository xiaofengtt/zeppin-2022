...
class MatchListView: UIView {
    ...
    init(frame: CGRect, matchList: Array<MatchModel>, hometeam: String) {
        ...
        for i in 0 ..< matchList.count{
            ...
            var ryqiuColor = UIColor.backgroundGray()
            ...
            if(hometeam == match.hometeam){
                if(homeScoreStr.length == awayScoreStr.length){
                    if(homeScoreStr > awayScoreStr){
                        ...
                        ryqiuColor = loseColor
                    }else{
                        ryqiuColor = drawColor
                    }
                }else{
                    if(homeScoreStr.length > awayScoreStr.length){
                        ryqiuColor = winColor
            ...
            let ryqiuRowLabel = UILabel(frame: CGRect(x: (frame.width - 35 * screenScale)/2, y: (rowView.frame.height - 18 * screenScale)/2, width: 35 * screenScale, height: 18 * screenScale))
            ryqiuRowLabel.layer.masksToBounds = true
            ryqiuRowLabel.layer.cornerRadius = 2 * screenScale
            ryqiuRowLabel.backgroundColor = ryqiuColor
            ryqiuRowLabel.text = match.finalresult.replacingOccurrences(of: "-", with: ":")
            ryqiuRowLabel.textColor = UIColor.white
            ryqiuRowLabel.font = dateRowLabel.font
            ryqiuRowLabel.textAlignment = scoreLabel.textAlignment
            rowView.addSubview(ryqiuRowLabel)
