...
class MatchCellView: UIView {
    ...
    init(frame: CGRect, match: MatchModel) {
        ...
        let leftView = UIView()
        let timeLabel = UILabel(frame: CGRect(x: 0 , y: 0, width: frame.width/4 - paddingLeft, height: UIFont.smallSize() * screenScale))
        ...
        homeScoreLabel.textColor = UIColor.fontBlack()
        homeScoreLabel.font = UIFont.boldFont(size: UIFont.biggestSize() * screenScale)
        ...
        if(match.finalresult.firstIndex(of: "(") == nil){
            if(match.status == "finished" || match.status == "living"){
                ...
                if(sorces.count == 2){
                    homeScoreLabel.text = "\(sorces[0].replacingOccurrences(of: " ", with: ""))"
                    awayScoreLabel.text = "\(sorces[1].replacingOccurrences(of: " ", with: ""))"
                }else{
                    homeScoreLabel.text = "-"
                    awayScoreLabel.text = "-"
                }
