...
import com.zeppin.service.impl.PeBudgetmpDetailServiceImpl;
import com.zeppin.service.impl.PeBudgetmpServiceImpl;
...
@Controller("peBudgetmpAction")
@Scope("prototype")
public class PeBudgetmpAction extends ActionSupport {
	...
	@Autowired
	private PeBudgetmpServiceImpl peBudgetmpService;

	@Autowired
	private PeBudgetmpDetailServiceImpl peBudgetmpDetailService;
	...
	private PeBudgetmp peFeeBudget;
	...
	public void yusuanList() {
		...
		sql_a.append(" LEFT OUTER JOIN PEBUDGETMP BUGET ON BUGET.FK_PRO_APPLYNO = APPLYNO.ID");
		sqlCount.append(" LEFT OUTER JOIN PEBUDGETMP BUGET ON BUGET.FK_PRO_APPLYNO = APPLYNO.ID");
	...
	public void initPeBudgetmp() {
		...
		String hql = "from PeBudgetmp pb where 1=1 and pb.fk_pro_applyno='" + this.peProApplyno.getId() + "'";
		...
		List<PeBudgetmp> lstmp = this.getPeBudgetmpService().getListByHSQL(hql);
		if (lstmp != null && lstmp.size() > 0) {
			...
			this.peFeeBudget = new PeBudgetmp();
	...
	public String savePeBudgetmp() {
		...
		PeBudgetmpDetail peFeeBudgetDetail = new PeBudgetmpDetail();

		boolean isadd = false;

		if (this.getPbid() != null && !this.getPbid().equals("")) {
			// 跟新
			PeBudgetmp tmp = this.getPeBudgetmpService().get(this.getPbid());
		...
		if (isadd) {
			this.getPeBudgetmpDetailService().add(peFeeBudgetDetail);
			this.peFeeBudget.setPeFeeBudgetDetail(peFeeBudgetDetail);
			this.getPeBudgetmpService().add(peFeeBudget);
			msg = "添加成功";
		} else {
			this.getPeBudgetmpDetailService().update(peFeeBudgetDetail);
			this.getPeBudgetmpService().update(peFeeBudget);
			msg = "编辑成功";
		}
	...
	public String toDetail() {
		...
		if (this.peFeeBudget != null) {
			PeBudgetmpDetail peFB = this.peFeeBudget.getPeFeeBudgetDetail();
			if (peFB != null) {
	...
	public String toPdf() throws DocumentException, IOException {
		...
		if (this.peFeeBudget != null && this.peFeeBudget.getId() != null && !this.peFeeBudget.getId().equals("")) {
			...
			PeBudgetmpDetail budgetDetal = this.peFeeBudget.getPeFeeBudgetDetail();
			String returnValue = "export/" + this.peFeeBudget.getId() + ".pdf";
			String TemplatePDF = path + "export/budget.pdf";
	...
	public String toPdf2() throws DocumentException, IOException {
		...
		this.peFeeBudget = this.getPeBudgetmpService().get(peId);
	...
	public void yusuanList2() {
		...
		sql_a.append(" FROM PEBUDGETMP PB,PE_PRO_APPLYNO APPLYNO,PE_UNIT UNIT ,ENUM_CONST EC ");
		sqlCount.append(" FROM PEBUDGETMP PB,PE_PRO_APPLYNO APPLYNO,PE_UNIT UNIT,ENUM_CONST EC ");
		...
		for (int i = 0; i < li.size(); i++) {
			...
			String t = "{\"id\":\"" + peId + "\",\"cell\":[\"" + unitName + "\",\"" + name + "\",\"" + number + "\",\"" + year + "\",\"" + type + "\",\"" + time + "\",\"" + standand + "\",\"" + limitStr + "\",\"" + intime + "\",\"" + statusName + "\"]}";
	...
	public void changeStatus() {
		...
		for (String str : ids) {
			if (method.equals("pass")) {
				String sql = "UPDATE PEBUDGETMP set FLAG = 1 where id ='" + str + "' ";
				this.peBudgetmpService.executeSQLUpdate(sql);
			} else if (method.equals("nopass")) {
				String sql = "UPDATE PEBUDGETMP set FLAG = 2 where id ='" + str + "' ";
				this.peBudgetmpService.executeSQLUpdate(sql);
			}
		}
	...
	public String toMorePdf() throws DocumentException, IOException {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			PeBudgetmp peBudgetmp = this.getPeBudgetmpService().get(ids[i]);
			PeBudgetmpDetail budgetDetal = peBudgetmp.getPeFeeBudgetDetail();
	...
	public PeBudgetmpServiceImpl getPeBudgetmpService() {
		return peBudgetmpService;
	}

	public void setPeBudgetmpService(PeBudgetmpServiceImpl peBudgetmpService) {
		this.peBudgetmpService = peBudgetmpService;
	}

	public PeBudgetmpDetailServiceImpl getPeBudgetmpDetailService() {
		return peBudgetmpDetailService;
	}

	public void setPeBudgetmpDetailService(PeBudgetmpDetailServiceImpl peBudgetmpDetailService) {
		this.peBudgetmpDetailService = peBudgetmpDetailService;
	}
	...
	public PeBudgetmp getPeFeeBudget() {
		return peFeeBudget;
	}

	public void setPeFeeBudget(PeBudgetmp peFeeBudget) {
