...
import com.zeppin.service.impl.PeActualBudgetNewTwoDetailServiceImpl;
import com.zeppin.service.impl.PeActualBudgetNewTwoServiceImpl;
 ...
 * 
 */
@Controller("peActualBudgetNewTwoAction")
@Scope("prototype")
public class PeActualBudgetNewTwoAction extends ActionSupport {
	...
	@Autowired
	private PeActualBudgetNewTwoServiceImpl peActualBudgetNewTwoService;

	@Autowired
	private PeActualBudgetNewTwoDetailServiceImpl peActualBudgetNewTwoDetailService;
	...
	private PeActualBudgetNewTwo peFeeBudgetNew;
	...
	public PeActualBudgetNewTwoAction() {
	...
	public void actualList() {
		...
		sql_a.append(" LEFT OUTER JOIN PEACTUALBUDGETNEWTWO BUGET ON BUGET.FK_PRO_APPLYNO = APPLYNO.ID AND BUGET.FK_UNIT= UNIT.ID");
		sqlCount.append(" LEFT OUTER JOIN PEACTUALBUDGETNEWTWO BUGET ON BUGET.FK_PRO_APPLYNO = APPLYNO.ID AND BUGET.FK_UNIT= UNIT.ID");
...
//			String limitStr = obj[7] == null ? "" : obj[7].toString();
	...
	public void initPeBudgetmp() {
		...
		String hql = "from PeActualBudgetNewTwo pb where 1=1 and pb.fk_pro_applyno='" + this.peProApplyno.getId() + "'";
...
//		this.getPeActualBudgetNewService().executeHSQL(hql);
		List<PeActualBudgetNewTwo> lstmp = this.getPeActualBudgetNewTwoService().getListByHSQL(hql);
		if (lstmp != null && lstmp.size() > 0) {
//			initSubjectApplyNo();
			this.peFeeBudgetNew = lstmp.get(0);
		} else {
			this.peFeeBudgetNew = new PeActualBudgetNewTwo();
	...
	public String savePeActualBudget() {
		...
		PeActualBudgetNewTwoDetail peFeeBudgetNewDetail = new PeActualBudgetNewTwoDetail();
		...
		try {
			...
			hql += " order by pb.input_date desc ";
			List<PeActualBudgetNewTwo> lstmp = this.getPeActualBudgetNewTwoService().getListByHSQL(hql);
			PeActualBudgetNewTwo pbn = new PeActualBudgetNewTwo();
			if (lstmp != null && lstmp.size() > 0) {
				pbn = lstmp.get(0);
			} 
...
//			}
			...
			if (isadd) {
				int status = this.getPeActualBudgetNewTwoService().addAndUpdate(isadd, peFeeBudgetNewDetail, this.peFeeBudgetNew);
				if(status == 1){
					this.msg = "添加成功";
				}else{
					this.msg = "添加失败，请重新提交";
	...
	public String toDetail() {
		...
		if (this.peFeeBudgetNew != null) {
			...
			PeActualBudgetNewTwoDetail peFB = this.peFeeBudgetNew.getPeFeeBudgetNewTwoDetail();
			if (peFB != null) {
...
//						+ peFB.getFeeArgument() + peFB.getFeeTrafficStu() + peFB.getFeeElectronCourse()+peFB.getFeeMealExpert()+peFB.getFeeOfficeSupplies());
	...
	public String toPdf() throws DocumentException, IOException {
		...
		if (this.peFeeBudgetNew != null && this.peFeeBudgetNew.getId() != null && !this.peFeeBudgetNew.getId().equals("")) {
			...
			PeActualBudgetNewTwoDetail budgetDetal = this.peFeeBudgetNew.getPeFeeBudgetNewTwoDetail();
			String returnValue = "export/" + this.peFeeBudgetNew.getId() + ".pdf";
			String TemplatePDF = path + "export/actualbudgetNewTwo.pdf";
			...
			form.setField("unitName", this.peFeeBudgetNew.getUnitName() + "（本单位财务专用章）");
...
//				form.setField("total", total.toString());
	...
	public void actualList2() {
		...
		sql_a.append(" FROM PEACTUALBUDGETNEWTWO PB,PE_PRO_APPLYNO APPLYNO,PE_UNIT UNIT ,ENUM_CONST EC ");
		sqlCount.append(" FROM PEACTUALBUDGETNEWTWO PB,PE_PRO_APPLYNO APPLYNO,PE_UNIT UNIT,ENUM_CONST EC ");
...
//					+ peFB.getFeeTrafficStu() + peFB.getFeeElectronCourse() + peFB.getFeeMealExpert() + peFB.getFeeOfficeSupplies());
	...
	public void changeStatus() {
		...
		for (String str : ids) {
			...
			PeActualBudgetNewTwo pab = this.peActualBudgetNewTwoService.get(str);
			...
			this.peActualBudgetNewTwoService.update(pab);
//			if (method.equals("pass")) {
...
//			}
	...
	public String toDetail2() {
		...
		this.peFeeBudgetNew = this.getPeActualBudgetNewTwoService().get(peId);
...
//				form.setField("total", total.toString());
	...
	public String toMorePdf() throws DocumentException, IOException {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			PeActualBudgetNewTwo peBudgetmp = this.getPeActualBudgetNewTwoService().get(ids[i]);
			PeActualBudgetNewTwoDetail budgetDetal = peBudgetmp.getPeFeeBudgetNewTwoDetail();
...
//				form.setField("total", total.toString());
	...
	public PeActualBudgetNewTwoServiceImpl getPeActualBudgetNewTwoService() {
		return peActualBudgetNewTwoService;
	}

	public void setPeActualBudgetNewTwoService(
			PeActualBudgetNewTwoServiceImpl peActualBudgetNewTwoService) {
		this.peActualBudgetNewTwoService = peActualBudgetNewTwoService;
	}

	public PeActualBudgetNewTwoDetailServiceImpl getPeActualBudgetNewTwoDetailService() {
		return peActualBudgetNewTwoDetailService;
	}

	public void setPeActualBudgetNewTwoDetailService(
			PeActualBudgetNewTwoDetailServiceImpl peActualBudgetNewTwoDetailService) {
		this.peActualBudgetNewTwoDetailService = peActualBudgetNewTwoDetailService;
	}

	public PeActualBudgetNewTwo getPeFeeBudgetNew() {
		return peFeeBudgetNew;
	}

	public void setPeFeeBudgetNew(PeActualBudgetNewTwo peFeeBudgetNew) {
