$(
	...
	function(){
		...
		function initGrid() {
			...
			$("#list2").jqGrid({
				url : "budget_yusuanList2.action",
				datatype : 'json',
				mtype : 'POST',
				height : 'auto',
				altRows : 'true',
				colNames : ['承训单位', '项目名称', '培训项目编号', '所属年度', '项目类型', '申报时限', '人均经费','申报学科上限','提交时间','审核状态','预算操作' ],
				...
				gridComplete : function() {
					...
					for (var i = 0; i < ids.length; i++) {
						...
						var st2 = '<a style="color:blue" target="_blank"  href="budget_toDetail2.action?peId=' + cl + '">查看详情</a>';
						var st1 = '<a style="color:blue" target="_blank"  href="budget_toPdf2.action?peId=' + cl + '">导出PDF</a>';
						var st =  st2+'  '+st1;
			...
			$('#t_list2').on('click','.toPDF',function(){
				...
				if(ids.length>0){
					...
					$(this).attr("href","budget_toMorePdf.action?id="+pIds);
				}
				else{
					alert("请选择要打印的选项");
				}
			...
			$('#list2').navButtonAdd('#pager2', {
				...
				onClickButton : function() {
					...
					$.post('budget_changeStatus.action', {
						'id' : pIds,
						'method' : 'pass'
					}, function(e, t) {
