$(
	...
	function(){
		...
		function initGrid() {
			...
			$("#list2").jqGrid({
				url : "actualnewtwo_actualList2.action",
				datatype : 'json',
				mtype : 'POST',
				height : 'auto',
				altRows : 'true',
				...
				gridComplete : function() {
					...
					for (var i = 0; i < ids.length; i++) {
						...
						var st2 = '<a style="color:blue" target="_blank"  href="actualnewtwo_toDetail2.action?peId=' + cl + '">查看详情</a>';
						var st1 = '<a style="color:blue" target="_blank"  href="actualnewtwo_toPdf2.action?peId=' + cl + '">导出PDF</a>';
						var st =  st2 +'  '+st1;
			...
			$('#t_list2').on('click','.toPDF',function(){
				...
				if(ids.length>0){
					...
					$(this).attr("href","actualnewtwo_toMorePdf.action?id="+pIds);
				}
				else{
					alert("请选择要打印的选项");
				}
			...
			$('#list2').navButtonAdd('#pager2', {
				...
				onClickButton : function() {
					...
					$.post('actualnewtwo_changeStatus.action', {
						'id' : pIds,
						'method' : 'pass'
					}, function(e, t) {
