...
// 			 $("input[name^=person]").sum("keyup", "#qty_item_13");
	 ...
	 function recalc(){
		...
		$("[id^=total_item]").calc(
			...
			function ($this){
				 var sumstr=0;
				 sumstr = $('.item_19').sum().toFixed(2);
				 $('#total_item_19').val(sumstr);
	...
	function check(){ 
		...
		//console.log($('input[name="timearray"]').length);
...
// 		changdi = $("[id^=total_item]:lt(15):gt(11)").sum()/total;
		changdi = $("[id^=total_item]:lt(11):gt(7)").sum()/total;
		//宣传费
...
// 			$("[id^=td_item]:lt(11):gt(7)").css("color","red");
// 			$("#tijiao").removeAttr("disabled");
// 			 a = false;
// 		}else{
// 			changdiInfor.innerHTML='<div style="color: black;">培训所需会议室、教室或实验室的场地租金、网络研修平台及相关设备的租用和服务费。</div>';
// 			$("[id^=td_item]:lt(11):gt(7)").css("color","black");
// 			 a = true;
// 		}
...
// 			pxziliaoInfor.innerHTML='<div style="color: red;">学习资料费加办公用品费,为:'+peixunfei+',超过300元!</div>';
...
// 			$("[id^=td_item]:eq(0)").css("color","red");
// 			o = false;
// 		}else{
// 			xuexiziliaoInfor.innerHTML='<div style="color: black;">聘请师资授课所需费用。</div>';
// // 		    $("[id^=td_item]:eq(6)").css("color","black");
// 			$("[id^=td_item]:eq(0)").css("color","black");
// 		    o = true;
// 		}
...
// 		}
		...
		/*
		if(document.getElementById('peFeeBudgetNew.trainingSpace').value==''){
				$("#msg").html("<b>培训地点不能为空！</b>");
				document.getElementById('peFeeBudgetNew.trainingSpace').focus();
				return false;
		}

		if(document.getElementById('peFeeBudgetNew.projectDirector').value==''){
				$("#msg").html("<b>项目负责人不能为空！</b>");
				document.getElementById('peFeeBudgetNew.projectDirector').focus();
				return false;
		}
		*/
		...
		if(document.getElementById('peFeeBudgetNew.payeeName').value==''){
				$("#msg").html("<b>收款单位名称不能为空！</b>");
				document.getElementById('peFeeBudgetNew.payeeName').focus();
				$("#tijiao").removeAttr("disabled");
				return false;
		}
		if(document.getElementById('peFeeBudgetNew.openingBank').value==''){
				$("#msg").html("<b>开户银行及联行号不能为空！</b>");
				document.getElementById('peFeeBudgetNew.openingBank').focus();
				$("#tijiao").removeAttr("disabled");
				return false;
		}
		if(document.getElementById('peFeeBudgetNew.accountNumber').value==''){
				$("#msg").html("<b>账号不能为空！</b>");
				document.getElementById('peFeeBudgetNew.accountNumber').focus();
				$("#tijiao").removeAttr("disabled");
				return false;
		}
		if(document.getElementById('peFeeBudgetNew.contactInfo').value==''){
				$("#msg").html("<b>联系人及电话不能为空！</b>");
				document.getElementById('peFeeBudgetNew.contactInfo').focus();
				$("#tijiao").removeAttr("disabled");
				return false;
		}
		if(document.getElementById('pbid').value==''){
		...
		var k = (num_fenpei1&&num_a&&num_b&&num_c&&num_d&&num_e&&num_f&&num_g&&num_h&&a&&b&&i&&j&&o&&p&&q);
		if(!k){
			$("#tijiao").removeAttr("disabled");
		}
		return k;
...
<body>
	<div id="main_content">
		<div class="content_title">
			<div align="center" class="STYLE1">“国培计划”——示范性项目培训经费决算表（适用于培训团队专项研修和紧缺领域骨干教师培训项目）</div>
		</div>
		<div class="cntent_k" align="center">
			<form action="actualnewtwo_savePeActualBudget.action"
				...
				<table width="80%" border="0" style="border: 1px dashed #8bbedc;"
					...
					<tr>
						...
						<td align="left"><input type="text"
							name="peFeeBudgetNew.unitName" id="peFeeBudgetNew.unitName"
							value="<s:property value='peFeeBudgetNew.unitName'/>" maxlength="25"
							style="width: 73%" /></td>
						<td>&nbsp;</td>
					</tr>
					...
					<tr>
						...
						<td align="left" colspan="2">
							...
							<s:if test="subjectList!=null">
								...
								<s:if test="%{peFeeBudgetNew.personCount!=null}">
									<s:set name="tempStr" value='peFeeBudgetNew.personCount.split("、")[1]' />
							...
							<s:else>
								<input type="hidden" id="total_p" name="total_p"
									value="<s:property value='peFeeBudgetNew.personCount'/>" />
								<s:property value="peFeeBudgetNew.personCount" />
					...
					<tr>
						<td>培训地点：</td>
						<td align="left"><input type="text"
							name="peFeeBudgetNew.trainingSpace" id="peFeeBudgetNew.trainingSpace"
							value="<s:property value='peFeeBudgetNew.trainingSpace'/>"
					...
					<tr>
						<td>项目负责人：</td>
						<td align="left"><input type="text"
							name="peFeeBudgetNew.projectDirector"
							id="peFeeBudgetNew.projectDirector"
							value="<s:property value='peFeeBudgetNew.projectDirector'/>"
							maxlength="50" style="width: 73%" /></td>
						<td>&nbsp;</td>
					</tr>
					...
					<tr>
						...
						<td align="left">
							...
							<div id="timeContent">
								<s:iterator value="peFeeBudgetNew.training_data.split('\\\\;')" id="tdata">
				...
				<table width="80%" border="0" style="border: 1px dashed #8bbedc;"
					...
					<tr>
						<td colspan="2" align="center" width="27%"><strong>决算科目</strong></td>
					...
					<tr>
						<td rowspan="4" align="center"><strong>师资费</strong></td>
						<td align="center">讲课费</td>
						<td align="left">
							
							经费: <input type="text" name="total_item_5" id="total_item_5"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeMealAccExpert'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_5">
						
							<input type="text" name="price_item_5" id="price_item_5"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealAccExpert.split('\\\\*')[0]"/>"
								size="2" /> 
							
							<span id="span_1">元/（人•半天）</span>× 
							
							<input type="text" id="qty_item_5" name="qty_item_5"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealAccExpert.split('\\\\*')[1]"/>"
								size="2" /> 
						...
						<td align="left" id="xuexiziliaoInfor">聘请师资授课所需费用。</td>
					</tr>
					<tr>
						<td align="center">住宿费</td>
						<td align="left">
							经费: <input type="text" name="total_item_2" id="total_item_2"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeAccommodation'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_2">
						
							<input type="text" name="price_item_2" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAccommodation.split('\\\\*')[0]"/>" /> 元/人/天× 
							
							<input type="text" id="qty_item_2" name="qty_item_2"
								value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAccommodation.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAccommodation.split('\\\\*')[1]"/></s:if>"
								size="2" /> 
							<span id="span_qty_2">人</span>× 
							
							<input type="text" 	name="time_item_2" size="2"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAccommodation.split('\\\\*')[2]"/>" /> 天
						</td>
						<td align="left" id="zhuanjiazhusuInfor">聘请师资在培训期间的住宿费。</td>
					</tr>
					<tr>
						<td align="center">伙食费</td>
						<td align="left">
							经费: <input type="text" name="total_item_15" id="total_item_15"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeMealExpert'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_15">
						
							<input type="text" name="price_item_15" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealExpert.split('\\\\*')[0]"/>" /> 元/人次×
							
							<input type="text" name="qty_item_15"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealExpert.split('\\\\*')[1]"/>"
								size="2" /> 人次
							
							<input type="hidden" name="time_item_15" value="1" size="2" /> 
						</td>
						<td align="left" id="zhuanjiahuoshiInfor">聘请师资在培训期间的伙食费。</td>
					</tr>
					<tr>
						...
						<td align="left">
							...
							经费: <input type="text" name="total_item_4" id="total_item_4"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeTrafficExpert'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_4">
						
							<input type="text" name="price_item_4"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTrafficExpert.split('\\\\*')[0]"/>"
								size="2" /> 元/人次× 
								
							<input type="text" name="qty_item_4"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTrafficExpert.split('\\\\*')[1]"/>"
								size="2" /> 人次
							
							<input type="hidden" name="time_item_4" value="1" size="2" /> 
						
						</td>
						<td align="left" id="zhuanjiajiaotongInfor">聘请师资城市间往返交通费。</td>
					</tr>
					<tr>
						...
						<td align="left">
							...
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeMeal'/>"
							size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_1">
							
							<input type="text" name="price_item_1" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMeal.split('\\\\*')[0]"/>"/> 元/人/天× 
							
							<input type="text" id="qty_item_1" 	name="qty_item_1" size="2" 
							value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMeal.split('\\\\*')[1] != null  "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMeal.split('\\\\*')[1]"/></s:if><s:else><s:property value="#total_persons"/></s:else>"/> 
							<span id="span_qty_1">人</span>× 
							
							<input type="text"	name="time_item_1" size="2" 
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMeal.split('\\\\*')[2]"/>"/> 天
							
						</td>
						<td align="left" id="xueyuanzhusuInfor">学员在培训期间的住宿费。</td>
					</tr>
					<tr>
						<td align="center">工作人员住宿费</td>
						<td align="left">
							经费: <input type="text" name="total_item_17" id="total_item_17"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeStaffAccommodation'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_17">
						
							<input type="text" name="price_item_17" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteStaffAccommodation.split('\\\\*')[0]"/>" /> 元/人/天× 
							
							<input type="text" id="qty_item_17" name="qty_item_17"
								value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteStaffAccommodation.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteStaffAccommodation.split('\\\\*')[1]"/></s:if>"
								size="2" /> 
							<span id="span_qty_2">人</span>× 
							
							<input type="text" 	name="time_item_17" size="2"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteStaffAccommodation.split('\\\\*')[2]"/>" /> 天
						</td>
						<td align="left" id="zhuanjiazhusuInfor">工作人员在培训期间的住宿费。</td>
					</tr>
					<tr>
						<td rowspan="2" align="center"><strong>伙食费</strong></td>
						<td align="center">学员伙食费</td>
						<td align="left">
							经费: <input type="text" name="total_item_14"  id="total_item_14"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeElectronCourse'/>"
							size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_14">
							
							<input type="text" name="price_item_14" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteElectronCourse.split('\\\\*')[0]"/>"/> 元/人/天× 
							
							<input type="text" id="qty_item_14" 	name="qty_item_14" size="2" 
							value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteElectronCourse.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteElectronCourse.split('\\\\*')[1]"/></s:if><s:else><s:property value="#total_persons"/></s:else>"/> 
							<span id="span_qty_14">人</span>× 
							
							<input type="text"	name="time_item_14" size="2" 
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteElectronCourse.split('\\\\*')[2]"/>"/> 天
							
						</td>
						<td align="left" id="xueyuanhuoshiInfor">学员在培训期间的伙食费。</td>
					</tr>
					<tr>
						<td align="center">工作人员伙食费</td>
						<td align="left">
							经费: <input type="text" name="total_item_18" id="total_item_18"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeMealStaff'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_18">
						
							<input type="text" name="price_item_18" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealStaff.split('\\\\*')[0]"/>" /> 元/人次×
							
							<input type="text" name="qty_item_18"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMealStaff.split('\\\\*')[1]"/>"
								size="2" /> 人次
							
							<input type="hidden" name="time_item_18" value="1" size="2" /> 
						</td>
						<td align="left" id="gongzuohuoshiInfor">工作人员在培训期间的伙食费。</td>
					</tr>
					<tr>
						...
						<td align="left">经费: <input type="text" name="total_item_10"
							id="total_item_10"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeResearch'/>"
						...
						<td align="left" id="td_item_10">
							...
							<input type="text"
								name="price_item_10"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteResearch.split('\\\\*')[0]"/>"
								size="2" /> 元/班次/天× 
								
							<input type="text" name="qty_item_10"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteResearch.split('\\\\*')[1]"/>"
								size="2" /> 班次× 
								
							<input type="text" name="time_item_10"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteResearch.split('\\\\*')[2]"/>"
								size="2" /> 天
						
						</td>
						<td rowspan="3" align="left" id="changdiInfor">培训所需会议室、教室或实验室的场地租金、网络研修平台及相关设备的租用和服务费。</td>
					</tr>
					<tr>
						<td align="center">网络研修平台租用费</td>
						<td align="left">经费: <input type="text" name="total_item_11"
							id="total_item_11"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeArgument'/>"
							size="5" readonly="true" />
						</td>
						...
						<input type="text"
							name="price_item_11"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteArgument.split('\\\\*')[0]"/>"
							size="2" /> 元/人×
						
						<input type="text" id="qty_item_11"
							name="qty_item_11"
							value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteArgument.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteArgument.split('\\\\*')[1]"/></s:if><s:else><s:property value="#total_persons"/></s:else>"
					...
					<tr>
						...
						<td align="left">经费: <input type="text" name="total_item_12"
							id="total_item_12"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeTrafficStu'/>"
							size="5" readonly="true" /></td>
						<td align="left" id="td_item_12">
						
						<input type="text"
							name="price_item_12"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTrafficStu.split('\\\\*')[0]"/>"
							size="2" /> 元/班次/天× 
							
						<input type="text" name="qty_item_12"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTrafficStu.split('\\\\*')[1]"/>"
							size="2" /> 班次× 
							
						<input type="text" name="time_item_12"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTrafficStu.split('\\\\*')[2]"/>"
							size="2" /> 天
					...
					<tr>
						...
						<td align="left">经费: <input type="text" name="total_item_9"
							id="total_item_9"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeSurvey'/>"
							size="5" readonly="readonly" /></td>
						<td align="left" id="td_item_9">
						
							<input type="text" name="price_item_9"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteSurvey.split('\\\\*')[0]"/>"
								size="2" /> 元/件× 
							
							<input type="text" name="qty_item_9"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteSurvey.split('\\\\*')[1]"/>"
								size="2" /> 件 
					...
					<tr>
						<td align="center">学习资料费</td>
							<td align="left">经费: <input type="text" name="total_item_8"
								id="total_item_8"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeEquipmentRent'/>"
								size="5" readonly="readonly" />
							</td>
							<td align="left" id="td_item_8">
							
								<input type="text" name="price_item_8" id="price_item_8"
									value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteEquipmentRent.split('\\\\*')[0]"/>"
									size="2" /> 元/人× 
									
								<input type="text" id="qty_item_8" name="qty_item_8"
									value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteEquipmentRent.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteEquipmentRent.split('\\\\*')[1]"/></s:if><s:else><s:property value="#total_persons"/></s:else>"
					...
					<tr>
						<td align="center">办公用品费</td>
							<td align="left">经费: <input type="text" name="total_item_16"
								id="total_item_16"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeOfficeSupplies'/>"
								size="5" readonly="readonly" />
							</td>
							<td align="left" id="td_item_16">
							
								<input type="text" name="price_item_16" id="price_item_16"
									value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOfficeSupplies.split('\\\\*')[0]"/>"
									size="2" /> 元/人× 
									
								<input type="text" id="qty_item_16" name="qty_item_16"
									value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOfficeSupplies.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOfficeSupplies.split('\\\\*')[1]"/></s:if><s:else><s:property value="#total_persons"/></s:else>"
					...
					<tr>
						<td align="center"><strong>交通费</strong></td>
						<td align="center">考察、调研等交通费</td>
						<td align="left">
						...
						<td align="left" id="td_item_3">
							...
							<input type="text" name="price_item_3"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTeach.split('\\\\*')[0]"/>"
								size="2" /> 元/次× 
							
							<input type="text" name="qty_item_3"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteTeach.split('\\\\*')[1]"/>"
								size="2" /> 次
								
							<input type="hidden" name="time_item_3" value="1" size="2" />
						
						</td>
						<td align="left" id="xueyuanjiaotongInfor">用于接送以及统一组织的与培训有关的考察、调研等发生的交通支出。</td>
					</tr>
					
					<tr>
						<td rowspan="3" align="center"><strong>其他费用</strong></td>
						<td align="center"><p align="center">现场教学费</p></td>
						<td align="left">
							经费: <input type="text" name="total_item_6" id="total_item_6"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeMaterials'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_6">
							<input type="text" name="price_item_6" id="price_item_6"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMaterials.split('\\\\*')[0]"/>"
								size="2" /> 
							
							<span id="span_1">元/人次</span>× 
							
							<input type="text" id="qty_item_6" name="qty_item_6"
								value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMaterials.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteMaterials.split('\\\\*')[1]"/></s:if>"
								size="2" /> <span id="span_qty_6">人次</span> 
								
							<input type="hidden" name="time_item_6" value="1" size="2" />
						</td>
						<td rowspan="3" align="left" id="zhuanjialaowuInfor">用于现场教学、班主任、文体活动、医药费支出以及与培训有关的其他支出。</td>
					</tr>
					<tr>
						<td align="center"><p align="center">班主任费</p></td>
						<td align="left">经费: <input type="text" name="total_item_7"
							id="total_item_7"
							value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeAreaRent'/>"
							size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_7">
						
							<input type="text" name="price_item_7"  id="price_item_7"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAreaRent.split('\\\\*')[0]"/>"
								size="2" /> <span id="span_1">元/人次/天</span>× 
								
							<input type="text" id="qty_item_7" name="qty_item_7"
								value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAreaRent.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAreaRent.split('\\\\*')[1]"/></s:if>"
								size="2" /> <span id="span_qty_6">人次</span>×
								
							<input type="text" 	name="time_item_7" size="2"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteAreaRent.split('\\\\*')[2]"/>" /> 天
						</td>
					</tr>
					<tr>
						<td align="center">文体活动费、医药费及与培训有关的其他支出</td>
						<td align="left">
							经费: <input type="text" name="total_item_19" id="total_item_19"
								value="<s:property value='peFeeBudgetNew.peFeeBudgetNewTwoDetail.feeOthers'/>"
								size="5" readonly="readonly" />
						</td>
						<td align="left" id="td_item_19">
							文体活动费
							<input type="text" class="item_19" name="price_item_19" size="2"
								value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOthers.split('\\\\*')[0]"/>" /> 元、 
							医药费
							<input type="text" class="item_19" id="qty_item_19" name="qty_item_19"
								value="<s:if test="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOthers.split('\\\\*')[1] != null "><s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOthers.split('\\\\*')[1]"/></s:if>"
								size="2" /> 
							<span id="span_qty_2">元、</span> 
							其他支出
							<input type="text" class="item_19"	name="time_item_19" size="2"
							value="<s:property value="peFeeBudgetNew.peFeeBudgetNewTwoDetail.noteOthers.split('\\\\*')[2]"/>" /> 元
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" align="left">
							<div id="grandTotal" style="color: red;float: left;margin-left: 40px;"></div><span style="float: left;color: red;">元</span>
						</td>
						<td>&nbsp;</td>
					</tr>
					<tr>
				...
				<table width="80%" border="0" style="border: 1px dashed #8bbedc;"
					...
					<tr>
						<td width="50%" align="right">
							收款单位名称： 
							<input type="text"
								name="peFeeBudgetNew.payeeName" id="peFeeBudgetNew.payeeName"
								value="<s:property value='peFeeBudgetNew.payeeName'/>"
								maxlength="50" style="width: 73%" />
						</td>
						<td width="50%" align="right">
							开户银行及联行号： <input type="text"
							name="peFeeBudgetNew.openingBank" id="peFeeBudgetNew.openingBank"
							value="<s:property value='peFeeBudgetNew.openingBank'/>"
							maxlength="50" style="width: 73%" />
						</td>
					</tr>
					<tr>
						<td align="right">账号： <input type="text"
							name="peFeeBudgetNew.accountNumber" id="peFeeBudgetNew.accountNumber"
							value="<s:property value='peFeeBudgetNew.accountNumber'/>"
							maxlength="25" style="width: 73%" />
						</td>
						<td align="right">
							项目联系人及电话：						
							<input type="text"
								name="peFeeBudgetNew.contactInfo" id="peFeeBudgetNew.contactInfo"
								value="<s:property value='peFeeBudgetNew.contactInfo'/>"
								maxlength="25" style="width: 73%" />
						</td>
					</tr>
				</table>
				...
				<!--预算表ID  -->
				<input type="hidden" name="pbid" id="pbid" value="<s:property value='peFeeBudgetNew.id'/>" />
