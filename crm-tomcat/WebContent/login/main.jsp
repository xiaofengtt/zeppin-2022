...
if (user_id.intValue() == 21){//金汇要求去掉一部分提示框
	if ("intro|widget2|widget3|widget4|widget5|widget6|widget7|widget8|widget9|widget10".equals(workbench)){
		workbench="widget99|intro|widget2|widget4";
	}
}
...
<%}%>
	...
	function showProductInfo(preproduct_id,product_id){
		var url ="";
		if (preproduct_id==0){
			url = "<%=request.getContextPath()%>/marketing/base/product_list_detail.jsp?product_id="+product_id;
		}else{
			url = "<%=request.getContextPath()%>/wiki/product_info_view.jsp?preproduct_id="+preproduct_id;
		}
		window.open(url);
	}
...
//查看产品预约统计信息
function showTeamInfo(pre_product_id, pre_product_name, pre_money,product_id){
	var url = '<%=request.getContextPath()%>/marketing/sell/product_pre_total_team.jsp?pre_product_id=' + pre_product_id+'&product_id='+product_id + '&pre_product_name=' + pre_product_name + '&pre_money=' + pre_money;
	showModalDialog(url, '', 'dialogWidth:1000px;dialogHeight:600px;status:0;help:0');
}

//直接预约
function reservAddInfo(pre_product_id,product_id){
	if (product_id>0)
		location = "<%=request.getContextPath()%>/marketing/sell/bespeak_add.jsp?from=main&product_id="+product_id;
	else
		location = "<%=request.getContextPath()%>/marketing/sell/direct_reserve_add.jsp?from=main&pre_product_id="+pre_product_id+"&product_id="+product_id;
}
		...
		<ul id="product1" class="column9">
		 	<li class="widget9" id="widget99" style="display:none">
	            <div class="widget-head">
	                <h2><img src="/images/home1.gif" />&nbsp;在售产品预约情况</h2>&nbsp;&nbsp;&nbsp;
	            </div>
...
<%}else{ %>
							<td align="center" width="*"><font size="1">小额份数/总预约数</font></td>
							<td align="center" width="*"><font size="1">剩余预约规模</font></td>
<%} %>
				...
				<%	for(int i=0;i<pre_len;i++){
						...
						<tr class="tr1">
							<td align="left"><a href="#" class="a2" title="查看产品信息" onclick="javascript:showProductInfo('<%=preproduct_id%>','<%=Utility.trimNull(preMap.get("BIND_PRODUCT_ID"))%>');" ><%=Utility.trimNull(preMap.get("PREPRODUCT_NAME"))%></a></td>
							...
							<td align="right" width="*"><a href="#" class="a2" title="查看产品预约统计信息" onclick="javascript:showTeamInfo('<%=preproduct_id%>','<%=java.net.URLEncoder.encode(Utility.trimNull(preMap.get("PREPRODUCT_NAME")))%>','<%=Utility.trimNull(Format.formatMoney(pre_money))%>','<%=Utility.trimNull(preMap.get("BIND_PRODUCT_ID"))%>');" ><font color="<%=max_ratio.compareTo(stand_ratio) == 1 ? "red" : "" %>"><%=Format.formatMoney(pre_money)%></font></a></td>
...
<%}else{ %>
							<td align="center" width="*"><%=Utility.parseInt(Utility.trimNull(preMap.get("LESS300_NUMBER")),new Integer(0))%> /  <%=Utility.parseInt(Utility.trimNull(preMap.get("VALID_COUNT")),new Integer(0))%></td>
							<td align="right" width="*"><%=Format.formatMoney(available_money)%></td>
<%} %>
							<td align="center" width="*">
		       					<a href="#" class="a2" onclick="javascript:reservAddInfo('<%=preproduct_id %>','<%=Utility.trimNull(preMap.get("BIND_PRODUCT_ID"))%>');" title="产品预约">预约</a>
							</td>
						</tr> 
				<%	}%>
					</table>
	        ...
	        <ul id="column3" class="column">
	            ...
	            <li class="widget" id="widget7" style="display:none">  
	                ...
	                <div class="widget-content">
						...
						<%}else{%>
							<ul>	
						<% 	for (int j=0; j<appl_len; j++) {
								Map applyCheckMap = (Map)applyCheckList.get(j);
	            ...
	            <li class="widget" id="widget8" style="display:none">  
	                ...
	                <div class="widget-content">
						...
						<%}else{%>
							<ul>	
						<%	for(int j=0;j<activeTask_len;j++){
								Map activeTaskMap = (Map)activeTaskList_1.get(j); %>
								<li>		
	         ...
	         <li class="widget" id="widget9" style="display:none">  
					...
					<%}else{%>
						<ul>	
					<% 	for (int j=0; j<activeTask_len2; j++){
							Map activeTaskMap2 = (Map)activeTaskList_2.get(j); %>
							<li>	
				...
				<li class="widget" id="widget12" style="display:none">  
	                ...
	                <div class="widget-content">    
						<% 
						//客户移交审核用到的变量
						List activeList_11 = activeList.getRsList();
						...
						<%}else{%>
							<ul>	
						<% 	for(int j=0;j<custManagerChangesList1_len;j++){
								 Map custManagerChangesMap = (Map)custManagerChangesList1.get(j);
				...
				<li class="widget" id="widget15" style="display:none"> 
					...
					<%}else{
						...
						for (int j=0; j<preContract_total_list.size(); j++) {	
							Map map = (Map)preContract_total_list.get(j);
							ytemName[j] = Utility.trimNull(map.get("TEAM_NAME"));
							xValue[j] = Utility.trimNull(Utility.parseDecimal(Utility.trimNull(map.get("RG_MONEY")),new BigDecimal(0)).divide(new BigDecimal(10000),2));
						}
				...
				<li class="widget" id="widget17" style="display:<%if(list.size()==0) out.print("none"); %>"> 
					...
					<% 	for (int j=0; j<list.size(); j++) {
							Map contractStatM = (Map)list.get(j); %>
...
System.out.println("============="+daysBeforeDue+"-----");
	if (daysBeforeDue>=0) {  %>
				<li class="widget" id="widget18" style="display:none"> 
					...
					<%} else {%>
							<ul>
					<%	for (int j=0; j<dueProductList.size(); j++) {	
							Map map = (Map)dueProductList.get(j);
