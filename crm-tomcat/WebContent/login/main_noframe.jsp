...
Map workConfigMap =null,workDetailMap=null,workCountMap=null;
...
if(workConfigList != null){
	...
	List workCountList = businessLogicLocal.listWorkCount(showSql);
		  for(int i=0;i<workCountList.size();i++){ 
		    workCountMap = (Map)workCountList.get(i);
...
<script language=javascript>
	...
	function showWorkBench(){
		...
		for(i=0;i<workbenchArray.length;i++){
			...
			if(document.getElementById(workbenchId)!=null){
				document.getElementById(workbenchId).style.display = "";	
			}
			
			if(document.getElementById(workbenchCheckId)!=null){
				document.getElementById(workbenchCheckId).checked = true;
			}	
		}
		<%if(Argument.getSyscontrolValue("FLOW_CONTROL") == 1){%>
		//显示待处理流程
		if(document.getElementById("widget98")!=null){
			document.getElementById("widget98").style.display = "";
		}
		if(document.getElementById("workbench_widget98")!=null){
	...
	function setAction(){
		...
		if(sl_confirm("<%=LocalUtilis.language("message.save",clientLocale)%> ")){//保存
			loginService.modiWorkBench(input_operatorCode,v_workbench,
				{callback: 	function(data){if(data=="100"){
...
<div id="accordion" style="margin-left:5px;height:98%;width:100%;">
	<div id="product" style="margin-left:5px;height:30%;width:99%;display:">
		<ul id="product1" class="column9">
		 	<li class="widget9" id="widget99" style="display:">
		 ...
		 <div id="columns">	        
	        <ul id="column1" class="column">
					...
					for(int i=0;i<workConfigList.size();i++){
			    		...
			    		if(Integer.parseInt(workCount)>0){ 
							for(int j=0;j<workDetailList.size();j++){
	    						...
	    						iNumber=iNumber+1;
								countToShow_1=countToShow_1+1;
								if(tempName.length() >charSum){
								tempName = tempName.substring(0,charSum-1)+"…";
								}
							if(iNumber>5) break;
	            ...
	            <li class="widget" id="intro" style="display:none">
	                <div class="widget-head">
	                    <h2><img src="/images/home1.gif" />&nbsp;<%=LocalUtilis.language("main.menu.mySchedule",clientLocale)%> </h2>
						<!--我的日程-->
	                </div>
	                <div class="widget-content">	           
						<%
	             ...
	             <li class="widget" id="widget2" style="display:none">  
	                <div class="widget-head">
	                    <h2><img src="/images/home2.gif" />&nbsp;<%=LocalUtilis.language("main.menu.promotionProduct",clientLocale)%> </h2>
						<!--最新推介期产品-->
	                </div>
	                <div class="widget-content">
					<%
					...
					%>		
						...
						<%=LocalUtilis.language("main.message.noPromotion",clientLocale)%> 。</div>
						<!--最近没有推介期产品-->
					<% }
						else{
					%>
						...
						<%	
							for(int i=0;i<p_len;i++){
								productMap = (Map)productlist.get(i);
	            ...
	            <li class="widget" id="widget3" style="display:none">  
	                <div class="widget-head">
	                    <h2><img src="/images/home3.gif" />&nbsp;<%=LocalUtilis.language("class.productMarketing",clientLocale)%> </h2>
						<!--在售产品统计 -->
	                </div>
	                	  
	                	<% 
	                		int iCurrent = 0;
							Map marketingMap = null;
					 ...
					 <tr class="trh" bgcolor=#fcfdfd>
						...
						<td align="center" width="*"><%=LocalUtilis.language("class.linkMan2",clientLocale)%><%=LocalUtilis.language("class.toMoney3",clientLocale)%></td><!--我的已售金额-->
								...
								<% for(int i=0;i<marketing.size();i++){
									marketingMap = (Map)marketing.get(i);
				...
				<li class="widget" id="widget10" style="display:none" >
					...
					<%
						...
						for(int i=0;i<channel_list.size();i++){
							channelMap = (Map)channel_list.get(i);
			                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

			                serial_no = Utility.parseInt(Utility.trimNull(channelMap.get("SERIAL_NO")),new Integer(0));
			                channel_id = Utility.parseInt(Utility.trimNull(channelMap.get("CHANNEL_ID")),new Integer(0));
							plan_time = Utility.trimNull(channelMap.get("PLAN_TIME"));
							service_content = Utility.trimNull(channelMap.get("SERVICE_CONTENT"));
						...
						%>
							<ul>
									<% for(int i=0;i<outsideList.size();i++){
												outsideMap = (Map)outsideList.get(i);
									%>
	            ...
	            <li class="widget" id="widget14" style="display:none">
	                ...
	                <div class="widget-content">	           
						<%
							...
							Map tcoMoneyTaskMap=null;
							Integer tcoMoneyTask_serial_no=new Integer(0);
							String record_id="";
							String task_type="";
						...
						%>
							...
							<%
								for(int i=0;i<tcoMoneyTaskLength;i++){
									tcoMoneyTaskMap = (Map)tcoMoneyTaskList.get(i);
									tcoMoneyTask_serial_no = Utility.parseInt(Utility.trimNull(tcoMoneyTaskMap.get("SERIAL_NO")),new Integer(0));
									record_id=Utility.trimNull(tcoMoneyTaskMap.get("RECORD_ID"));
									task_type=Utility.trimNull(tcoMoneyTaskMap.get("TASK_TYPE"));
							%>	
								<li>
	        ...
	        <ul id="column2" class="column">
	        	<li class="widget" id="widget4" style="display:none">  
	                <div class="widget-head">
	                    <h2><img src="/images/home4.gif" />&nbsp;<%=LocalUtilis.language("main.menu.customerAffairs",clientLocale)%> </h2>
						<!--待处理客户事务-->
	                </div>
	                <div class="widget-content">
						<%	
	         ...
	         <li class="widget" id="widget9" style="display:none">  
	                ...
	                <div class="widget-content">    
						...
						<%								
							  for(int i=0;i<custManagerChangesList1_len;i++){
								 custManagerChangesMap = (Map)custManagerChangesList1.get(i);
								 serial_no = Utility.trimNull(custManagerChangesMap.get("SERIAL_NO"));
								 manager_before = Utility.trimNull(custManagerChangesMap.get("MANAGERNAME_BEFORE"));
								 manager_now = Utility.trimNull(custManagerChangesMap.get("MANAGERNAME_NOW"));	
						%>
				...
				<li class="widget" id="widget17" style="display:<%if(list.size()==0) out.print("none"); %>"> 
	                ...
	                <div class="widget-content">    
						...
						<%}else{Map contractStatM = null;
						%>
							<ul>	
						<%
							for (int i=0; i<list.size(); i++){
							contractStatM = (Map)list.get(i);
						%>
...
<%} %>
	    ...
	    <div id="setMenu">
			<button type="button"  class="xpbutton2" id="btnSave" onclick="javascript:showDialog();"> <%=LocalUtilis.language("index.msg.set",clientLocale)%> </button><!--设置-->
		</div>
	</div>
...
<div id="setContent" style="display:none;">
	<table cellspacing="2" cellpadding="2" width="500px">
		<tr bgcolor="#FFFFFF">
			...
			<%if(Argument.getSyscontrolValue("FLOW_CONTROL") == 1){%>
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget98" class="selectAllBox" value="widget98" onclick="">
				&nbsp;&nbsp;事务流程
			</td>
			<td align="left">&nbsp;&nbsp;</td>
			<%} %>
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_intro" class="selectAllBox" value="intro" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.mySchedule",clientLocale)%> <!--我的日程-->
			</td>
		</tr>
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget2" class="selectAllBox" value="widget2" onclick="">
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget4" class="selectAllBox" value="widget4" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.customerAffairs",clientLocale)%> <!--待处理客户事务-->
			</td>

			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget5" class="selectAllBox" value="widget5" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.myMeetings",clientLocale)%> <!--我的会议-->
			</td>
		</tr>
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget6" class="selectAllBox" value="widget6" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.myActivity",clientLocale)%> <!--我的活动-->
			</td>

			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget7" class="selectAllBox" value="widget7" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.auditingPurchaseContract",clientLocale)%> <!--待审核申购合同-->
			</td>
		</tr>
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget8" class="selectAllBox" value="widget8" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.main.yourActivityTask",clientLocale)%> <!--待处理活动任务-->
			</td>

			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget9" class="selectAllBox" value="widget9" onclick="">
				&nbsp;&nbsp;<%=LocalUtilis.language("main.menu.auditingActivityTask",clientLocale)%> <!--待审核活动任务-->
			</td>
		</tr>
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget10" class="selectAllBox" value="widget10" onclick="">&nbsp;&nbsp;
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget12" class="selectAllBox" value="widget12" onclick="">&nbsp;&nbsp;
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget3" class="selectAllBox" value="widget3" onclick="">
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget13" class="selectAllBox" value="widget13" onclick="">
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget16" class="selectAllBox" value="widget16" onclick="">
				&nbsp;&nbsp;产品销售统计分析 <!--在售产品销售统计-->
			</td>
			<%} %>
		</tr>
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				<input type="checkbox" name="workbenchOptions" id="workbench_widget18" class="selectAllBox" value="widget18" onclick="">
				&nbsp;&nbsp;将到期产品
			</td>
			<td align="left">&nbsp;&nbsp;</td>
		</tr>
	...
	<div style="float:right;margin-top:5px;margin-bottom:5px;margin-right:25px;">
		<button type="button"  class="xpbutton2" id="btnSave" onclick="javascript:setAction();"><%=LocalUtilis.language("message.save",clientLocale)%> <!--保存--></button>	
	</div>
</div>
