    	...
    	function checkMenu(obj){
            ...
            if(".CLL"==(URLSuffix.toLocaleUpperCase())){
			   ...
			   if(menu_url.indexOf('?')<=0){
				   menu_url="<%=request.getContextPath()%>"+menu_url+"?menu_id="+menu_id+"&first_flag=1&parent_id=&log=<%=LOG0001.toString()%>"+"&UNQUERY=1";
			   }else{
			   	   menu_url="<%=request.getContextPath()%>"+menu_url+"&menu_id="+menu_id+"&first_flag=1&parent_id=&log=<%=LOG0001.toString()%>"+"&UNQUERY=1";
			   }
		  	}					
		...
		<div id="home-header"> 
			...
			<div id="scroll"> 
				<div id="scrollMe" style="overflow:hidden;height:27px;margin-right:10px;">
					<!-- 您好 空闲 -->
					<a href="#"><%=LocalUtilis.language("index.msg.hello",clientLocale)%>!<%=input_operatorName%></a>
			...
			<div class="nav-secondary">
				...
				<span class="region-selector">
					...
					<!-- 呼叫中心 -->
					<%if(Argument.getSyscontrolValue("DT_CALL")!=0){%>
					<span><font  color="#000000"><%=LocalUtilis.language("index.menu.callcenter",clientLocale)%></font> <img src="<%=request.getContextPath()%>/images/icon-phone.gif" /></span>&nbsp;&nbsp;					
					<%}%>
					<%if(Argument.getSyscontrolValue("DT_CALL")==0){%>
