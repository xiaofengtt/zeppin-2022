var CalendarWebControl = new atCalendarControl();//日期控件（2004.8.25）
function atCalendarControl(){
  ...
  this.addCalendarPad=function(){
   document.write("<div id='divCalendarpad' style='position:absolute;top:100;left:0;width:255;height:167;display:none;z-index:99;'>");
...
var tbBoard=document.createElement("table");
   ...
   tbBoard.className="dateSelect-table"
  ...
  /************** 设置各功能按钮的功能 *********************/
   ...
   calendar.prevMonth.onmousedown=function(){
    ...
    if(calendar.currentDate[2].toString().length<2)
  ...
  this.show=function(targetObject,defaultDate,sourceObject)
  {
   ...
   if(defaultDate.length==8)
	     defaultDate=exchangeDateStyle(defaultDate)
   ...
   else
   {
    ...
    {
     alert("默认日期的格式不正确\n\n默认日期可接受格式为:'yyyy-mm-dd'或'yyyymmdd'");
     return;
    }
   ...
   for(i=0;i<theRows;i++){
    ...
    for(j=0;j<7;j++){
     ...
     if(n>theFirstDay && n<=theMonthLen){
      day=n-theFirstDay;
      calendar.insertBodyCell(theRow,j,day,targetObject);
     }
   ...
   if((document.body.offsetWidth-(offsetPos.x+calendar.source.offsetWidth-document.body.scrollLeft))>calendar.calendarPad.style.pixelWidth){
    ...
    var calLeft=offsetPos.x-calendar.calendarPad.style.pixelWidth;
   }
...
//************* 插入日期单元格 **************/
  this.insertBodyCell=function(theRow,j,day,targetObject){
   ...
   theCell.onclick=function()
   {
    ...
    targetObject.focus(); //取得时间后，把光标指定到当前的文本框中 2008-11-05 ADD
   }
  }
  /************** 取得月份的第一天为星期几 *********************/
