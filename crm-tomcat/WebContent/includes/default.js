...
function table_sort_string(obj, n, mode, len)
{
	...
	for(i = 1; i < len - 1; i++)
	{
		...
		for(j = i + 1; j < len; j++)
		{
			...
			if (mode)
			{
				if(tab.rows[j].cells[n].innerText.localeCompare(tab.rows[i].cells[n].innerText)) //(tab.rows[i].cells[n].innerText > tab.rows[j].cells[n].innerText)
				{
			...
			{
				if(tab.rows[i].cells[n].innerText.localeCompare(tab.rows[j].cells[n].innerText)) //(tab.rows[i].cells[n].innerText < tab.rows[j].cells[n].innerText)
				{
					tab.rows[i].swapNode(tab.rows[j]);
					if (multi)	swapHistoryRow(tab, i, j, len);
				}
...
{　
	return sl_confirm("通过恢复");
}

function sl_update_ok(url)
{
	sl_alert("保存成功！");  // in default.vbs
	if (url != null)
		location.replace(url);
}
function sl_refer_ok(url)
{
	sl_alert("提交成功！");  // in default.vbs
	if (url != null)
		location.replace(url);
}
function sl_check_ok(url)
{
	sl_alert("审核成功！");  // in default.vbs
	if (url != null)
		location.replace(url);
}
...
function confirmRefer1(element,mess)
{
	...
	if(checkedCount(element) == 0)
	{
		sl_alert('请选定要'+mess+'的记录！');
		return false;
	}
	return sl_confirm('提交'+mess+'的记录');
...
function setWaitingTitle(title)
{
	...
	{
		waittingTitle.innerText = "正在"+title+"，请稍候...";
	}
}
...
function Selecthide()
{
	if(parent.leftshow)
	{
		...
		document.all.menuSwitch.title='隐藏';
	}
	else
	{
		...
		document.all.menuSwitch.title='显示';
	}
}
...
// 把number类型的date转换成separator分隔的String类型
function formatDate(date, separator) {  
    if (separator==null) separator = "-";
    var str = ""+date;
    return str.substring(0,4) 
       + separator + str.substring(4,6) 
       + separator + str.substring(6,8);
}
...
function formatNum(num) {
    ...
    var integral = str;
    var decimal = "";
    var idxPoint = str.indexOf(".");
    if (idxPoint>=0) {
        decimal = str.substring(idxPoint, str.length);
        integral = str.substring(0,idxPoint);
    }
    var array = [];
    for (var i=integral.length-1; i>=0; i-=3) {
        var temp;
        if (i-2<0)
            temp = integral.substring(0,i+1);
        else 
            temp = integral.substring(i-2,i+1);
...
//判断身份证号码是否合法
function checkCardId1(socialNo){   
      if(socialNo == "")   
      {   
        alert("输入身份证号码不能为空!");   
        return false;   
