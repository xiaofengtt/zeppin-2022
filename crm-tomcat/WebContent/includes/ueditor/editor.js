...
function getDomNode(node, start, ltr, startFromChild, fn, guard) {
    ...
    while (!tmpNode && (parent = (parent || node).parentNode)) {
        if (parent.tagName == 'BODY' || guard && !guard(parent)) return null;
        tmpNode = parent[ltr];
    }
...
var domUtils = dom.domUtils = {
    ...
    findParentByTagName: function (node, tagName, includeSelf, excludeFn) {
        if (node && node.nodeType && !this.isBody(node) && (node.nodeType == 1 || node.nodeType)) {
            ...
            while (node && node.tagName && node.nodeType != 9) {
                if (excludeFn && excludeFn(node)) break;
    ...
    isSameElement: function (nodeA, nodeB) {
        ...
        if (!ie && thisAttribs.length != otherAttribs.length) return 0;
        ...
        for (var i = 0; attrA = thisAttribs[i++];) {
            ...
            if (attrA.nodeValue != attrB.nodeValue) return 0;
    ...
    isSameStyle: function (elementA, elementB) {
        ...
        for (var i = 0, ci; ci = styleA[i++];) {
            if (utils.indexOf(styleB, ci) == -1) return 0
        }
        return 1;
    },
    ...
    isEmptyInlineElement: function (node) {
        ...
        if (node.nodeType != 1 || !dtd.$removeEmpty[node.tagName]) return 0;
    ...
    removeStyle: function (node, name) {
        ...
        if (!node.style.cssText) domUtils.removeAttributes(node, ['style'])
    },
    ...
    isEmptyBlock: function (node) {
        ...
        if (node[browser.ie ? 'innerText' : 'textContent'].replace(reg, '').length > 0) return 0;

        for (var n in dtd.$isNotEmpty) {
            if (node.getElementsByTagName(n).length) return 0;
        }
...
(function () {
    ...
    Range.prototype = {
        ...
        deleteContents: function () {
            if (!this.collapsed) execContentsAction(this, 1);
            if (browser.webkit) {
        ...
        shrinkBoundary: function (ignoreEnd) {
            ...
            if (!ignoreEnd) {
                ...
                &&
                me.endOffset > 0 && (child = me.endContainer.childNodes[me.endOffset - 1]) && child.nodeType == 1 && !domUtils.isBookmarkNode(child) && !dtd.$empty[child.tagName] && !dtd.$nonChild[child.tagName]) {
            ...
            if (start.nodeType == 3) {
                if (offset == 0) {
                    ...
                    if (offset >= start.nodeValue.length) {
                        ...
                        if (start === end) this.setEnd(textNode, this.endOffset - offset);
                        else if (start.parentNode === end) this.endOffset += 1;
        ...
        enlarge: function (toBlock, stopFn) {
            ...
            if (!this.collapsed) {
                while (this.startOffset == 0) {
                    if (stopFn && stopFn(this.startContainer)) break;
                    if (isBody(this.startContainer)) break;
                    this.setStartBefore(this.startContainer);
                }
                while (this.endOffset == (this.endContainer.nodeType == 1 ? this.endContainer.childNodes.length : this.endContainer.nodeValue.length)) {
                    if (stopFn && stopFn(this.endContainer)) break;
    ...
    Selection.prototype = {
        ...
        getStart: function () {
            ...
            if (ie) {
                ...
                if (start.nodeType == 1 && start.hasChildNodes()) start = start.childNodes[Math.min(start.childNodes.length - 1, range.startOffset)];
...
(function () {
    ...
    Editor.prototype = {
        ...
        _selectionChange: function (delay, evt) {
            ...
            var me = this;
            if (!me.selection.isFocus()) return;
        ...
        hasContents: function (tags) {
            if (tags) {
                for (var i = 0, ci; ci = tags[i++];) {
                    if (this.document.getElementsByTagName(ci).length > 0) return true;
                }
            }
...
(function () {
    ...
    function genColorPicker(noColorText) {
        ...
        for (var i = 0; i < COLORS.length; i++) {
            ...
            html += i < 70 ? '<td style="padding: 0 2px;"><a hidefocus title="' + COLORS[i] + '" onclick="return false;" href="javascript:" unselectable="on" class="edui-box edui-colorpicker-colorcell"' + ' data-color="#' + COLORS[i] + '"' + ' style="background-color:#' + COLORS[i] + ';border:solid #ccc;' + (i < 10 || i >= 60 ? 'border-width:1px;' : i >= 10 && i < 20 ? 'border-width:1px 1px 0 1px;' :
    ...
    SplitButton.prototype = {
        ...
        setPopup: function (popup) {
            ...
            popup.addListener('postrender', utils.bind(function () {
                ...
                uiUtils.createElementByHtml('<div id="' + this.popup.id + '_bordereraser" class="edui-bordereraser edui-background" style="width:' + (uiUtils.getClientRect(this.getDom()).width - 2) + 'px"></div>'));
...
(function () {
    ...
    AutoTypeSetPicker.prototype = {
        ...
        getHtmlTpl: function () {
            ...
            return '<div id="##" class="edui-autotypesetpicker %%">' + '<div class="edui-autotypesetpicker-body">' + '<table >' + '<tr><td colspan="2"><input type="checkbox" name="mergeEmptyline" ' + (opt["mergeEmptyline"] ? "checked" : "") + '>�ϲ�����</td><td colspan="2"><input type="checkbox" name="removeEmptyline" ' + (opt["removeEmptyline"] ? "checked" : "") + '>ɾ�����</td></tr>' + '<tr><td colspan="2"><input type="checkbox" name="removeClass" ' + (opt["removeClass"] ? "checked" : "") + '>�����ʽ</td><td colspan="2"><input type="checkbox" name="indent" ' + (opt["indent"] ? "checked" : "") + '>��������2��</td></tr>' + '<tr><td colspan="2"><input type="checkbox" name="textAlign" ' + (opt["textAlign"] ? "checked" : "") + '>���뷽ʽ��</td><td colspan="2" id="textAlignValue"><input type="radio" name="textAlignValue" value="left" ' + ((opt["textAlign"] && opt["textAlign"] == "left") ? "checked" : "") + '>�����<input type="radio" name="textAlignValue" value="center" ' + ((opt["textAlign"] && opt["textAlign"] == "center") ? "checked" : "") + '>���ж���<input type="radio" name="textAlignValue" value="right" ' + ((opt["textAlign"] && opt["textAlign"] == "right") ? "checked" : "") + '>�Ҷ��� </tr>' + '<tr><td colspan="2"><input type="checkbox" name="imageBlockLine" ' + (opt["imageBlockLine"] ? "checked" : "") + '>ͼƬ����</td>' + '<td colspan="2" id="imageBlockLineValue">' + '<input type="radio" name="imageBlockLineValue" value="none" ' + ((opt["imageBlockLine"] && opt["imageBlockLine"] == "none") ? "checked" : "") + '>Ĭ��' + '<input type="radio" name="imageBlockLineValue" value="left" ' + ((opt["imageBlockLine"] && opt["imageBlockLine"] == "left") ? "checked" : "") + '>�󸡶�' + '<input type="radio" name="imageBlockLineValue" value="center" ' + ((opt["imageBlockLine"] && opt["imageBlockLine"] == "center") ? "checked" : "") + '>��ռ�о���' + '<input type="radio" name="imageBlockLineValue" value="right" ' + ((opt["imageBlockLine"] && opt["imageBlockLine"] == "right") ? "checked" : "") + '>�Ҹ���</tr>' +

            '<tr><td colspan="2"><input type="checkbox" name="clearFontSize" ' + (opt["clearFontSize"] ? "checked" : "") + '>����ֺ�</td><td colspan="2"><input type="checkbox" name="clearFontFamily" ' + (opt["clearFontFamily"] ? "checked" : "") + '>�������</td></tr>' + '<tr><td colspan="4"><input type="checkbox" name="removeEmptyNode" ' + (opt["removeEmptyNode"] ? "checked" : "") + '>ȥ�������html����</td></tr>' + '<tr><td colspan="4"><input type="checkbox" name="pasteFilter" ' + (opt["pasteFilter"] ? "checked" : "") + '>ճ����� (��ÿ��ճ�������Ӧ�����Ϲ��˹���)</td></tr>' + '<tr><td colspan="4" align="right"><button >ִ��</button></td></tr>' + '</table>' + '</div>' + '</div>';
...
(function () {
    ...
    var dialogBtns = {
        ...
        ok: ['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage', 'insertvideo', 'highlightcode', 'insertframe', 'edittd']
            ...
            indexByValue: function (value) {
                if (value) {
                    for (var i = 0, ci; ci = this.items[i]; i++) {
                        if (ci.value.join(',').indexOf(value) != -1) return i;
                    }
                }
    ...
    editorui.printelement = function(editor,list,title){
        title = title || editor.options.labelMap['printelement'] || '';
        list = list || editor.options['printelement'] || [];
        var items = [];
        for (var i = 0; i < list.length; i++) {
            items.push({
                label: list[i].label,
                value: list[i].tag,
                renderLabelHtml: function () {
                    return '<div class="edui-label %%-label" style="line-height:1;font-size:12px">' + (this.label || '') + '</div>';
                }
            });
        }
        var ui = new editorui.Combox({
            ...
            initValue: title,
            onselect: function (t, index) {
                editor.execCommand('PrintElement', this.items[index].value);
            },
            onbuttonclick: function () {
                this.showPopup();
            },
            className: 'edui-for-printelement'
        });
        editor.addListener('selectionchange', function (type, causeByUi, uiReady) {
            if (!uiReady) {
                var state = editor.queryCommandState('PrintElement');
                if (state == -1) {
                    ui.setDisabled(true);
                } else {
                    ui.setDisabled(false);
...
(function () {
    ...
    EditorUI.prototype = {
            ...
            if (editor.options.imagePopup) {
                editor.addListener('mouseover', function (t, evt) {
                    ...
                    if (editor.ui._dialogs.insertframeDialog && /iframe/ig.test(el.tagName)) {
                        var html = popup.formatHtml('<nobr>����: <span onclick=$$._setIframeAlign(-2) class="edui-clickable">Ĭ��</span>&nbsp;&nbsp;<span onclick=$$._setIframeAlign(-1) class="edui-clickable">�����</span>&nbsp;&nbsp;<span onclick=$$._setIframeAlign(1) class="edui-clickable">�Ҷ���</span>&nbsp;&nbsp;' + '<span onclick=$$._setIframeAlign(2) class="edui-clickable">����</span>' + ' <span onclick="$$._updateIframe( this);" class="edui-clickable">�޸�</span></nobr>');
                        if (html) {
                ...
                editor.addListener('selectionchange', function (t, causeByUi) {
                    ...
                    if (img && img.tagName == 'IMG') {
                        ...
                        if (img.getAttribute("anchorname")) {
                            ...
                            html = popup.formatHtml('<nobr>����: <span onclick=$$._onImgEditButtonClick("anchorDialog") class="edui-clickable">�޸�</span>&nbsp;&nbsp;' + '<span onclick=$$._onRemoveButtonClick(\'anchor\') class="edui-clickable">ɾ��</span></nobr>');
                        }
                        ...
                        if (!dialogs[dialogName]) {
                            return;
                        }!html && (html = popup.formatHtml('<nobr>����: <span onclick=$$._onImgSetFloat("none") class="edui-clickable">Ĭ��</span>&nbsp;&nbsp;' + '<span onclick=$$._onImgSetFloat("left") class="edui-clickable">����</span>&nbsp;&nbsp;' + '<span onclick=$$._onImgSetFloat("right") class="edui-clickable">����</span>&nbsp;&nbsp;' + '<span onclick=$$._onImgSetFloat("center") class="edui-clickable">����</span>&nbsp;&nbsp;' + '<span onclick="$$._onImgEditButtonClick(\'' + dialogName + '\');" class="edui-clickable">�޸�</span></nobr>'))
                    ...
                    if (editor.ui._dialogs.linkDialog) {
                        ...
                        if (link && (url = (link.getAttribute('data_ue_src') || link.getAttribute('href', 2)))) {
                            ...
                            html += popup.formatHtml('<nobr>����: <a target="_blank" href="' + url + '" title="' + url + '" >' + txt + '</a>' + ' <span class="edui-clickable" onclick="$$._onEditButtonClick();">�޸�</span>' + ' <span class="edui-clickable" onclick="$$._onRemoveButtonClick(\'unlink\');"> ���</span></nobr>');
        ...
        getHtmlTpl: function () {
            return '<div id="##" class="%%">' + '<div id="##_toolbarbox" class="%%-toolbarbox">' + (this.toolbars.length ? '<div id="##_toolbarboxouter" class="%%-toolbarboxouter"><div class="%%-toolbarboxinner">' + this.renderToolbarBoxHtml() + '</div></div>' : '') + '<div id="##_toolbarmsg" class="%%-toolbarmsg" style="display:none;">' + '<div id = "##_upload_dialog" class="%%-toolbarmsg-upload" onclick="$$.showWordImageDialog();">����ϴ�</div>' + '<div class="%%-toolbarmsg-close" onclick="$$.hideToolbarMsg();">x</div>' + '<div id="##_toolbarmsg_label" class="%%-toolbarmsg-label"></div>' + '<div style="height:0;overflow:hidden;clear:both;"></div>' + '</div>' + '</div>' + '<div id="##_iframeholder" class="%%-iframeholder"></div>' + '<div id="##_bottombar" class="%%-bottomContainer"><table><tr>' + '<td id="##_elementpath" class="%%-bottombar"></td>' + '<td id="##_wordcount" class="%%-wordcount"></td>' + '</tr></table></div>' + '</div>';
        },
                        ...
                        for (var i = 0, ci; ci = me.currentSelectedArr[i++];) {
                            if (ci.style.display != 'none') {
                                ...
                                if (cmdName == 'superscript' || cmdName == 'subscript') {
                                    ...
                                    if (!obj || obj.tagName.toLowerCase() != cmdName) range.removeInlineStyle(['sub', 'sup'])
    ...
    me.commands['removeformat'] = {
        execCommand: function (cmdName, tags, style, attrs, notIncludeA) {
            ...
            function isRedundantSpan(node) {
                if (node.nodeType == 3 || node.tagName.toLowerCase() != 'span') return 0;
                if (browser.ie) {
                        ...
                        if (!dtd.$empty[current.tagName.toLowerCase()] && !domUtils.isBookmarkNode(current)) {
                            if (tagReg.test(current.tagName)) {
                                ...
                                if (!dtd.$tableContent[current.tagName] && !dtd.$list[current.tagName]) {
                                    ...
                                    if (isRedundantSpan(current)) domUtils.remove(current, true);
                                }
        ...
        (function (cmd, style) {
            UE.commands[cmd] = {
                ...
                queryCommandState: function (cmdName) {
                    ...
                    if (!(cmdName == 'underline' || cmdName == 'strikethrough')) return 0;
            ...
            while (current && !(domUtils.getPosition(current, bookmark2.end) & domUtils.POSITION_FOLLOWING)) {
                if (current.nodeType == 3 || !block(current)) {
                    ...
                    if (attrs) {
                        ...
                        if (sourceCmdName && sourceCmdName == 'customstyle' && attrs.style) para.style.cssText = attrs.style;
                    }
                    ...
                    if (block(parent) && !domUtils.isBody(para.parentNode) && utils.indexOf(notExchange, parent.tagName) == -1) {
                        ...
                        if (attrs && /h\d/i.test(parent.tagName) && !/h\d/i.test(para.tagName)) {
                            ...
                            if (sourceCmdName && sourceCmdName == 'customstyle' && attrs.style) parent.style.cssText = attrs.style;
                            domUtils.remove(para, true);
                            para = parent;
                        } else domUtils.remove(para.parentNode, true);
        ...
        me.commands['source'] = {
            execCommand: function () {
                ...
                if (sourceMode) {
                    ...
                    if (browser.gecko) me.body.contentEditable = false;
...
UE.plugins['undo'] = function () {
    ...
    me.addListener('reset', function (type, exclude) {
        if (!exclude) me.undoManger.reset();
    });
...
UE.plugins['selectall'] = function () {
    ...
    me.commands['selectall'] = {
        execCommand: function () {
            ...
            if (domUtils.isEmptyBlock(this.body)) range.collapse(true);
            range.select(true);
            this.selectAll = true;
        },
        notNeedUndo: 1
    ...
    me.addListener('keydown', function (type, evt) {
        ...
        if (keyCode == 9) {
            ...
            if (range.collapsed) {
                ...
                } else range.insertNode(span.cloneNode(true).firstChild).setCursor(true);
                ...
                for (var i = 0; i < s.length; i++) {
                    ...
                    if (parts.length == 2) {
                        ...
                        case "mso-vertical-align-alt":
                            if (!/<table/.test(tag)) n[i] = name.replace(/^mso-|-alt$/g, "") + ":" + ensureUnits(value);
                            continue;
                        case "horiz-align":
    ...
    function transOutNode(node) {
        ...
        case 'div':
            if (node.attributes._ue_div_script) {
                ...
                node.children = [{
                    type: 'cdata',
                    data: decodeURIComponent(node.attributes._ue_script_data) || '',
                    parent: node
                }];
    ...
    UE.plugins['paste'] = function () {
        ...
        me.addListener('ready', function () {
            ...
            domUtils.on(me.body, browser.ie ? 'keydown' : 'paste', function (e) {
                if (browser.ie && (!e.ctrlKey || e.keyCode != '86')) return;
...
UE.commands['preview'] = {
    execCommand: function () {
        ...
        var me = this,
            ...
            style = "<style type='text/css'>p{height:20px;}" + (me.document.getElementById("editorinitialstyle") && me.document.getElementById("editorinitialstyle").innerHTML) + "</style>",
            cont = me.getContent();
        if (browser.ie) {
        ...
        d.write('<html><head>' + style + '<link rel="stylesheet" type="text/css" href="' + utils.unhtml(this.options.iframeCssUrl) + '"/>' + (css ? '<link rel="stylesheet" type="text/css" href="' + css.href + '"/>' : '')

        + (css && js ? ' <script type="text/javascript" charset=gbk src="' + js.src + '"></script>' : '') + '<title></title></head><body >' + cont + (css && js ? '<script type="text/javascript">' + (baidu.editor.browser.ie ? 'window.onload = function(){SyntaxHighlighter.all()};' : 'SyntaxHighlighter.all();') + 'setTimeout(function(){' + 'for(var i=0,di;di=SyntaxHighlighter.highlightContainers[i++];){' + 'var tds = di.getElementsByTagName("td");' + 'for(var j=0,li,ri;li=tds[0].childNodes[j];j++){' + 'ri = tds[1].firstChild.childNodes[j];' + 'ri.style.height = li.style.height = ri.offsetHeight + "px";' + '}' + '}},100)</script>' : '') + '</body></html>');
        d.close();
    },
    notNeedUndo: 1
};
