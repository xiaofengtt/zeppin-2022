...
{ 
  ...
  if (this.hreference && (this.hreference.indexOf('#')==-1)) 
  { 
	if (USEFRAMES)
	  docW = docW + "<a class = 'menu' href='" + this.hreference + "'  TARGET=\"content\" "
	else
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='javascript:clickOnNode(\""+this.getID()+"\");return false;'"	//fuck 这里要加上return false;不然会响应href

    docW = docW + ">"
  }
  else if(this.hreference && (this.hreference.indexOf('#')!=-1))
  { 
	if (USEFRAMES)
	  docW = docW + "<a class = 'menu' style=cursor:hand oncontextmenu = showMenu('"+this.hreference+"') TARGET=\"content\" "
	else
	  docW = docW + "<a class = 'menu' style=cursor:hand oncontextmenu = showMenu('"+this.hreference+"') TARGET=_top "
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='javascript:clickOnNode(\""+this.getID()+"\");return false;'"
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
      ...
      if (this.isOpen)
        srcStr = ICONPATH + "elbow-minus-nl.gif"  
      else
        srcStr = ICONPATH + "elbow-plus-nl.gif"  
  } 
  else 
  { 
    if (this.nChildren == 0)
      ...
      if (this.isOpen)
        srcStr = ICONPATH + "elbow-end-minus-nl.gif"
      else
        srcStr = ICONPATH + "elbow-end-plus-nl.gif"
  }   
  return srcStr;
}
...
{ 
  ...
  this.iconSrc = ICONPATH + "leaf.gif" 
  this.isRendered = 0
  this.isLastNode = false
  this.level = 0
  this.leftSideCoded = ""
...
{ 
  ...
  var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" oncontextmenu = showMenu('"+this.link+"') onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');return false;\"";
  this.isRendered = 1
  //var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" onDBClick=\"showMenu('"+this.link+"');return false;\"";
  if (this.level>0) 
...
function preLoadIcons() {
	...
	auxImg.src = ICONPATH + "elbow-end-minus-nl.gif";
	auxImg.src = ICONPATH + "elbow-minus-nl.gif";
...
{
    menuForm.pass_menuid.value = id;
    var hrefs=menuForm.pass_menuid.value
    startid = hrefs.indexOf("bottom_flag")+12;
    
	if(startid==-1)
		return true;
	strSubID = hrefs.substring(startid,startid+1);
	if(strSubID=='0')
		popMenu(itemMenu,100,"0000");//显示添加，
	else if(strSubID=='1')
		popMenu(itemMenu,100,"1010");//显示添加，刷新
    else if(strSubID=='2')
		popMenu(itemMenu,100,"0000");//不显示添加，取消，刷新
	else if(strSubID=='3')
		popMenu(itemMenu,100,"0010");//显示添加菜单夹，重命名
    else if(strSubID=='4')
		popMenu(itemMenu,100,"1101");//显示取消,添加菜单夹
...
function popMenu(menuDiv,width,rowControlString)
{
    ...
    pop.show(event.clientX-1,event.clientY,width,rowCount*22,document.body);
    return true;
}

function opmenu(action)
{
	var hrefs=menuForm.pass_menuid.value
    
    startid = hrefs.indexOf("?")+1;
    
	if(startid==-1)
		return false;
	strSubID = hrefs.substring(startid,hrefs.length); 
	...
	if (action == 2)//取消菜单从我的菜单
		url = '../portal/my_menu_update.jsp?inflag=2&'+strSubID;
	else
		url = '../portal/folder_name.jsp?inflag='+action+'&'+strSubID;
		
	if(showModalDialog(url, '', 'dialogWidth:400px;dialogHeight:220px;status:0;help:0'))
	{
		if(action != 1)
			location.reload();
		return false;
