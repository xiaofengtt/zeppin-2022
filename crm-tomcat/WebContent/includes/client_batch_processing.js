...
var TreeEvent = {
	...
	checkAction : function(imgObj){
		...
		var queryId = classId+"_"+parentId;
		//父节点
		if(parentId==10||parentId==11||parentId==12||parentId==1001||parentId==1002){
			sysFlag = 1;
			if(DataProxy.initFlag){
				sl_alert('该节点为系统自动评级节点');
				return false;
			}
			var v_joinId = "imgJoin_"+ classId + "_"+ parentId;
			//本身图片变更
			imgObj.onclick = function(){};
			imgObj.src = IMG.ICONS["checkbox2"];
			imgObj.title="checkbox1";

			$(v_joinId).onclick = function(){};
			$(v_joinId).title = "joinbottom";
			$(v_joinId).src = IMG.ICONS["joinbottom"];
		}
		else if(checkType=="checkbox0"){
...
var DataProxy = {
	...
	paramsObjs 	: [],
	paramsObjs2	: [],//保存缓存
	getCustLevelArray : function(classId,parentId,levelId){
		DataProxy.dwrFlag = false;
		CustomerService.getCustClassDefine(classId,parentId,levelId,oState.cd_no,{callback: function(data){
			oState.accept1 +=1;
			...
			if(data.length>1){
				...
				for(var i = 0;i<dataArray.length;i++){
					...
					if(dataObj.getBottomFlag()==2){
						...
						if(v_parentId==null||v_parentId==''){
							v_parentId=0;
						}
						var paramsObj = {classId:v_classId,parentId:v_parentId,levelId:v_LevelId};
	...
	getCustLevelDataById : function(cust_id){
		oState.send2 +=1;
		DataProxy.dwrFlag = false;
		CustomerService.getCustomerClass(cust_id,{callback: function(data){
			DataProxy.dwrFlag = true;
			oState.accept2 +=1;
	...
	getAllCustLevel : function(){
		if(DataProxy.paramsObjs.length>0){
			...
			if(DataProxy.dwrFlag){
				DataProxy.getCustLevelArray(v_classId,v_parentId,v_LevelId);
			}
	...
	setDataArray : function(sTreePanel,data){
		...
		while(queryArray.length>0){
			...
			for(var j = 0;(dataArray!=null)&&j<dataArray.length;j++){
				...
				var queryId = classId +"_"+ parentId;
				var tempArray = oState.TEMP_MAP[queryId];
				if(classId==10||classId==11||classId==12||classId==1001||classId==1002){
	...
	updateCustLevel : function(){
		if(DataProxy.paramsObjs2.length>0){
			...
			alert('保存成功！');
			window.location.href='client_class.jsp';
		}
	},
	init : function(cust_id){
		var paramsObj = {classId:0,parentId:0,levelId:1};
		DataProxy.paramsObjs.push(paramsObj);
		oState.send1 +=1;
		DataProxy.getAllCustLevel();
