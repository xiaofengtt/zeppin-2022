...
String isNewFlow =  ConfigUtil.getPropertyNameById("flow_catalog","isnewflow","flow_no",flow_no);
if("1".equals(isNewFlow)) interfaceType_code = "FlowTask050";
...
//设置修改字段后缀名
if("1".equals(isNewFlow)){
	
}else{
	if(!("010".equals(nodeNo) || "300".equals(nodeNo))){
		fieldShowList=ConfigCheck.setFieldValue(fieldShowList,"FLOW_DESC","SHOW_UNIT","<input type='button' value='美化摘要' name='genbutton' class=selectbtn onclick='javascript:beautifyFlowDesc();' tabindex='13'>");
	}
}
...
function selectNodeOpinion()
{
	...
	var returnV = checkLogic("CRMBusinessCheck","checkBusinessInfo",<%=identity_code%>+"@@"+objectType+"@@work");
	//选择意见界面
	var returnValue=showModalDialog('/project/flow/nodeopinion_query.jsp?flow_no='+flowNo+'&node_no='+nodeNo+'&query_type=nodeopinion&checkFlag='+returnV, '', 'dialogWidth:400px;dialogHeight:300px;status:0;help:0')
...
function sucSubmit(){
	...
	//获得初始化流程信息
	if(<%=identity_code%>==0){
		var returnValue=checkLogic("BusinessCheck","getInitFlow",flowNo);
		if(typeof(returnValue)!="undefined" && returnValue.length>0){
			var s=returnValue.split("@@");
			document.theform.NODE_NO.value=s[0];
			document.theform.NODE_NAME.value=s[1];
			document.theform.NODE_FLAG.value=s[2];
		}
	}
...
<table border="0" width="100%">
	<tr>
		<td align="right">
		<button class="xpbutton3" accessKey=s id="btnSave" name="btnSave" onclick="javascript: sucSubmit();">暂存(<u>S</u>)</button>
		&nbsp;&nbsp;
		...
		&nbsp;&nbsp;
		<button class="xpbutton3" accessKey=m id="btnSave" name="btnSave" onclick="javascript: referflow();">提交(<u>M</u>)</button>
		&nbsp;&nbsp;		
		</td>
	</tr>
</table>
