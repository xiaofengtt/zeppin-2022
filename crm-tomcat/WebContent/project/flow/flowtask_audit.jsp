...
{
	var returnValue=showModalDialog('/project/flow/flowobject_tab.jsp?task_id='+identity_code+'&object_type='+object_type+'&flow_no='+flow_no+'&node_no='+node_no+'&show_flag=FlowTask&edit_right=no&object_id='+object_no, '', 'dialogWidth:1000px;dialogHeight:760px;status:0;help:0');
	if(returnValue=="success") location.reload();
}

//--------------退回功能-----------------//
function backflow(taskID,object_no,object_type,flow_no,node_no)
{   
   if(confirm("确定要退回吗？"))
	{
		var returnValue=actionFlow("FlowUtil","untreadFlow",taskID+'@@'+object_no+'@@'+object_type+'@@'+flow_no+'@@'+node_no);
...
//--------------委托功能-----------------//
function consignflow(taskID,object_no,object_type,flow_no,node_no)
{   
   if(confirm("确定要委托吗？"))
	{
		var returnValue=showModalDialog('/system/project/operator_query.jsp', '', 'dialogWidth:400px;dialogHeight:500px;status:0;help:0')
		if(typeof(returnValue)!="undefined" && returnValue!=""){
			s=returnValue.split("@@");
			var userID=s[0];
			var returnValue=actionFlow("FlowUtil","consignFlowTask",taskID+'@@'+object_no+'@@'+object_type+'@@'+flow_no+'@@'+node_no+'@@'+userID);
			if(returnValue=="success"){
				alert("任务委托成功！")
				location.reload();
			}else{
