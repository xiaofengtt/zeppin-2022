...
if(request.getMethod().equals("POST")){//删除信息
	...
	if(s!=null && s.length>0){
		if("submitAll".equals(modiTypeFlag)){
			CRMBusinessCheck.referAll(input_operatorCode+"");
			for(int i = 0;i < s.length; i++){
				CRMBusinessCheck.changeAll(s[i]);//修改要汇总的标志位
			}
			String serailId=Utility.trimNull(ConfigUtil.getSqlResult("select top 1 serail_id from TSTAFFREIMBURSESUM where input_man="+input_operatorCode+" order by  serail_id desc"));

			CRMBusinessCheck.updateReferAll(serailId);
		}else{
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=yes&object_type=<%=object_type%>&object_id=<%=object_id%>&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:400px;status:0;help:0') != null)
	{
		sl_update_ok();
			...
			<TR>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<FONT size="4 " color="red"> 总金额：【<%=totalInfo %>】元</FONT>
							...
							<button class="xpbutton3" accessKey=m id="btnAll" name="btnAll" title="汇总" onclick="javascript:referAll('<%=delStr%>');">生成汇总(<u>M</u>)</button>
							&nbsp;&nbsp;&nbsp;
