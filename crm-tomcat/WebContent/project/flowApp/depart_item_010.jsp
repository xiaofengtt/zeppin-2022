...
sqlContent =sqlContent + " and input_user='"+input_operatorCode+"'";
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=yes&logType='+returnValue+'&object_type=<%=object_type%>&object_id='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:500px;status:0;help:0') != null)
	{
		sl_update_ok();
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&object_type=<%=object_type%>&edit_right=yes&identity_code=0&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:500px;status:0;help:0') != null)
	{
		//sl_update_ok();
		document.theform.submit(); 
...
//--------------权限设置功能-----------------//
function roleInfo(identity_code)
{   
	location='/project/flowApp/itemrole.jsp?item_id='+identity_code
}
</script>
