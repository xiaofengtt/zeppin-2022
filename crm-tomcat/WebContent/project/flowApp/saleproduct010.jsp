...
String object_type="SaleProduct";
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=yes&object_type=<%=object_type%>&object_id='+identity_code+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:1000px;dialogHeight:350px;status:0;help:0') != null)
	{
		//sl_update_ok();
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&object_type=<%=object_type%>&edit_right=yes&identity_code=0&table_code='+tableCode, '', 'dialogWidth:1000px;dialogHeight:350px;status:0;help:0') != null)
	{
		//sl_update_ok();
		document.theform.submit();
  ...
  var checkFlag="1";
	 if(checkFlag=="1")
   {
	   if(confirm("确定要提交吗？"))
		{
			disableAllBtn(true);
			var returnValue=actionFlow("FlowUtil","initFlow",object_id+"@@"+"<%=object_type%>"+"@@"+"SaleProductFlow"+"@@"+"<%=input_operatorCode%>");
