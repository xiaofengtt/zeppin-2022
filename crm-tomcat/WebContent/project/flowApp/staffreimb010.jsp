...
<%
//初始化参数信息
String interfaceType_code = "staffreimb010";
...
if(request.getMethod().equals("POST")){//删除信息
	...
	if(s!=null && s.length>0){
		if("cutDel".equals(modiTypeFlag)){
			
			for(int i = 0;i < s.length; i++){
				CRMBusinessCheck.cutDel(s[i]);//
			}
...
sqlContent =sqlContent+"  and apply_id='"+object_id+"'";
String totalInfo=CRMBusinessCheck.getRelationInfo("StaffReimbSum"+"@@"+sqlContent);
...
<%if(bSuccess && "cutDel".equals(modiTypeFlag)){
	bSuccess = false;
%>
	alert("去除成功");
<%}%>
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=<%=edit_right%>&object_type=<%=object_type%>&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:400px;status:0;help:0') != null)
	{
		sl_update_ok();
...
function newInfo()
{	
	if(showModalDialog('/project/flowApp/staffreimb030.jsp?object_id=<%=object_id%>', '', 'dialogWidth:800px;dialogHeight:450px;status:0;help:0')!= null){
		document.theform.submit();
	}
	
}
//----------去除------------
function cutDel(AllId)
{
	...
	if(confirm("确定要去除信息吗？")){
		disableAllBtn(true);
		document.getElementById('modiTypeFlag').value='cutDel';
		document.theform.submit();
	}
}
//--------------提交审批功能-----------------//
		...
		<TABLE cellSpacing=0 cellPadding=4 width="100%" align=center border=0>
			<TR>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td align="center"><FONT size="4 " color="red"> 总金额：【<%=totalInfo %>】元</FONT></td>
						<td align="right">
							<div class="btn-wrapper">
							<button class="xpbutton3" accessKey=f id="queryButton" name="queryButton">查询(<u>F</u>)</button>
							&nbsp;&nbsp;&nbsp;
							<%if(edit_right.equals("yes")){%>
							<button class="xpbutton3" accessKey=n id="btnNew" name="btnNew" title="新建记录" onclick="javascript:newInfo();">添加(<u>N</u>)</button>
							&nbsp;&nbsp;&nbsp;
							<button class="xpbutton3" accessKey=m id="btnNew" name="btnNew" title="去除" onclick="javascript:cutDel('<%=delStr%>');">去除(<u>M</u>)</button>
							&nbsp;&nbsp;&nbsp;
							<%} %> 
							</div>
						</td>
