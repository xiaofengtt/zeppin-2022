...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=yes&logType='+returnValue+'&object_type=<%=object_type%>&object_id='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:900px;dialogHeight:700px;status:0;help:0') != null)
	{
		//sl_update_ok();
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&object_type=<%=object_type%>&edit_right=yes&identity_code=0&table_code='+tableCode, '', 'dialogWidth:300px;dialogHeight:150px;status:0;help:0') != null)
	{
		//sl_update_ok();
		document.theform.submit();
		var returnValue=checkLogic("CRMBusinessCheck","getObjectKeyID","<%=table_code%>"+"@@"+"<%=identityCode%>"+"@@"+"<%=input_operatorCode%>");
...
{   
   ...
   var checkFlag=showModalDialog('/project/flow/checkerror_info.jsp?check_flag=DiscInfoCheck&object_type=WorkLogFlow&object_id=' + object_id, '', 'dialogWidth:560px;dialogHeight:450px;status:0;help:0');
   if(checkFlag=="1")
   {
	   if(confirm("确定要提交吗？"))
		{
			disableAllBtn(true);
			var returnValue=actionFlow("FlowUtil","initFlow",object_id+"@@"+"<%=object_type%>"+"@@"+"WorkLogFlow"+"@@"+"<%=input_operatorCode%>");
