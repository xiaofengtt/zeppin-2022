...
if(request.getMethod().equals("POST")){
	...
	preVO.setSub_product_id(sub_product_id);
...
function changeProduct(value){
	var v1=value.split("|");
	var product_id=v1[1];
	document.theform.s_product_id.value = product_id;
	document.theform.product_id.value = product_id;
	if(product_id>0){
		utilityService.getSubProductFlag(product_id,getSubFlag);
	}
	document.theform.s_product_code.value = v1[0];
...
function getSubFlag(data){
	...
	if(is_product_sub_flag==1) //有子产品
	{
		...
		var p_id = document.theform.s_product_id.value;
		utilityService.getSubProductOptionS(p_id,0,getSubProductOption);//获得子产品结果集
	}else{ //无子产品的
...
<input type="hidden" name="s_product_id" value="">
<div align="left">
	<br>
	<div>
		<table border="0" width="100%" cellspacing="0" cellpadding="1">
			<tr>
				...
				<td><button type="button"  class="xpbutton3" accessKey=q onclick="javascript:queryInfo();">搜索 (<u>Q</u>)</button>	&nbsp;<!--搜索--></td>							
			</tr>
			<tr>
				<td colspan="5"><br></td>
			</tr>
			<tr>
				...
				<td colspan="3">
					...
					<%
						...
						while(iterator.hasNext()){	
							...
							product_id_s = Utility.trimNull(map.get("PRODUCT_ID"));
					%>
						<option value="<%=product_code%>|<%=product_id_s%>" <%if(product_code.equals(s_product_code)){%>selected<%}%>>						
							<%=product_name%>	
						</option>
					<%
						}
				...
				<td><button type="button"  class="xpbutton3" accessKey=q onclick="javascript:if(document.theform.onsubmit()) document.theform.submit();">匹配 (<u>P</u>)</button>	&nbsp;</td>							
			</tr>
			<tr id="sub_product_flag" style="display:<%if (sub_product_id.intValue()==0) out.print("none");%>">
				<td align="right" width="120px"><font color="red"><b>*</b></font><%=LocalUtilis.language("class.subProductID",clientLocale)%>:</td><!--子产品选择-->
				<td align=left id="select_id_2" colspan="4">
				    <select name="sub_product_id" id="sub_product_id">
				        <option value="0">请选择</option>
						<%if (sub_product_id.intValue()>0) out.println(Argument.getSubProductOptions(product_id, new Integer(0), sub_product_id)); %>
				    </select>
				</td>
			</tr>		
		</table>		
