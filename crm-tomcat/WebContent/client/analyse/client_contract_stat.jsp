...
sUrl += "&"+Utility.getQueryString(request, new String[]{"showFlag","q_cust_type","q_group_id","q_class_detail_id","q_productCode"
													 ,"q_productId","q_stat_scope","q_rg_date_start","q_rg_date_end"}); 
...
vo.setFunc_id(new Integer(0)); // 统计客户经理的客户汇总
...
String qs = Utility.separateQueryString(request,"~"
				,new String[]{"page","pagesize","showFlag","q_cust_type","q_group_id"
				,"q_class_detail_id","q_productCode","q_productId","q_stat_scope"
				,"q_rg_date_start","q_rg_date_end"});

/*
String qs = t_sPage + "$"
		  + Utility.trimNull(request.getParameter("pagesize")) + "$"
		  + Utility.trimNull(request.getParameter("showFlag")) + "$"
		  + Utility.trimNull(request.getParameter("q_cust_type")) + "$"
		  + Utility.trimNull(request.getParameter("q_group_id")) + "$"
          + Utility.trimNull(request.getParameter("q_class_detail_id")) + "$"
		  + Utility.trimNull(request.getParameter("q_productCode")) + "$"
		  + Utility.trimNull(request.getParameter("q_productId")) + "$"
		  + Utility.trimNull(request.getParameter("q_stat_scope")) + "$"
          + Utility.trimNull(request.getParameter("q_rg_date_start")) + "$"
		  + Utility.trimNull(request.getParameter("q_rg_date_end"));*/
...
function refreshPage(page){
	...
	location.search = "?page="+(page?page:1)+"&pagesize="+ document.theform.pagesize.value
...
function detail() {
	location.href = "client_contract_stat0.jsp?from=<%=qs%>&page=<%=sPage%>&pagesize="+ document.theform.pagesize.value
						+ "&q_cust_type=" + document.getElementById("q_cust_type").value
						+ "&q_stat_scope=" + document.getElementById("q_stat_scope").value
						+ "&q_productId=" + document.getElementById("q_productId").value
						+ "&q_group_id=" + document.getElementById("q_group_id").value	
...
<div id="headDiv" class="btn-wrapper">
	<TABLE cellSpacing=0 cellPadding=0 width="100%" border="0" >
		<TBODY>
			<TR>											
				<!--列表-->
                <TD id="d1" style="background-repeat: no-repeat;font-size:13;font-family:微软雅黑;display:none" onclick=show(1) vAlign=bottom width=60 height=20 
					background='<%=request.getContextPath()%>/images/headdark_00_01.gif'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=LocalUtilis.language("message.list",clientLocale)%> </TD>
				<!--图表-->
                <TD id="d2" style="background-repeat: no-repeat;font-size:13;font-family:微软雅黑;display:none" onclick=show(2) vAlign=bottom width=60 height=20 
					background='<%=request.getContextPath()%>/images/headdark_00_01.gif'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=LocalUtilis.language("message.icon",clientLocale)%> </TD>	
				...
				<%if (input_operator.hasFunc(menu_id, 107)) {%>
                    ...
                    <button type="button"  class="xpbutton3" accessKey=r id="reportButton" name="reportButton" title='<%=LocalUtilis.language("message.report",clientLocale)%>' 
						onclick="javascript:window.open('<%=request.getContextPath()%>/webreport/noparam_init.jsp?filename=/webreport/Cells/106_0.cll&cllParamStr=<%=cllParamStr%>')"><%=LocalUtilis.language("message.report",clientLocale)%> (<u>R</u>)</button>
				<%}%>
				</TD>				
			</TR>
		</TBODY>
			...
			<td>
				<table cellSpacing="1" cellPadding="2"  bgcolor="#CCCCCC">
					<tr style="background:F7F7F7;">
						...
						<!--个人-->
	                    <td width="90px" align="center" id="td1" <%if (q_cust_type.intValue()==1) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="javascript:changeCustType(1);" class="a2">个人客户</a></font></td>
						<!--机构-->
	                    <td width="90px" align="center" id="td2" <%if (q_cust_type.intValue()==2) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="javascript:changeCustType(2);" class="a2">机构客户</a></font></td>
					</tr>
				</table>
			</td>
			<td>
				<table cellSpacing="1" cellPadding="2" bgcolor="#CCCCCC">
					<tr style="background:F7F7F7;">				
						<td width="60px" align="center" id="td_s0" <%if (q_stat_scope.intValue()==0) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="javascript:changeStatScope(0);" class="a2">所有</a></font></td>
...
<style>
.qcMain{left:auto!important; right:30px;}
...
<div  id="r1"  align="left"  style="display:none;">
	<table border="0"  width="99%" cellspacing="1" cellpadding="2"	class="tablelinecolor">
		<tr class="trh">
			<td align="center" width="20%"><%=LocalUtilis.language("class.accountManager",clientLocale)%></td> <!-- 客户经理 -->
			<td align="center" width="10%">所属团队</td>
			<td align="center" width="10%">总客户认购次数</td>
			<td align="center" width="*">总客户认购金额</td>
			<td align="center" width="*">总客户存量金额</td>
			<td align="center" width="*">总客户受益份额</td>
		</tr>
	<%
	...
	while(iterator.hasNext()){
		...
		Integer op_code = Utility.parseInt(Utility.trimNull(map.get("SERVICE_MAN")),new Integer(0));
		boolean self = op_code.intValue()==input_operatorCode.intValue();
		String op_name = Utility.trimNull(map.get("OP_NAME"));
		Integer rg_times = Utility.parseInt(Utility.trimNull(map.get("RG_TIMES")),new Integer(0));
		BigDecimal rg_money =Utility.parseDecimal( Utility.trimNull(map.get("RG_MONEY")),new BigDecimal(0.00));
		BigDecimal current_money = Utility.parseDecimal( Utility.trimNull(map.get("CURRENT_MONEY")),new BigDecimal(0.00));
	...
	%>			
			<tr class="tr<%=iCount%2%>">
				...
				<%if (self) { %>
					<a href="javascript:detail();"><%=op_name%></a></td>             
				<%}else{ %>
					<input type="text" style="width:100%;" class="ednone" value="<%=op_name%>" readonly>
				<%} %>
				 <td align="center" width="10%"><%=Utility.trimNull(map.get("TEAM_NAME"))%></td>        
				 <td align="center" width="10%"><%=rg_times%></td>        
				 <td align="right" width="*"><%=Format.formatMoney2(rg_money)%></td>   
				 <td align="right" width="*"><%=Format.formatMoney2(current_money)%></td>    
				 <td align="right" width="*"><%=Format.formatMoney2(ben_amount)%></td>    
			</tr>
	<%
	}
	...
	<%}%> 
			...
			<tr class="trbottom">
				<!--合计--><!--项-->
                <td class="tdh" align="left"><b>&nbsp;<%=LocalUtilis.language("message.total",clientLocale)%>&nbsp;<%=pageList.getTotalSize()%>&nbsp;<%=LocalUtilis.language("message.entries",clientLocale)%> </b></td>		
	            <td align="center"></td>				
