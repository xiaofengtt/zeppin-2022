...
List list = custStatLocal.getCategoryStat(vo);
...
String fileName1 = "客户类别分布-人数占比";
String fileName2 = "客户类别分布-金额占比";
for(int i=0;i<list.size();i++) {
	Map map = (Map)list.get(i);
	if(Utility.parseInt(Utility.trimNull(map.get("CUST_TYPE")),1) == 1){
		ItemName1[i] = LocalUtilis.language("class.personal",clientLocale);
		ItemName2[i] = LocalUtilis.language("class.personal",clientLocale);
	}else{
		ItemName1[i] = LocalUtilis.language("message.organization",clientLocale);	
		ItemName2[i] = LocalUtilis.language("message.organization",clientLocale);	
	}
	ItemValue1[i] = Utility.trimNull("");
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
	<tr>
		<td align="left">
			<table  border="0"  width="440" cellspacing="1" cellpadding="2"	class="tablelinecolor">
				<tr class="trh">
					<td><%=LocalUtilis.language("class.levelID2",clientLocale) %></td><!-- 类别 -->
...
%>	
				<tr class="tr<%=inCount%2 %>">
					<td><%if(Utility.parseInt(Utility.trimNull(map.get("CUST_TYPE")),1)==1) out.print(LocalUtilis.language("class.personal",clientLocale)); 
					else out.print(LocalUtilis.language("message.organization",clientLocale));%></td>
