			...
			while(it.hasNext()){
				...
				<div <%if(print_deply_bill.intValue()==0){%>class="Noprint"<% }%>>
					...
					<tr>
						...
						<td align="right"><%=Utility.trimNull(Format.formatMoney(Utility.stringToDouble(Utility.trimNull(ben_map.get("BEN_AMOUNT")))))%></td>
						<td align="center"><%=Format.formatDateCn(Utility.parseInt(Utility.trimNull(ben_map.get("BEN_DATE")), new Integer(0)))%></td>
						<td align="right"><%=Format.formatMoney(Utility.stringToDouble(Utility.trimNull(ben_map.get("SY_MONEY"))))%></td>
