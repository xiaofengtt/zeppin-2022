...
if(serial_no.intValue()>0){
	...
	if(map_service!=null){	
		...
		step_flag = Utility.trimNull(map_service.get("STEP_FLAG"),"0"); 
		info_level = Utility.trimNull(map_service.get("INFO_LEVEL"),"0"); 
		executor = Utility.parseInt(Utility.trimNull(map_service.get("EXECUTOR")),new Integer(0));
		service_man2 = Utility.parseInt(Utility.trimNull(map_service.get("SERVICE_MAN")),new Integer(0));
	}
}
//客户信息详细
...
if(request.getMethod().equals("POST")){
	...
	custServiceLogVO.setSerialNo(serial_no);
	...
	custServiceLogLocal.modi(custServiceLogVO);
	bSuccess = true;
}
%>
...
window.onload = function(){
	...
	if(v_bSuccess=="true"){		
		sl_alert("客户维护记录编辑成功！");//客户维护记录添加成功
...
//用来取值,设值的 hesl 2011-05-18
	/*function onloadInfo(){
		try{
			var _serviceTime = document.getElementsByName("serviceTime")[0];
			var _serviceInfo=document.getElementsByName("serviceInfo")[0];
			var _serviceSummary=document.getElementsByName("serviceSummary")[0];
			var _serial_no=document.getElementsByName("serial_no")[0];
			_serviceTime.setAttribute("value",oState.data.serviceTime);
			_serviceInfo.setAttribute("value",oState.data.serviceInfo);
			_serviceSummary.setAttribute("value",oState.data.serviceSummary);
...
function refreshPage(){
	disableAllBtn(true);
	document.theform.action="service_list.jsp";
...
function getMarketingCustomer(prefix,readonly){
	...
	var url = '<%=request.getContextPath()%>/marketing/customerInfo.jsp?readonly=1&select_flag=2&prefix='+ prefix+ '&cust_id=' + cust_id+'&readonly='+readonly ;
...
<form name="theform" method="post" action="service_edit.jsp" onsubmit="javascript:return validateForm(this);">
...
<table cellSpacing=0 cellPadding=4 width="100%" border=0>
			<tr>
				...
				<td align="left">
					<button type="button"  class="xpbutton3" accessKey=e name="btnEdit" title="<%=LocalUtilis.language("enu.customerInformation",clientLocale)%> " onclick="javascript:getMarketingCustomer('customer','0');">查看 
					</button><!--客户信息--><!--请选择-->
				</td>
			<tr>
			<tr>
