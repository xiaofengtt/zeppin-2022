...
int level_id = Utility.parseInt(request.getParameter("level_id"), 1);//节点数
Integer detail_id = Utility.parseInt(request.getParameter("detail_id"), new Integer(0));//分级ID

int isNotFlag = Utility.parseInt(request.getParameter("isNotFlag"),-1);
...
System.out.println("____define_id:"+define_id+"__detail_id:"+detail_id);	
//保存客户分级信息
if(request.getMethod().equals("POST"))
{
	...
	if(isNotFlag==1)
	{
		...
		int class_define_id = 0;
			...
			define_vo.setLevel_id(new Integer(0));//一级菜单
...
//		}
		if(class_define_id != 0)
		{
			...
			define_vo.setParent_id(define_id);
			define_vo.setParent_value(detail_id);
			define_vo.setCD_no(new Integer(2));
...
{	
	...
	if(document.theform.isFlag[0].checked)
		document.theform.isNotFlag.value = 1;
	else
		document.theform.isNotFlag.value = 2;
...
<input type="hidden" name="isNotFlag" value="">
...
<div align="center">
	<table border="0" width="100%" cellspacing="5" cellpadding="0">
		...
		<tr>
			<td align="right">是否为末级: </td>
			<td>
				<input type="radio" name="isFlag" value="1" checked />分类&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" name="isFlag" value="2" />分类值
			</td>
		</tr>
