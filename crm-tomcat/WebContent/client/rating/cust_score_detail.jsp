...
Integer scoring_date = Utility.parseInt(request.getParameter("scoring_date"),new Integer(0));
...
vo.setScoring_date(scoring_date);
vo.setInput_man(input_man);
IPageList pageList = rating.pageList_tcustscoredetail(vo, totalColumn, t_sPage, t_sPagesize);

sUrl = sUrl+ "&subject_id =" +subject_id+ "&operand_id =" +operand_id
		+ "&cust_no =" +cust_no+ "&cust_name =" +cust_name+ "&scoring_date =" +scoring_date;
%>
...
{
	disableAllBtn(true);
	syncDatePicker(document.theform.scoring_date_picker, document.theform.scoring_date);
	location = 'cust_score_detail.jsp?page=<%=sPage%>&pagesize=' + document.theform.pagesize.value 
							...
							+ '&scoring_date=' + document.theform.scoring_date.value;
}
...
<BODY class="BODY" >
<form name="theform" method="POST" action="cust_score_detail.jsp">
			...
			<table border="0" width="100%" cellspacing="2" cellpadding="2">
				...
				<tr>
					...
					<td align="left">
						...
						<%=Argument.getOperandOptions(operand_id,new Integer(0),new Integer(0))%>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td align="left" colspan="3">
						<input TYPE="text" style="width:120" NAME="scoring_date_picker" class=selecttext value="<%=Format.formatDateLine(scoring_date)%>" onKeyDown="javascript:nextKeyPress(this)"> 
				        <input TYPE="button" value="▼" class=selectbtn onClick="javascript:CalendarWebControl.show(theform.scoring_date_picker,theform.scoring_date_picker.value,this);" onKeyDown="javascript:nextKeyPress(this)"> 
			...
			<TR>
				...
				<table id="table3" border="0" cellspacing="1" cellpadding="2"
					...
					<tr class=trtagsort>
						...
						<td align="center"><%=LocalUtilis.language("class.scoringDate",clientLocale)%></td><!--打分日期-->
						<td align="center"><%=LocalUtilis.language("class.score",clientLocale)%></td><!--分值-->
...
%>
					<tr class="tr<%=(iCurrent % 2)%>">
						...
						<td align="center"><%=Format.formatDateLine(Utility.parseInt(Utility.trimNull(map.get("SCORING_DATE")),new Integer(0))) %></td>
						<td align="right"><%=Utility.trimNull(map.get("CUST_SOURCE")) %>&nbsp;</td>
