...
if (request.getMethod().equals("POST")){	
	...
	try{
		...
		out.println("<script language=\"javascript\">alert(\""+e.getMessage()+"！"+""+"\");window.returnValue=null;window.close();</script>");
		bSuccess = false;
	}
...
function validateForm(form) {
	if(!<%=Argument.checkRoleIdByOpCode(input_operatorCode, 4) %> && document.theform.card_type.value=='110802'){
		sl_alert("证件类型不能为护照");//证件类型不能为护照
	...
	if (event.keyCode == 13||event.keyCode == 9){
		if(document.theform.cust_type.value==1)
		{
			if(document.theform.birthday_picker.value!="")
			{
				if(!sl_checkDate(document.theform.birthday_picker,"<%=LocalUtilis.language("class.birthday",clientLocale)%> "))	return false;//出生日期
				var current_date = new Date();	
				if(current_date.getYear() >= 2000)
				{				
					document.theform.age.value = current_date.getYear() - document.theform.birthday_picker.value.substring(0,4);			
				}
				else
				{
					document.theform.age.value = 1900 + current_date.getYear() - document.theform.birthday_picker.value.substring(0,4);			
...
<div class="page-title-select" style="margin-left:40px;margin-top:5px;">
...
<div  id="r2" align="left"  style="display:none;">
	<table border="0" width="100%" cellspacing="2" cellpadding="2" class="table-popup">
			...
			<tr>
				<td align="right"><span style="color:red">*</span><%=LocalUtilis.language("class.customerHTel",clientLocale)%> ：</td><!--家庭电话-->
				<td><input name="h_tel" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%> onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_H_TEL"))%>" /></td>
				<td align="right"><%=LocalUtilis.language("class.companyPhone",clientLocale)%> ：</td><!--公司电话-->
				<td><INPUT name="o_tel" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%> onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_O_TEL"))%>" /></td>
			</tr>
			
			<tr>
				<td align="right"><span style="color:red">*</span><%=LocalUtilis.language("class.customerMobile",clientLocale)%> ：</td><!--手机-->
				<td><input name="mobile" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%> onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_MOBILE"))%>" /></td>
				<td align="right"><%=LocalUtilis.language("class.customerMobile",clientLocale)%> 2：</td><!--手机-->
				<td><INPUT name="bp" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%> onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_BP"))%>" /></td>
			</tr>
...
<div  id="r3" align="left"  style="display:none;">
	...
	<tr>
		<td>
			<table  border="0" width="100%" cellspacing="0" cellpadding="3" style="border: 1px; border-style: dashed; border-color: blue; margin-top:5px;">	
			<tr>
				<td colspan = "4"><b><%=LocalUtilis.language("message.estateStatus",clientLocale)%> </b></td>  <!--房产状况-->
			</tr>
			<tr>
