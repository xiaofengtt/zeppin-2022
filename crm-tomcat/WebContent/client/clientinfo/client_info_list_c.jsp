...
<table width="99%" cellspacing="0" cellpadding="2">
	...
	<tr>
		<td align="right" width="20%">客户联系方式修改申请状态:</td>
...
<!-- 							<button type="button"  class="xpbutton5" title="真伪设置"  onclick="javascript:setTrueFlag();">真伪设置 </button> -->
<!-- 							&nbsp;&nbsp;&nbsp; -->
<%-- 						<%} %> --%>
<!-- 							导入客户--><!-- 导入 -->
<%-- 						<%if (input_operator.hasFunc(menu_id, 100)) {%> --%>
...
<%-- 						<%}%> --%>
				...
				<table cellSpacing="1" cellPadding="2" style="margin-top:5px" class="table-select" >
						<tr style="background:F7F7F7;" style="margin-top:5px">
							...
							<!--全部-->
							<td width="50px" align="center" id="td1" <%if (cust_type.intValue()==1) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeCustType(1);" class="a2"><%=LocalUtilis.language("class.personal",clientLocale)%></a></font></td>
							<!--个人-->
							<td width="50px" align="center" id="td2" <%if (cust_type.intValue()==2) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeCustType(2);" class="a2"><%=LocalUtilis.language("message.organization",clientLocale)%></a></font></td>
							<!--机构-->
...
<%}%>
						...
						<td align="center">修改明细</td><!--修改明细-->	
						<td align="center">是否确认/审核</td><!--修改明细-->	
					</tr>
			<%
				int iCount = 0;
				int iCurrent = 0;
							...
							<div id="detailsDiv<%=map.get("CUST_ID")%>" style="overflow-y:auto;" align="center">
								<table id="detailsTable<%=map.get("CUST_ID")%>" class="tablelinecolor-inner" border="0" width="100%" bgcolor="#000000" cellspacing="1">
									<tr style="background:F7F7F7;" >
										<td class="text-right"><%=LocalUtilis.language("class.companyPhone",clientLocale)%> ：</td><!--公司电话-->
										<td  width="*"><a href="#" class="a2" onclick="javascript:callCust(<%=map.get("CUST_ID")%>)"><%=Utility.trimNull(map.get("O_TEL"))%></a></td>
									</tr>
									<tr style="background:F7F7F7;" >
										<td class="text-right"><%=LocalUtilis.language("class.mobile",clientLocale)%> ：</td><!--手机号码-->
										<td  width="*"><a href="#" class="a2" onclick="javascript:callCust(<%=map.get("CUST_ID")%>)"><%=Utility.trimNull(map.get("MOBILE"))%></a></td>
									</tr>
									<tr style="background:F7F7F7;" >
										<td class="text-right"><%=LocalUtilis.language("class.email",clientLocale)%> ：</td><!--电子邮件-->
										<td  width="*"><%=Utility.trimNull(map.get("E_MAIL"))%></td>
									</tr>
									<tr style="background:F7F7F7;">
										<td  width="120px" class="text-right"><%=LocalUtilis.language("class.postcode",clientLocale)%> ：</td><!--邮政编码-->
					...
					<tr id="detailsTr1<%=map.get("CUST_ID")%>" style="display: none" bgcolor="#FFFF99">
						<td class="tdh" colspan="17">
							<table id="detailsTable1<%=map.get("CUST_ID")%>" bgcolor="#000000" style="display: none" border="0" cellpadding="2" cellspacing="1" width="100%">
								<tr>
									<td align="center" bgcolor="#FFFFFF">修改后手机号</td>
									<td align="center" bgcolor="#FFFFFF">登记时间</td>
									<td align="center" bgcolor="#FFFFFF">录入人</td>
									<td align="center" bgcolor="#FFFFFF">状态</td>
									<td align="center" bgcolor="#FFFFFF">确认人</td>
									<td align="center" bgcolor="#FFFFFF">确认时间</td>
									...
									<td align="center" bgcolor="#FFFFFF">确认</td>
								...
								%>
									<tr>
										...
										<%if (isModi) { //可编辑%>
												...
												<img src="<%=request.getContextPath()%>/images/edit.gif" width="16" height="16" title="编辑客户联系方式修改信息"/></a>
										<%} else { %>
										...
										<td bgcolor="#FFFFFF" align="center">
											...
											<img src="<%=request.getContextPath()%>/images/search.gif" width="16" height="16" title="查看客户联系方式修改信息"/></a>
										</td>										
										<td bgcolor="#FFFFFF" align="center">
										<%if (input_operator.hasFunc(menu_id, 100)){ %>	
										<%if (isCheck) { //可确认%>
												<a href="#" onclick="javascript:checkInfo('<%=Utility.trimNull(preMap.get("SERIAL_NO"))%>','<%=map.get("CUST_NAME")%>');">					 	
												<img src="<%=request.getContextPath()%>/images/check.gif" width="16" height="16" title="确认客户联系方式修改信息"/></a>
										<%} else { %>
										...
										<%if (isRecheck) { //可审核%>
												...
												<img src="<%=request.getContextPath()%>/images/check.gif" width="16" height="16" title="审核客户联系方式修改信息"/></a>
										<%} else { %>
