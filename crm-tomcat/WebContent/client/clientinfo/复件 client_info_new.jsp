...
if (request.getMethod().equals("POST")){	
	...
	file.parseRequest();
	file.uploadCustAttchment(cust_id, "", input_operatorCode);*/
	// 逐一提取上传文件信息，同时可保存文件。 
	for (int i = 0; i < su.getFiles().getCount(); i++) {
		com.jspsmart.upload.File file = su.getFiles().getFile(i);
		// 若文件不存在则继续 
...
<%//if(user_id.intValue()==17/*方正*/ || user_id.intValue()==22/*中投*/ ||
	//	user_id.intValue()==999) { %>//方正东亚需求在界面录入客户名称时动态判断，开启该功能时数据量大时明显会慢很多
	alert33("ok1");jsAutoInstance.display();alert("ok2");
<%//}%>
}
...
<%} else { %>
	<input name="cust_name" onkeydown="javascript:nextKeyPress(this)" maxlength="200" size="20"
		value="<%=Utility.trimNull(map.get("CUST_NAME"))%>" onkeyup="javascript:test(this);" onfocus="javascript:focused(this)" onblur="blured2()"/>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<font size="2" face="宋体"><%=LocalUtilis.language("class.custType",clientLocale)%> ：</font><!--客户类型-->
