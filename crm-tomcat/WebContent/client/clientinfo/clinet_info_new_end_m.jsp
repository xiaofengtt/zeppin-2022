...
if (request.getMethod().equals("POST")){
	...
	try{
		...
		if (cust_id.intValue() == 0){
			...
			if(is_deal.intValue()==1){
				if (user_id.intValue()!=15) customer_inst.cope_customers_m(cust_vo); //建信CRM的客户信息不同步给INTRUST
				cust_vo.setComplete_flag(new Integer(2)); //客户信息不完整
				cust_local.modify_m(cust_vo);
			}
			else if(is_deal.intValue()==2){
				cust_vo.setComplete_flag(new Integer(2)); //客户信息不完整
				cust_local.modify2_m(cust_vo);
			}
...
if (cust_id != null && cust_id.intValue() != 0){
	cust_vo.setCust_id(cust_id);
	cust_vo.setInput_man(input_operatorCode);
	list = cust_local.listProcAll_m(cust_vo);
...
function changeCustType(cust_type) {
	if (cust_type=="1") {
		document.getElementById("reload").href = "client_info_new_m.jsp?cust_id=<%=cust_id%>"; 
		document.getElementById("reload").click();
	} else if (cust_type=="2") {
		document.getElementById("reload").href = "clinet_info_new_end_m.jsp?cust_id=<%=cust_id%>"; 
		document.getElementById("reload").click();
	}
}
...
<form name="theform" method="post" action="clinet_info_new_end_m.jsp" enctype="multipart/form-data">
...
<a style="display:none" href="clinet_info_new_end_m.jsp?cust_id=" id="reload"></a>
