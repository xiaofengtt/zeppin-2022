	...
	try{
		...
		if (cust_id.intValue() == 0){
			if(is_deal.intValue()==1){
				...
				cust_vo.setComplete_flag(new Integer(2)); //客户信息不完整
				new_cust_id =  cust_local.append(cust_vo);//必须同步到业务系统，不然两边客户编号会重复
			}
			...
			if(is_deal.intValue()==1){
				...
				cust_vo.setComplete_flag(new Integer(2)); //客户信息不完整
				cust_local.modify(cust_vo); //必须同步到业务系统，不然两边客户编号会重复
			}
...
function validateForm(form) {
	...
	//手机号码必填 长度为11的数字
	<%if (cust_id==null || "".equals(cust_id) || cust_id.intValue()==0){//添加%>
...
function checkfolder(folder_no){
	...
	if (folder_no == 1) {
		...
		form.print_post_info.value= form.print_post_info.checked? 1: 0;
			...
			if (!sl_check(form.contact_man, "<%=LocalUtilis.language("class.contact",clientLocale)%> ", 20, 1)) return false;//联系人
			...
			if(form.touch_type.value=="110905" || form.touch_type.value=="110906"){
				if(!sl_checkNum(form.mobile, "<%=LocalUtilis.language("class.mobile",clientLocale)%> ", 30, 1))	return false;//手机号码
				if(!(form.mobile.value.length==11)){
					sl_alert("手机号码必须为11位");//手机号码必须为11位
					form.mobile.focus();
...
<div class="page-title-select" style="margin-left:40px">
...
<div id="r1" align="center" style="display:none;">
	<table border="0" width="100%" cellspacing="3" cellpadding="3" class="table-popup">
		<tr>
		...
		<tr>
			...
			<td align="left">
				...
				<button type="button"  class="xpbutton3" onclick="javascript:document.theform.card_valid_date_picker.value='2500-01-01';">长期有效</button>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><select size="1" name="service_man" disabled="disabled" onkeydown="javascript:nextKeyPress(this)" style="WIDTH: 125px">
		...
		<tr id="forbidFileTr" style="display:<%=Utility.trimNull(map.get("STATUS")).equals("112806")?"":"none"%>">
			...
			<%
				...
				<tr><td>
					...
					&nbsp;<button type="button"  class="xpbutton2" onclick="javascript:deleteAttachment(this,<%=attaId%>);">删除</button>
...
<div  id="r5" align="center" style="display:none;">
	<table border="0" width="100%" cellspacing="3" cellpadding="3" class="table-popup">
		<tr>
...
<div  id="r2" align="center" style="display:none;">
	<table border="0" width="90%" cellspacing="3" cellpadding="3" class="table-popup">
		<tr>
			<td align="right"><span style="color:red">*</span><%=LocalUtilis.language("class.contact",clientLocale)%> ：</td><!--联系人-->
			...
			<td><input name="h_tel" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%>  onkeydown="javascript:nextKeyPress(this)"  size="20" value='<%=Utility.trimNull(map.get("H_H_TEL")).trim()%>'></td>
		</tr>
		...
		<tr>
			...
			<td><input name="mobile" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%> onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_MOBILE")).trim()%>" /></td>
			<td align="right"><%=LocalUtilis.language("class.customerMobile",clientLocale)%> 2：</td><!--手机2-->
			<td><INPUT name="bp" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%>  onkeydown="javascript:nextKeyPress(this)" size="20"  value="<%=Utility.trimNull(map.get("H_BP")).trim()%>" /></td>
		</tr>

		<tr>
			<td align="right"><%=LocalUtilis.language("class.companyPhone",clientLocale)%> ：</td><!--公司电话-->
			<td><INPUT name="o_tel" <%if (cust_id!=null&&!"".equals(cust_id)&&cust_id.intValue()!=0){%>readonly="readonly"<%}%>  onkeydown="javascript:nextKeyPress(this)" size="20" value="<%=Utility.trimNull(map.get("H_O_TEL")).trim()%>" /></td>
...
<div  id="r3" align="left"  style="display:none;">
...
<div  id="r4" align="left"  style="display:none;">
	<table border="0" width="100%" cellspacing="2" cellpadding="2" class="table-popup">
			<tr>
...
<div align="right" style="margin-right:5px;">
	...
	&nbsp;&nbsp;
	<button type="button"  class="xpbutton3" accessKey=b id="btnReturn" name="btnReturn" onclick="javascript:window.close();"><%=LocalUtilis.language("message.close",clientLocale)%> (<u>B</u>)
	</button><!--关闭-->
</div>
<br>
</form>
