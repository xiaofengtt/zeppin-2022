...
gt_vo.setCheck_flag(new Integer(11));//审核标志(只显示未审核的记录)
...
tempUrl = "&grade_id=" + grade_id + "&cust_name=" + cust_name + "&start_trade_date=" + start_trade_date
			+ "&end_trade_date=" + end_trade_date + "&product_id=" + product_id;
sUrl = sUrl + tempUrl;
...
{	
	...
	location = 'client_mark_grade_check_list.jsp?page=<%=sPage%>&pagesize=' + document.theform.pagesize.value 
		+'&grade_id=' + document.theform.grade_id.value+'&cust_name=' + document.theform.cust_name.value
		+'&start_trade_date=' + document.theform.start_trade_date.value+'&end_trade_date=' + document.theform.end_trade_date.value
...
function check_grade(check_flag)
{
	...
	if (selcount > 0)
	{
		if(check_flag=="1"){
			...
			if(confirm("<%=LocalUtilis.language("message.auditWithout",clientLocale)%> ？")){//确认审核不通过吗
				disableAllBtn(true);
		...
		sl_alert("<%=LocalUtilis.language("message.auditTip",clientLocale)%> ！");//请选择要审核的记录
}
...
<form name="theform" method="post" action="client_mark_grade_check.jsp">
			...
			<TR>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<!--审核评级--><!--审核通过-->
                        <button type="button"  class="xpbutton3" accessKey=n id="btnPass" name="btnPass"  title='<%=LocalUtilis.language("message.auditRating",clientLocale)%>' onclick="javascript:check_grade('1');"><%=LocalUtilis.language("message.pass",clientLocale)%> </button>
						&nbsp;&nbsp;&nbsp; <%}%> <%if (input_operator.hasFunc(menu_id, 101)) {%>
						<!--审核评级--><!--审核不通过-->
                        <button type="button"  class="xpbutton3" accessKey=n id="btnUnPass" name="btnUnPass"  title='<%=LocalUtilis.language("message.auditRating",clientLocale)%>' onclick="javascript:check_grade('2');"><%=LocalUtilis.language("message.notPass2",clientLocale)%> </button>
						<%}%>
					</div>
					</td>
					</tr>
				...
				<table id="table3" border="0" cellspacing="1" cellpadding="2"class="tablelinecolor" width="100%" sort="multi">
					...
					while (it.hasNext()) {
						map = (Map)it.next();
						String checkFlag=enfo.crm.tools.LocalUtilis.language("message.unaudited",clientLocale);//未审核
						if(Utility.parseInt(Utility.trimNull(map.get("CHECK_FLAG")),new Integer(0)).equals(new Integer(2))){
							checkFlag=enfo.crm.tools.LocalUtilis.language("message.audit",clientLocale);//审核中
						}
						if(Utility.parseInt(Utility.trimNull(map.get("CHECK_FLAG")),new Integer(0)).equals(new Integer(3))){
							checkFlag=enfo.crm.tools.LocalUtilis.language("class.auditBy2",clientLocale);//审核通过
						}
					%>
