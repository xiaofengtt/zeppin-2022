		...
		<TR>
			...
			<table id="table3" border="0" cellspacing="1" cellpadding="2" class="tablelinecolor" width="100%">
				<tr class="trh">
					...
					<td><%=LocalUtilis.language("class.typeName",clientLocale)%> </td><!--类别名称-->
					<td><%=LocalUtilis.language("class.indexName",clientLocale)%> </td><!-- 细则名称 -->
					<td><%=LocalUtilis.language("class.flagName1",clientLocale)%> </td><!--值来源-->		
					<td><%=LocalUtilis.language("class.flagName2",clientLocale)%> </td><!--有效标志-->
					<td><%=LocalUtilis.language("class.stValue",clientLocale)%> </td>	<!--标准值-->
					<td><%=LocalUtilis.language("class.dfgs",clientLocale)%> </td><!--得分公式-->
					<td><%=LocalUtilis.language("class.inputMan2",clientLocale)%> </td><!--设置人员-->
				...
				{  
					...
					String flag_name1="";
					String flag_name2=enfo.crm.tools.LocalUtilis.language("message.invalid",clientLocale);//无效
					if(value_flag.equals(new Integer(1))){
						flag_name1=enfo.crm.tools.LocalUtilis.language("message.manual Entry",clientLocale);//手工录入
					}
					if(value_flag.equals(new Integer(2))){
						flag_name1=enfo.crm.tools.LocalUtilis.language("message.calculate",clientLocale);//计算
					}
					if(value_flag.equals(new Integer(3))){
						flag_name1=enfo.crm.tools.LocalUtilis.language("message.boolean",clientLocale);//布尔型
					}
					if(valid_flag == "true"){
						flag_name2=enfo.crm.tools.LocalUtilis.language("message.valid",clientLocale);//有效
					}
				%>
				<tr class="tr<%=(iCurrent % 2)%>">
					...
					<a href="#" onclick="javascript:showInfo(<%=map.get("GRADE_ID")%>,<%=map.get("INDEX_ID")%>);">
						<img src="<%=request.getContextPath()%>/images/edit2.gif" width="16" height="16" alt='<%=LocalUtilis.language("message.edit",clientLocale)%> '/><!--编辑-->
					</a>
					</td>					
				</tr>
				<%
