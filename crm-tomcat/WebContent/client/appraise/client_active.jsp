...
Integer start_date = Utility.parseInt(request.getParameter("start_date"), new Integer(0));//开始日期
Integer end_date = Utility.parseInt(request.getParameter("end_date"), new Integer(0));//结束日期
...
function showInfo(cust_id,cust_type)
{
	var iQuery = document.theform.cust_no.value 
		...
		if(cust_type == 1)							
			location = 'client_info_new.jsp?cust_id=' + cust_id + '&iQuery=' + iQuery;
		else if(cust_type == 2)
			location = 'clinet_info_new_end.jsp?cust_id=' + cust_id + '&iQuery=' + iQuery;
}
...
function newInfo(cust_type)
{
	...
	//如果是机构客户
	if(cust_type == 2)						
		location = 'clinet_info_new_end.jsp?iQuery=' + iQuery;
	else if(cust_type == 1)
		location = 'client_info_new.jsp?iQuery=' + iQuery;
}
...
{
	disableAllBtn(true);
	var url = 'client_info_list.jsp?page=1&pagesize=' + document.theform.pagesize.value 
				+'&cust_name=' + document.theform.cust_name.value 
				+ '&cust_no=' + document.theform.cust_no.value
				+ '&product_id=' + document.theform.product_id.value ;
	location = url;				
...
<form name="theform" method="get" action="../clientinfo/customer_remove.jsp">
...
<table>
	...
	<tr>
		<td align="right"><%=LocalUtilis.language("class.dateRange",clientLocale)%> :</td><!--日期范围-->
		<!--从-->
        <td align="left"><%=LocalUtilis.language("message.start",clientLocale)%> <input type="text" name="start_date_picker" class=selecttext
			value="<%=Format.formatDateLine(start_date)%>"> <input
			...
			tabindex="13"> <INPUT type="hidden" name="start_date">
		</td>
		<!--到-->
        <td align="left"><%=LocalUtilis.language("message.end",clientLocale)%> <input type="text" name="end_date_picker" class=selecttext
			value="<%=Format.formatDateLine(end_date)%>"> <input
			TYPE="button" value="▼" class=selectbtn
		...
		<td align="right"><%=LocalUtilis.language("class.subscriptionAmount",clientLocale)%> :</td><!--认购额度-->
		...
		<TABLE cellSpacing=0 cellPadding=4 width="100%" align=center border=0>
			<TR>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td><%=LocalUtilis.language("class.livenessLevel",clientLocale)%>:</td><%=LocalUtilis.language("class.livenessLevel",clientLocale)%> 
						...
						<!--查询-->
                        <button type="button"  class="xpbutton3" accessKey=f id="queryButton" title='<%=LocalUtilis.language("message.query",clientLocale)%> ' 
							name="queryButton"><%=LocalUtilis.language("message.query",clientLocale)%> (<u>F</u>)</button>
						&nbsp;&nbsp;&nbsp; <%}
						if (input_operator.hasFunc(menu_id, 108)) {%>
						<button type="button"  class="xpbutton3" accessKey=n id="btnNew" name="btnNew"
							title='<%=LocalUtilis.language("message.assess",clientLocale)%>' onclick="javascript:assessLeve();"><%=LocalUtilis.language("message.assess",clientLocale)%> (<u>A</u>)</button><!--评定-->
						&nbsp;&nbsp;&nbsp;<%}%></td>
					</tr>
					<tr>
						<td colspan=3>
