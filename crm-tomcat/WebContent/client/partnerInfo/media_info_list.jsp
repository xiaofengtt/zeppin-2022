...
System.out.println("--------------"+q_partn_type);
Integer q_partn_type2_flag = new Integer(2);//媒体类型
String q_partn_type2_name =enfo.crm.tools.LocalUtilis.language("message.media",clientLocale);//媒体
...
function AppendAction(partn_type){
	//1个人 2机构
	if(partn_type ==1){
		var url = "partner_info_action.jsp?actionFlag=1&q_partn_type2_flag=2";
		showModalDialog(url,'','dialogWidth:950px;dialogHeight:450px;status:0;help:0');
		window.location.reload();
	}
	else if(partn_type == 2){
		var url = "partner_info_action2.jsp?actionFlag=1&q_partn_type2_flag=2";
		showModalDialog(url,'','dialogWidth:950px;dialogHeight:450px;status:0;help:0');
		window.location.reload();
	}
}
...
function ModiAction(partn_type,partn_id){
	//1个人 2机构
	if(partn_type ==1){
		var url = "partner_info_action.jsp?actionFlag=2&q_partn_type2_flag=2&partn_id="+partn_id;
		showModalDialog(url,'','dialogWidth:950px;dialogHeight:450px;status:0;help:0');
		window.location.reload();
	}
	else if(partn_type == 2){
		var url = "partner_info_action2.jsp?actionFlag=2&q_partn_type2_flag=2&partn_id="+partn_id;
		showModalDialog(url,'','dialogWidth:950px;dialogHeight:450px;status:0;help:0');
		window.location.reload();
	}
}
...
function QueryAction(){
	var url = "media_info_list.jsp?page=1&pagesize="+ document.theform.pagesize.value;	
...
<div id="queryCondition" class="qcMain" style="display:none;width:480px;height:130px;">
	...
	<table border="0" width="100%" cellspacing="2" cellpadding="2">
		<tr>
			<td  align="right"><%=q_partn_type2_name%><%=LocalUtilis.language("class.ID",clientLocale)%> ： </td><!--渠道编号-->
			<td  align="left">
					<input type="text" name="q_partn_no" id="q_partn_no" value="<%= q_partn_no%>"onkeydown="javascript:nextKeyPress(this)" style="width:120px">
			</td>	
			<td  align="right"><%=q_partn_type2_name%><%=LocalUtilis.language("class.name",clientLocale)%> ： </td><!--渠道名称-->
			<td  align="left">
		...
		<tr>
			<td  align="right"><%=q_partn_type2_name%><%=LocalUtilis.language("class.levelID2",clientLocale)%> ： </td><!--渠道类型-->
			<td  align="left">
		...
		<tr>
			<td align="center" colspan="4">
				<button type="button"  class="xpbutton3" accessKey=O id="btnQuery" onclick="javascript:QueryAction();"><%=LocalUtilis.language("message.ok",clientLocale)%> (<u>O</u>)</button><!--确定-->
				&nbsp;&nbsp;&nbsp;&nbsp;	 				
			</td>
		</tr>	
	</table>
...
<div align="center" >
	<table border="0"  width="100%" cellspacing="1" cellpadding="2"	class="tablelinecolor">
		<tr class="trh">
				...
				<td align="center" width="15%"><%=q_partn_type2_name%><%=LocalUtilis.language("class.name",clientLocale)%> </td><!--名称-->
				<td align="center" width="8%"><%=q_partn_type2_name%><%=LocalUtilis.language("class.levelID2",clientLocale)%> </td><!--类别-->
				<td align="center" width="8%"><%=LocalUtilis.language("class.reportType",clientLocale)%> </td><!--报道形式-->
...
while(iterator.hasNext()){
	...
	report_type_name = Utility.trimNull(map.get("REPORT_TYPE_NAME"));
