...
//Integer q_partnerCustId = Utility.parseInt(Utility.trimNull(request.getParameter("q_partnerCustId")),new Integer(0));
...
if(request.getMethod().equals("POST")){
	...
	nonproduct.modi(vo,input_operatorCode);
	bSuccess = true;
}
else {
	//产品信息显示
	...
	if(map != null){
		...
		q_validPriodUnit = Utility.parseInt(Utility.trimNull(map.get("VALID_PRIOD_UNIT")),
				new Integer(0));
		q_validPriod = Utility.parseInt(Utility.trimNull(map.get("VALID_PRIOD")),
				new Integer(0));
		q_expectMoney = Utility.parseDecimal(Utility.trimNull(map.get("EXPECT_MONEY")),
				new BigDecimal(0), 2, "1");
		q_expectRate1 = Utility.parseDecimal(Utility.trimNull(map.get("EXPECT_RATE1")),
				new BigDecimal(0),4, "100");
		q_expectRate2 = Utility.parseDecimal(Utility.trimNull(map.get("EXPECT_RATE2")),
				new BigDecimal(0), 4, "100");
		startDate = Utility.parseInt(Utility.trimNull(map.get("START_DATE")),
				new Integer(0));
		endDate = Utility.parseInt(Utility.trimNull(map.get("END_DATE")),
				new Integer(0));   
		q_description = Utility.trimNull(map.get("DESCRIPTION"));
	}
...
function getLastOptions(){	
	...
	var temp = temp + '&q_nonproductId=' + document.theform.q_nonproductId.value;
...
function selectProductAndCustomer(){	
	var url = "noproduct_update.jsp?page=<%=sPage%>" + getLastOptions();
...
function validateForm(form){
	...
	if(theform.q_expectRate1.value>theform.q_expectRate2.value){
...
<form name="theform" method="post" action="noproduct_update.jsp" onsubmit="javascript:return validateForm(this);">
<!--新增成功标志-->
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	...
	<tr>
		<td align="right"><b>*<%=LocalUtilis.language("class.productName",clientLocale)%></b> :&nbsp;&nbsp;</td><!--名称-->
		<td align="left" >
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	<tr>   
		...
		<td colspan="3"><textarea  rows="10" name="q_description" cols="120"><%=Utility.trimNull(q_description) %></textarea></td>
	</tr>
