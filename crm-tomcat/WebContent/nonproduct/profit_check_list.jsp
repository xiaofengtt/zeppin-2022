...
function no_pass()
{
	location = 'profit_check_action.jsp?q_serialNo='+document.theform.q_serialNo.value;
}
...
<form name="theform" method="post" action="profit_check_actionOK.jsp" onsubmit="javascript:return validateForm(this);">
<!--新增成功标志-->
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	...
	<tr>
		...
		<td align="LEFT">
			<input readonly class='edline' maxlength="16" name="q_profitMoney" value="<%=q_profitMoney%>" size="20"  onkeydown="javascript:nextKeyPress(this)" >
		</td>
		<td align="right"><b>*<%=LocalUtilis.language("message.date",clientLocale)%></b> :&nbsp;&nbsp;</td><!--日期-->
		<td>
			<input readonly class='edline' maxlength="16" name="q_syDate" value="<%=q_syDate%>" size="20"  onkeydown="javascript:nextKeyPress(this)" >
		</td>
	</tr>
</table>
<br/>
<p class="title-table"><b><%=LocalUtilis.language("menu.customerInformation",clientLocale)%> </b></td><!--客户信息--></p>
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	<tr>
...
<div align="right">
	<br>
	<!--审核-->
	<button type="button" class="xpbutton4" id="btnCheck" name="btnCheck" onclick="javascript:op_check(<%=q_serialNo %>);"><%=LocalUtilis.language("class.auditBy2",clientLocale)%> </button>
	&nbsp;&nbsp;
	<!--审核未通过-->
	<button type="button" class="xpbutton4" id="btnnotPass" name="btnnotPass" onclick="javascript:no_pass();"><%=LocalUtilis.language("message.notPass2",clientLocale)%> </button>
	&nbsp;&nbsp;
	<!--返回-->
