...
if(request.getMethod().equals("POST")){
	...
	subscribe.modi(vo,input_operatorCode);
	bSuccess = true;
}
else{
	vo.setSubscribe_id(q_subscribeId);
...
function selectProductAndCustomer(){	
	var url = "subscribe_edit.jsp?page=<%=sPage%>" + getLastOptions();
...
function selectBank(value){
	if (value!=""){	
		location = 'subscribe_edit.jsp?page=<%=sPage%>&bank_id=' + value+ getLastOptions();
	}
}
...
function SaveAction(){		
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action="subscribe_edit.jsp";
...
<form name="theform" method="post" action="subscribe_edit.jsp" onsubmit="javascript:return validateForm(this);">
<!--新增成功标志-->
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	<tr>
		...
		<td align="LEFT">
			<input readonly type="text" maxlength="16" name="q_nonproductId" value="<%=q_nonproductId%>" size="20">
		</td>
		<td>&nbsp;&nbsp;</td>
		<td>&nbsp;&nbsp;</td>
	</tr>
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	...
	<tr>   
		<!--请选择-->
        <td align="center"><button class="xpbutton3" accessKey=E name="btnEdit" title="<%=LocalUtilis.language("menu.customerInformation",clientLocale)%> " onclick="javascript:getMarketingCustomer('customer','0');"><%=LocalUtilis.language("message.select2",clientLocale)%> </button></td>
	</tr>
	<tr>
...
<br>
<p class="title-table"><b><%=LocalUtilis.language("message.contractInfo",clientLocale)%> </b></p><!-- 合同信息 -->
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">	
	<!--认购合同信息-->
	<tr>
		<td align="right"><b>*<%=LocalUtilis.language("class.rg_money",clientLocale)%> </b> :&nbsp;&nbsp;</td><!--认购金额-->
		<td><input name="rg_money" size="20" value="<%=subscribe_money %>" onkeydown="javascript:nextKeyPress(this)" onkeyup="javascript:showCnMoney(this.value,rg_money_cn)" onblur="javascript:checkSellInfo();"> 
