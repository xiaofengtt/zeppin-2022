...
if(request.getMethod().equals("POST")){
	...
	profit.modi(quotientvo,input_operatorCode);
	bSuccess = true;
}
else{
	...
	//收益信息
	quotientvo.setSerial_no(q_serialNo);
	quotientvo.setNonproduct_id(q_nonproductId);
	quotientvo.setNonproduct_name(q_nonproductName);
	quotientvo.setCust_name(q_custName);
	quotientvo.setSubscribe_bh(subscribe_bh);
	quotientvo.setSy_date(q_syDate);
	List profitlist = profit.load(quotientvo, input_operatorCode);
	if (profitlist!=null && profitlist.size()>0)
	{
		map_profit = (Map)profitlist.get(0);
	}
	if(map_profit != null){
		q_profitMoney = Utility.parseDecimal(Utility.trimNull(map_profit.get("PROFIT_MONEY")),new BigDecimal(0),2,"1");
		q_syDate = Utility.parseInt(Utility.trimNull(map_profit.get("SY_DATE")),new Integer(0));
	}
...
function selectProductAndCustomer(){	
	var url = "profit_update.jsp?page=<%=sPage%>" + getLastOptions();
...
function SaveAction(){	
	...
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action="profit_update.jsp";
...
<form name="theform" method="post" action="profit_update.jsp" onsubmit="javascript:return validateForm(this);">
<!--新增成功标志-->
...
<table  border="0" width="100%" cellspacing="0" cellpadding="3" class="table-popup">
	...
	<tr>
		...
		<td align=left colspan=3>
			<input readonly class='edline' maxlength="16" name="q_nonproductName" value="<%=q_nonproductName%>" size="50">
		</td>
   </tr>
   <tr>
   		<!--合同序号-->
		...
		<td align=left colspan=3>
			<input readonly class='edline' maxlength="16" name="q_subscribeId" value="<%=subscribe_bh%>" size="50">
		</td>
   </tr>
</table>
<br>
