...
BigDecimal q_syRate = Utility.parseDecimal(Utility.trimNull(request.getParameter("q_syRate")),new BigDecimal(0),2,"1");
...
if(request.getMethod().equals("POST")){
	...
	quotientvo.setSy_rate(q_syRate);
	quotientvo.setSy_date(q_syDate);
	profit.cal(quotientvo,input_operatorCode);
	bSuccess = true;
}
else{
...
<HTML>
<HEAD>
<TITLE><%=LocalUtilis.language("menu.addSubscriptionInfo",clientLocale)%>（<%=LocalUtilis.language("message.withoutBooked",clientLocale)%> ）</TITLE>
...
function getLastOptions(){	
	...
	var temp = temp + "&q_syRate="+ document.theform.q_syRate.value;	
...
function selectProductAndCustomer(){	
	var url = "profit_cal.jsp?page=<%=sPage%>" + getLastOptions();
...
function SaveAction(){	
	...
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action="profit_cal.jsp";
...
function validateForm(form){
	...
	if(!sl_checkDecimal(form.q_syRate, '<%=LocalUtilis.language("class.profitMoney",clientLocale)%> ', 13, 3, 1)){return false;}//份额	
...
<form name="theform" method="post" action="profit_cal.jsp" onsubmit="javascript:return validateForm(this);">
<!--新增成功标志-->
...
<div>
<br/>
<table class="table-popup"   border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>   
...
<table class="table-popup" border="0" width="100%" cellspacing="0" cellpadding="0" >	
	...
	<tr>
		...
		<td><input readonly class='edline' name="rg_money" size="50" value="<%=subscribe_money %>" > 
		</td>
	...
	<tr>
		...
		<td >
			<input readonly class='edline' name="bank_name" size="20" onkeydown="javascript:nextKeyPress(this)" value="<%=bank_name%>">
...
<table class="table-popup"   border="0" width="100%" cellspacing="0" cellpadding="0"  >	
	<tr>
		<td><b><%=LocalUtilis.language("message.incomeInfo",clientLocale)%> </b></td>  <!--合同信息-->
	</tr>
	<TR>
	   	<td align="right" width="120px"><%=LocalUtilis.language("class.returnRate",clientLocale)%> :&nbsp;&nbsp;</td><!--收益率-->
		<td align="LEFT">
			<input type="text" maxlength="16" name="q_syRate" value="<%=q_syRate%>" size="20"  onkeydown="javascript:nextKeyPress(this)" >%
		</td>
		<td align="right">*<%=LocalUtilis.language("message.date",clientLocale)%> :</td><!--日期-->
		<td>
