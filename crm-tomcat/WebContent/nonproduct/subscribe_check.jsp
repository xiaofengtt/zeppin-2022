...
function op_check()
{
	...
	if (sl_check_pass()){
	disableAllBtn(true);
	var url = 'subscribe_check_action.jsp';
...
function showInfo(q_subscribeId,cust_id)
{
	...
	location = 'subscribe_check_list.jsp?q_subscribeId='+q_subscribeId+'&cust_id='+cust_id+'&page=1&sQuery=' + sQuery;
}

/*刷新*/
function refreshPage(){
	disableAllBtn(true);		
	var url = "subscribe_check.jsp?page=1&pagesize="+ document.theform.pagesize.value;	
...
<form name="theform" method="get"  action="subscribe_check.jsp">
...
<div>
	...
	<div align="right" class="btn-wrapper">
		...
		<%if (input_operator.hasFunc(menu_id, 103)){%>
        <!-- 合同结清审核不通过 -->
		<button type="button" class="xpbutton4" name="btnCheck" id="btnCheck" title='<%=LocalUtilis.language("message.notPass2",clientLocale)%>' onclick="javascript:StartQuery();"><%=LocalUtilis.language("message.notPass2",clientLocale)%> </button>
		<%}%>
	</div>
	<br/>
</div>
