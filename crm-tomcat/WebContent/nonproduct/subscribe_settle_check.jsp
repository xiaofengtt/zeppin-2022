...
function op_check()
{
	...
	if (sl_check_pass()){
	disableAllBtn(true);
	var url = 'subscribe_settle_check_actionOK.jsp';
...
function se_check()
{
	...
	if (sl_check_unpass()){
	disableAllBtn(true);
	var url = 'subscribe_settle_check_action.jsp';
	document.getElementsByName("theform")[0].setAttribute("action",url);
	document.getElementsByName("theform")[0].submit();
	return true;
	}
...
function showInfo(q_subscribeId,cust_id)
{
	...
	location = 'subscribe_settle_check_list.jsp?q_subscribeId='+q_subscribeId+'&cust_id='+cust_id+'&page=1&sQuery=' + sQuery;
}

/*刷新*/
function refreshPage(){
	disableAllBtn(true);
	var url = "subscribe_settle_check.jsp?page=1&pagesize="+ document.theform.pagesize.value;
...
<form name="theform" method="get"  action="subscribe_settle_check.jsp">
...
<div>
	...
	<div align="right" class="btn-wrapper">
        ...
        <!-- 合同结清审核 -->
		<button type="button" class="xpbutton4" name="btnCheck" id="btnCheck" title='<%=LocalUtilis.language("message.auditBy2",clientLocale)%>' onclick="javascript:op_check();"><%=LocalUtilis.language("class.auditBy2",clientLocale)%> </button>&nbsp;&nbsp;&nbsp;
		<%}%>
		<%if (input_operator.hasFunc(menu_id, 103)){%>
        <!-- 合同结清审核未通过 -->
		<button type="button" class="xpbutton4" name="btnCheck" id="btnCheck" title='<%=LocalUtilis.language("message.notPass2",clientLocale)%>' onclick="javascript:se_check();"><%=LocalUtilis.language("message.notPass2",clientLocale)%> </button>
		<%}%>
	</div>
	<br/>
</div>
...
while(iterator.hasNext()){
	...
	temp_status = Utility.trimNull(map.get("TEMP_STATUS"));
