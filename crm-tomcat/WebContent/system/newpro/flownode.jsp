...
<%
//初始化参数信息
String interfaceType_code = "FlowNode010";
...
if(!"".equals(flow_no)){//自己拼接的其他查询条件语句
...
sPage="page="+sPage+"&flow_no="+flow_no+"&flow_name="+flow_name;
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:600px;dialogHeight:360px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	var dialogUrl = '<%=freeform_name%>?flow_no=<%=flow_no%>&flow_name=<%=flow_name%>&interfacetype_code='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode;
	if(showModalDialog(dialogUrl, '', 'dialogWidth:600px;dialogHeight:360px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
//---------设置流程节点流转---------
function viewInfo(flow_no,flow_name,node_no,node_name)
{
	showModalDialog('/system/newpro/flowdrive.jsp?flow_no='+flow_no+'&flow_name='+flow_name+'&node_no='+node_no+'&node_name='+node_name, '', 'dialogWidth:700px;dialogHeight:560px;status:0;help:0')
}

//---------设置流程节点操作对象---------
function configInfo(flow_no,flow_name,node_no,node_name)
{
	showModalDialog('/system/newpro/flowobject.jsp?flow_no='+flow_no+'&flow_name='+flow_name+'&node_no='+node_no+'&node_name='+node_name, '', 'dialogWidth:700px;dialogHeight:560px;status:0;help:0')
}
