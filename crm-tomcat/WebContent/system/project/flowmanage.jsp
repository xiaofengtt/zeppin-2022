...
<%
//初始化参数信息
String interfaceType_code = "FlowManage010";
...
String object_type="FlowManage010";
...
if(query_type.equals("")) sqlContent =sqlContent + " and 1=2";
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=<%=edit_right%>&object_type=<%=object_type%>&object_id=<%=object_id%>&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:600px;status:0;help:0') != null)
	{
		sl_update_ok();
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&edit_right=<%=edit_right%>&object_type=<%=object_type%>&object_id=<%=object_id%>&identity_code=0&table_code='+tableCode, '', 'dialogWidth:800px;dialogHeight:600px;status:0;help:0') != null)
	{
		sl_update_ok();
		document.theform.submit();
	}
...
//---------查看详情---------
function itemflow(object_no,object_type)
{
	location='/system/project/flowtaskmanage.jsp?object_no='+object_no+'&object_type='+object_type;
}
</script>
...
<input name="QueryType" type="hidden" value="<%=query_type%>" />
