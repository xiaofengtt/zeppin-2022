...
<%
	...
	//组装符合业务条件的行
	String checkStr =ConfigCheck.getInterfaceCheck(pageList,"INTERFACE_FIELD","INTERFACETYPE_CODE");
...
<script language=javascript>
	...
	function showInfo(identity_code)
	{
		...
		if(showModalDialog('<%=freeform_name%>?interfacetype_codeone='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:670px;dialogHeight:520px;status:0;help:0') != null)
		{
			sl_update_ok();
			location.reload();
		}
	...
	{	
		var tableCode = document.theform.table_code.value;
		var interfaceTypeCode = document.theform.interfacetype_code.value;
		if(showModalDialog('<%=freeform_name%>?interfacetype_codeone='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode, '', 'dialogWidth:670px;dialogHeight:520px;status:0;help:0') != null)
		{
			sl_update_ok();
			location.reload();
		}
	...
	//----------查看--------------
	function viewInfo(interfacetype_code)
	{	
		location ='/system/config/interface_content.jsp?otherinterfacetype_code='+interfacetype_code;
	}
	//--------------预览-------------
	function previewInfo(viewId){
		if(showModalDialog('/system/config/basic_preview_select.jsp?id='+viewId, '', 'dialogWidth:280px;dialogHeight:150px;status:0;help:0') != null)
		{
			sl_update_ok();
			location.reload();
		}
...
<TABLE height="100%" cellSpacing=0 cellPadding="0" width="100%" border=0>
	<TR>
		...
		<jsp:include page="/system/config/basiclist_table.jsp" flush="true">
			...
			<jsp:param name="checkName" value="INTERFACETYPE_ID"/>
