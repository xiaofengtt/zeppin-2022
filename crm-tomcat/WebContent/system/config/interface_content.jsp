...
<%
String interfaceType_code = "020";
ConfigLocal configLocal = EJBFactory.getConfig();
boolean bSuccess=false;

//通过页面编码查找该表对应的主键字段
String otherInterfaceTypeCode = Utility.trimNull(request.getParameter("otherinterfacetype_code"),"0");
...
if(!otherInterfaceTypeCode.equals("0")){//自己拼接的其他查询条件语句
		sqlContent =sqlContent + " and INTERFACETYPE_CODE='"+otherInterfaceTypeCode+"'";
...
sPage="page="+sPage+"&otherinterfacetype_code="+otherInterfaceTypeCode;
sUrl = sUrl +"&otherinterfacetype_code="+otherInterfaceTypeCode;
%>
...
function showInfo(identity_code)
{
	...
	var otherinterfacetype_code = document.theform.otherinterfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code_type='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:1050px;dialogHeight:660px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
function newInfo()
{
	...
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	var otherinterfacetype_code = document.theform.otherinterfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?thisinterfacetype_code=<%=otherInterfaceTypeCode%>&interfacetype_code_type='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode, '', 'dialogWidth:1050px;dialogHeight:660px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
function doBack(){
	location ='interface_collect.jsp';
}
...
<input name="otherinterfacetype_code" type="hidden" value="<%=otherInterfaceTypeCode%>">
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td align="right">
							...
							&nbsp;&nbsp;&nbsp;
							<%if(!("0".equals(otherInterfaceTypeCode))){%> 
