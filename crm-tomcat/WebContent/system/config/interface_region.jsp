...
<%
//初始化参数信息
String interfaceType_code = "130";
ConfigLocal configLocal = EJBFactory.getConfig();
boolean bSuccess = false;

//通过页面编码查找该表对应的主键字段
String region_code = Utility.trimNull(request.getParameter("region_code"));
String region_name = Utility.trimNull(request.getParameter("region_name"),"");
...
if(!region_code.equals("0") && !"".equals(region_code)){//自己拼接的其他查询条件语句
		sqlContent =sqlContent + " and REGION_CODE='"+region_code+"'";
		if("IndustryType010".equals(region_code) || "AreaType010".equals(region_code))
			pageList= configLocal.showPageDataOther(interfaceType_code,table_code,queryFieldColStr,queryValueStr,queryConditionValueStr,
											sqlContent,Utility.parseInt(sPage,1),Utility.parseInt(sPagesize,20));
		else
			pageList= configLocal.showPageDataOther(interfaceType_code,table_code,queryFieldColStr,queryValueStr,queryConditionValueStr,
...
sPage="page="+sPage+"&region_code="+region_code;
sUrl = sUrl +"&region_code="+region_code;
%>
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:560px;dialogHeight:550px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	var region_code=document.theform.region_code.value;
	var region_name=document.theform.region_name.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode+'&region_name='+region_name+'&region_code='+region_code, '', 'dialogWidth:560px;dialogHeight:550px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
//----------返回-----------
function doBack(){
	location ='/system/config/interface_category.jsp';
}
</script>
...
<input name="region_code" type="hidden" value="<%=region_code%>">
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td align="right">
							...
							&nbsp;&nbsp;&nbsp; 
							<%if(!region_code.equals("0") && !"".equals(region_code)){%>
