...
//组装符合业务条件的行
String checkStr =ConfigCheck.getInterfaceTypeCheck(pageList);						
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:560px;dialogHeight:300px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
{	
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	if(showModalDialog('<%=freeform_name%>?interfacetype_code='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode, '', 'dialogWidth:560px;dialogHeight:300px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
//----------配置-----------
function viewInfo(catalog_code)
{	
	location ='/system/config/interface_element.jsp?catalog_code='+catalog_code;
}
//----------预览-----------
function previewInfo(catalog_code){
	if(showModalDialog('/system/config/assist_preview_select.jsp?catalog_code='+catalog_code, '', 'dialogWidth:800px;dialogHeight:600px;status:0;help:0') != null)
		{
			sl_update_ok();
			location.reload();
		}
...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
	<TR>
		...
		<jsp:include page="/system/config/basiclist_table.jsp" flush="true">
			<jsp:param name="checkName" value="CATALOG_ID"/>
