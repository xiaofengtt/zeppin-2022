<%@ page contentType="text/html;charset=GBK" pageEncoding="GBK" import="enfo.crm.system.*,enfo.crm.vo.*,enfo.crm.dao.*,enfo.crm.tools.*,java.util.*"%>
<%
	...
	String flow_no = Utility.trimNull(request.getParameter("flow_no"),"");//流程编号
	String flow_name = Utility.trimNull(request.getParameter("flow_name"),"");//流程名称
	String node_code = Utility.trimNull(request.getParameter("node_code"),"");//节点状态编码
	String node_name = Utility.trimNull(request.getParameter("node_name"),"");//节点名称
	String node_flag = Utility.trimNull(request.getParameter("node_flag"),"");//节点类型
	/**if("".equals(flow_name)){
		//flow_name = Utility.trimNull(ConfigUtil.getSqlResult("select FLOW_NAME from FLOW_CATALOG where FLOW_NO='" + flow_no + "'"));
	}*/
	//System.out.println("======flow_no=" + flow_name +", node_code=" + node_code + ", node_name=" + node_name+ ", node_flag=" + node_flag);
%>
...
<%
	...
	String codeName="ItemView020",menu_id="",menu_name="";
	menuList = configLocal.showSelectInfo("code",codeName,"");
	menuList = new ArrayList();
	Map m = new HashMap();
	m.put("Item_Code", "010");
	m.put("Item_Name", "节点基本信息");
	m.put("Relative_Name", "/system/workflow/wkflownode_info.jsp");
	m.put("Attribute_Name", "object_id=#OBJECT_ID#&edit_right=#EDIT_RIGHT#&object_type=#OBJECT_TYPE#");
	Map m2 = new HashMap();
	m2.put("Item_Code", "020");
	m2.put("Item_Name", "操作对象信息");
	m2.put("Relative_Name", "/system/workflow/wkflowobject.jsp");
	m2.put("Attribute_Name", "object_id=#OBJECT_ID#&edit_right=#EDIT_RIGHT#&object_type=#OBJECT_TYPE#");
	menuList.add(m);
	menuList.add(m2);
...
<script language="javascript">
	...
	//选择菜单信息
  	function loadMain(nodeno,nodename,nodetype,jspurl,jspparam) {
  		...
  		jspparam = jspparam + "&flow_no=<%=flow_no%>&flow_name=<%=flow_name%>&node_code=<%=node_code%>&node_name=<%=node_name%>"; 
  		jspparam = jspparam + "&node_flag=<%=node_flag%>"; 
  		if(nodetype=="leaf"){
  ...
  //初始化展示客户基本信息
  	Ext.get('home-column-main').dom.src = "/system/workflow/wkflownode_info.jsp"
  		+ "?object_type=<%=object_type%>&object_id=<%=object_id%>&edit_right=<%=edit_right%>"
  		+ "&flow_no=<%=flow_no%>&flow_name=<%=flow_name%>&node_code=<%=node_code%>&node_name=<%=node_name%>&node_flag=<%=node_flag%>";
</script>
