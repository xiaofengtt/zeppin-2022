...
String table_code = Utility.trimNull(request.getParameter("table_code"),"FLOW_NODE");
String interfaceType_code = Utility.trimNull(request.getParameter("interfacetype_code"), "WorkFlowNode010");
//评级目录类别
String flow_no = Utility.trimNull(request.getParameter("flow_no"),"0");
String flow_name = Utility.trimNull(request.getParameter("flow_name"),"");
String node_code = Utility.trimNull(request.getParameter("node_code"),"");
String node_name = Utility.trimNull(request.getParameter("node_name"),"");
String node_flag = Utility.trimNull(request.getParameter("node_flag"),"");

if(identity_code.intValue() ==0 && !"".equals(node_code)){
	String nodeIdSql =  "select NODE_ID from FLOW_NODE where FLOW_NO='" + flow_no + "' and NODE_CODE ='" + node_code + "'";
	identity_code = Utility.parseInt(ConfigUtil.getSqlResult(nodeIdSql), new Integer(0));
}
...
function initValue(){
	...
	if("<%=identity_code%>"==0){
		...
		document.theform.NODE_CODE.value=trim('<%=node_code%>');
	}
	var nodeName = document.theform.NODE_NAME.value;
	if('<%=node_name%>' !='' && '<%=node_name%>' != nodeName){
		document.theform.NODE_NAME.value=trim('<%=node_name%>');
	}
	var nodeFlag = document.theform.NODE_FLAG.value;
	if('<%=node_flag%>' !='' && '<%=node_flag%>' != nodeFlag){
		document.theform.NODE_FLAG.value=trim('<%=node_flag%>');
		var nodeFlagName =document.getElementsByName('NODE_FLAG')[0].options[document.getElementsByName('NODE_FLAG')[0].selectedIndex].text;
		document.theform.NODE_FLAG_NAME.value=nodeFlagName;
	}
}
</script>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
		<tr>
			<td>
				<img border="0" src="/images/member.gif" align="absbottom" width="32" height="28"><font color="#215dc6"><b>流程图节点</b></font>
			</td>
		</tr>
		<tr>
