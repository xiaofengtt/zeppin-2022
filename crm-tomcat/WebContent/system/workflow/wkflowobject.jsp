...
<%
	//初始化参数信息
	String interfaceType_code = "WorkFlowObject010";
	...
	String flow_name = Utility.trimNull(request
			.getParameter("flow_name"));
	String node_code = Utility.trimNull(request
			.getParameter("node_code"));
	String node_no = Utility.trimNull(request.getParameter("node_no"));
	String node_name = Utility.trimNull(request
			.getParameter("node_name"));
	if (!"".equals(node_code) && ("".equals(node_no) || "0".equals(node_no))) {
		String getNodeSql = "select NODE_NO from FLOW_NODE where FLOW_NO='" + flow_no + "' and NODE_CODE='" + node_code + "'";
		node_no = Utility.trimNull(ConfigUtil.getSqlResult(getNodeSql));
	}
	//通过页面编码查找该表对应的主键字段和表名
	String identityCode = ConfigUtil.getIdentityCode(configLocal,
			interfaceType_code);
	List catalogList = ConfigUtil.getTableName(configLocal, interfaceType_code);
	String table_code = catalogList.get(0).toString();
	String grid_name = catalogList.get(1).toString();
	String freeform_name = catalogList.get(2).toString();
	...
	if (request.getMethod().equals("POST")) {//删除信息
		...
		if (s != null && s.length > 0) {
			ConfigUtil
			.delInfo(configLocal, s, table_code, identityCode);
			delSuc = true;
		}
	}
	...
	ConfigUtil.queryConditionBlock(request, queryFieldList,
			queryFieldColStr, queryFieldNameStr, queryInputTypeStr,
			queryValueTypeStr, queryValueContentStr,
			queryConditionTypeStr, queryValueStr,
			queryConditionValueStr);

	List tableFieldList = configLocal
			.tableFieldShowList(interfaceType_code);//查询需要显示有效的页面表格字段
	String[] fieldStrs = new String[tableFieldList.size()];//显示字段编码
	String[] fieldNameStrs = new String[tableFieldList.size()];//显示字段名称
	ConfigUtil.pageValidColShowBlock(tableFieldList, fieldStrs,
			fieldNameStrs);//获取字段和字段名称集合

	//其他查询字段
	List replaceColValue = new ArrayList();//需要替换的值
	String sqlContent = ConfigUtil.getOtherQuerySql(interfaceType_code,
			replaceColValue);

	sqlContent = sqlContent + " and flow_no='" + flow_no + "'"
			+ " and node_no='" + node_no + "'";

	IPageList pageList = configLocal.showPageDataOther(
			interfaceType_code, table_code, queryFieldColStr,
			queryValueStr, queryConditionValueStr, sqlContent, Utility
			.parseInt(sPage, 1), Utility
			.parseInt(sPagesize, -1));

	sUrl = sUrl + "&flow_no=" + flow_no + "&flow_name=" + flow_name
			+ "&node_no=" + node_no + "&node_name=" + node_name;
	sUrl = ConfigUtil.getSUrl(sUrl, queryFieldColStr,
			queryConditionTypeStr, queryValueStr,
			queryConditionValueStr);
%>
...
<HTML>
	...
	<SCRIPT LANGUAGE="javascript"
		SRC="/includes/queryEx/scripts/queryEx.js"></SCRIPT>
...
function showInfo(identity_code)
{
	...
	if(showModalDialog('<%=freeform_name%>?node_code=<%=node_code%>&interfacetype_code='+interfaceTypeCode+'&identity_code='+identity_code+'&table_code='+tableCode, '', 'dialogWidth:560px;dialogHeight:400px;status:0;help:0') != null)
	{
		sl_update_ok();
...
function newInfo()
{	
	if('<%=node_no%>'==0 || '<%=node_no%>'==''){
		alert('新建操作对象前，请先保存节点基本信息！');
		return false;
	}
	var tableCode = document.theform.table_code.value;
	var interfaceTypeCode = document.theform.interfacetype_code.value;
	var dialogUrl = '<%=freeform_name%>?node_code=<%=node_code%>&interfacetype_code='+interfaceTypeCode+'&identity_code=0&table_code='+tableCode;
	...
	<BODY class="BODY">
		<form name="theform" method="POST" action="<%=grid_name%>">
			<input name="interfacetype_code" type="hidden"
				value="<%=interfaceType_code%>" />
			...
			<input name="node_code" type="hidden" value="<%=node_code%>" />
...
<%}%>
			...
			<%
				...
				request
						.setAttribute("queryConditionTypeStr",
						queryConditionTypeStr);//查询字段查询类型集
				request.setAttribute("queryConditionValueStr",
						queryConditionValueStr);//查询字段选择的查询类型
			...
			<%
				request.setAttribute("bSuccess", new Boolean(delSuc));
				request.setAttribute("queryFieldColStr", queryFieldColStr);
				request
						.setAttribute("queryConditionTypeStr",
						queryConditionTypeStr);
				request.setAttribute("queryConditionValueStr",
						queryConditionValueStr);
							...
							<TR>
								<TD>
									<table border="0" width="100%" cellspacing="0" cellpadding="0">
										<tr>
											<td>
												<IMG src="/images/member.gif" align="absBottom" border="0"
													width="32" height="28">
												<b>流程图节点操作对象</b>
											</td>
										</tr>
										<tr>
											<td align="right">
												...
												&nbsp;&nbsp;&nbsp;
												<button type="button"  class="xpbutton3" accessKey=n id="btnNew"
													name="btnNew" title="新建记录" onclick="javascript:newInfo();">
													新增(
													<u>N</u>)
												</button>
												&nbsp;&nbsp;&nbsp;
												<button type="button"  class="xpbutton3" accessKey=l id="btnDel"
													name="btnDel" title="删除"
													onclick="javascript: remove('<%=delStr%>')">
													删除(
													<u>L</u>)
												</button>
												&nbsp;&nbsp;&nbsp;
									...
									%>
									<jsp:include page="/system/config/basiclist_table.jsp"
										flush="true">
