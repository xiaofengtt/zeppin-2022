		...
		<TABLE cellSpacing=0 cellPadding=3 width="100%" border=0 class="product-list">
			...
			<tr>
				...
				<td>
					...
					<button type="button"  class="xpbutton3" title="<%=LocalUtilis.language("menu.customerInformation",clientLocale)%> " onclick="javascript:getMarketingCustomer('<%=Utility.trimNull(custMap.get("CUST_ID"))%>');">详细</button>
				</td>
			...
			<tr>
				...
				<td >
					<input  class="edline" readonly="readonly" type="text" name="valid_period" size="20"  value="<%if(contractMap.get("FEE_JK_TYPE")!=null&&((Integer)contractMap.get("FEE_JK_TYPE")).intValue()==0){out.print("无需缴纳");}else if(contractMap.get("FEE_JK_TYPE")!=null&&((Integer)contractMap.get("FEE_JK_TYPE")).intValue()==1){out.print("从本金扣除");}else{out.print("额外缴纳");}%>">					
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<input class="edline" readonly="readonly" TYPE="text" name="expect_ror_lower" size="12" 
						value="<%=Utility.trimZero(((BigDecimal)(contractMap.get("EXPECT_ROR_LOWER")==null?new BigDecimal(0):contractMap.get("EXPECT_ROR_LOWER"))).multiply(new BigDecimal(100)))%>" ><font color="red">%</font> 到 
