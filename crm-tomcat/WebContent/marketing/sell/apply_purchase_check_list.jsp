...
<%
Integer product_id = Utility.parseInt(request.getParameter("product_id"), overall_product_id); // overall_product_id 当前产品
...
<%
	...
	//int totalWidth = 0+100+30; //合同编号+审核
							...
							<tr>
								<td align=right>
									...
									<%if (input_operator.hasFunc(menu_id, 108)){%>
							 		<button type="button"  class="xpbutton3" accessKey=q id="queryButton" name="queryButton"><%=LocalUtilis.language("message.query",clientLocale)%> <!--查询-->(<u>Q</u>)
									</button>&nbsp;&nbsp;&nbsp; 
			...
			<table id="table3" border="0" cellspacing="1" cellpadding="2" class="tablelinecolor" width="100%" >
				<tr class="trh">
					<td align="center" width="100px">
						<input type="checkbox" name="btnCheckbox" class="selectAllBox" onclick="javascript:selectAllBox(document.theform.serial_no,this);">&nbsp;&nbsp;合同编号
					</td><!--合同编号-->
				<%
					for(int j=0;j<fieldsArr.length;j++){
						String setWithName = Utility.trimNull(((Map)fieldsMap.get(fieldsArr[j])).get("FIELD_NAME"));
						if ("合同编号".equals(setWithName)) continue;

						String setWith = "";
						String other = "";
						
						if ("产品名称".equals(setWithName) || "证件号码".equals(setWithName) || "电话号码".equals(setWithName) 
								|| "银行名称".equals(setWithName)||"银行账号".equals(setWithName) || "签署日期".equals(setWithName))
							setWith = "120px";
						else if ("地址".equals(setWithName))
							setWith = "180px";	
						else if ("受益级别".equals(setWithName) || "收益级别".equals(setWithName) || "渠道合作方式".equals(setWithName))
							setWith = "60px";
						else if ("状态".equals(setWithName)) 
							setWith = "80px";
						else 
							setWith = "100px";
						
						if ("申购金额".equals(setWithName)) other = "sort='num'";
							
					%>
					<td align="center" width="<%=setWith%>" <%=other%>><%=((Map)fieldsMap.get(fieldsArr[j])).get("FIELD_NAME")%></td>
					<%}%>						
					<td align="center" width="30px"><%=LocalUtilis.language("message.check",clientLocale)%> </td><!--审核-->
				</tr>
<%
int iCount = 0;
int iCurrent = 0;
