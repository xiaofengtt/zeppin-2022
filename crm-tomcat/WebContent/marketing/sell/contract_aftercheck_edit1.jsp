...
int inputflag2=Utility.parseInt(request.getParameter("inputflag2"), 2); // 银行帐号的输入方式 默认是 输入
...
if(request.getMethod().equals("POST")){
	...
	inputflag2=Utility.parseInt(file.getParameter("inputflag2"), 2); // 银行帐号的输入方式 默认是 输入
...
function changeInput2(obj){
	if(document.theform.inputflag2.value==1){ //选择
...
<input type="hidden" name="inputflag2" value="<%=inputflag2%>"/><!--银行账号输入标志-->
...
<div>
	<table  border="0" width="100%" cellspacing="0" cellpadding="2" class="product-list">
		...
		<%if(product_sub_flag.intValue() == 1){%>
			<tr>	
				<td align="right" width="120px"><b>*<%=LocalUtilis.language("class.subProductID",clientLocale)%> </b></td><!--子产品选择-->
		...
		<table  border="0" width="100%" cellspacing="0" cellpadding="2" class="product-list">
			...
			<tr>
				<td align="right" width="120px">*<%=LocalUtilis.language("class.bankName3",clientLocale)%> :</td><!--银行名称-->
				<td >
				...
				<td align="right" width="120px">*<%=LocalUtilis.language("class.bankAcct3",clientLocale)%> :</td><!--银行帐号-->
				<td>
		...
		<tr>
			...
			<td ><input name="gain_acct" size="20"  <%if(user_id.intValue()!=25/*25重庆*/){%>class="edline" readonly<%}%> onkeydown="javascript:nextKeyPress(this)" value="<%=gain_acct%>"/></td>
		</tr>
		<tr>
			<td align="right" width="120px">*<%=LocalUtilis.language("class.feeType",clientLocale)%> :</td><!--缴款方式-->
			<td>
