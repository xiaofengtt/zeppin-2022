...
Integer q_check_flag=Utility.parseInt(request.getParameter("q_check_flag"),new Integer(2));
...
function editAction(serialno, checkflag,ht_status,bank_id,pre_flag){
	...
	if (checkflag!=3) {
		...
		location = 'contract_aftercheck_check_aciton.jsp?serial_no='+serialno+'&view_flag=true&sQuery=' + sQuery;
	}
}
	...
	<tr>
		...
		<td  align="left" >
			<select size="1" name="q_check_flag" onkeydown="javascript:onkeyQuery(this);" style="WIDTH: 100px">
				...
				<OPTION value="91" <%if(q_check_flag.intValue()==91)out.print("selected") ;%>>修改待审核</OPTION>
				<OPTION value="93" <%if(q_check_flag.intValue()==93)out.print("selected") ;%>>审核未通过</OPTION>
			</select>
		</td>
		...
		<TABLE cellSpacing=0 cellPadding=4 width="100%" align=left border=0>
			<TR>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td colspan="6">
							<table cellSpacing="0" cellPadding="0"  bgcolor="#CCCCCC" class="table-select">
				...
				<tr class="trh">
					...
					<td align="center" width="35px;"><%=q_check_flag.intValue()==2 || q_check_flag.intValue()==91?LocalUtilis.language("message.edit",clientLocale):"查看"%> </td><!--编辑-->
				</tr>
<%
int iCurrent = 0;
List rsList = null;
...
%>
					...
					<td align="left" ><a href="#" onclick="javascript:showProduct('<%=rowMap.get("PRODUCT_ID")%>');"><%=Utility.trimNull(rowMap.get("PRODUCT_NAME"))%></a></td>
