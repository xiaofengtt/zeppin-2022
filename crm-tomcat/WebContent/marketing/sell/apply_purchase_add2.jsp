...
String q_subproductId = Utility.trimNull(request.getParameter("sub_product_id"));
...
if(request.getMethod().equals("POST")){
	...
	q_subproductId = Utility.trimNull(file.getParameter("sub_product_id"));  //子产品信息ID

	q_cust_id = Utility.trimNull(file.getParameter("customer_cust_id")); //客户ID

	q_cust_no=Utility.trimNull(file.getParameter("cust_no")); //客户编号
...
Integer cust_no=Utility.parseInt(q_cust_no,new Integer(0));
...
Integer sub_product_id = Utility.parseInt(q_subproductId,new Integer(0));
...
if(request.getMethod().equals("POST")&&flash_flag==0){
	...
	Integer r_cust_id = Utility.parseInt(file.getParameter("cust_id"), null);
	...
	Integer r_fee_jk_type = Utility.parseInt(file.getParameter("fee_jk_type"),new Integer(0));
	...
	s_appl_vo.setCust_id(r_cust_id);	
	s_appl_vo.setProduct_id(r_product_id);
	...
	s_appl_vo.setBonus_flag(r_bonus_flag);
	s_appl_vo.setJk_type(r_jk_type);
	s_appl_vo.setBank_id(r_bank_id);
	s_appl_vo.setBank_sub_name(r_bank_sub_name);
	s_appl_vo.setGain_acct(r_gain_acct);
	s_appl_vo.setStart_date(Utility.parseInt(file.getParameter("start_date"),new Integer(0)));//改参数已作废，所以随便传入当前时间
	
	if (inputflag==1)
		s_appl_vo.setBank_acct(r_bank_acct);
	else if (inputflag==2)
		s_appl_vo.setBank_acct(r_bank_acct);

	s_appl_vo.setService_man(s_link_man);
	s_appl_vo.setSummary(r_summary);
	...
	s_appl_vo.setSq_date(r_qs_date);
	s_appl_vo.setJk_date(r_jk_date);   //改参数已作废，所以随便传入当前时间
	s_appl_vo.setCheck_man(r_check_man);
	s_appl_vo.setInput_man(input_operatorCode);
	...
	s_appl_vo.setCity_serial_no(r_city_serialno);
	s_appl_vo.setContract_sub_bh(r_contract_sub_bh);
	s_appl_vo.setFee_jk_type(r_fee_jk_type);
	s_appl_vo.setBank_acct_type(r_bank_acct_type);
	s_appl_vo.setSub_product_id(r_sub_product_id);
	...
	s_appl_vo.setChannel_cooperation(channel_cooperation);
	...
	Argument.addSysMessage(r_cust_id, "录入新申购合同"
	...
	if(product_id.intValue()>0){
		...
		if (preCodeOptions == null || preCodeOptions.equals("")){
			preCodeOptions = Argument.getPreContractOptionsCrm(input_bookCode, product_id, sub_product_id, preSerialNo, input_operatorCode,q_cust_name);		
		}
		//总行导入预约列表
...
System.out.println("---------@^^^^^---"+user_id+"&&&&&&&&&&&&&&&&&&&&&&"+product_id+"-----$$$----------"+q_cust_id+"------^^--"+cust_type);
%>
<HTML>
<HEAD>
...
function SaveAction(){
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action="apply_purchase_add2.jsp" ;
...
function CancelAction(){
	...
	location = 'apply_purchase_list.jsp?page=1&q_productId='+tempArray[0]+'&q_productCode='+tempArray[1]+'&q_cust_name='
				+tempArray[3]+'&query_contract_bh='+tempArray[4]+'&pagesize='+tempArray[5]+'&q_check_flag='+tempArray[6]+'&q_product_name='+tempArray[7];
}
...
<%}%>
	...
	if(!sl_checkDecimal(form.rg_money, '申购金额 ', 13, 3, 1))return false;//申购金额
	...
	if(parseFloat(form.pre_money.value)<parseFloat(form.rg_money.value)){
		strTemp="申购金额已大于预约金额 ";//认购金额已大于预约金额
		sl_alert(strTemp);
		return false;
	}
...
function selectPrecodeItem(preSerialNo){
	...
	if(v[0]>0){ <%-- intrust..tprecontract.serial_no --%>
		contract.getPreContractInfoCRM(preSerialNo,
			{ callback: 
				function(data){
					...
					document.theform.cust_id.value = data[0];		
...
/*设置产品及客户信息*/
function selectProductAndCustomer(value){
	disableAllBtn(true);
...
window.onload = function(){
		...
		if(v_bSuccess=="true"){
			if(v_selfbenflag==0){
			  ...
			  if (confirm("申购成功！<%=LocalUtilis.language("class.constractBH",clientLocale)%><%=LocalUtilis.language("message.is",clientLocale)%>:"+v_contract_bh+"，<%=LocalUtilis.language("message.contractMaintenanceTip",clientLocale)%> ？")){
...
function addline() {
	...
	newline.insertCell().innerHTML="<input type='file' name=file_info"+n+" size='60'>"+"<button onclick='javascript:removeline(this)'><%=LocalUtilis.language("messgae.remove",clientLocale)%> </button>";//移除
}
...
function selectTeam(value){
	if (value!=""){
		location = 'apply_purchase_add2.jsp?page=<%=sPage%>&link_man=' + value+ getLastOptions();
	}
}
...
function getCustomer(cust_id){
	if(cust_id !=null && cust_id!=""){
		...
		if(v!=null){
			document.theform.cust_no.value = v[14];
			document.theform.cust_name.value = v[0];
		}
	}else{
		alert("请选择预约编号后，查看客户信息！");
	}
...
<form name="theform" method="post" onsubmit="javascript:return validateForm(this);" ENCTYPE="multipart/form-data" action="apply_purchase_add2.jsp">
...
<div>
	<table  border="0" width="100%" cellspacing="0" cellpadding="2" style="border: 1px; border-style: dashed; border-color: blue; margin-top:5px;">
		<tr>
		...
		<tr>
			...
			<td>
				...
				<!--<button name="show_info" title="查看产品信息" onclick="javascript:openQuery('<!%=product_id%>','<!%=item_id%>');"<!%if(product_id.intValue()!=0){out.print("style=display:'';");}else{out.print("style=display:'none';");}%>>查看</button>-->
			</td>
		</tr>
	<%if(product_sub_flag.intValue() == 1){%>
		<tr>
			...
			<td align=left colspan=3>
				<select size="1" id="sub_product_id" name="sub_product_id" onkeydown="javascript:nextKeyPress(this)" class="productname" onchange="javascript:selectProductAndCustomer(this.value);">
					<%=Argument.getSubProductOptions(product_id,new Integer(0),sub_product_id)%>
				</select>&nbsp;&nbsp;
				<button class="xpbutton2" name="show_sub_info" title="查看子产品信息" onclick="javascript:openSubProduct();"><%=LocalUtilis.language("message.view",clientLocale)%></button><!-- 查看 -->
			</td>
		</tr>
	<%}%>
	</table>
	<table  border="0" width="100%" cellspacing="0" cellpadding="3" style="border: 1px; border-style: dashed; border-color: blue; margin-top:5px;">
		<TR>
			...
			<td>
				<input readonly class="edline" name="cust_no" id="cust_no" size="20" onkeydown="javascript:nextKeyPress(this)" value=""/>
				<input type="hidden" name="cust_id" value="">
				<button class="xpbutton5" accessKey=E name="customers" title="客户信息" onclick="javascript:getCustomer(cust_id.value);">查客户信息 </button>
		...
		<TR> 
			...
			<td colspan=3>
				<input name="cust_name" id="cust_name" size="50" value=""/>&nbsp;&nbsp;
				<button class="xpbutton5" accessKey=E name="btnEdit" title="<%=LocalUtilis.language("menu.customerInformation",clientLocale)%> " onclick="javascript:selectProductAndCustomer();">查询预约编号 </button>
			</td>
		</TR> 
		...
		<tr>
			...
			<td>
				...
				<button class="xpbutton2" accessKey=x name="btnPrint" 
		...
		<tr>
			...
			<td>
				<input readonly class="edline" name="pre_money" size="20" onkeydown="javascript:nextKeyPress(this)" value="">
			</td>
		</tr>
	</table>
	<table border="0" width="100%" cellspacing="0" cellpadding="2" style="border:1px;border-style:dashed;border-color:blue;margin-top:5px">
		<tr>
			...
			<td <%if(Argument.getSyscontrolValue_intrust("C30902")==1){out.print("style='display:none'");}%>>
				<input name="contract_bh" size="20" maxlength=20 value="<%=contract_bh%>" onkeydown="javascript:nextKeyPress(this)" <%if(Argument.getSyscontrolValue_intrust("C30902")==1){out.print("tabindex='-1'");}%>>
			</td>
<%
...
<%if (user_id.intValue()==2 /*2北国投*/|| user_id.intValue()==999) { %>
		<tr>
			...
			<td>
				...
				<select name="touchtype" id="period_unit"><%=Argument.getPeriodValidUnitOptions(period_unit)%></select>
			</td>
		</tr>
<%} %>
		<tr>
			...
			<td>
				<input type="text" name="bank_acct" style="display:<%=inputflag2==1?"none":""%>" value="<%=Utility.trimNull(bank_acct)%>" 
					onkeydown="javascript:nextKeyPress(this)" onkeyup="javascript:showAcctNum(this.value)" onblur="luhmCheck(this.value);" size="30"/>
			...
			<%if(inputflag2==1){%>
				<!-- 输入 -->
                <button class="xpbutton2" accessKey=x name="btnSelect" title='<%=LocalUtilis.language("message.input",clientLocale)%>' 
		...
		<tr>
			<td align="right" width="120px"><font color="red"><b>*</b></font>申购金额 :&nbsp;&nbsp;</td><!--认购金额-->
			<td>
			...
			<%if (user_id.intValue()==24/*厦门24*/) { %>
				&nbsp;&nbsp;<button class="xpbutton2" onclick="javascript:readCard(this);">刷卡</button>
			<%} %>
		...
		<td>
			...
			<button class="xpbutton2" id="newChannel1" name="newChannel1" onclick="javascript:newChannel();">新建</button>
		</td>
		...
		<tr>
			...
			<tr>
				...
				<td>
					...
					&nbsp;<button class='xpbutton4' accessKey=r id='btnSetcity' name='btnSetcity' onclick='javascript:newCity(document.theform.product_id.value);' ><%=LocalUtilis.language("class.promotionSet",clientLocale)%> </button>
				</td>
			</tr>
			<tr>
