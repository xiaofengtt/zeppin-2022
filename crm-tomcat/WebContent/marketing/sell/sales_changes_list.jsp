...
<table>
	...
	<tr>
		...
		<td align="center" width="20%">原销量客户经理:</td>
		<td align="left" width="30%">
...
<table id="table3"  border="1px" bordercolor="#000000" cellspacing="0px" style="border-collapse:collapse"  width="100%" class="tablelinecolor">
	<tr class="trh">
		...
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">产品销售类型</td>
		...
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">原销量客户经理</td>
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">转销量后客户经理</td>
		<td align="center" sort="num" style="ie2:expression(onclick=sortTable__2)">转让金额</td>
		<td align="left">转销量事由</td>
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">录入人员</td>
		<td align="center" width="*" sort="string" style="ie2:expression(onclick=sortTable__2)">录入时间</td>
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">记录状态</td>
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">确认人员</td>
		<td align="center" width="*" sort="string" style="ie2:expression(onclick=sortTable__2)">确认时间</td>
<!-- 		<td align="left">确认意见</td> -->
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">审核人员</td>
		<td align="center" width="*" sort="string" style="ie2:expression(onclick=sortTable__2)">审核时间</td>
...
<%
	...
	for(int j=0; j<list.size(); j++){
		...
		String statusName = map.get("STATUS_NAME") == null ? "无" : map.get("STATUS_NAME").toString();
...
<%-- 		<td align="center" height="28px;"><%=re_check_reason%></td> --%>
		...
		<%if (isModi) { //可编辑%>
			...
			<img src="<%=request.getContextPath()%>/images/edit.gif" width="16" height="16" title="编辑转销量信息"/></a>
		<%} else { %>
		...
		<td bgcolor="#FFFFFF" align="center">
			...
			<img src="<%=request.getContextPath()%>/images/search.gif" width="16" height="16" title="查看转销量信息"/></a>
		</td>	
		<td bgcolor="#FFFFFF" align="center">
		<%if (input_operator.hasFunc(menu_id, 103)){ %>	
		<%if (isCancel) { //可撤销%>
				<a href="#" onclick="javascript:cancelInfo('<%=Utility.trimNull(map.get("TC_SERIAL_NO"))%>');">					 	
				<img src="<%=request.getContextPath()%>/images/close.gif" width="16" height="16" title="撤销转销量信息"/></a>
		<%} else { %>
		...
		<%if (isCheck) { //可确认%>
				...
				<img src="<%=request.getContextPath()%>/images/check.gif" width="16" height="16" title="确认转销量信息"/></a>
		<%} else { %>
		...
		<%if (isRecheck) { //可审核%>
				...
				<img src="<%=request.getContextPath()%>/images/check.gif" width="16" height="16" title="审核转销量信息"/></a>
		<%} else { %>
...
<%}%>
	<tr > 
		<td class="tdh" colspan="21" align="center"><b>&nbsp;<%=LocalUtilis.language("message.total",clientLocale)%><!--合计-->&nbsp;<%=list.size()%>&nbsp;<%=LocalUtilis.language("message.entries",clientLocale)%> <!--项--></b></td>
	</tr>				
</table>
</div>
<%sclocal.remove();%>
