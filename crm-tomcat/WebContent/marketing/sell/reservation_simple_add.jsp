...
String h_e_mail = "";
String h_bp = "";
String cust_moble = Utility.trimNull(request.getParameter("cust_moble"));
//获取对象
...
if(request.getMethod().equals("POST")){
	...
	cust_moble = Utility.trimNull(request.getParameter("cust_moble"));
	...
	if("".equals(card_ids))
		card_ids = " ";//默认空值
	cust_vo.setCust_name(customer_cust_name);
	cust_vo.setCust_type(is_deal_picker);
	cust_vo.setCust_tel(cust_moble);
	cust_vo.setCard_type(card_types);
	cust_vo.setCard_id(card_ids);
	cust_vo.setInput_man(input_operatorCode);
	cust_id =  customer.append(cust_vo);
...
function SaveAction(){		
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action="reservation_simple_add.jsp";
...
function validateForm(form){
	...
	if(!sl_checkNum(form.cust_moble, "联系电话", 11, 1))	return false;//手机号码
...
<table cellSpacing=0 cellPadding=4 width="100%" border=0>
	...
	<tr>
		...
		<td width="28%">
			<input maxlength="100" name="cust_moble" size="27" onkeydown="javascript:nextKeyPress(this);" value="<%=cust_moble %>">
		</td>
	</tr>
	<tr>
