...
function showInfo2(serialno, checkflag,ht_status,bank_id,pre_flag){  
	var sQuery = document.theform.productid.value + "$" +document.theform.product_id.value + "$" + document.theform.cust_name.value + "$" + document.theform.query_contract_bh.value  + "$" + document.theform.card_id.value + "$" + document.theform.check_flag.value + "$" + document.theform.pagesize.value+ "$" + "<%=sPage%>";
	  
	if(pre_flag==1)
		location = 'subscribe_edit1.jsp?serial_no='+serialno+'&bank_id='+bank_id+'&check_flag='+checkflag+'&ht_status='+ht_status+'&page=1&sQuery=' + sQuery +"&purchase_mode_flag=1";
	else
		location = 'subscribe_edit2.jsp?serial_no='+serialno+'&check_flag='+checkflag+'&bank_id='+bank_id+'&page=1&sQuery=' + sQuery+"&purchase_mode_flag=1";
}
...
function showBenifiter(contract_id){
   var sQuery = document.theform.productid.value + "$" +document.theform.product_id.value + "$" + document.theform.cust_name.value + "$" + document.theform.query_contract_bh.value  + "$" + document.theform.card_id.value + "$" + document.theform.check_flag.value + "$" + document.theform.pagesize.value;
   location = '<%=request.getContextPath()%>/marketing/sell/benifiter.jsp?from_flag_benifitor=4&contract_id='+contract_id+'&page=1&sQuery=' + sQuery;
}
...
<div id="queryCondition" class="qcMain" style="display:none;width:450px;">
	...
	<table>
		<tr>
			...
			<td><input type="text" name="productid" value="<%=Utility.trimNull(product_code)%>" onkeydown="javascript:setProduct(this.value);" size="15">						
	...
	<%}%>
		...
		<tr>
			...
			<td  align="left" >
				<input type="text" onkeydown="javascript:nextKeyPress(this)" name="query_contract_bh" size="15" value="<%=query_contract_bh%>"></td>
...
<div>
	<div align="left" class="page-title">
		<font color="#215dc6"><b>合同信息补全</b></font>
	</div>
...
Integer mend_flag = new Integer(0);
String mend_flag_name = "";
String bank_id = "";
Integer check_man = new Integer(0);
String check_man_name = "";
...
while (iterator.hasNext()) {
	...
	qs_date = Utility.parseInt(Utility.trimNull(map.get("QS_DATE")),new Integer(0));
	check_flag_name = Argument.getCheckFlagName(check_flag);
	ht_status_name = Utility.trimNull(map.get("HT_STATUS_NAME"));
	mend_flag = Utility.parseInt(Utility.trimNull(map.get("MEND_CHECK_FLAG")),new Integer(0));
	pre_flag = Utility.parseInt(Utility.trimNull(map.get("PRE_FLAG")),new Integer(0));
	bank_id = Utility.trimNull(map.get("BANK_ID")); 
	check_man = Utility.parseInt(Utility.trimNull(map.get("CHECK_MAN")),new Integer(0));
	check_man_name = Argument.getOperatorName(check_man);
	...
	String check_summary = Utility.trimNull(map.get("CHECK_SUMMARY"));
 %>	
	<tr class="tr<%=iCount%2%>" style="color:<%=color%>">
		...
		<td align="center" ><%=check_summary%></td>
		<td align="center" ><%=mend_flag_name%></td>
		<td align="center" ><%=Argument.getPreflag(pre_flag)%></td>
    	<td align="center" >
		<%if (input_operator.hasFunc(menu_id, 102)&&mend_flag.intValue()!=2) {%>
			<button type="button"  class="xpbutton2" name="btnEdit" onclick="javascript:disableAllBtn(true);showInfo2(<%=serial_no%>,<%=check_flag%>,<%=ht_status%>,'<%=bank_id%>',<%=pre_flag%>);">&gt;&gt;</button>
		<%}%>
		</td>
		<td align="center" >
		<%if (input_operator.hasFunc(menu_id, 102)){%>
			<button type="button"  class="xpbutton2" name="btnBenifitor" title="查看受益人信息" onclick="javascript:disableAllBtn(true);showBenifiter(<%=serial_no%>);">&gt;&gt;</button>
		<%}%>
		</td>
		<td align="center" >
		<%if (input_operator.hasFunc(menu_id, 102)){%>
			<button type="button"  class="xpbutton3" accessKey=p id="btnPrint" name="btnPrint" onclick="javascript:disableAllBtn(true);print(<%=serial_no%>);">&gt;&gt;</button>
		<%}%>
		</td>
	</tr>
<%}
...
<%}%>   	
	<tr class="trbottom">
		<td class="tdh" align="left" colspan="13"><b>&nbsp;合计&nbsp;<%=pageList.getTotalSize()%>&nbsp;项</b></td>							
	</tr>	
</table>
</div>
<br>
