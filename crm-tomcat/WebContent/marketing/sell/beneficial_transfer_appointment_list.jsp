...
String tempUrl = "&from_cust_name=" + from_cust_name + "&to_cust_name=" + to_cust_name
...
<html>
<head>
<title><%=LocalUtilis.language("message.benTrans",clientLocale)%> </title>
<!--受益权转让-->
...
{
	disableAllBtn(true);
	var url = 'beneficial_transfer_appointment_list.jsp.jsp?page=<%=sPage%>&pagesize=' + document.theform.pagesize.value
...
//新建
function newInfo(){
	location = "beneficial_transfer_appointment_add.jsp";
}

//查看
function showInfo(value,flag){
	if(flag==2){
		//showModalDialog("contract_exchange_infobyCheck.jsp?serial_no="+value+"&product_id="+document.theform.product_id.value+"&query_contract_sub_bh="+document.theform.query_contract_sub_bh.value , '','dialogWidth:1100px;dialogHeight:500px;STATUS=0;help=0;');
	}
	else{
		window.location.href = "beneficial_transfer_info.jsp?serial_no="+value+"&product_id="+document.theform.product_id.value+"&contract_bh="+document.theform.contract_bh.value + "&first=true";
	}	
} 
		...
		<td>
			<table width="100%" class="tablelinecolor" cellspacing="1" cellpadding="1">
				...
				<tr class="tr<%=(iCurrent%2)%>">
					...
					<td align="center"><%=Utility.trimNull(Argument.getCheckFlagName(Utility.parseInt(Utility.trimNull(map.get("CHECK_FLAG")), new Integer(0))))%></td>
