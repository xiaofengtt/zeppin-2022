<%@ page contentType="text/html; charset=GBK" import="enfo.crm.marketing.*,enfo.crm.customer.*,enfo.crm.intrust.*,enfo.crm.affair.*,enfo.crm.tools.*,enfo.crm.dao.*,enfo.crm.system.*,enfo.crm.vo.*,java.math.*,java.util.*" %>
...
if(request.getMethod().equals("POST")){	//保存数据	
	...
	//预约信息数据集	
	pre_vo.setSerial_no(serial_no);	
	List list = preContract.listPreContractCrm(pre_vo);	
	if(list.size()>0){
		...
		List rsList_single = customer.listByControl(c_vo); //客户单个值数据集
...
function SaveAction(){		
	if(document.theform.onsubmit()){
		disableAllBtn(true);
		document.theform.action = "reserve_edit1.jsp";
...
<%if (! notNeedQuota) {%>
	if(new Number(form.pre_money.value) - new Number(form.current_money.value) > 0){
		sl_alert("预约金额不能大于可预约规模!");
		return false;
	}
	if (form.this_pre_money.value >= 3000000 && form.pre_money.value < 3000000 
			&& parseInt(form.pre_num.value) > parseInt(form.input_info4.value)) {
...
<%}%>
	...
	if(new Number(form.pay_money.value) - new Number(form.pre_money.value) > 0)	{
...
<%}%>
	...
	if (! sl_checkDate(document.theform.pre_date_picker,"<%=LocalUtilis.language("class.preDate",clientLocale)%> ")) return false; //预约日期
...
function getMarketingCustomer(prefix,readonly){
	...
	if (v) showMarketingCustomer(prefix, v);	
	return v != null;
}
/*用于销售管理里*/
function showMarketingCustomer(prefix, v){	
	...
	if(getElement(prefix, "e_mail"))
		getElement(prefix, "e_mail").value = v[18];		
	if(getElement(prefix, "gain_acct"))
		getElement(prefix, "gain_acct").value = v[0];  
		 
	prodid=0;		
	for (var i=0; i<document.theform.link_man.options.length; i++) {	
...
function showPreinfo(pre_product_id){
	...
	autoSetGainLevel(pre_product_id, document.theform.pre_money.value);
}

//查看客户信息
function getCustomer(cust_id){
	showModalDialog('<%=request.getContextPath()%>/marketing/customerInfo2.jsp?cust_id='+cust_id
				, '','dialogWidth:720px;dialogHeight:720px;status:0;help:0;');
}
	...
	if(channel_type != "")
		document.getElementById("channel_td1").style.display = 
			document.getElementById("channel_td2").style.display = 
				channel_type.substring(0,6)=="550003"? "": "none";
}
...
<table cellSpacing=0 cellPadding=4 width="100%" border=0 class="product-list">
		...
		<tr>
			...
			<td><input readonly class="edline" name="product_total_money" size="27"/></td>
		</tr>
		<tr>
			<td align="right">小额份数/累计份数 :</td>
			<td><input readonly class="edline" name="pre_total_num"  size="27"/></td>
			<td align="right">截止日期 :</td>
			<td><input readonly class="edline" name="pre_end_date_display" size="27"/><input type="hidden" name="pre_end_date"/></td>
		</tr>	
		<tr>
			<td align="right">已预约规模 :</td>
			<td><input readonly class="edline" name="pre_total_money" size="27"/></td>
			<td align="right">可预约规模 :</td>
			<td><input readonly class="edline" name="product_sy_money" size="27"/></td>
		</tr>
		<tr>
			<td align="right">预约截止时间 :</td>
			<td><input readonly class="edline" name="expre_end_time" size="27"/></td>
			<td align="right">预约有效天数 :</td>
			<td><input readonly class="edline" name="pre_valid_days" size="27"/></td>
		</tr>
		<tr>
			<td colspan="4"><hr size="1"></td>
		</tr>
		<tr style="display:<%=notNeedQuota?"none":""%>">
			...
			<td><input readonly class="edline" name="input_info1" id="input_info1" size="27"/></td>
			<td align="right">已预约小额份数/合同投资人数量配额 :</td>
			<td><input readonly class="edline" name="input_info2" id="input_info2" size="27"/></td>
		</tr>
		<tr>
			<td align="right">当前可预约金额 :</td>
			<td><input readonly class="edline" name="input_info3" id="input_info3" size="27"/></td>
			<td align="right">当前可预约份数 :</td>
			<td><input readonly class="edline" name="input_info4" id="input_info4" size="27"/></td>
		</tr>
		<tr>
			<td align="right">*预约团队 :</td><!--预约登记人-->
			<td><select size="1" name="team_id" onkeydown="javascript:nextKeyPress(this)" style="WIDTH: 160px" disabled="disabled">
			...
			<td >
				<select size="1" name="link_man" onkeydown="javascript:nextKeyPress(this)" style="WIDTH: 160px">
					<%=Argument.getManagerTeamMemberList(team_id,link_man)%>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<input name="pre_money" size="27" value="<%=Format.formatMoney(pre_money)%>" onkeydown="javascript:nextKeyPress(this)" 
