...
Integer pre_product_type = Utility.parseInt(Utility.trimNull(request.getParameter("pre_product_type")),new Integer(0));
//页面辅助参数
...
if(UNQUERY.intValue()==0)
    list = local.querySalesResultForStatisticMore(preVo);
else
    list = new ArrayList();
...
tempUrl = tempUrl+"&pre_product_type="+pre_product_type;
sUrl = sUrl + tempUrl;
...
function refreshPage(){
	...
	var url = "sales_changes_result_list.jsp?page=1&pagesize="+ document.theform.pagesize.value;	
	...
	var url = url + '&pre_product_type=' + document.theform.pre_product_type.value;
	location = url;	
}
...
function exportInfo()
{
	if(sl_confirm("导出数据"))
	{
		...
		var query_term = 'q_pre_product_id=' + document.theform.q_pre_product_id.value
					...
					+ '&pre_product_type=' + document.theform.pre_product_type.value;
	  	setWaittingFlag(false);
	 	var url = 'sales_changes_result_export.jsp?'+query_term;
	 	location = url;
	}
}
...
function newInfo(){	
// 	disableAllBtn(true);
	if (showModalDialog('sales_changes_result_add.jsp',
		 	'', 'dialogWidth:600px;dialogHeight:480px;status:0;help:0')) {
		sl_update_ok();
		location.reload();
	}
}
//修改信息
function modiInfo(serial_no,pre_product_type){

	if (showModalDialog('sales_changes_result_update.jsp?serial_no='+serial_no+'&pre_product_type='+pre_product_type,
		 	'', 'dialogWidth:600px;dialogHeight:480px;status:0;help:0')) {
		sl_update_ok();
		location.reload();
	}
...
function modiInfo(serial_no,pre_product_type){
	...
	if (showModalDialog('sales_changes_result_cancel.jsp?serial_no='+serial_no+'&pre_product_type='+pre_product_type,
		 	'', 'dialogWidth:600px;dialogHeight:480px;status:0;help:0')) {
		sl_update_ok();
		location.reload();
	}
...
//查看转销量申请信息
function loadInfo(serial_no,pre_product_type){

	if (showModalDialog('sales_changes_result_query.jsp?serial_no='+serial_no+'&pre_product_type='+pre_product_type,
		 	'', 'dialogWidth:600px;dialogHeight:480px;status:0;help:0')) {
		sl_update_ok();
		location.reload();
	}
...
<table>
	...
	<tr>
		<td align="right" width="15%">到账开始日期:</td>
		<td width="35%">
		...
		<td align="right" width="12%">到账结束日期:</td>
	...
	<tr>
		<td align="right" width="20%">产品分类:</td><!--产品类型-->
		<td align="left" width="30%">
			<select size="1" name="pre_product_type" onkeydown="javascript:nextKeyPress(this)" style="width: 120px;">
				<%=Argument.getSalecChangesResultProductTypeOptionsMore(pre_product_type+"")%>
			</select>
		</td>
	</tr>	
	<tr>
...
<table id="table3"  border="1px" bordercolor="#000000" cellspacing="0px" style="border-collapse:collapse"  width="100%" class="tablelinecolor">
	<tr class="trh">
		...
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">客户类型</td>
		<td sort="string" style="ie2:expression(onclick=sortTable__2)">客户编号</td>
...
<%
	...
	for(int j=0; j<list.size(); j++){
		...
		boolean isModi = input_operator.hasFunc(menu_id, 102);
		boolean isLoad = input_operator.hasFunc(menu_id, 101);
...
%>
	<tr>
		<td class="tdh" height="28px;" align="center"><%=product_name %></td>
		<td align=center><%=cust_type_name%></td>
		<td align=center><%=Utility.trimNull(map.get("CUST_ID"))%></td>
		<%if(record_count == 1) {%>
			<td class="tdh" height="28px;" align="center"><%=cust_name %></td>
		...
		<%} else if(record_count == -1) { %><%--其他产品分类（手工录入） --%>
			<td class="tdh" height="28px;" align="center"><%=cust_name %></td>
			<td align="center" height="28px;"> <%=Utility.trimNull(map.get("RG_SERVICE_MAN_NAME")) %></td>
			<td align="center" height="28px;"> <%=Utility.trimNull(map.get("RG_TEAM_NAME")) %></td>
		<%}  else { %>
			<%if(preProductType == 1) {%>
				<td class="tdh" height="28px;" align="center"></td>
				<td align="center" height="28px;"> <%=Utility.trimNull(map.get("TO_SERVICE_MAN_NAME")) %></td>
				<td align="center" height="28px;"> <%=Utility.trimNull(map.get("TT_TEAM_NAME")) %></td>
			<%} else {%>
				<td class="tdh" height="28px;" align="center"></td>
				<td align="center" height="28px;"> <%=Utility.trimNull(map.get("RG_SERVICE_MAN_NAME")) %></td>
				<td align="center" height="28px;"> <%=Utility.trimNull(map.get("RG_TEAM_NAME")) %></td>
		...
		<td align="center" height="28px;"><%=Utility.trimNull(map.get("PRE_PRODUCT_TYPE_NAME"))%></td>
		<td align="left" height="28px;"><%=Utility.trimNull(map.get("REMARK")) %></td>
		<td>
		<%if (isModi) { //可编辑%>
			<a href="#" onclick="javascript:modiInfo('<%=Utility.trimNull(map.get("SERIAL_NO"))%>','<%=preProductType %>');">						 	
			<img src="<%=request.getContextPath()%>/images/edit.gif" width="16" height="16" title="编辑信息"/></a>
		<%} else { %>
		...
		<%if (isCancel) { //可撤销%>
			<a href="#" onclick="javascript:modiInfo('<%=Utility.trimNull(map.get("SERIAL_NO"))%>','<%=preProductType %>');">						 	
			<img src="<%=request.getContextPath()%>/images/close.gif" width="16" height="16" title="撤销信息"/></a>
		<%} else { %>
		    				 
		<%} %>
		</td>
		<td bgcolor="#FFFFFF" align="center">
			<a href="#" onclick="javascript:loadInfo('<%=Utility.trimNull(map.get("SERIAL_NO"))%>','<%=preProductType %>');">						 	
			<img src="<%=request.getContextPath()%>/images/search.gif" width="16" height="16" title="查看信息"/></a>
		</td>
	</tr>
