...
UNQUERY = Utility.parseInt(Utility.trimNull(request.getParameter("UNQUERY")),new Integer(0));
//页面辅助参数
...
// preVo.setPre_product_type(1);//直销工作台
List list = null;
if(UNQUERY.intValue()==0){
	boolean flag = input_operator.hasFunc(menu_id, 100);
	if(flag){
		list = local.querySalesResultForStatistic(preVo);
	} else {
		list = local.querySalesResultForStatistic_q(preVo);
	}
...
<HTML>
<HEAD>
<TITLE><%=LocalUtilis.language("message.new",clientLocale)%> </TITLE>
...
function refreshPage(){
	...
	var url = "sales_changes_list_new.jsp?page=1&pagesize="+ document.theform.pagesize.value;	
...
//新建转销量处理
function newPreMoneyInfo(serial_no, cust_name, product_name, dz_money){
	var url = "sales_changes_list_new_add.jsp";
	if (showModalDialog(url+'?serial_no='+serial_no+'&cust_name='+cust_name+'&cust_type='+'&product_name='+product_name
						+'&dz_money='+dz_money+'&rg_money='+"&cust_id="+"&product_id="+"&sub_product_id=", 
...
<fieldset>
<legend>
	<b style="color:blue;">查询条件：</b>
</legend>
<table class="table-popup" style="width:100%;margin-top:10px;">
	<tr>
		<td align="right"><%=LocalUtilis.language("class.productID",clientLocale)%>:</td><!--产品编号-->
		<td >
			<input type="text" name="q_productCode" value="<%= q_productCode%>" onkeydown="javascript:setProduct(this.value);" size="25">&nbsp;
			<button type="button" style="position:relative;top:-4px" class="searchbutton" onclick="javascript:searchProduct(document.theform.q_productCode.value);" />
		</td>
		<td  align="right">产品名称 :</td>
		<td  align="left" >
			<input type="text" name="q_product_name" value="" onkeydown="javascript:nextKeyPress(this);" size="25">&nbsp;
			<button type="button" style="position:relative;top:-4px" class="searchbutton" onclick="javascript:searchProductName(document.theform.q_product_name.value);" />		
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td width="30%">
			<INPUT TYPE="text" NAME="start_date_picker"value="<%=Format.formatDateLine(start_date)%>" size="25">
		...
		<td align="left" width="30%">
			<INPUT TYPE="text" NAME="end_date_picker"value="<%=Format.formatDateLine(end_date)%>" size="25">
	...
	<tr>
		...
		<td align="left" width="30%">
			<input type="text" name="q_cust_name" value="<%=q_cust_name%>" size="25">
		</td>
		<td align="right" width="20%">产品状态:</td>
		<td align="left" width="30%">
			<select size="1" name="pre_status" onkeydown="javascript:nextKeyPress(this)" style="width: 150px;">
	...
	<tr>
		...
		<td align="left" width="30%">
			<select size="1" name="q_pre_level" onkeydown="javascript:nextKeyPress(this)" style="width: 150px;">
				<%=Argument.getDictParamOptions(1182, q_pre_level)%>
			</select>
		</td>
		<td align="right" width="20%">团队名称:</td>
		<td align="left" width="30%">
			<select size="1" name="team_id" onkeydown="javascript:nextKeyPress(this)" style="width: 150px;">
...
<%
	...
	for(int j=0; j<list.size(); j++){
		...
		Integer records_count = Utility.parseInt(Utility.trimNull(map.get("RECORDS_COUNT")),new Integer(0));
...
%>
	<tr>
		...
		<%} else if(records_count == -1) { %><%--其他产品分类（手工录入） --%>
			<td class="tdh" height="28px;" align="center"><%=cust_name %></td>
...
<%-- 		<td class="center" height="28px;" align="center"> <%=Utility.trimNull(map.get("PRE_LEVEL_NAME")) %></td> --%>
		...
		<td align="center" >
			<%if(records_count == 1 || records_count == -1){ %>
	       	<a href="#" onclick="javascript:newPreMoneyInfo('<%=Utility.trimNull(map.get("SERIAL_NO"))%>','<%=cust_name%>','<%=product_name%>','<%=dz_money%>');">
				<img src="<%=request.getContextPath()%>/images/FUNC20076.gif" width="16" height="16" title="转销量"/>
			</a>
			<%} %>
		</td>
	</tr>
