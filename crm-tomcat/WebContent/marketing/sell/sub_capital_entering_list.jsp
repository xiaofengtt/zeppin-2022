...
if (need_preliminary_check) {
	check_flag = Utility.parseInt(request.getParameter("check_flag"),new Integer(11));	// 未初审
} else {
	check_flag = Utility.parseInt(request.getParameter("check_flag"),new Integer(1)); // 已初审、未审核
}
...
function editAction(serialno,checkflag){
	spellQuery();
	location = 'sub_capital_entering_add.jsp?serial_no='+serialno+'&check_flag='+checkflag+'&action_from=1&sQuery=' + sQuery;
}

function newAction(){
	spellQuery();
	location = 'sub_capital_entering_add.jsp?action_from=1&sQuery=' + sQuery;
}
...
function StartQuery(){
	...
	location = 'sub_capital_entering_list.jsp?page=1&pagesize=' + document.theform.pagesize.value 
...
<form name="theform" method="POST" action="sub_capital_enter_remove.jsp">
...
<div id="queryCondition" class="qcMain" style="display:none;width:450px;">
	...
	<select size="1" name="check_flag" onkeydown="javascript:onkeyQuery(this);" style="WIDTH: 100px">
		<%if (need_preliminary_check) { %>
			...
			<option value="2" <%if(check_flag.intValue()==2) out.println("selected");%>>已审核</option>
		<%} else { %>
			<option value="1" <%if(check_flag.intValue()==1) out.println("selected");%>>未审核</option>
			<option value="2" <%if(check_flag.intValue()==2) out.println("selected");%>>已审核</option>
		<%} %>
		</select></td>
	</tr><tr>
...
<div>
	...
	<div align="right">
		...
		<%if (input_operator.hasFunc(menu_id, 100)) {%>
			<button type="button"  class="xpbutton3" accessKey=n name="btnNew" title="新建" onclick="javascript:newAction();">新建(<u>N</u>)
			</button><!--新建-->
		&nbsp;&nbsp;&nbsp; <%}%>	
		<%if (input_operator.hasFunc(menu_id, 101)) {%>
			<button type="button"  class="xpbutton3" accessKey=d name="btnCancel" title="删除"	onclick="javascript:if(confirmRemove(document.theform.serial_no)) {disableAllBtn(true);document.theform.submit();}">删除(<u>D</u>)
			</button><!--删除-->
		&nbsp;&nbsp;&nbsp;<%}%>						
	</div>
...
%>
	<tr class="tr<%=(iCurrent % 2)%>">
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
			<tr>
				<td width="10%"><%if(check_flag2.intValue()!=2){%> <input  type="checkbox" name="serial_no" value="<%=serial_no%>" class="flatcheckbox"> <%}%></td>
		...
		<td align="center" ><%=need_preliminary_check?(check_flag2.intValue()==11?"未初审":(check_flag2.intValue()==1?"已初审":"已审核")):(check_flag2.intValue()==1?"未审核":"已审核")%></td>		
		<td align="center" >
		<%if (input_operator.hasFunc(menu_id, 102)) {%>
		<button type="button"  class="xpbutton2" name="btnEdit" onclick="javascript:disableAllBtn(true);editAction('<%=serial_no%>','<%=check_flag2%>');">&gt;&gt;</button>
		<%}%>
		</td>
	</tr>
<%}%>
