...
if (request.getMethod().equals("POST"))
{
	...
	customer.setLink_type(Utility.parseInt(request.getParameter("link_type"),new Integer(-1)));
	customer.setLink_gd_money(Utility.parseDecimal(request.getParameter("link_gd_money"),new java.math.BigDecimal(0.00))); 	
...
function continueCreate()
{
	...
	if(form.customer_cust_type.value==2){
		<%if(requiredList.contains("LEGAL_MAN")){%>
			if(!sl_check(form.customer_legal_man, "法人姓名", 40, 1))		return false;
		<%}%>

		<%if(requiredList.contains("CONTRACT_MAN")){%>
			if(!sl_check(form.customer_contact_man, "联系人", 40, 1))		return false;
		<%}%>
	}
	...
	if(form.customer_cust_type.value==1)
	{
		...
		<%if(requiredList.contains("AGE")){%>
			if(!sl_checkNum(form.customer_age, "年龄", 3, 1))	return false;
		<%}%>
	...
	<%if(requiredList.contains("H_TEL")){%>
		if(!sl_checkNum(form.customer_h_tel, "联系电话", 11, 0))	return false;
	<%}%>

	<%if(requiredList.contains("O_TEL")){%>
		if(!sl_checkNum(form.customer_o_tel, "公司电话", 11, 0))	return false;
	<%}%>

	<%if(requiredList.contains("MOBILE")){%>
		if(!sl_checkNum(form.customer_mobile, "手机号码", 11, 0))	return false;
	<%}%>

	<%if(requiredList.contains("BP")){%>
		if(!sl_checkNum(form.customer_bp, "手机号码", 11, 0))	return false;	
	<%}%>
	...
	<%if(requiredList.contains("POST_ADDRESS")){%>
		if(!sl_check(form.customer_post_address, "联系地址", 60, 1))	return false;	  
	<%}%>
	...
	<%if(requiredList.contains("SUMMARY")){%>
		if(!sl_check(document.theform.customer_summary,'备注信息')) return false;
	<%}%>
	
	/*
	if(form.customer_touch_type.value=="110901"){
		if(form.customer_cust_type.value==1){
			if(form.customer_h_tel.value.length==0&&form.customer_o_tel.value.length==0
			&&form.customer_mobile.value.length==0&&form.customer_bp.value.length==0){
				sl_alert("请至少填一个联系号码");
				return false;
			}
		}
		if(form.customer_cust_type.value==2){
			if(form.customer_h_tel.value.length==0&&form.customer_mobile.value.length==0&&form.customer_bp.value.length==0){
				sl_alert("请至少填一个联系号码");
				return false;
			}
		}
	...
	if(form.customer_touch_type.value=="110905"){
		if(!sl_checkNum(form.customer_mobile, "手机号码", 11, 1))	return false;
	}
	*/
...
function newInfo(value)
{
	document.theform.cust_id.value = "";
	document.theform.customer_cust_type.value = value;
...
function okInfo()
{
	...
	}
	{
		...
		if(document.theform.customer_o_tel != null)
...
<form name="theform" method="post" action="customer_info_simple.jsp">
...
<input type="hidden" name="customer_cust_type" value="<%=customer.getCust_type()%>">
	...
	<TBODY>
		<TR>
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="1">
				<tr>
				<td><img border="0" src="<%=request.getContextPath()%>/images/ico_area.gif" width="32" height="32"><b>客户信息</b></td>
				</tr>
				...
				<tr>
					...
					<%}if(readonly!=1){%>
					&nbsp;<button type="button"  class="xpbutton3" accessKey=n id="btnNew" name="btnNew" onclick="javascript:newInfo(1);">新建个人</button>
					&nbsp;<button type="button"  class="xpbutton3" accessKey=n id="btnNew" name="btnNew" onclick="javascript:newInfo(2);">新建机构</button>
					<%}%>
...
<%/*}
				...
				<tr>
					...
					<td >
					<input readonly class="edline" onkeydown="javascript:nextKeyPress(this)" name="customer_cust_type_name" size="18" maxlength="20" value="<%if(customer.getCust_type()!=null){if(customer.getCust_type().intValue()==1) out.print("个人");else out.print("机构");}%>">
				...
				<td colspan=3>
<%if(cust_id==null)	{ %>	
			    <input <%if (!sReadonly.equals("")) out.print(sReadonly); else out.print("type='hidden'");%> onkeydown="javascript:nextKeyPress(this)" name="customer_voc_type_name" size="50" maxlength="20" value="">
				...
				<input <%if (!sReadonly.equals("")) out.print(sReadonly); else out.print("type='hidden'");%> onkeydown="javascript:nextKeyPress(this)" name="customer_voc_type_name" size="50" maxlength="20" value="<%=Utility.trimNull(customer.getVoc_type_name())%>">			
...
<%}%>
				<tr>
					<td align="right"><%if(requiredList.contains("H_TEL")) out.print("<font color='red'>*</font>");%>联系电话:</td>
				...
				<tr>
					<td align="right"><%if(requiredList.contains("MOBILE")) out.print("<font color='red'>*</font>");%>手机:</td>
				...
				<tr>
					...
					<td align="right"><%if(requiredList.contains("POST_ADDRESS")) out.print("<font color='red'>*</font>");%>联系地址:</td>
					<td colspan="3"><input <%=sReadonly%> onkeydown="javascript:nextKeyPress(this)" name="customer_post_address" size="60" maxlength="60" value="<%=Utility.trimNull(customer.getPost_address())%>"></td>
				...
				<tr>
					...
					<td align="right"><%if(requiredList.contains("IS_LINK")) out.print("<font color='red'>*</font>");%>关联方:</td>
...
<!-- ************************* -->
			<tr id="linkoption" style="display:<%if(customer.getIs_link()!=null ){if(customer.getIs_link().intValue()!=1) out.print("none");}%>;">
				...
				<td >
			    	<select  <%if (!sReadonly.equals("")) out.print("disabled");%> onkeydown="javascript:nextKeyPress(this)" size="1" name="link_type" style="width: 230px">
				...
				<td>
					<input <%=sReadonly%> onkeydown="javascript:nextKeyPress(this)" onkeyup="javascript:showCnMoney(this.value,link_gd_money_cn)" name="link_gd_money"   size="20" maxlength="60" value="<%=Format.formatMoney(customer.getLink_gd_money())%>">
...
%>
	...
	<td align="center" height="25">
		<input  type="text" name="fieldvalue_<%=iCount%>" size="15" onkeydown="javascript:nextKeyPress(this)" <%if(cust_id!=null){%>value="<%=Utility.trimNull(local2.getField_value())%>"<%}%>>
	</td>
			...
			<table id="tablePro<%=local1.getSerial_no()%>" border="0" width="100%" bgcolor="#000000" cellspacing="0">
				<tr>
					<td bgcolor="#FFFFFF" align="right">
						<%
							String summary = Utility.trimNull(local1.getSummary().replace('@',' '));							
							String[] ss = Utility.splitString(summary,";");
							for(int d=0;d<ss.length;d++){%>
								<input type="<%=checkname%>" id="checkbox2" class="flatcheckbox" name="checkbox2_<%=iCount%>" value="<%=Utility.trimNull(ss[d])%>" onclick="javascript:changeStyle(document.theform.checkbox2_<%=iCount%>,document.theform.fieldvalue_<%=iCount%>)" >						
								<%=Utility.trimNull(ss[d])%>
						<%}%>	
					</td>
				</tr>
			</table>
