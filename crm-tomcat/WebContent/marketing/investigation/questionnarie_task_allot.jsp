...
<BASE TARGET="_self">
<title><%=LocalUtilis.language("menu.questionnarieTaskAllot",clientLocale)%> </title>
<!--服务分配-->
...
function refreshPage(){
	disableAllBtn(true);		
	window.location = 'service_allot.jsp?page=<%=sPage%>&pagesize=' + document.theform.pagesize.value+'<%=tempUrl%>';	
}
...
function QueryAction(){		
	...
	//url 组合
	var url = "questionnarie_task_allot.jsp?1=1" ;			
...
function allotAction(){
	...
	if(checkedCount(document.getElementsByName("check_serial_no")) == 0){
		sl_alert("<%=LocalUtilis.language("message.selectRecord",clientLocale)%> ！");//请选定要分配的记录
		return false;
	}
...
<div>
	...
	<div align="right" class="btn-wrapper">
		...
		<button type="button"  class="xpbutton3"  id="queryButton1" name="queryButton1" title="<%=LocalUtilis.language("message.assign",clientLocale)%> " onclick="javascript:void(0);"><%=LocalUtilis.language("message.assign",clientLocale)%> </button>
		<%} %><!--分配-->
	</div>
	<br/>
</div>
	...
	<TABLE id="table3" border="0" cellspacing="1" cellpadding="2" class="tablelinecolor" width="100%" >
		<tr class="trh">
	         ...
	         <td width="14%" align="center"><%=LocalUtilis.language("class.serviceWayName",clientLocale)%> </td><!--任务方式-->  
	          <td width="8%" align="center"><%=LocalUtilis.language("class.serviceStatusName2",clientLocale)%> </td><!--状态2-->
...
<%}%>           
		...
		<tr class="trbottom">
			<td class="tdh" align="left" colspan="7" ><b>&nbsp;<%=LocalUtilis.language("message.total",clientLocale)%><!--合计-->&nbsp;<%=pageList.getTotalSize()%>&nbsp;<%=LocalUtilis.language("message.entries",clientLocale)%> </b></td><!--项-->
