...
Integer quotacheck_flag = Utility.parseInt(request.getParameter("quotacheck_flag"),new Integer(2));
//页面辅助参数
...
<script language=javascript>
	...
	{
		var url ="quota_query_list.jsp?page=1&pagesize=" + document.theform.pagesize.value
	...
	/*审核*/
	function showInfo(preProductId){
		window.location.href="quota_query_info.jsp?preProductId="+preProductId;
	}
</script>
</HEAD>
		...
		<tr>
			...
			<td>				
				<select name="quotacheck_flag" style="width:122">
					...
					<option value="2" <%if(quotacheck_flag.intValue()==2)out.print("selected"); %>>设置已审核</option>
					<option value="3" <%if(quotacheck_flag.intValue()==3)out.print("selected"); %>>调整未审核</option>
					<option value="4" <%if(quotacheck_flag.intValue()==4)out.print("selected"); %>>调整已审核</option>	
				</select>
			</td>
		</tr>
		<tr>
 ...
 %>
	<tr class="tr<%=(iCurrent % 2)%>">
		...
		<td align="center" width="50px">
           	<a href="#" onclick="javascript:showInfo(<%=preProductId %>);">
           		<img border="0" src="<%=request.getContextPath()%>/images/dot_5.gif" width="16" height="16">
           	</a>
		</td>
	</tr>
<%iCurrent++;
