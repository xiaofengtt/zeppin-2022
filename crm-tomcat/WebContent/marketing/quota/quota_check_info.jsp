...
if(request.getMethod().equals("POST")){
	...
	if(quotacheck_flag.intValue() == 1)
		vo.setQuotacheck_flag(new Integer(2));
	else
		vo.setQuotacheck_flag(new Integer(4));
	vo.setInput_man(input_operatorCode);
	
	//新增修改CRM中产品表
	preProduct.checkQuota(vo);
	bSuccess = true ;
}
if(!request.getMethod().equals("POST")){
...
function showInfo(preProductId,team_id){
	if(showModalDialog('quota_update.jsp?preProductId=' + preProductId + '&team_id=' + team_id, '', 'dialogWidth:360px;dialogHeight:200px;status:0;help:0') != null)
	{
		sl_update_ok();
		location.reload();
	}
...
function CancelAction(){
	window.location.href="quota_check_list.jsp?";
}
...
function checkAction(){	
	if (sl_check_pass()){	
		disableAllBtn(true);
		document.theform.action="quota_check_info.jsp" ;
		document.theform.submit();
		//sl_alert("<%=LocalUtilis.language("message.checkOK2",clientLocale)%> ");//审核成功;
	}
}
...
<div align="right">
    <button type="button"  class="xpbutton3" onclick="javascript:javascript:checkAction();"><%=LocalUtilis.language("message.pass",clientLocale)%> </button>
	&nbsp;&nbsp;
	<!--返回-->
