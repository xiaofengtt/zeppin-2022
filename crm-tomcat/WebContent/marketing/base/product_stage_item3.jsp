...
//设置查询参数
vo.setSerial_no(null);//删除时，有个setSerial_no,这里重新Set，否则删除后查不出数据
...
{
	if(showModalDialog('product_stage_item3_info.jsp?product_id=<%=product_id%>&serial_no=' + serial_no, '', 'dialogWidth:420px;dialogHeight:360px;status:0;help:0') != null)
	{
		sl_update_ok();
...
function newInfo()
{
	if(showModalDialog('product_stage_item3_info.jsp?product_id=<%=product_id%>', '', 'dialogWidth:420px;dialogHeight:360px;status:0;help:0') != null)
	{
		sl_update_ok();
		document.theform.submit();
	}
...
<form name="theform" method="get" action="product_stage_item3.jsp">
				...
				<TBODY>
					<TR>
						...
						<table id="table3" border="0" cellspacing="1" cellpadding="2" class="tablelinecolor" width="100%">
							<tr class="trh">
								<td width="11%" align="center" height="25"><%=LocalUtilis.language("class.serialNumber",clientLocale)%> </td><!--序号-->
								<td width="11%" align="center" height="25" sort="num"><%=LocalUtilis.language("class.beginMoney",clientLocale)%> </td><!--起始金额-->
								<td width="11%" align="center" height="25" sort="num"><%=LocalUtilis.language("class.endMoney",clientLocale)%> </td><!--结束金额-->
								<td width="11%" align="center" height="25" sort="num"><%=LocalUtilis.language("class.divMoney",clientLocale)%> </td><!--提取固定报酬-->
								<%if(readonly==0){%>
...
int iCount2 = 0;
Integer serial_no2;
BigDecimal begin_money;
BigDecimal end_money;
BigDecimal div_money;
...
while(it.hasNext()){
	...
	begin_money = Utility.parseDecimal(Utility.trimNull(map.get("BEGIN_MONEY")),new BigDecimal(0));
	end_money = Utility.parseDecimal(Utility.trimNull(map.get("END_MONEY")),new BigDecimal(0));
	div_money = Utility.parseDecimal(Utility.trimNull(map.get("DIV_MONEY")),new BigDecimal(0));
...
%>
							...
							<tr class="tr<%=(iCount % 2)%>">
								...
								<td width="10%" align="right" height="25"><%=Utility.trimNull(Utility.trimZero(begin_money))%></td>
								<td width="10%" align="right" height="25"><%=Utility.trimNull(Utility.trimZero(end_money))%></td>
								<td width="10%" align="right" height="25"><%=Utility.trimNull(Utility.trimZero(div_money))%></td>
								<%if(readonly==0){%>
							...
							<tr class="trbottom">
								<td class="tdh" width="9%" align="center" height="25">
									<b><%=LocalUtilis.language("message.total",clientLocale)%><!--合计--> <%=iCount2%><%=LocalUtilis.language("message.entries",clientLocale)%><!--项--></b>
								</td>
								<td width="10%" align="center" height="25">-</td>
								<td width="10%" align="center" height="25">-</td>
								<td width="10%" align="center" height="25">-</td>
