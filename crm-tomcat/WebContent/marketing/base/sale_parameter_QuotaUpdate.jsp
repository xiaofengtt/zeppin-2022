...
BigDecimal team_money = Utility.parseDecimal(request.getParameter("team_money"),new BigDecimal(0));//可分配销售配额
...
<%}%>
	function validateForm(form){
		var total_money = <%=team_money.intValue()%>+<%=quota_money.intValue()%>;
		var total_num = <%=team_q_num.intValue()%>+<%=quota_qualified_num.intValue()%>;
		//验证销售配额
		if(document.theform.quota_money != null && document.theform.quota_money.value >total_money&&document.theform.quota_money.value<0){
			alert("您输入的配额不合法，请输入不大于"+total_money);
			return false;
		}
		//验证数量配额
		if(document.theform.quota_qualified_num != null && document.theform.quota_qualified_num.value >total_num&&document.theform.quota_qualified_num.value){
			alert("您输入的配额不合法，请输入不大于"+total_num);
			return false;
		}
