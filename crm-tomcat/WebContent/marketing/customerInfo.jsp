...
<HEAD>
	...
	function queryInfo(){
		...
		document.theform.action = "customerInfo.jsp";
		disableAllBtn(true);
...
<BODY onload="javascript:document.all.q_cust_name.focus();" class="body body-nox">
	...
	<table border="0" width="100%" cellspacing="2" cellpadding="2"  >
		...
		<tr <%if(readonly==1) out.print("style='display:none'");%>>
			<td colspan="4" align="right"  >
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<button type="button"   class="xpbutton3" accessKey=q onclick="javascript:queryInfo();"><%=LocalUtilis.language("message.search",clientLocale)%> (<u>Q</u>)</button>	&nbsp;&nbsp;
				<!--搜索-->
			<%if (user_id.intValue()!=15){//建信不能对客户新建/修改 %>
				<% if (input_operator.hasFunc(menu_id, 121)){ %>
				<button type="button"   class="xpbutton3" accessKey=n id="btnNew" name="btnNew" onclick="javascript:newInfo();"><%=LocalUtilis.language("message.new",clientLocale)%> (<u>N</u>)</button>&nbsp;&nbsp;
				<!--新建-->
				<%} %>
			<%if(cust_id.intValue()>0 && modi_flag==1 && input_operator.hasFunc(menu_id, 122)){%>
				<button type="button"   class="xpbutton4" onclick="javascript:uploadCard();" <%if(user_id.intValue() != 2 ){ %> style="display:none"<%} %>>更新证件附件</button>
				&nbsp;&nbsp;&nbsp;
...
<div align="left" style="margin-left:40px;margin-top:10px;<%if (sReadonly.equals("") || readonly==1 && cust_id.intValue()>0) out.print("display:none;");%>">			
		<select onkeydown="javascript:nextKeyPress(this)"  name="id" style="width:550px;" onchange="javascript:changeCustomer();">
			<option value=""><%=LocalUtilis.language("message.pleaseSelect",clientLocale)%> </option><!--请选择-->
				...
				%>
					<option value="<%=r_cust_id%>$<%=modi_flag %>" <% if(r_cust_id.equals(cust_id)){%>selected<%}%>>							
...
<iframe src ="customerInfo_details_view.jsp?cust_id=<%=cust_id%>&select_flag=<%=select_flag%>" name="sonIframe" frameborder="no" border="0" 
	height="<%=user_id.intValue()==2?1010:1010%>" width="700" scrolling="no"></iframe>
</fieldset>
</div>
<!--子窗口 和 父窗口 的 联系节点-->
