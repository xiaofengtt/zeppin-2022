...
Integer coProduct_id=Utility.parseInt(request.getParameter("coProduct_id"),new Integer(0));
boolean bSuccess = false;
tcoProductVO.setCoProduct_id(coProduct_id);
List tcoProductList=tcoProductLocal.queryByList(tcoProductVO);
Map tcoProductMap=(Map)tcoProductList.get(0);
String coProduct_name=Utility.trimNull(tcoProductMap.get("COPRODUCT_NAME"));
String publish_date=Utility.trimNull(tcoProductMap.get("PUBLISH_DATE"));
Integer team_id=Utility.parseInt(Utility.trimNull(tcoProductMap.get("TEAM_ID")),new Integer(0));
Integer coProduct_manager=Utility.parseInt(Utility.trimNull(tcoProductMap.get("COPRODUCT_MANAGER")),new Integer(0));
String coProduct_type=Utility.trimNull(tcoProductMap.get("COPRODUCT_TYPE"));
String selfMade_type=Utility.trimNull(tcoProductMap.get("SELFMADE_TYPE"));
String coProduct_price=Utility.trimNull(tcoProductMap.get("COPRODUCT_PRICE"));
String coProduct_summary=Utility.trimNull(tcoProductMap.get("COPRODUCT_SUMMARY"));


if(request.getMethod().equals("POST")){
	tcoProductVO.setCoProduct_id(Utility.parseInt(request.getParameter("coProduct_id"),new Integer(0)));
	...
	tcoProductLocal.modi(tcoProductVO);
...
function saveAction(){
	...
	document.theform.action="tcoProduct_edit.jsp";	
...
<input type="hidden" name="coProduct_id" value="<%=coProduct_id %>"/>
...
<table border="0" width="98%" cellspacing="3">
	<tr>
		...
		<td align="left">
			<input type="text" name="coProduct_name" size="25" value="<%=coProduct_name %>" onkeydown="javascript:nextKeyPress(this)">
		</td>
		<td  align="right">首次发行日期: </td>
		<td>
			<INPUT TYPE="text" NAME="publish_date_picker" class=selecttext value="<%=publish_date %>" >
	...
	<tr>	
		...
		<td align="left" >
			<select name="coProduct_type" style="width: 145px;">
				<%=Argument.getDictParamOptions(5001,coProduct_type) %>
			</select>
		</td>
		<td  align="right">研发情况:</td>
		<td>
			<select name="selfMade_type" style="width: 145px;">
				<%=Argument.getDictParamOptions(5002,selfMade_type)%>
			</select>
		</td>
	</tr>	
	<tr>	
		...
		<td align="left" colspan="3">
			<input type="text" name="coProduct_price" size="25" value="<%=coProduct_price %>" onkeydown="javascript:nextKeyPress(this)" onkeyup="javascript:sl_checkDecimal(form.coProduct_price,'合同金额',13,3,0);showCnMoney(this.value)">
	...
	<tr>
		...
		<td colspan="3">
			<textarea rows="" cols="68" name="coProduct_summary"><%=coProduct_summary %></textarea>
		</td>
	</tr>
</table>
