...
Integer q_extension = Utility.parseInt(Utility.trimNull(request.getParameter("q_extension")),new Integer(-1));
...
IPageList pageList = seatLocal.query_page_seatCallDetail(vo,totalColumn,t_sPage,t_sPagesize);
%>

<HTML>
<HEAD>
<TITLE>坐席电话呼叫明细报表</TITLE>
...
function refreshPage(){
	...
	window.location = 'tellout_detail_list.jsp?page=<%=sPage%>&pagesize=' + _pagesize[_pagesize.selectedIndex].getAttribute("value")+'<%=tempUrl%>';
}
...
function QueryAction(){		
		...
		//url 组合
		var url = "tellout_detail_list.jsp?page=<%=sPage%>&pagesize=" + _pagesize[_pagesize.selectedIndex].getAttribute("value");		
	...
	<tr>
	  ...
	  <td>
		...
		<tr>
	    	...
	    	<td align="left">
	    	    <input type="text" name="q_start_date_picker" class=selecttext value="<%=Format.formatDateLine(q_start_date)%>"> 
			    <input TYPE="button" value="▼" class=selectbtn
			    onclick="javascript:CalendarWebControl.show(theform.q_start_date_picker,theform.q_start_date_picker.value,this);"tabindex="13"> 
			    <INPUT type="hidden" name="q_start_date">
		    </td>
		    <td align="right">结束日期:</td>
		    <td align="left">
		        <input type="text" name="q_end_date_picker" class=selecttext value="<%=Format.formatDateLine(q_end_date)%>"> 
			    <input TYPE="button" value="▼" class=selectbtn
			    onclick="javascript:CalendarWebControl.show(theform.q_end_date_picker,theform.q_end_date_picker.value,this);"tabindex="13"> 
			    <INPUT type="hidden" name="q_end_date">
		    </td>
	    </tr>
	 <tr>
