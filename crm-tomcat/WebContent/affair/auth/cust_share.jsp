...
authorizationShareVO.setShareType(new Integer(1)); //1共享

String[] totalColumn = new String[0];

IPageList pageList =authorizationShareLocal.pagelist_query_share(authorizationShareVO,totalColumn,Utility.parseInt(sPage,1),Utility.parseInt(sPagesize,8));
...
<script language="javascript">
	...
	function AppendAction(){
		var url = "<%=request.getContextPath()%>/affair/auth/cust_share_new.jsp";
	...
	function ModiAction(){
		...
		var url = "<%=request.getContextPath()%>/affair/auth/cust_share_edit.jsp";
		oState.flag = "edit";
	...
	function DelAction(){
		var url = "<%=request.getContextPath()%>/affair/auth/cust_share_remove.jsp";
		var bool = false;
	...
	function ModiStatusAction(statusValue){
		var url = "<%=request.getContextPath()%>/affair/auth/cust_share_modi_status.jsp?statusValue="+statusValue;
		var bool = false;
		document.getElementsByName("theform")[0].setAttribute("action",url);
		var check = document.getElementsByName("btnCheckbox");
		for(var i=0; i != check.length; i++ ){
	...
	function refreshPage(){
		...
		window.location = '<%=request.getContextPath()%>/affair/auth/cust_share.jsp?page=<%=sPage%>&pagesize=' + _pagesize[_pagesize.selectedIndex].getAttribute("value")+'<%=tempUrl%>';
	}
	
	function QueryAction(){
		var _pagesize = document.getElementsByName("pagesize")[0];
		//url 组合
		var url = '<%=request.getContextPath()%>/affair/auth/cust_share.jsp?page=<%=sPage%>&pagesize=' + _pagesize[_pagesize.selectedIndex].getAttribute("value");
...
<td>
	...
	<tr class="trh">
         <td width="9%" align="center"><%=LocalUtilis.language("class.shareNo",clientLocale)%> </td><!--共享编号-->
         <td width="15%" align="center"><%=LocalUtilis.language("class.sourceManager",clientLocale)%> </td><!-- 源客户经理 -->
         <td width="9%"  align="center"><%=LocalUtilis.language("class.sharedManager",clientLocale)%></td><!-- 已共享经理 -->
		 <td width="9%"  align="center"><%=LocalUtilis.language("class.cust_sharedDescription",clientLocale)%></td><!-- 授权描述 -->
