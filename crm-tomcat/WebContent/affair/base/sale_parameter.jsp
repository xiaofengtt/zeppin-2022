...
BigDecimal team_money = new BigDecimal(0);//可分配销售配额
BigDecimal quota_money = new BigDecimal(0);//销售配额
BigDecimal quota_money_total = new BigDecimal(0);//总销售配额
Integer quota_qualified_num = new Integer(0);//可分配数量配额
int	quota_qualified_num_total = 0;//总合格投资人数量配额
Integer team_q_num = new Integer(0);//合格投资人数量配额
...
//设置团队配额
	function showInfo(productId,sub_productId,team_id,team_name,team_money,quota_money,quota_qualified_num,team_q_num){
		if(showModalDialog('<%=request.getContextPath()%>/marketing/base/sale_parameter_QuotaUpdate.jsp?productId=' + productId + '&sub_productId='+ sub_productId +'&team_id='
							 + team_id+'&team_name='+team_name +'&team_money='+team_money+'&quota_money='+quota_money+'&quota_qualified_num='+quota_qualified_num+'&team_q_num='
							 + team_q_num, '', 'dialogWidth:380px;dialogHeight:350px;status:0;help:0') != null)
		{
		sl_update_ok();
		location="sale_parameter.jsp?productId="+productId+"&sub_productId="+sub_productId;
		}
	}
...
<form id="theform" name="theform" method="post" action="sale_parameter.jsp">
	<div align="left">
...
<%//查看团队销售信息
	...
	List  listTTeam = sale_parameter.queryTTeamValue1(productId,input_operatorCode,sub_productId);
						...
						<tr class="tr0">
							...
							<td align="center" width="50px">
								<%if(team_money.intValue()>0){ %>
				               	<a href="#" onclick="javascript:showInfo(<%=productId %>,<%=sub_productId %>,<%=team_id %>,'<%=team_name %>',<%=team_money %>,<%=quota_money %>,<%=quota_qualified_num %>,<%=team_q_num %>);">
	...
	<%} %>
					...
					<tr class="trbottom">
						<!--合计--><!--项-->
				        <td class="tdh" align="left" ><b>&nbsp;<%=LocalUtilis.language("message.total",clientLocale)%>&nbsp;<%=iCount%>&nbsp;<%=LocalUtilis.language("message.entries",clientLocale)%> </b></td>
						<td align="center"></td>
