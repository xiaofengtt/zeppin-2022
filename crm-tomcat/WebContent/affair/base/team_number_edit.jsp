...
if(request.getMethod().equals("POST")){
	...
	managerid = Utility.parseInt(Utility.trimNull(request.getParameter("managerid")),null);
...
<script language=javascript>
	...
	window.onload = function (){
		try{
			var _description = document.getElementsByName("team_position")[0];
...
<form name="theform" method="post" action="<%=request.getContextPath()%>/affair/base/team_number_edit.jsp" onsubmit="javascript:return validateForm(this);">
...
<input type="hidden" name="managerid" value="<%=Utility.trimNull(request.getParameter("managerid"))%>" />
...
<div align="center">
	<table border="0" width="100%" cellspacing="5" cellpadding="0" class="product-list">
		...
		<tr>
			...
			<td>
				<select style="width: 110px;" disabled="disabled">
					<%=Argument.getManager_Value(Utility.parseInt(Utility.trimNull(request.getParameter("managerid")),null))%>
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td colspan="3">
				<textarea rows="3" name="description" onkeydown="javascript:nextKeyPress(this)" cols="62"><%=Utility.trimNull(request.getParameter("description"))%></textarea>			
			</td>
		</tr>
	</table>
</div>
