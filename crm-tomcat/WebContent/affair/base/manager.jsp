...
$(function(){
	...
	$("#tree").fancytree({
		...
		tooltip:false,
		// source: SOURCE,
		source: { url: "/affair/base/managerTreeJson3.jsp"},
	...
	$("#tree").contextmenu({
		...
		menu: [
			{title: "添加下一级", cmd: "addLevel", uiIcon: "ui-icon-arrowreturn-1-e" },
			{title: "查看", cmd: "viewMembers", uiIcon: "ui-icon-note" },
...
{	
	if(key == "0" || key == ""){
		...
		depart_id=key.split("_")[0];
		manager_id=key.split("_")[1];
	}
    if(depart_id==''||depart_id=='0')//depart_id=0或者为空，右键修改实效
    	return;
	var level_no=title.split("_")[0];
	var level_name=title.split("_")[1].split("(")[0];
