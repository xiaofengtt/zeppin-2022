...
Integer begin_plan_date = Utility.parseInt(Utility.trimNull(request.getParameter("begin_plan_date")),new Integer(Utility.getCurrentDate()));
Integer end_plan_date = Utility.parseInt(Utility.trimNull(request.getParameter("end_plan_date")),new Integer(Utility.getCurrentDate()));
...
vo_sms.setCheck_flag(new Integer(0));
...
//url设置
String tempUrl = "&sms_content="+sms_content
					+"&begin_plan_date="+begin_plan_date
...
function QueryAction(){
	syncDatePicker(document.theform.begin_plan_date_picker,document.theform.begin_plan_date);
	syncDatePicker(document.theform.end_plan_date_picker,document.theform.end_plan_date);
	var url = "send_info.jsp?page=<%=sPage%>&pagesize="+ document.theform.pagesize.value;	
...
function newInfo(){
	var url = "<%=request.getContextPath()%>/affair/message/send_create_action_bycust.jsp";		
	dialog("","iframe:"+url,"850px","470px","show","<%=request.getContextPath()%>"); 
	showWaitting(0);
}

function AppendAction(){
	var url = "<%=request.getContextPath()%>/affair/message/send_create_action.jsp";		
	dialog("","iframe:"+url,"850px","600px","show","<%=request.getContextPath()%>"); 
	showWaitting(0);	
}
/*修改方法*/
function modiAction(serial_no){				
	var url = "<%=request.getContextPath()%>/affair/message/send_create_action_bycust.jsp?close_flag=1&serial_no="+serial_no;		
	dialog("","iframe:"+url,"850px","470px","show","<%=request.getContextPath()%>"); 
	showWaitting(0);
}
function delAction(){
	...
	if(sl_confirm("发送所选的短信")){
		document.getElementsByName('theform')[0].action="send_info_del.jsp";
...
<div id="queryCondition" class="qcMain" style="display:none;width:510px;">
	...
	<table border="0" width="100%" cellspacing="2" cellpadding="2">
		...
		<tr>
			...
			<td align="left" >
				<select id="way_type" name="way_type" style="width:60px;">
...
<div>
	...
	<div align="right" class="btn-wrapper">
        ...
        <!--新增-->
		<button type="button"  class="xpbutton3" accessKey=n id="btnAppend" name="btnAppend" onclick="javascript:newInfo();">按客户发送</button>
		&nbsp;&nbsp;&nbsp;
		<button type="button"  class="xpbutton3" accessKey=s id="btnAppend" name="btnAppend"  onclick="javascript:AppendAction();">自定义发送</button>
		&nbsp;&nbsp;&nbsp;
        <!--删除-->
		<button type="button"  class="xpbutton3" title="<%=LocalUtilis.language("message.delete",clientLocale)%>" onclick="javascript:delAction();"><%=LocalUtilis.language("message.delete",clientLocale)%></button>
		<%}%>
...
%>
	<tr class="tr<%=iCount%2%>">
		...
		<td align="center"><%if(check_flag.intValue() ==-1)out.print("审核不通过");else if(check_flag.intValue() ==1)out.print("待审核");else if(check_flag.intValue() ==2) out.print("已审核");else out.print("");%></td> 
		...
		<td align="center">       	
	      	<a href="javascript:modiAction(<%=serial_no%>);">
				<%if(input_operator.hasFunc(menu_id, 100) && check_flag.intValue()==0){ %>	
