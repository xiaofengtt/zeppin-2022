...
<BASE TARGET="_self">
<title><%=LocalUtilis.language("message.taskArchiving",clientLocale)%> </title><!-- 任务归档 -->
...
function QueryAction(){		
	if(validate()){	
		//url 组合
		var url = "<%=request.getContextPath()%>/affair/service/service_pigeonhole.jsp?page=1&pagesize="+ document.theform.pagesize.value;	
...
function showMenuView(serial_no,serviceType){
	var url = "<%=request.getContextPath()%>/affair/service/service_main_view.jsp?serial_no="+serial_no+"&serviceType="+serviceType;	
...
function showRemark(serial_no){
	...
	showModalDialog(url,'', 'dialogWidth:850px;dialogHeight:500px;status:0;help:0');
	showWaitting(0);
}

function showDetail(serial_no){
	var url = "<%=request.getContextPath()%>/affair/service/service_pigeonhole_detail.jsp?task_serial_no="+serial_no;	
	showModalDialog(url,'', 'dialogWidth:800px;dialogHeight:600px;status:0;help:0');
	showWaitting(0);
}
</script>
</head>
...
<div id="queryCondition" class="qcMain" style="display:none;width:540px;height:130px;">
	...
	<table border="0" width="95%" cellspacing="2" cellpadding="2">
		<tr>
			...
			<td  align="left">
				<select name="q_serviceType" id="q_serviceType" onkeydown="javascript:nextKeyPress(this)" style="width:122px">	
					<%= Argument.getService_typeName(q_serviceType) %>
				</select>
			</td>	
			<td  align="right"><%=LocalUtilis.language("class.serviceWay",clientLocale)%> ： </td><!-- 联系方式 -->
			<td  align="left">
				<select name="q_serviceWay" id="q_serviceWay" style="width:122px">
...
<div id="headDiv" style="margin-left:0px">
	<table cellSpacing="1" cellPadding="2" width="750px"  bgcolor="#CCCCCC" class="table-select">
			<tr style="background:F7F7F7;">
				...
				<!-- 全部 -->
                <td width="60px" align="center" id="td0" <%if (q_service_status.intValue()==0) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(0);" class="a2"><%=LocalUtilis.language("message.all",clientLocale)%> </a></font></td>
				<!-- 未处理 -->
                <td width="60px" align="center" id="td1" <%if (q_service_status.intValue()==1) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(1);" class="a2"><%=LocalUtilis.language("message.undone",clientLocale)%> </a></font></td>
				<!-- 待处理 -->
                <td width="60px" align="center" id="td2" <%if (q_service_status.intValue()==2) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(2);" class="a2"><%=LocalUtilis.language("class.pending",clientLocale)%> </a></font></td>
				<!-- 处理中 -->
                <td width="100px" align="center" id="td3" <%if (q_service_status.intValue()==3) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(3);" class="a2"><%=LocalUtilis.language("message.beingProcessed",clientLocale)%> </a></font></td>
				<!-- 已处理 -->
                <td width="60px" align="center" id="td4" <%if (q_service_status.intValue()==4) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(4);" class="a2"><%=LocalUtilis.language("class.processed",clientLocale)%> </a></font></td>
				<!-- 作废 -->
                <td width="60px" align="center" id="td5" <%if (q_service_status.intValue()==5) out.print(" class='active'"); %>><font size="2" face="微软雅黑"><a href="#" onclick="javascript:changeStatus(5);" class="a2"><%=LocalUtilis.language("class.invalid",clientLocale)%> </a></font></td>
			</tr> 
	</table>
</div>
...
<div style="margin-top:5px;">
	<TABLE id="table3" border="0" cellspacing="1" cellpadding="2" class="tablelinecolor" width="100%">
		<tr class="trh">
	         ...
	         <td width="8%" align="center"><%=LocalUtilis.language("class.created",clientLocale)%> </td><!-- 创建者 -->
	         ...
	         <td align="center"><%=LocalUtilis.language("class.customerCount",clientLocale)%> </td><!-- 目标客户数 -->
	         <td align="center"><%=LocalUtilis.language("class.completeCount",clientLocale)%> </td><!-- 完成客户数 -->
			 <td align="center">任务明细</td>
	         <td align="center"><%=LocalUtilis.language("messag.memo",clientLocale)%> </td><!-- 备忘 -->
	     </tr>
<%
//声明字段
Integer serial_no;
...
%>
	<tr class="tr<%=iCount%2%>">
		 ...
		 <td align="center">            	
          	<a href="javascript:showDetail(<%=serial_no%>)">
           		<img border="0" src="<%=request.getContextPath()%>/images/auditing.gif"  width="16" height="16">
           	</a>
	  	 </td>    	
 		 <td align="center">         
...
<%}
		...
		<tr class="trbottom">
			<!-- 合计  --><!-- 项 -->
			<td class="tdh" align="left" colspan="12"><b>&nbsp;<%=LocalUtilis.language("message.total",clientLocale)%>&nbsp;<%=pageList.getTotalSize()%>&nbsp;<%=LocalUtilis.language("message.entries",clientLocale)%> </b></td>				
		</tr>
	</TABLE>
</div>
<br>
