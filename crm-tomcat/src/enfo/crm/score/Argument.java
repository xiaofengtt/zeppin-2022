...
//	查询积分活动
	public static String getActivity(Integer current_date, int flag, Integer inputMan,String activity_id) throws Exception {
		String listSql = "{call SP_QUERY_TSCORECARD_ACTIVITY (?,?,?)}";
		...
		stmt.setInt(1, 0);
		stmt.setInt(2, current_date.intValue());
		stmt.setInt(3, flag);
		ResultSet rs = stmt.executeQuery();
		try {
			while (rs.next()){
				appendOptions(sb, rs.getString("ACTIVITY_ID"), rs.getString("SUMMARY"), activity_id);
			}
		} finally {
			rs.close();
			stmt.close();
