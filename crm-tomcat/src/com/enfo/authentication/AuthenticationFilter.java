...
public class AuthenticationFilter implements Filter{
	...
	protected void doLoginFilter(HttpServletRequest request,HttpServletResponse response, FilterChain filterChain) throws Exception { 
		...
		if (request.getRequestURI().endsWith(this.loginUrl)) {
			doLogout(request, response);
			//filterChain.doFilter(request, response);
			//return;
		}
		...
		//登陆标志
		String inuri = request.getRequestURI();
		...
		InputMan input_operator = (InputMan)request.getSession().getAttribute("OPERATOR");
		 ServletContext application = request.getSession().getServletContext();
		_is_token = _is_token && application.getAttribute("USER_ID") != null && (input_operator != null);
