...
GO
CREATE PROCEDURE SP_MODI_TCOMAINTAINMONEYMX @IN_MONEYMX_ID    INT,     --到账明细ID，自增长
...
AS
    ...
    SELECT @SBUSI_NAME = N'合同管理之修改维护合同到帐明细'
    SELECT @SSUMMARY = N'合同管理之修改维护合同到帐明细'
    ...
    UPDATE TCOCONTRACTMAINTAIN
        SET ARRIVE_MONEY = ISNULL(ARRIVE_MONEY,0) + (ISNULL(@IN_ARRIVE_MONEY,0) - ISNULL(@V_OLD_ARRIVE_MONEY,0)),
            ARRIVEMONEY_FLAG =CASE WHEN (@V_WH_MONEY <=  (ISNULL(@V_ARRIVE_MONEY,0) + (ISNULL(@IN_ARRIVE_MONEY,0) - ISNULL(@V_OLD_ARRIVE_MONEY,0)))) THEN 3 ELSE 2 END
    ...
    END
    --
    SELECT @SSUMMARY = N'合同管理之修改维护合同到帐明细，合同编号：'+@V_COCONTRACTMAINTAIN_SUB_BH + '，到帐日期：'+CONVERT(NVARCHAR(8),@IN_ARRIVE_TIME)+'，到帐金额：'+CONVERT(VARCHAR(30),@IN_ARRIVE_MONEY)
