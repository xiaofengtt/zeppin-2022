...
GO
ALTER PROCEDURE dbo.SP_QUERY_TPRECONTRACT_M  @IN_SERIAL_NO       INT,                    --序号
                                        ...
                                        @IN_SL_STATUS       INT,					--双录状态
                                        @IN_SL_TYPE			NVARCHAR(20) = NULL		--双录模式
			...
			BEGIN
				SELECT A.SERIAL_NO,A.PREPRODUCT_ID,A.CUST_ID,A.PRE_CODE,A.PRE_MONEY,A.RG_MONEY,A.RG_DATE,A.LINK_MAN,
					...
					FROM TPRECONTRACT A LEFT JOIN TPREPRODUCT B ON (A.PREPRODUCT_ID = B.PREPRODUCT_ID)
						...
						--20180615列表查询增加按双录信息录入状态筛选
						LEFT JOIN 
						(SELECT * FROM (select * , ROW_NUMBER() over(partition by PRE_SERIAL_NO order by INPUT_TIME DESC) as rownum 
							from TPREDOUBLERECORDINFO) em WHERE em.rownum=1) PR
						ON A.SERIAL_NO=PR.PRE_SERIAL_NO
			...
			BEGIN
				SELECT A.SERIAL_NO,A.PREPRODUCT_ID,A.CUST_ID,A.PRE_CODE,A.PRE_MONEY,A.RG_MONEY,A.RG_DATE,A.LINK_MAN,
					...
					FROM TPRECONTRACT A LEFT JOIN TPREPRODUCT B ON (A.PREPRODUCT_ID = B.PREPRODUCT_ID)
						...
						LEFT JOIN 
						(SELECT * FROM (select * , ROW_NUMBER() over(partition by PRE_SERIAL_NO order by INPUT_TIME DESC) as rownum 
							from TPREDOUBLERECORDINFO) em WHERE (ISNULL(@IN_SL_TYPE,'') = '' OR em.RECORD_TYPE=@IN_SL_TYPE) 
							AND em.rownum=1) PR
						ON A.SERIAL_NO=PR.PRE_SERIAL_NO
					WHERE (A.PREPRODUCT_ID = @IN_PREPRODUCT_ID OR ISNULL(@IN_PREPRODUCT_ID,0)=0 OR (A.PREPRODUCT_ID=0 AND A.PRODUCT_ID=@V_PRODUCT_ID))
						AND(ISNULL(B.PREPRODUCT_NAME,'') LIKE '%'+@IN_PREPRODUCT_NAME+'%')
						...
						AND(ISNULL(A.CHANNEL_FARE,0) BETWEEN @IN_CHANNEL_FARE1 AND @IN_CHANNEL_FARE2)
						AND(A.PRE_LEVEL = @IN_PRE_LEVEL OR ISNULL(@IN_PRE_LEVEL,'') = '')
						AND (ISNULL(@IN_SL_STATUS,-2) = -2 OR PR.[STATUS] = @IN_SL_STATUS)
						AND (ISNULL(@IN_SL_TYPE,'') = '' OR PR.RECORD_TYPE=@IN_SL_TYPE)
			END
