...
GO
CREATE PROCEDURE SP_QUERY_TCOCONTRACTPAYPLAN_UNRECIEVE @IN_PAYPLAN_ID         INT,            --付款计划ID
                                                       @IN_COCONTRACT_SUB_BH  NVARCHAR(200),  --合同编号
                                                       @IN_CUST_NAME          NVARCHAR(60),   --客户名称
                                                       @IN_PAY_NUM_NAME       NVARCHAR(60),   --期数说明
                                                       @IN_EXP_DATE_BEGIN     INT,            --预计付款日期从
                                                       @IN_EXP_DATE_END       INT,            --预计付款日期到
...
AS
    ...
    IF ISNULL(@IN_EXP_DATE_BEGIN,0) =0
        SET @IN_EXP_DATE_BEGIN =20000101
    IF ISNULL(@IN_EXP_DATE_END,0) =0
        SET @IN_EXP_DATE_END =20991231

    SELECT A.*,B.COCONTRACT_SUB_BH,C.CUST_NAME
        FROM TCOCONTRACTPAYPLAN A,TCOCONTRACT B,TCustomers C
            WHERE (ISNULL(@IN_PAYPLAN_ID,0) = 0 OR A.PAYPLAN_ID =@IN_PAYPLAN_ID)
                AND (ISNULL(@IN_PAY_NUM_NAME,'') ='' OR A.PAY_NUM_NAME LIKE '%'+@IN_PAY_NUM_NAME+'%')
                AND (A.EXP_DATE BETWEEN @IN_EXP_DATE_BEGIN AND @IN_EXP_DATE_END)
                AND (ISNULL(@IN_ACCT_NAME,'') ='' OR A.ACCT_NAME LIKE '%'+@IN_ACCT_NAME+'%' )
                ...
                AND A.COCONTRACT_ID =B.COCONTRACT_ID
                AND (ISNULL(@IN_COCONTRACT_SUB_BH,'') ='' OR B.COCONTRACT_SUB_BH LIKE '%'+@IN_COCONTRACT_SUB_BH+'%')
                AND B.CUST_ID =C.CUST_ID
                AND (ISNULL(@IN_CUST_NAME,'') ='' OR C.CUST_NAME LIKE '%'+@IN_CUST_NAME+'%')
                AND( B.INPUT_MAN = @IN_INPUT_MAN OR B.INPUT_MAN IN (SELECT TYPE_CONTENT FROM TDICTPARAM WHERE TYPE_ID=9111 AND TYPE_VALUE=CONVERT(NVARCHAR(30),@IN_INPUT_MAN)) OR B.CHECK_MAN = @IN_INPUT_MAN OR C.SERVICE_MAN = @IN_INPUT_MAN OR @V_FLAG_ACCESS_ALL = 1
                    OR EXISTS(SELECT 1 FROM @V_TEMPCUST2 Z WHERE C.CUST_ID = Z.CUST_ID))
GO
