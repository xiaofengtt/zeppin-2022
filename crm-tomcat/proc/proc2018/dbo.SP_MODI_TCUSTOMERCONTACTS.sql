...
GO
CREATE PROCEDURE SP_MODI_TCUSTOMERCONTACTS   @IN_CONTACTID         INTEGER,         --联系人ID
...
AS
    ...
    SELECT @SBUSI_NAME = N'修改客户联系人信息', @SSUMMARY = N'修改客户联系人信息'
    IF NOT EXISTS(SELECT 1 FROM TCUSTOMERS WHERE CUST_ID = @IN_CUST_ID )
        RETURN @V_RET_CODE - 1  --客户信息不存在

    BEGIN TRANSACTION
    UPDATE TCustomerContacts
    SET CUST_ID = @IN_CUST_ID,
        Contactor = @IN_CONTACTOR,
        PhoneHome = @IN_PHONEHOME,
        PhoneOffice = @IN_PHONEOFFICE,
        Moblie = @IN_MOBLIE,
        Fax = @IN_FAX,
        Email = @IN_EMAIL,
        Address = @IN_ADDRESS,
        ...
        Anniversary = @IN_ANNIVERSARY,
        IsMarried = @IN_ISMARRIED,
        Spouse = @IN_SPOUSE,
        Boss = @IN_BOSS,
        ...
        City = @IN_CITY,
        Role = @IN_ROLE,
        Assistant = @IN_ASSISTANT,
        AssistantTelphone = @IN_ASSISTANTTELPHONE,
        Manager = @IN_MANAGER,
        ManagerTelphone = @IN_MANAGERTELPHONE,
        ContactWay = @IN_CONTACTWAY,
        PreferredDate = @IN_PREFERREDDATE,
        PreferredTime = @IN_PREFERREDTIME,
        ReceiveContactWay = @IN_RECEIVECONTACTWAY,
        ReceiveServices = @IN_RECEIVESERVICES,
    ...
    BEGIN
        ...
        RETURN -100
    END
    SET @SSUMMARY = N'修改客户联系人信息，操作员：' + CAST(@IN_INPUT_MAN AS NVARCHAR(10))
