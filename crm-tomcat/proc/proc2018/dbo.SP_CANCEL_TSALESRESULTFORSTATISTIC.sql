...
GO
ALTER PROCEDURE SP_CANCEL_TSALESRESULTFORSTATISTIC @IN_SERIAL_NO  INT, --序号 
											  @IN_STATUS     INT, --审核状态
											  @IN_INPUT_MAN  INT,  --操作员
											  @IN_CHECK_REASON  NVARCHAR(200)  = NULL  --意见
WITH ENCRYPTION
AS
    SET NOCOUNT ON
    ...
    SELECT @SBUSI_NAME = N'撤销到帐结果信息'
    SELECT @SSUMMARY = N'撤销到帐结果信息'
	...
	SELECT @V_PRE_SERIAL_NO = PRE_SERIAL_NO,@V_PREPRODUCT_NAME = RG_PRODUCT_NAME, @V_CUST_NAME=RG_CUST_NAME
	FROM TSALESRESULTFORSTATISTIC WHERE SERIAL_NO = @IN_SERIAL_NO
    
    

	SET @V_STATUS_NAME = N'已撤销'

    ------------------------------------------------------------------------
    BEGIN TRANSACTION

    UPDATE TSALESRESULTFORSTATISTIC SET [STATUS] = @IN_STATUS  
        WHERE SERIAL_NO = @IN_SERIAL_NO


    --2.日志
    SELECT @SSUMMARY = N'撤销到帐结果信息，产品名称：'+@V_PREPRODUCT_NAME
						+N'客户名称：' + @V_CUST_NAME
						+N'状态'+@V_STATUS_NAME
