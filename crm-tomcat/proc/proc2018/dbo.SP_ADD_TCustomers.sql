...
GO
CREATE PROCEDURE SP_ADD_TCustomers @IN_CUST_NAME                    NVARCHAR(100),
...
AS
    ...
    DECLARE @V_CUST_ID INT,@V_CUST_NO NVARCHAR(8),@V_CARD_TYPE_NAME NVARCHAR(30),@V_MAX_ID INT, @V_ReceiveServices BIGINT,
    ...
    DECLARE @V_TRUE_FLAG INT,@V_CARD_ID_CHECK INT,@V_IS_DEAL INT
    ...
    /*--证件号码的验证，放到JSP提交前的JS提示判断(相关过程SP_QUERY_TCustomers_NAME),有可能允许重复证件号码的客户
    IF @V_CARD_ID_CHECK=1 AND ISNULL(@IN_CARD_ID,'') <> '' AND EXISTS (SELECT * FROM TCustomers WHERE CARD_TYPE = @IN_CARD_TYPE AND CARD_ID = @IN_CARD_ID AND STATUS <> '112805' )
       RETURN @V_RET_CODE - 3  --该身份证号已存在
	*/
    IF @IN_CUST_TYPE = 1
    BEGIN
        IF ISNULL(@IN_CARD_ID,'') <> '' AND EXISTS (SELECT * FROM TCustomers WHERE CARD_TYPE = @IN_CARD_TYPE AND CARD_ID = @IN_CARD_ID AND STATUS <> '112805' )
			RETURN @V_RET_CODE - 3  --该身份证号已存在
    ...
    INSERT INTO TCustomers(ReceiveServices, --添加客户时，默认接收所有服务
          ...
          ImageIdentification,POTENTIAL_MONEY,PinYinSimple,IS_DEAL,GOV_PROV_REGIONAL,GOV_PROV_REGIONAL_NAME,GOV_REGIONAL,
    ...
    VALUES(@V_ReceiveServices,
          ...
          @IN_ImageIdentification,@IN_POTENTIAL_MONEY,@V_PinYinSimple,@V_IS_DEAL,@IN_GOV_PROV_REGIONAL,@V_GOV_PROV_REGIONAL_NAME,@IN_GOV_REGIONAL,
          @V_GOV_REGIONAL_NAME,@IN_RECOMMENDED,@IN_COMPLETE_FLAG,@IN_EAST_JG_TYPE,@V_EAST_JG_TYPE_NAME,@IN_JG_WTRLX2,@V_TRUE_FLAG,
    ...
    --
    BEGIN
        INSERT INTO INTRUST.dbo.TCUSTOMERINFO(BOOK_CODE,
              CUST_ID,CUST_NO,CUST_NAME,CUST_TEL,POST_ADDRESS,POST_ADDRESS2,
              CARD_TYPE,CARD_TYPE_NAME,CARD_ID,AGE,SEX,SEX_NAME,POST_CODE2,
              O_TEL,H_TEL,MOBILE,BP,FAX,E_MAIL,CUST_TYPE,CUST_TYPE_NAME,
              ...
              SUMMARY,INPUT_MAN,POST_CODE,CHECK_FLAG,LIST_ID, LEGAL_MAN, LEGAL_ADDRESS, BIRTHDAY,
              PRINT_DEPLOY_BILL, PRINT_POST_INFO, IS_LINK, SERVICE_MAN,VIP_CARD_ID,VIP_DATE,HGTZR_BH,
              VOC_TYPE,VOC_TYPE_NAME,CARD_VALID_DATE,COUNTRY,JG_CUST_TYPE, CONTACT_MAN,FACT_CONTROLLER,
              RECOMMENDED,COMPLETE_FLAG,EAST_JG_TYPE,EAST_JG_TYPE_NAME,JG_WTRLX2)
        VALUES(@V_BOOK_CODE,
              @V_CUST_ID,@V_CUST_NO,@IN_CUST_NAME,@IN_CUST_TEL,@IN_POST_ADDRESS,@IN_POST_ADDRESS2,
              @IN_CARD_TYPE,@V_CARD_TYPE_NAME,@IN_CARD_ID,@IN_AGE,@IN_SEX,@V_SEX_NAME,@IN_POST_CODE2,
              @IN_O_TEL,@IN_H_TEL,@IN_MOBILE,@IN_BP,@IN_FAX,@IN_E_MAIL,@IN_CUST_TYPE,@V_CUST_TYPE_NAME,
              ...
              @IN_SUMMARY,@IN_INPUT_MAN,@IN_POST_CODE,2,@V_MAX_ID, @IN_LEGAL_MAN, @IN_LEGAL_ADDRESS, @IN_BIRTHDAY,
              @IN_PRINT_DEPLOY_BILL, @IN_PRINT_POST_INFO, @IN_IS_LINK, @IN_SERVICE_MAN,@IN_VIP_CARD_ID,@IN_VIP_DATE,@IN_HGTZR_BH,
              @IN_VOC_TYPE,@V_VOC_TYPE_NAME,@IN_CARD_VALID_DATE,@IN_COUNTRY,@IN_JG_CUST_TYPE, @IN_CONTACT_MAN,
              @IN_FACT_CONTROLLER,@IN_RECOMMENDED,@IN_COMPLETE_FLAG,@IN_EAST_JG_TYPE,@V_EAST_JG_TYPE_NAME,@IN_JG_WTRLX2)
    END
    /*当EFCRM与INTRUST在同一实例下时，分布式事务内部调用过程嵌套事务会报错：其他会话正在使用事务的上下文
    EXEC @RET = SP_MODI_CustomerClass @V_CUST_ID,NULL,NULL,@IN_INPUT_MAN
    IF @@ERROR <> 0
    BEGIN
