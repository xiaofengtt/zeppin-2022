...
GO
CREATE PROCEDURE SP_INIT_FLOW_WORKLOG_DAY @IN_ROLE_ID  INT
WITH ENCRYPTION
AS
	...
	IF EXISTS(SELECT 1 FROM INTRUST..TWORKDAYS 
				WHERE WORK_FLAG = 2 AND WORK_DATE = CONVERT(NVARCHAR(10),CONVERT(NVARCHAR(10),GETDATE(),112)))	
	BEGIN
		--非工作日不生成工作日报
		COMMIT TRANSACTION
		RETURN 100
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO TSTAFFWORKLOG(DATE, LOG_TYPE, LOG_TYPE_NAME, LOG_STATE, INPUT_MAN)
				VALUES(@V_CURRENT_DATE, '1', '工作日报', '1020', @V_OP_CODE)
			IF @@ERROR <> 0
			BEGIN
