...
GO
CREATE PROCEDURE SP_QUERY_TCOCONTRACTMAINTAIN_UNRECIEVE @IN_MAINTAIN_ID                   INT,           --维护合同(维护费)ID
                                                        ...
                                                        @IN_INVOICE_TIME_BEGIN            INT,           --开票时间从
                                                        @IN_INVOICE_TIME_END              INT,           --开票时间到
                                                        @IN_ARRIVEMONEY_FLAG              INT,           --到帐标志：1 未到帐 2 部分到帐 3 已到帐 0 全部
...
AS
    ...
    SELECT A.*,B.CUST_NAME FROM TCOCONTRACTMAINTAIN A,TCustomers B
        WHERE (ISNULL(@IN_MAINTAIN_ID,0) =0 OR A.MAINTAIN_ID = @IN_MAINTAIN_ID)
            AND (ISNULL(@IN_COCONTRACTMAINTAIN_SUB_BH,'') ='' OR A.COCONTRACTMAINTAIN_SUB_BH LIKE '%'+@IN_COCONTRACTMAINTAIN_SUB_BH+'%')
            AND (ISNULL(@IN_MAIN_PRO_NAME,'') ='' OR A.MAIN_PRO_NAME LIKE '%'+@IN_MAIN_PRO_NAME+'%')
            AND (ISNULL(@IN_COLLECT_TIME_BEGIN,0) =0 OR (ISNULL(@IN_COLLECT_TIME_BEGIN,0)<>0 AND A.COLLECT_TIME >= @IN_COLLECT_TIME_BEGIN))
            AND (ISNULL(@IN_COLLECT_TIME_END,0) =0 OR (ISNULL(@IN_COLLECT_TIME_END,0)<>0 AND A.COLLECT_TIME <= @IN_COLLECT_TIME_END))
            AND (ISNULL(@IN_START_DATE_BEGIN,0) =0 OR (ISNULL(@IN_START_DATE_BEGIN,0)<>0 AND A.START_DATE >= @IN_START_DATE_BEGIN))
            AND (ISNULL(@IN_START_DATE_END,0) =0 OR (ISNULL(@IN_START_DATE_END,0)<>0 AND A.START_DATE <= @IN_START_DATE_END))
            AND (ISNULL(@IN_END_DATE_BEGIN,0) =0 OR (ISNULL(@IN_END_DATE_BEGIN,0)<>0 AND A.END_DATE >= @IN_END_DATE_BEGIN))
            AND (ISNULL(@IN_END_DATE_END,0) =0 OR (ISNULL(@IN_END_DATE_END,0)<>0 AND A.END_DATE <= @IN_END_DATE_END))
            AND (ISNULL(@IN_INVOICE_TIME_BEGIN,0) =0 OR (ISNULL(@IN_INVOICE_TIME_BEGIN,0)<>0 AND A.INVOICE_TIME >= @IN_INVOICE_TIME_BEGIN))
            AND (ISNULL(@IN_INVOICE_TIME_END,0) =0 OR (ISNULL(@IN_INVOICE_TIME_END,0)<>0 AND A.INVOICE_TIME <= @IN_INVOICE_TIME_END))
            AND (ISNULL(@IN_ARRIVEMONEY_FLAG,0) =0 OR A.ARRIVEMONEY_FLAG = @IN_ARRIVEMONEY_FLAG)
