...
GO
CREATE PROCEDURE SP_MODI_TCUSTADDRESS @IN_CUST_ID               INTEGER,        --客户ID
                                   @IN_LIST_ID                 INTEGER,        --每个客户的联系信息序号
...
AS
    ...
    SET @SBUSI_NAME = N'修改客户联系信息'
    
    BEGIN TRY
        BEGIN TRANSACTION
        
        SET @IN_LIST_ID=ISNULL(@IN_LIST_ID,0)
        --客户信息变更审核 1是 2否
		...
		SELECT @V_DEFAULT_FLAG=DEFAULT_FLAG FROM TCUSTADDRESS WHERE CUST_ID=@IN_CUST_ID AND LIST_ID=@IN_LIST_ID
		...
		END
		IF @IN_LIST_ID=1 AND LTRIM(RTRIM(ISNULL(@OLD_POST_ADDRESS,''))) <> LTRIM(RTRIM(ISNULL(@V_POST_ADDRESS,'')))
		BEGIN
			INSERT INTO TCustomerChanges(CUST_ID,FIELD_NAME,FIELD_CN_NAME,OLD_FIELD_INFO,NEW_FIELD_INFO,INPUT_MAN,CHECK_FLAG)
				VALUES(@IN_CUST_ID,N'POST_ADDRESS',N'邮寄地址',@OLD_POST_ADDRESS,@V_POST_ADDRESS,@IN_INPUT_MAN,@V_MODI_CHECK_FLAG)
		END
		IF @IN_LIST_ID=2 AND LTRIM(RTRIM(ISNULL(@OLD_POST_ADDRESS2,''))) <> LTRIM(RTRIM(ISNULL(@V_POST_ADDRESS,'')))
		BEGIN
			INSERT INTO TCustomerChanges(CUST_ID,FIELD_NAME,FIELD_CN_NAME,OLD_FIELD_INFO,NEW_FIELD_INFO,INPUT_MAN,CHECK_FLAG)
		...
		ELSE
		BEGIN
			UPDATE TCUSTADDRESS SET PROVINCE=@IN_PROVINCE,CITY=@IN_CITY,DISTRICT=@IN_DISTRICT,TOWN_DETAIL=@IN_TOWN_DETAIL
					,POST_CODE=@IN_POST_CODE,CONTACT_MAN=@IN_CONTACT_MAN,TEL=@IN_TEL,FAX=@IN_FAX
				WHERE CUST_ID=@IN_CUST_ID AND LIST_ID=@IN_LIST_ID
			IF @@ROWCOUNT=0
			BEGIN
				IF EXISTS (SELECT * FROM TCUSTADDRESS WHERE CUST_ID=@IN_CUST_ID AND DEFAULT_FLAG=1)
					SET @V_DEFAULT_FLAG=0
				ELSE SET @V_DEFAULT_FLAG=1
				IF @IN_LIST_ID=1
					INSERT INTO TCUSTADDRESS(CUST_ID,LIST_ID,DEFAULT_FLAG,PROVINCE,CITY,DISTRICT,TOWN_DETAIL,POST_CODE,CONTACT_MAN,TEL,FAX)
						VALUES(@IN_CUST_ID,@IN_LIST_ID,@V_DEFAULT_FLAG,@IN_PROVINCE,@IN_CITY,@IN_DISTRICT,@IN_TOWN_DETAIL,@IN_POST_CODE,@IN_CONTACT_MAN,@IN_TEL,@IN_FAX)
				ELSE IF @IN_LIST_ID=2
					INSERT INTO TCUSTADDRESS(CUST_ID,LIST_ID,DEFAULT_FLAG,PROVINCE,CITY,DISTRICT,TOWN_DETAIL,POST_CODE,CONTACT_MAN,TEL,FAX)
						VALUES(@IN_CUST_ID,@IN_LIST_ID,@V_DEFAULT_FLAG,@IN_PROVINCE,@IN_CITY,@IN_DISTRICT,@IN_TOWN_DETAIL,@IN_POST_CODE,@IN_CONTACT_MAN,@IN_TEL,@IN_FAX)
			END
		END
        IF @V_MODI_CHECK_FLAG=0 --不用审核时，直接更新客户联系信息
        BEGIN
			UPDATE TCustomers 
				SET O_TEL=@IN_O_TEL,
				    H_TEL=@IN_H_TEL,
			...
			IF @V_DEFAULT_FLAG=1 AND LTRIM(RTRIM(ISNULL(@OLD_POST_ADDRESS,''))) <> LTRIM(RTRIM(ISNULL(@V_POST_ADDRESS,'')))
				UPDATE TCustomers SET POST_ADDRESS=@V_POST_ADDRESS WHERE CUST_ID=@IN_CUST_ID
			ELSE IF LTRIM(RTRIM(ISNULL(@OLD_POST_ADDRESS2,''))) <> LTRIM(RTRIM(ISNULL(@V_POST_ADDRESS,'')))
				UPDATE TCustomers SET POST_ADDRESS2=@V_POST_ADDRESS WHERE CUST_ID=@IN_CUST_ID
        END
        --日志记录
