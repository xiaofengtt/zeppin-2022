...
GO
CREATE PROCEDURE SP_QUERY_TCustomersExt @IN_CUST_ID             INT,
...
AS
    ...
    INSERT INTO @V_TEMPCUST2(CUST_ID)
        ...
        WHERE (EXISTS( SELECT 1 FROM @V_MANAGER_IDS B WHERE A.SERVICE_MAN = B.MANAGERID)
            ...
            AND(A.SERVICE_MAN = @IN_ACCOUNTMANAGER OR @IN_ACCOUNTMANAGER IS NULL) --增加客户经理和推荐人的查询
...
--        WHERE A.CUST_ID = B.CUST_ID
    ...
    SET @V_SQL = N'
       ...
       ,A.TRUE_FLAG,
       A.TRUE_FLAG_NAME       
       , B.CLASS10, B.CLASS11, B.CLASS12 AS CUST_LEVEL_NAME,H.OP_NAME AS SERVICE_MAN_NAME
