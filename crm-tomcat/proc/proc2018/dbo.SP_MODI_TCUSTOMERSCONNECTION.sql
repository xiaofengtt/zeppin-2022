...
GO
ALTER PROCEDURE SP_MODI_TCUSTOMERSCONNECTION @IN_SERIAL_NO  INT, --序号 
											 @IN_CUST_ID INT, --客户ID(EFCRM..CUSTOMERS.CUST_ID)
											 @IN_INPUT_MAN     INT,
...
AS
    ...
    SELECT @SBUSI_NAME = N'编辑客户联系方式修改申请'
    SELECT @SSUMMARY = N'编辑客户联系方式修改申请'
    ...
    UPDATE TCUSTOMOERS_CONNECTION_MODI 
    SET N_CUST_TEL = @IN_CUST_TEL, N_MOBILE = @IN_MOBILE, N_O_TEL = @IN_O_TEL, N_H_TEL = @IN_H_TEL, N_BP = @IN_BP
    , APPLY_REASON = @IN_APPLY_REASON
        WHERE SERIAL_NO = @IN_SERIAL_NO

    --2.日志
    SELECT @SSUMMARY = N'编辑客户联系方式修改申请，客户名称：' + @V_CUST_NAME
