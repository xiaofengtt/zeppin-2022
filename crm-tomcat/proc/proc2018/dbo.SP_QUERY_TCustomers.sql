...
GO
ALTER PROCEDURE SP_QUERY_TCustomers @IN_CUST_ID    INT,                    --客户ID
	...
	BEGIN
		...
		A.[CUST_ID]
			...
			A.MODI_FLAG_2,
					...
					A.ACTIVITY_FEE,
					A.TOTAL_COUNT,
		B.OP_NAME AS SERVICE_MAN_NAME, C.Extension EXTENSION, (CASE WHEN ISNULL(D.TEAM_ID,0) = ISNULL(@V_INPUT_MAN_TEAM_ID,0) THEN 1 ELSE 0 END) IS_TEAM
				,E.CLASSDETAIL_NAME CUST_LEVEL_NAME
			FROM #TEMP_QUERY_TCustomers A
				LEFT JOIN TOPERATOR B ON A.SERVICE_MAN = B.OP_CODE
				LEFT JOIN TCustManagers C ON A.SERVICE_MAN = C.ManagerID
