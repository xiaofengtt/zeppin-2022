...
GO
ALTER PROCEDURE dbo.SP_QUERY_TSALESCHANGES @IN_SERIAL_NO      INT,            --序号
										  ...
										  @IN_FROM_SERVICE_MAN      INT = NULL,   --原经理
										  @IN_TO_SERVICE_MAN        INT          = NULL,    --转销量后经理
										  @IN_SA_STATUS       INT          = NULL    --转销量状态
WITH ENCRYPTION
AS
    SET NOCOUNT ON
	...
	IF ISNULL(@IN_PRE_SERIAL_NO,0) <> 0 and ISNULL(@IN_SERIAL_NO ,0)=0
		BEGIN
        SELECT  TC.SERIAL_NO AS TC_SERIAL_NO ,TC.FK_TPREMONEYDETAIL ,DBO.GETDATEINT(TC.DZ_DATE) AS TC_DZ_DATE, TC.DZ_DATE AS TC_DZ_TIME
				,TC.DZ_MONEY AS TC_DZ_MONEY,TC.FROM_SERVICE_MAN
				,TC.FROM_SERVICE_MAN_NAME,TC.TO_SERVICE_MAN ,TC.TO_SERVICE_MAN_NAME,TC.CHANGE_MONEY,TC.CHANGE_REASON
				,TC.INPUT_MAN AS TC_INPUT_MAN,TC.INPUT_TIME AS TC_INPUT_TIME,TC.STATUS,TC.STATUS_NAME ,TC.CHECKER,TC.CHECK_TIME,TC.CHECK_REASON,TC.RE_CHECKER
				,TC.RE_CHECK_TIME,TC.RE_CHECK_REASON,TC.FK_TSALESRESULT
				,A.REMARK, A.PRE_PRODUCT_TYPE,A.PRE_PRODUCT_TYPE_NAME, D.CUST_TYPE_NAME, A.RECORDS_COUNT
				, A.SERIAL_NO, A.PRE_SERIAL_NO, DBO.GETDATEINT(A.DZ_DATE) AS DZ_DATE, A.DZ_DATE AS DZ_TIME, A.DZ_MONEY, A.REFUND_DATE
                ...
                FROM TSALES_CHANGES TC
                LEFT JOIN TSALESRESULTFORSTATISTIC A ON TC.FK_TSALESRESULT=A.SERIAL_NO AND A.SERIAL_NO IS NOT NULL
    ...
    IF ISNULL(@IN_SERIAL_NO,0) <> 0
        SELECT @IN_PRE_SERIAL_NO = PRE_SERIAL_NO FROM TSALES_CHANGES WHERE SERIAL_NO = @IN_SERIAL_NO
    ...
    FROM  --用一层子查询，在外层判断过滤SERIAL_NO，是为了在子查询中得到缴款序号
        ( SELECT TC.SERIAL_NO AS TC_SERIAL_NO,TC.FK_TPREMONEYDETAIL ,DBO.GETDATEINT(TC.DZ_DATE) AS TC_DZ_DATE, TC.DZ_DATE AS TC_DZ_TIME
            ...
            FROM TSALES_CHANGES TC
                LEFT JOIN TSALESRESULTFORSTATISTIC A ON TC.FK_TSALESRESULT=A.SERIAL_NO AND A.SERIAL_NO IS NOT NULL
                ,TPRECONTRACT B LEFT JOIN INTRUST..TPRECONTRACT R WITH (NOLOCK) ON B.BIND_SERIAL_NO = R.SERIAL_NO,
            ...
            WHERE A.PRE_SERIAL_NO = B.SERIAL_NO 
                ...
                AND (TC.FROM_SERVICE_MAN = @IN_FROM_SERVICE_MAN OR ISNULL(@IN_FROM_SERVICE_MAN,0) = 0) 
                AND (TC.TO_SERVICE_MAN = @IN_TO_SERVICE_MAN OR ISNULL(@IN_TO_SERVICE_MAN,0) = 0) 
                AND (TC.[STATUS] = @IN_SA_STATUS OR ISNULL(@IN_SA_STATUS,0) = 0) 
        ) A  LEFT JOIN INTRUST..TCONTRACT Z WITH (NOLOCK) ON A.CONTRACT_SERIAL_NO = Z.SERIAL_NO
        WHERE (A.TC_SERIAL_NO = @IN_SERIAL_NO OR @IN_SERIAL_NO = 0)
		END
GO
