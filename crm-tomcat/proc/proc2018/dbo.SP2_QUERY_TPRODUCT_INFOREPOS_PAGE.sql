...
GO
--DROP PROCEDURE SP2_QUERY_TPRODUCT_INFOREPOS_PAGE
CREATE PROCEDURE SP2_QUERY_TPRODUCT_INFOREPOS_PAGE
						 ...
						 @IN_INPUT_MAN      INT,
						 @IN_PRODUCT_STATUS INT,            --产品状态（0全部 2推介期 3正常期 4结束）
						 @IN_OPENFLAG       INT,            --产品发行方式（0全部 1开放式 2封闭式）
						 @IN_SORTFIELD      NVARCHAR(200),  --排序字段
						 @IN_PRE_DATE1      INT,            --推介期下限
						 @IN_PRE_DATE2      INT,            --推介期上限
						 @IN_PRESTART_TIME1 DATETIME,       --预约开始时间下限
						 @IN_PRESTART_TIME2 DATETIME,       --预约开始时间上限
...
PRINT 'OK'
    ...
    INSERT INTO #TMP_PRODUCT_INFOREPOS(PRODUCT_ID,PRODUCT_NAME,INPUT_TIME,PRE_MONEY,PRE_START_DATE,PRE_END_DATE,ADMIN_MANAGER,
        ...
        WHERE A.ADMIN_MANAGER = B.OP_CODE AND A.INTRUST_FLAG1 = 2 --仅集合产品
            ...
            AND (@IN_CLASS_TYPE1='' OR 1=2)
            AND (@IN_PREPRODUCT_ID=0 OR 1=2)
	...
	BEGIN
		...
		IF ISNULL(@IN_PAGE_SIZE,0)<=0
			SET @IN_PAGE_SIZE=20
	END
	SELECT @V_RECORD_TOTAL=COUNT(0) FROM #TMP_PRODUCT_INFOREPOS WHERE PREPRODUCT_ID not in (13,111,115,117,118,139,262,357)
	IF ISNULL(@IN_SORTFIELD,'')=''
		SET @IN_SORTFIELD='INPUT_TIME DESC'
    SET @V_SQL = 'SELECT * FROM (SELECT *,EXPRE_START_DATE AS EXPRE_START_TIME,ROW_NUMBER() OVER(ORDER BY '+@IN_SORTFIELD+') RID,'+CAST(@V_RECORD_TOTAL AS VARCHAR)+' AS RECORD_TOTAL'
		+' FROM #TMP_PRODUCT_INFOREPOS WHERE 1 = 1 and PREPRODUCT_ID not in (13,111,115,117,118,139,262,357) ) B WHERE B.RID>='+CAST((@IN_PAGE_INDEX-1)*@IN_PAGE_SIZE+1 AS NVARCHAR)
