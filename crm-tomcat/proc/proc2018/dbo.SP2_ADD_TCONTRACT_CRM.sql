...
GO
CREATE PROCEDURE SP2_ADD_TCONTRACT_CRM @IN_PRODUCT_ID            INT,
...
AS
    ...
    DECLARE @V_PRPDUCT_ID_TMP INT,@V_PRE_SERIAL_NO INT,@V_INTRUST_SERIAL_NO INT
    ...
    IF ISNULL(@IN_SPOT_DEAL,0)=3 --移动端签约
	BEGIN
		SELECT @V_PRPDUCT_ID_TMP=PREPRODUCT_ID FROM TPREPRODUCT WHERE BIND_PRODUCT_ID=@IN_PRODUCT_ID
		IF @@ROWCOUNT=0
			RAISERROR('产品未绑定，不能签约！',16,1)
		SELECT @V_PRE_SERIAL_NO=SERIAL_NO,@V_CUST_ID=CUST_ID,@V_PRE_STATUS = PRE_STATUS,@V_LINK_MAN = LINK_MAN,@V_PRE_MONEY = PRE_MONEY,
			   @V_PRE_NUM1 = PRE_NUM
			FROM TPRECONTRACT WHERE PREPRODUCT_ID=@V_PRPDUCT_ID_TMP AND PRE_CODE=@IN_PRE_CODE
		IF @@ROWCOUNT=0
			RAISERROR('指定的预约不存在！',16,1)
		EXEC SP_SYNC_TPRECONTRACT @V_PRE_SERIAL_NO,@IN_INPUT_MAN, @V_INTRUST_SERIAL_NO OUTPUT
		SELECT @IN_PRE_CODE=PRE_CODE FROM INTRUST..TPRECONTRACT WHERE SERIAL_NO=@V_INTRUST_SERIAL_NO
	END
    ELSE IF NOT EXISTS(SELECT 1 FROM INTRUST..TPRECONTRACT WHERE PRODUCT_ID = @IN_PRODUCT_ID AND PRE_CODE = @IN_PRE_CODE )
    ...
    IF ISNULL(@IN_SPOT_DEAL,0)<>3 --非移动端签约
    BEGIN
