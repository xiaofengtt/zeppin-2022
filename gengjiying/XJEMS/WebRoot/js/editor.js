...
var QuestionGOpt = {
	...
	G_createjson : function(type) {
		...
		$('.module').each(function(g){//题型循环
			var th = $(this),mtype = th.data('mtype'),num = parseInt(g)+1,qtype= th.data('qtype');
			...
			$.post('../itemAddJson',{'questionItem':QuestionGOpt.G_createjson('add')},function(r){	
				...
				if(r.Status == 'success') {
					if(e.target.id == 'savenextbutton') {
						...
						location.href = url('host')+'/admin/paperLoadPaper?id='+_this.Pid;
					}else {
						location.href = url('host')+'/admin/itemList.html';
					}
...
function changeIcon (e) {
	if (e.attr('class') == 'o') {
		e.attr('class', 'c');
		var docClassName = "." + e.parent().attr("class");
		e.parent().nextUntil(docClassName).addClass("hide");
	} else {
		e.attr('class', 'o');
		var docClassName = "." + e.parent().attr("class");
		e.parent().nextUntil(docClassName).removeClass("hide");
	}
		    ...
		    for (var property in object) {
		        if (object.hasOwnProperty(property)) {
		            ...
		            if (typeof object[property] == "object"){
						...
						emClick = (object['haschild']) ? ' onclick="changeIcon($(this)); return false;"' : '';
...
var paperQuestion = {
	...
	settingjson : function() {
		...
		var jdata = '';
		var pb = $('.paper-set-gbox');
		if(pb.length > 0) {
			$('.paper-set-gbox').each(function(i){
				var _this = $(this);
				var childdata = '';
				var tb = _this.children('table'),
					tbd = tb.data(),
					na = tb.find('textarea');
//				var tb = $(this).find('table'),
//					tbd = tb.data(),
//					na = tb.find('textarea');
				var index = $(".title-td-num").index($(this).find(".title-td-num")) + 1;
				if(tbd.level==undefined)tbd.level="1";	
				var com = (i== (pb.length-1)) ? '' : ',';
				var ctb = _this.next();
				var cpb = ctb.children('.paper-set-box');
				cpb.each(function(i){
					var ctbh = $(this).find('table'),
						ctbd = ctbh.data(),
						cna = ctbh.find('textarea');
					var inx = $(".title-td-num").index($(this).find(".title-td-num")) + 1;
					if(ctbd.level==undefined)ctbd.level="1";	
					var ccom = (i== (cpb.length-1)) ? '' : ',';
					childdata += '{"name":"'+ string2json2(cna.val()) +'","inx":"'+ inx + '","level":"' + ctbd.level +'","itemType.id":"'+ ctbd.itemtypeid +'","itemType.name":"'+ ctbd.itemtypename +'"}' + ccom
				});
					
					
				jdata += '{"name":"'+ string2json2(na.val()) +'",'
							+'"inx":"'+ index + '",'
							+'"level":"' + tbd.level +'",'
							+'"itemType.id":"'+ tbd.itemtypeid +'",'
							+'"itemType.name":"'+ tbd.itemtypename +'",'
							+'"data":['
							+childdata
							+']'
							+'}' + com
			...
			$('.paper-set-box').each(function(i){
				...
				if(tbd.level==undefined)tbd.level="1";	
				var com = (i== (pb.length-1)) ? '' : ','
				jdata += '{"name":"'+ string2json2(na.val()) +'","inx":"'+ index + '","level":"' + tbd.level +'","itemType.id":"'+ tbd.itemtypeid +'","itemType.name":"'+ tbd.itemtypename +'"}' + com
			})
		}
	...
	openwin_settingjson : function(pid) {
		...
		var jdata = '';
		var pb = $('.openwin-box').find('.paper-set-gbox');
		var bb = $('.openwin-box').find('.paper-set-box');
		if(pb.length > 0) {
			pb.each(function(i){
				var _this = $(this);
				var childdata = '';
				var tb = _this.children('table'),
					tbd = tb.data(),
					na = tb.find('textarea');
				var index = $(".title-td-num").index(_this.find(".title-td-num")) + 1;
				if(tbd.level==undefined)tbd.level="1";	
				var com = (i== (pb.length-1)) ? '' : ',';
				var ctb = _this.next();
				var cpb = ctb.children('.paper-set-box');
				cpb.each(function(i){
					...
					if(ctbd.level==undefined)ctbd.level="1";	
					var ccom = (i== (cpb.length-1)) ? '' : ',';
					childdata += '{"id":"'+ ctbd.pid +'","name":"'+ string2json2(cna.val()) +'","inx":"'+ inx + '","level":"' + ctbd.level +'","itemType.id":"'+ ctbd.itemtypeid +'","itemType.name":"'+ ctbd.itemtypename +'"}' + ccom
				});
					
					
				jdata += '{"id":"'+ tbd.pid +'","name":"'+ string2json2(na.val()) +'",'
							+'"inx":"'+ index + '",'
							+'"level":"' + tbd.level +'",'
							+'"itemType.id":"'+ tbd.itemtypeid +'",'
							+'"itemType.name":"'+ tbd.itemtypename +'",'
			...
			bb.each(function(i){
				...
				var index = $(".title-td-num").index($(this).find(".title-td-num")) +1;
				if(tbd.level==undefined)tbd.level="1";	
				var com = (i== (bb.length-1)) ? '' : ','
				jdata += '{"id":"'+ tbd.pid +'","name":"'+ string2json2( na.val() ) +'","inx":"'+ index + '","level":"' + tbd.level +'","itemType.id":"'+ tbd.itemtypeid +'","itemType.name":"'+ tbd.itemtypename +'"}' + com
			})
		}
