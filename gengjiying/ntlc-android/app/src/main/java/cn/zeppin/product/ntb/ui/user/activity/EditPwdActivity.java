 ...
 * 设置密码
 */
public class EditPwdActivity extends BaseActivity<SetPwdPresenter, SetPwdModel> implements SetPwdContract.View {
     ...
     */
    private void toSave() {
        if (TextUtils.isEmpty(getInputOldPwd())) {
            showToastCenter("请输入原密码");
            return;
        }
        if (TextUtils.isEmpty(getInputNewPwd())) {
            showToastCenter("请输入新密码");
            return;
        }
        ...
        mPresenter.setPwd(AppApplication.getInstance().getUuid(), getInputNewPwd(), getInputOldPwd());
    }

    private String getInputNewPwd() {
        return cetNewPwd.getText().toString();
    }

    private String getInputOldPwd() {
        return cetOldPwd.getText().toString();
    }
    ...
    public void setPwdSuccess() {
        final DialogBinding dialogBinding = new DialogBinding(EditPwdActivity.this, "修改成功！", R.drawable.set_pwd_success, "个人中心");
