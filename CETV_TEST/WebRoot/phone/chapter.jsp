		...
		<s:iterator value='lstTextbookCs' id="tbc" >
	 		...
	 		<ul class="list">
	 			...
	 			<li <s:if test="%{#tbc.childs.size()==0}">class="nochild"</s:if>>
					...
					<!-- 
					<s:if test="%{#capterDegree!=null}">
						<a href="../phone/phPaperLoadItem?paper.id=<s:property value="#capterDegree.userTest.paper.id" />&usertest.id=<s:property value='#capterDegree.userTest.id' />&clienttype=1" class="icon icon-compose"></a>
					</s:if>
					<s:else>
					</s:else>
					 -->
					<a href="../phone/phPaperGroupPaper?subject.id=<s:property value="subjectId" />&textbookCapter.id=<s:property value="#tbc.id" />" class="icon icon-compose"></a>
							...
							<s:iterator value='#tbc.childs' id='tbchild'>
								<li class="nochild">
									...
									<s:else>
									 ...
									 -->
									<a href="../phone/phPaperGroupPaper?subject.id=<s:property value="subjectId" />&textbookCapter.id=<s:property value="#tbchild.id" />" class="icon icon-compose"></a>
