			...
			<s:iterator value='lstKnowledges' id='kl'>
				...
				<ul class="list">
					...
					<li <s:if test="%{#kl.childs.size()==0}">class="nochild"</s:if>>
						...
						<a href="../phone/phPaperGroupPaper?subject.id=<s:property value="subjectId" />&knowledge.id=<s:property value="#kl.id" />" class="icon icon-compose"></a>
							...
							<ul>
								...
								<s:iterator value='#kl.childs' id='klex'>
									...
									<li <s:if test="%{#klex.childs.size()==0}">class="nochild"</s:if>>
										...
										<!-- 
										<s:if test="%{#knowDegree!=null}">
											<a href="../phone/phPaperLoadItem?paper.id=<s:property value="#knowDegree.userTest.paper.id" />&usertest.id=<s:property value='#knowDegree.userTest.id' />&clienttype=1" class="icon icon-compose"></a>
										</s:if>
										<s:else>
										</s:else>
										 -->
										<a href="../phone/phPaperGroupPaper?subject.id=<s:property value="subjectId" />&knowledge.id=<s:property value="#klex.id" />" class="icon icon-compose"></a>
											...
											<s:iterator value='#klex.childs' id='klext'>
												<li class="nochild">
													...
													<s:else>
													 ...
													 -->
													<a href="../phone/phPaperGroupPaper?subject.id=<s:property value="subjectId" />&knowledge.id=<s:property value="#klext.id" />" class="icon icon-compose"></a>								
