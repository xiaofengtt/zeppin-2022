...
//			searchMap.put("paper.id", id);
//			List<Item> childItems = this.getItemService().getItemsForPhone(searchMap);
...
//		}
		if(paper != null && paper.getStatus() == 2){
			if(status == 3){
				result.init(FAIL_STATUS, "请先审核", null);
				Utlity.ResponseWrite(result, dataType, response);
				return;
			}
			paper.setStatus(status);
			...
			result.init(SUCCESS_STATUS, "操作成功", null);
		} else if(paper != null && paper.getStatus() == 1){
			...
			result.init(SUCCESS_STATUS, "操作成功", null);
		} else if(paper != null && paper.getStatus() == 3){
			if(status == 0){
				paper.setStatus(status);
				this.getPaperService().updatePaper(paper, null);
				...
				result.init(FAIL_STATUS, "当前试卷已发布", null);
			}
		} else if(paper != null && paper.getStatus() == 0){
			if(status == 3){
				result.init(FAIL_STATUS, "请先审核", null);
				Utlity.ResponseWrite(result, dataType, response);
				return;
