...
public class CountAction extends BaseAction {
	...
	public void SearchYearCount(){
		...
		for(int i=0;i<10;i++){
			...
			searchMap.put("endtime", ts.toString());
			int total=this.itemService.searchItemCount(searchMap);
			data.put("date", (date.getYear()+1900)+"年"+(date.getMonth()+1)+"月");
			data.put("total", total);
			lstData.add(data);
		}
		actionResult.init(SUCCESS_STATUS, "数据获成功", lstData);
	...
	public void submitETL() {
		Log log= LogFactory.getLog("ETL");
		log.info("更新完成");
	}
	
	public void submitCopy() {
		Log log= LogFactory.getLog("备份");
		log.info("完成");
	}
}
