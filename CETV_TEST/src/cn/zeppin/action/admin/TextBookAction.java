...
public class TextBookAction extends BaseAction implements IStandardAction {
	...
	private HashMap<String, Object> getParas() {
		...
		if (request.getParameterMap().containsKey("publisher")) {
			hMap.put("publisher", request.getParameter("publisher"));
		}
		...
		if (oldTextbook == null) {
			...
			if (textbookService.isExistByName(name, id)) {
				...
				if (textbook != null) {
					...
					oldTextbook.setResource(textbook.getResource());
	...
	private Textbook getFromFront(ActionResult actionResult) {
		...
		Integer resoourceId = null;

		if (request.getParameterMap().containsKey("resource.id")) {
			if (Utlity.isNumeric(request.getParameter("resource.id"))) {
				resoourceId = Integer.parseInt(request.getParameter("resource.id"));
				Resource resource = resourceService.getById(resoourceId);
				if (resource == null) {
					actionResult.init(FAIL_STATUS, "封面不存在", null);
					return null;
				}
				textbook.setResource(resource);
			}
		}
