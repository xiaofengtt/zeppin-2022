...
public class EncryptionUtil {
	 ...
	 * MD5
	 */
	public static byte[] md5(byte[] input) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException {
	...
	public static String byte2hex(byte[] b) {
		...
		for (int n = 0; n < b.length; n++) {
			stmp = stmp + (java.lang.Integer.toHexString(b[n] & 0XFF));
			if (stmp.length() == 1) {
				hs = hs + "0" + stmp;
			} else {
				hs = hs + stmp;
