...
function getData(){
	...
	$.get('../rest/backadmin/bankFinancialProduct/netvaluelist?pageNum=0&pageSize=10000&uuid='+uuid+'&deadline=all',function(r) {
		if(r.status =='SUCCESS') {
...
//				$('#queboxCnt').html(html);
//			}
			$(".addNew").attr('href','bankFinancialProductDailyAdd.jsp?uuid='+uuid);
		} else {
			layer.msg(r.message, {
				time: 2000 
			})
...
function getList(){
	...
	for(var i = start; i < end; i++){
		...
		html += '<td><a class="editDaily" href="bankFinancialProductDailyEdit.jsp?uuid=' + thisData.uuid + '&statistime=' + thisData.statistime + '&netValue=' + thisData.netValue + '">修改</a>';
	...
	if(flag){
		$('#pageTool').Paging({pagesize:pagesize,count:dataMap.size(),callback:function(page,size,count){
			pageNum = page;
			flag=false;
			getList();
		},render:function(ops){
...
$("#formsubmit").submit(function(){
	...
	datas += str
	$.post('../rest/backadmin/bankFinancialProduct/netvalueEdit?uuid=' + uuid,datas,function(data){
		if (data.status == "SUCCESS") {
			layer.msg('提交审核成功', {
				time: 1000 
			},function(){
				location.href='bankFinancialProductList.jsp'
			})
		}
	})
	return false;
