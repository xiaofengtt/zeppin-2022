...
//获取值
function getDate() {
    $.get('../rest/backadmin/fundPublish/operateNetvalueGet?uuid=' + uuid, function(r) {
        if (r.status == 'SUCCESS') {
            ...
            $('#custodian').html(r.data.fundPublishName);
        ...
        $.get('../rest/backadmin/fundPublish/operateCheck?uuid=' + uuid + '&status=checked&reason=' + reason, function(r) {
            if (r.status == 'SUCCESS') {
                layer.msg('操作成功', {
                    ...
                    window.location.href = "./fundPublishAuditingCheck.jsp";
                });
            } else {
