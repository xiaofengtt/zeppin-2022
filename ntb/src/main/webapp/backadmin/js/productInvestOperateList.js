...
function checkThis(t) {
	...
	layer.confirm('确定要变更状态吗?', function(index){
		...
		$.get('../rest/backadmin/bankFinancialProductInvest/operateCheck?uuid='+uuid+'&status='+status+'&reason='+reason, function(r) {
			if (r.status == 'SUCCESS') {
				layer.msg('操作成功', {
					time: 1000 
				}, function(){
...
function getStatusList(){
	...
	$("#allCount").html("(0)");
	$.get('../rest/backadmin/bankFinancialProductInvest/operateStatusList',function(r) {
		if(r.status =='SUCCESS') {
			var totalCount=0;
...
function getTypeList(){
	...
	$("#deleteCount").html("(0)");
	$("#checkCount").html("(0)");
	$("#redemingCount").html("(0)");
	$.get('../rest/backadmin/bankFinancialProductInvest/operateTypeList?status=' + status,function(r) {
		if(r.status =='SUCCESS') {
...
function getList(){
	...
	$.get('../rest/backadmin/bankFinancialProductInvest/operateList?pageNum='+page+'&pageSize=10&name='+name+str,function(r) {
		if(r.status =='SUCCESS') {
