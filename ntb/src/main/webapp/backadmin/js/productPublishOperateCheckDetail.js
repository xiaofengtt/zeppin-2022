    ...
    $.get('../rest/backadmin/bankFinancialProductPublish/operateGet?uuid=' + uuid, function(r) {
        if (r.status == 'SUCCESS') {
            ...
            if (r.data.type == 'add') {
                ...
                $('#operateType').html("删除");
            } else if (r.data.type == 'collect') {
                $('#operateType').html("开始认购");
            } else if (r.data.type == 'uninvest') {
                $('#operateType').html("结束认购");
            } else if (r.data.type == 'balance') {
                $('#operateType').html("结算还款");
            } else if (r.data.type == 'exception') {
                $('#operateType').html("异常下线");
            }
            if (r.data.type == 'delete' || r.data.type == 'collect' || r.data.type == 'uninvest' || r.data.type == 'balance' ||
                r.data.type == 'edit' || r.data.type == 'exception') {
                ...
                $('#collectAmounts').html(r.data.oldData.collectAmount);
            ...
            if (r.data.type == 'edit') {
                ...
                if (r.data.oldData.collectAmount != r.data.newData.collectAmount) {
                    $('#collectAmount').html("变更为：" + r.data.newData.collectAmount + "万元");
                }
...
function checked(t) {
    layer.confirm("确认要变更状态吗？", function() {
        if (flagSubmit == false) {
            return false;
        }
        flagSubmit = false;
