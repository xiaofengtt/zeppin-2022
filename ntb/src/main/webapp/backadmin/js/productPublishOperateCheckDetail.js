...
function getDate(){
	$.get('../rest/backadmin/bankFinancialProductPublish/operateGet?uuid='+uuid,function(r) {
		if(r.status =='SUCCESS') {
			...
			if(r.data.type=='add'){
				$('#operateType').html("添加");
			}else if(r.data.type=='edit'){
				$('#operateType').html("修改");
			}else if(r.data.type=='delete'){
				$('#operateType').html("删除");
			}else if(r.data.type=='exception'){
				$('#operateType').html("异常下线");
			}
			if(r.data.type=='delete' || (r.data.type=='edit' && r.data.status!='checked') || r.data.type=='exception'){
				...
				$('#statuss').html(r.data.oldData.statusName);
...
//				$('#flagCloseends').html(jsonDate.flagCloseend[r.data.oldData.flagCloseend]);
				$('#guaranteeStatuss').html(r.data.oldData.guaranteeStatusCN);
				$('#remarks').html(escape2Html(r.data.oldData.remark));
				$('#netWorths').html(r.data.oldData.netWorth);
			...
			}else if(r.data.type=='add' || (r.data.type=='edit' && r.data.status=='checked')){
				...
				$('#netWorths').html(r.data.newData.netWorth);
			...
			if(r.data.type=='edit' && r.data.status!='checked'){
				...
				if(r.data.oldData.typeCN != r.data.newData.typeCN){
					$('#type').html("变更为："+r.data.newData.typeCN);
				}
				if(r.data.oldData.targetCN != r.data.newData.targetCN){
				...
				if(r.data.oldData.collectStarttimeCN != r.data.newData.collectStarttimeCN){
					$('#collectStarttime').html("变更为："+r.data.newData.collectStarttimeCN);
				}
				if(r.data.oldData.collectEndtimeCN != r.data.newData.collectEndtimeCN){
					$('#collectEndtime').html("变更为："+r.data.newData.collectEndtimeCN);
				}
				if(r.data.oldData.term != r.data.newData.term){
					$('#term').html("变更为："+r.data.newData.term + "天");
				}
				if(r.data.oldData.recordDateCN != r.data.newData.recordDateCN){
					$('#recordDate').html("变更为："+r.data.newData.recordDateCN);
				}
				if(r.data.oldData.valueDateCN != r.data.newData.valueDateCN){
					$('#valueDate').html("变更为："+r.data.newData.valueDateCN);
				}
				if(r.data.oldData.maturityDateCN != r.data.newData.maturityDateCN){
					$('#maturityDate').html("变更为："+r.data.newData.maturityDateCN);
				}
				...
				if(r.data.oldData.guaranteeStatusCN != r.data.newData.guaranteeStatusCN){
					$('#guaranteeStatus').html("变更为："+r.data.newData.guaranteeStatusCN);
				}
				...
				if(r.data.oldData.document != r.data.newData.document){
					if(r.data.newData.document != null){
						...
						$("#document").html("<label>变更为：</label>"+r.data.newData.documentCN+'.'+r.data.newData.documentType);
					}else{
						$("#document").html("<label>变更为：</label>"+r.data.newData.documentCN);
					}
				}
			}
		} else {
...
function checked(t) {
	...
	$.get('../rest/backadmin/bankFinancialProductPublish/operateCheck?uuid='+uuid+'&status=checked&reason='+reason, function(r) {
		if (r.status == 'SUCCESS') {
			layer.msg('操作成功', {
				time: 1000 
			}, function(){
...
function unpassed(t) {
	var reason = $('#checkReason').val();
	$.get('../rest/backadmin/bankFinancialProductPublish/operateCheck?uuid='+uuid+'&status=unpassed&reason='+reason, function(r) {
		if (r.status == 'SUCCESS') {
			layer.msg('操作成功', {
				time: 1000 
			}, function(){
