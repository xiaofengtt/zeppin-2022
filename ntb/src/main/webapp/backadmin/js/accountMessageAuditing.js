$(document).ready(function() {
    ...
    //获取筛选项
    getStatusList("../rest/backadmin/companyAccount/operateStatusList");
    getTypeList();
    getList();
    ...
    function deleteThis(t) {
        layer.confirm('确定要删除吗?', function(index) {
            ...
            $.get('../rest/backadmin/companyAccount/operateDelete?uuid=' + uuid, function(r) {
                if (r.status == 'SUCCESS') {
                    layer.msg('操作成功', {
                        time: 2000
                    }, function() {
    ...
    function submitThis(t) {
        layer.confirm('确定要提交审核吗?', function(index) {
            ...
            var obj = $(t);
            var uuid = obj.attr('data-uuid');
            $.get('../rest/backadmin/companyAccount/operateSubmitCheck?uuid=' + uuid, function(r) {
                if (r.status == 'SUCCESS') {
                    layer.msg('操作成功', {
                        time: 1000
                    }, function() {
    ...
    function getList(name, sorts) {
        ...
        $.ajax({
                url: '../rest/backadmin/companyAccount/operateList',
                type: 'get',
                data: {
                    "pageNum": pageNum,
                    "pageSize": 10,
