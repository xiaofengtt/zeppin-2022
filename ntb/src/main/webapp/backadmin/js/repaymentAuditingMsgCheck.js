$(document).ready(function() {
    ...
    function checkThis(t) {
        ...
        layer.confirm('确定要变更状态吗?', function(index) {
            ...
            $.get('../rest/backadmin/productPublishBalance/operateCheck?uuid=' + uuid + '&status=' + status + '&reason=' + reason, function(r) {
                if (r.status == 'SUCCESS') {
                    layer.msg('操作成功', {
                        time: 2000
                    }, function() {
    ...
    function getItem() {
        $.ajax({
                url: '../rest/backadmin/productPublishBalance/operateGet',
                type: 'get',
                data: {
                    "uuid": uuid
                }
            ...
            .done(function(r) {
                if (r.status == "SUCCESS") {
                    returnRate = r.data.bankFinancialProductPublishInfo.targetAnnualizedReturnRate / 100;
                    term = r.data.bankFinancialProductPublishInfo.term / 365;
                    $("#useValue").val(r.data.value);
                    $("#useConfirm").html(r.data.value + "%");
                    if (r.data.status == "checked" || r.data.status == "unchecked") {
                ...
                if (r.status == "SUCCESS") {
                    ...
                    $('input[name="my-checkbox"]').on('switchChange.bootstrapSwitch', function(event, state) {
                        if (state == true) {
                            ...
                            $("#returnValue").html((collect * realReturn).toFixed(2));
                    ...
                    $("#useValue").bind("input", function() {
                        ...
                        $("#allPrice").html((collect + returnValue).toFixed(2));
                        if ($("#useValue").val() == "") {
                            $("#returnValue").html("0");
    ...
    function getInvestMessage() {
        $.ajax({
                ...
                type: 'get',
                data: {
                    "bankFinancialProductPublish": puuid,
                    "pageNum": 1,
                    "pageSize": 1000
                }
            })
            .done(function(r) {
                ...
                var totalReturn = 0;
                $(".totalReturn").each(function(index, el) {
                    if (isNaN($(".totalReturn").eq(index).html().replace(/,/g, "")) == false) {
    ...
    $("#allRightConfirm").click(function() {
        ...
        $(".btn_group").eq(1).hide();
        $(".repayment_confirm").hide();
        $(".confirm_item").eq(0).show();
    });

    function submit() {
        $.ajax({
                url: '../rest/backadmin/productPublishBalance/operateEdit',
                type: 'post',
                data: {
                    "uuid": uuid,
                    "realRate": RealV
