...
function deleteThis(t) {
	layer.confirm('确定要删除这条记录吗?', function(index){
		var obj = $(t);
		var uuid = obj.attr('data-uuid');
		$.get('../rest/backadmin/bankFinancialProductPublish/operateDelete?uuid='+uuid, function(r) {
			if (r.status == 'SUCCESS') {
				layer.msg('操作成功', {
					time: 1000 
				}, function(){
...
function getStatusList(){
	...
	$("#allCount").html("(0)");
	$.get('../rest/backadmin/bankFinancialProductPublish/operateStatusList',function(r) {
		if(r.status =='SUCCESS') {
			var totalCount=0;
...
function getTypeList(){
	...
	$.get('../rest/backadmin/bankFinancialProductPublish/operateTypeList?status=' + status,function(r) {
		if(r.status =='SUCCESS') {
...
function getList(){
	...
	$.get('../rest/backadmin/bankFinancialProductPublish/operateList?pageNum='+page+'&pageSize=10&name='+name+str,function(r) {
		if(r.status =='SUCCESS') {
