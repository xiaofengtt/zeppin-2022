...
function checked(t) {
    layer.confirm("确认要变更状态吗？", function(index) {
        ...
        $.get('../rest/backadmin/fund/operateCheck?uuid=' + uuid + '&status=checked&reason=' + reason, function(r) {
            if (r.status == 'SUCCESS') {
                layer.msg('操作成功', {
                    time: 1000
                }, function() {
...
function unpassed(t) {
    layer.confirm("确认要变更状态吗？", function(index) {
        var reason = $('#checkReason').val();
        $.get('../rest/backadmin/fund/operateCheck?uuid=' + uuid + '&status=unpassed&reason=' + reason, function(r) {
            if (r.status == 'SUCCESS') {
                layer.msg('操作成功', {
                    time: 1000
                }, function() {
