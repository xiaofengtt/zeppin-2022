$(function(){
	if($(window).width()>1250){
		...
		$(".contain-right .main-contain").css({"min-height":$(window).height()-210+"px"});
	}	
	menu();
...
$(document).on("click", function(e) {
	if (!$(e.target).parents().is('.header-right')){
		...
		$(".header .header-right .userInfo a").css({"color":"#a7a6a5","text-decoration":"none"});
	}	
});
        ...
        success: function (data) {
        	...
        	if(json.status=="SUCCESS"){
        		for(var i=0;i<json.data.length;i++){
        			str+='<li class="lip" name="'+json.data[i].scode+'"><a onclick="menuSecond(\''+json.data[i].scode+'\',this)" style="background:#161616 url('+json.data[i].icon+'.png) 29px center no-repeat;">'+json.data[i].title+'<img src="img/aleft.png"></a></li>';
        		}
        		$("#menu").html(str);
		...
		$.ajax({
	        ...
	        success: function (data) {
	        	...
	        	if(json.status=="SUCCESS"){
	        		var str='<ul class="ul">';
	        		for(var i=0;i<json.data.length;i++){
	        			str+='<li class="lic" name="'+json.data[i].scode+'"><a href="../'+json.data[i].url+'" icon-url="'+json.data[i].icon+'" style="background: url('+json.data[i].icon+'.png) 29px center no-repeat;">'+json.data[i].title+'</a></li>';
	        		}
	        		str += '</ul>';
...
function match(){
	...
	$(".contain-left ul li").each(function(){
	    if($(this).attr("name")==scodeSecond){
	    	...
	    	thisa.addClass("lia");
	    	thisa.css("background","url("+thisa.attr("icon-url")+"_0.png) 29px center no-repeat");
