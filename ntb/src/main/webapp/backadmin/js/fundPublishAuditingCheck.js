...
function checkThis(t) {
    ...
    layer.confirm('确定要变更状态吗?', function(index) {
        ...
        $.get('../rest/backadmin/fundPublish/operateCheck?uuid=' + uuid + '&status=' + status + '&reason=' + reason, function(r) {
            if (r.status == 'SUCCESS') {
                layer.msg('操作成功', {
                    time: 1000
                }, function() {
...
function submitThis(t) {
    layer.confirm('确定要提交审核吗?', function(index) {
        ...
        var obj = $(t);
        var uuid = obj.attr('data-uuid');
        $.get('../rest/backadmin/fundPublish/operateSubmitCheck?uuid=' + uuid, function(r) {
            if (r.status == 'SUCCESS') {
                layer.msg('操作成功', {
                    time: 1000
                }, function() {
...
function getStatusList() {
    ...
    $("#allCount").html("(0)");
    $.get('../rest/backadmin/fundPublish/operateCheckStatusList', function(r) {
        if (r.status == 'SUCCESS') {
            var totalCount = 0;
...
function getTypeList() {
    ...
    $.get('../rest/backadmin/fundPublish/operateCheckTypeList?status=' + status, function(r) {
        if (r.status == 'SUCCESS') {
...
function getList() {
    ...
    $.get('../rest/backadmin/fundPublish/operateCheckList?sorts=submittime-desc&pageNum=' + page + '&pageSize=10&name=' + name + str, function(r) {
        if (r.status == 'SUCCESS') {
