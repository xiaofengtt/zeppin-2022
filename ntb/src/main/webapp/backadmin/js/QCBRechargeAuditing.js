$(document).ready(function() {
    ...
    function getStatusList() {
        ...
        $.ajax({
                url: "../rest/backadmin/qcbCompanyTransfer/operateStatusList",
                type: "get",
                data: {
    ...
    function deleteThis(t) {
        layer.confirm('确定要删除吗?', function(index) {
            ...
            $.get('../rest/backadmin/qcbCompanyTransfer/operateDelete?uuid=' + uuid, function(r) {
                if (r.status == 'SUCCESS') {
                    layer.msg('操作成功', {
                        time: 2000
                    }, function() {
    ...
    function submitThis(t) {
        layer.confirm('确定要提交审核吗?', function(index) {
            ...
            var obj = $(t);
            var uuid = obj.attr('data-uuid');
            $.get('../rest/backadmin/qcbCompanyTransfer/operateSubmitCheck?uuid=' + uuid, function(r) {
                if (r.status == 'SUCCESS') {
                    layer.msg('操作成功', {
                        time: 1000
                    }, function() {
    ...
    function getTypeList() {
        ...
        $.ajax({
                url: "../rest/backadmin/qcbCompanyTransfer/operateTypeList",
                type: "get",
                data: {
                    "status": $(".filter").eq(0).find(".statusLight").prop("id"),
                    "transferType": "outside"
                }
            })
            .done(function(r) {
    ...
    function getList(name, sorts) {
        var sort = "";
        var nameValue = "";
        var statusValue = $(".filter").eq(0).find(".statusLight").prop("id");
        var typeValue = $(".filter").eq(1).find(".statusLight").prop("id");
        if (sorts != undefined) {
            sort = sorts;
        }
        if (name != undefined) {
        ...
        $.ajax({
                url: '../rest/backadmin/qcbCompanyTransfer/operateList',
                type: 'get',
                data: {
                    "pageNum": pageNum,
                    "pageSize": 10,
