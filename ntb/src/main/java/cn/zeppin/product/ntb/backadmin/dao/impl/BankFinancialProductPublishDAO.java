...
public class BankFinancialProductPublishDAO extends BaseDAO<BankFinancialProductPublish, String> implements IBankFinancialProductPublishDAO {
	...
	public List<Entity> getListForPage(Map<String, String> inputParams, Integer pageNum, Integer pageSize, String sorts,
		...
		builder.append(" select bfp.uuid, bfp.bank_financial_product as bankFinancialProduct, bfp.name,bfp.scode,bfp.url,bfp.type,bfp.status,bfp.stage,bfp.target,bfp.custodian,bfp.target_annualized_return_rate as targetAnnualizedReturnRate,"
				...
				+ "bfp.risk_level as riskLevel,bfp.net_worth as netWorth,bfp.creator,bfp.createtime,bfp.area as area,bfp.min_invest_amount as minInvestAmount,bfp.max_invest_amount as maxInvestAmount, bfp.min_invest_amount_add as minInvestAmountAdd from bank_financial_product_publish bfp where 1=1 ");
		//名称
	 ...
	 */
	@Override
	public List<Entity> getWebListForPage(Map<String, String> inputParams, Integer pageNum, Integer pageSize, String sorts,
			Class<? extends Entity> resultClass) {
		StringBuilder builder = new StringBuilder();
		builder.append(" select bfp.uuid, bfp.bank_financial_product as bankFinancialProduct, bfp.name,bfp.scode,bfp.url,bfp.type,bfp.status,bfp.stage,bfp.target,bfp.custodian,bfp.target_annualized_return_rate as targetAnnualizedReturnRate,"
				+ "bfp.total_amount as totalAmount,bfp.collect_starttime as collectStarttime,bfp.collect_endtime as collectEndtime,bfp.term,bfp.record_date as recordDate,bfp.guarantee_status as guaranteeStatus,"
				+ "bfp.value_date as valueDate,bfp.maturity_date as maturityDate,bfp.flag_purchase as flagPurchase, bfp.flag_redemption as flagRedemption, bfp.flag_flexible as flagFlexible,bfp.currency_type as currencyType,"
				+ "bfp.risk_level as riskLevel,bfp.net_worth as netWorth,bfp.creator,bfp.createtime,bfp.area as area,bfp.min_invest_amount as minInvestAmount,bfp.max_invest_amount as maxInvestAmount, bfp.min_invest_amount_add as minInvestAmountAdd from bank_financial_product_publish bfp where  bfp.status='checked' ");
		//名称
		if (inputParams.get("name") != null && !"".equals(inputParams.get("name"))) {
			builder.append(" and bfp.name like '%" + inputParams.get("name") + "%' ");
		}
	...
	public Integer getWebCount(Map<String, String> inputParams) {
		StringBuilder builder = new StringBuilder();
		builder.append(" select count(*) from bank_financial_product_publish bfp where bfp.status='checked' ");
		//名称
		if (inputParams.get("name") != null && !"".equals(inputParams.get("name"))) {
			builder.append(" and bfp.name like '%" + inputParams.get("name") + "%' ");
		}
	...
	public List<Entity> getStageList(Class<? extends Entity> resultClass) {
		StringBuilder builder = new StringBuilder();
		builder.append("select bfpp.stage as status, count(*) as count from bank_financial_product_publish bfpp where bfpp.status='checked' group by bfpp.stage");
		return super.getBySQL(builder.toString(), resultClass);
	}
	...
	public void updateStageCollect() {
		...
		StringBuilder builder = new StringBuilder();
		builder.append("update bank_financial_product_publish set stage='collect' where status='checked' and ");
	...
	public void updateStageIncome() {
		Timestamp time = new Timestamp(System.currentTimeMillis());
		StringBuilder builder = new StringBuilder();
		builder.append("update bank_financial_product_publish set stage='income' where status='checked' and ");
	...
	public void updateStageFinished() {
		Timestamp time = new Timestamp(System.currentTimeMillis());
		StringBuilder builder = new StringBuilder();
		builder.append("update bank_financial_product_publish set stage='finished' where status='checked' and stage != 'exception' and");
