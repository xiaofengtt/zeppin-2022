...
public class QcbEmployeeCouponVO implements Entity {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -7277580059362513285L;
	...
	private String status;
 	...
 	public QcbEmployeeCouponVO(){
		
	}
	
	public QcbEmployeeCouponVO(QcbEmployeeCoupon qe){
		this.uuid = qe.getUuid();
		this.coupon = qe.getCoupon();
		this.status = qe.getStatus();
		this.expiryDate = qe.getExpiryDate();
		if(qe.getExpiryDate() != null){
			this.expiryDateCN = Utlity.timeSpanToDateString(qe.getExpiryDate());
		} else {
			this.expiryDateCN = "";
		}
	}
