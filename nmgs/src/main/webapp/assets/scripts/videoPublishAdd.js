...
function backToList(){
	...
	window.location.href="../views/videoPublishList.html?parent="+parent;
}
$(function() {
	var parent = (url('?cid') != null) ? url('?cid') : '';
	$("#category").val(parent);
	var html="";
	$.getJSON('../front/admin/videoinfo!execute?uid=g0001&pagesize=&sort=&pagenum=&status=checked', function(r) {
		...
		$.post('../front/admin/videoPublish!execute?uid=h0003&' + str, function(data) {
			if (data.status == "success") {
				if(buttonType==1){
					...
					layer.msg("添加成功！");
					setTimeout('window.location.href="../views/videoPublishList.html?parent="+parent',1000);
				}
			} else {
