...
public class ResourceAction extends BaseAction {
	 ...
	 * @throws Exception
	 */
	public Map<String,Object> writeFile(File file) throws Exception {
		...
		for (String sPath : dir) {
			...
			if (!tfFile.exists()) {
				tfFile.mkdir();
				if (!tfFile.exists()) {
					throw new Exception(serverPath + "/" + basePath+ "路径创建失败");
				}
			}
		}
		...
		if("jpg".equals(fileType.toLowerCase())||"jpeg".equals(fileType.toLowerCase())||"png".equals(fileType.toLowerCase())
				||"bmp".equals(fileType.toLowerCase())||"tiff".equals(fileType.toLowerCase())||"gif".equals(fileType.toLowerCase())
				||"psd".equals(fileType.toLowerCase())){
			res.put("type","2");
			BufferedImage src = ImageIO.read(file2);
			res.put("dpi",src.getWidth(null) + "x" + src.getHeight(null));
		}else if("mp4".equals(fileType.toLowerCase())||"rmvb".equals(fileType.toLowerCase())||"avi".equals(fileType.toLowerCase())
				||"mov".equals(fileType.toLowerCase())||"3gp".equals(fileType.toLowerCase())||"flv".equals(fileType.toLowerCase())
				||"wmv".equals(fileType.toLowerCase())||"mpg".equals(fileType.toLowerCase())){
			res.put("type","1");
			try {
