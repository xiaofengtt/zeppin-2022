...
public class ResourceAction extends BaseAction {
	...
	// }
	// if(sis.available() > 0){
	// String path = "";//临时文件存储路径
	// Map map = FileUtlity.saveToImgByInputStream(sis, serverPath+"/temp",
	// fileName+"_temp",path);//创建临时文件，并获取相关信息
	// if(map.isEmpty()){
	// actionResult.init(FAIL_STATUS, "未读取到文件,资源上传失败", null);
	// Utlity.ResponseWrite(actionResult, dataType, response);
	// return;
	// }
	// if(map.get("staff")!=null && "error".equals(map.get("staff"))){
	// actionResult.init(FAIL_STATUS, "未识别的文件类型,资源上传失败", null);
	// Utlity.ResponseWrite(actionResult, dataType, response);
	// return;
	// }
	...
	// resource.setFilesize(Integer.parseInt(map.get("size").toString()) );
	// resource.setSuffix("." + map.get("staff").toString());
	// resource.setSourcePath(basePath + resource.getName() +
	// resource.getSuffix());
	...
	// resource.setSysUser(0);
	// path = map.get("path").toString();
	// if(path != null && "".equals(path)){
	// actionResult.init(FAIL_STATUS, "临时文件创建失败,资源上传失败", null);
	// Utlity.ResponseWrite(actionResult, dataType, response);
	// return;
	// }
	// File fileTemp = new File(path);
	//
	// boolean file2 = fileTemp.renameTo(new File(serverPath + "/" +
	...
	// } else{
	// actionResult.init(FAIL_STATUS, "文件处理失败，资源上传失败", null);
	// }
	//
	//
	// } catch (Exception e) {
	...
	public void Add() {
		...
		try {
			...
			e.getStackTrace().toString();
			actionResult.init(FAIL_STATUS, "文件类型必须为：.jpg .jpeg .png", null);
			Utlity.ResponseWrite(actionResult, dataType, response);
			return;
		}
		Utlity.ResponseWrite(actionResult, dataType, response);
