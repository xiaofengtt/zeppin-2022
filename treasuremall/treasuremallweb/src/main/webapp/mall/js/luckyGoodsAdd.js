...
$(function(){
	...
	$("input[name='dPrice']").val(window.localStorage.getItem("price"));
				    		...
				    		if (r.status == "SUCCESS") {
				    			for(var m=0;m<r.data.imageList.length;m++){
									if(r.data.imageList[m].type=="type_list"&&r.data.imageList[m].sort=='0'){
										$(".goodsImg img").attr("src","http://backadmin.jumeiygc.com/"+r.data.imageList[m].imageUrl).show();
									}
								}
