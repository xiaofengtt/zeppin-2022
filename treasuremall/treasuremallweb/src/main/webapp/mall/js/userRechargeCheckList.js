	...
	$.ajax({
		...
		if (r.status == "SUCCESS") {
			var tableHtml = '<tr>'+
				...
				'<th width="11%" class="text-center">用户</th>'+
				'<th width="8%" class="text-center">充值金额/元</th>'+
				'<th width="17%" class="text-center">充值信息</th>'+
				'<th width="7%" class="text-center">备注</th>'+
				'<th width="7%" class="text-center">充值金币</th>'+
			...
			for(var i=0;i<r.data.length;i++){
				...
				tableHtml += '<tr data-showId="'+r.data[i].frontUser+'">'
					...
					+'<td class="text-center">'+r.data[i].transData+'</td>'
...
function exportFile(){
	...
	link.href = '../back/userRecharge/export?status='+status+'&orderNum='+orderNum+'&frontUserShowId='+frontUserShowId+
