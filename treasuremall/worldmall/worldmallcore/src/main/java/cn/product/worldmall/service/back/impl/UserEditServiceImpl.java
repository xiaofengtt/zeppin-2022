...
import cn.product.worldmall.vo.back.FrontUserEditVO;
...
//					content.append("您在"+Utlity.timeSpanToChinaDateString(frontUserEdit.getCreatetime())+"申请修改昵称，由"+frontUserEdit.getInfoBefore()+"变更为"+ frontUserEdit.getInfoAfter() +",");
					content.append("You applied to change your nickname "
//							+ "on " + Utlity.timeSpanToUsString(frontUserEdit.getCreatetime()) 
					+ " , and changed from "+frontUserEdit.getInfoBefore()+" to "+ frontUserEdit.getInfoAfter() +",");
					if(FrontUserEditStatus.CHECKED.equals(status)) {
						content.append("and it was approved!");
						fu.setNickname(frontUserEdit.getInfoAfter());
						this.frontUserEditDao.check(frontUserEdit, fu);
					}else {
						content.append("the review failed! Reason for failure:"+reason);
