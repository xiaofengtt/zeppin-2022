...
import cn.product.worldmall.entity.Version.VersionStatus;
import cn.product.worldmall.service.back.VersionService;
...
public class VersionServiceImpl implements VersionService{
	...
	public void add(InputParams params, DataResult<Object> result) {
		...
		String adverturl = paramsMap.get("adverturl") == null ? "" : paramsMap.get("adverturl").toString();
		Boolean flagupdate = paramsMap.get("flagupdate") == null ? null : Boolean.valueOf(paramsMap.get("flagupdate").toString());
		Boolean flagcompel = paramsMap.get("flagcompel") == null ? null : Boolean.valueOf(paramsMap.get("flagcompel").toString());
		
		try {
			//排他校验
			if(this.versionDao.getByChannelVersion(channel, name) != null) {
				result.setStatus(ResultStatusType.FAILED);
				result.setMessage("渠道版本已存在！");
				return;
			}
