...
//		return new Queue(environment.getProperty("mq.lottery.ready.queue"), true);
	...
	/**
	 * 用户绑定推送设备获取AWS-SNS-endpointArn信息消息模型
	 * @return
	 */
	//用户获取AWS-SNS-endpointArn信息队列
	@Bean
	public Queue getEndpointArnStartQueue() {
		return new Queue(environment.getProperty("mq.endpointarn.start.queue"), true);
	}
	//用户获取AWS-SNS-endpointArn信息交换机
	@Bean
	public TopicExchange getEndpointArnStartExchange() {
		return new TopicExchange(environment.getProperty("mq.endpointarn.start.exchange"), true, false);
	}
	//用户获取AWS-SNS-endpointArn信息绑定
	@Bean
	public Binding getEndpointArnStartBinding() {
		return BindingBuilder.bind(getEndpointArnStartQueue())
				.to(getEndpointArnStartExchange())
				.with(environment.getProperty("mq.endpointarn.start.routing.key"));
	}
	
	
	/**
	 * 发送AWS-SNS点对点推送信息消息模型
	 * @return
	 */
	//发送AWS-SNS点对点推送信息队列
	@Bean
	public Queue sendNoticeMessageStartQueue() {
		return new Queue(environment.getProperty("mq.noticemessage.start.queue"), true);
	}
	//发送AWS-SNS点对点推送信息交换机
	@Bean
	public TopicExchange sendNoticeMessageStartExchange() {
		return new TopicExchange(environment.getProperty("mq.noticemessage.start.exchange"), true, false);
	}
	//发送AWS-SNS点对点推送信息绑定
	@Bean
	public Binding sendNoticeMessageStartBinding() {
		return BindingBuilder.bind(sendNoticeMessageStartQueue())
				.to(sendNoticeMessageStartExchange())
				.with(environment.getProperty("mq.noticemessage.start.routing.key"));
	}
	...
	//为设备绑定主题队列
	@Bean
	public Queue bindingTopicStartQueue() {
		return new Queue(environment.getProperty("mq.bindingtopic.start.queue"), true);
	}
	//为设备绑定主题交换机
	@Bean
	public TopicExchange bindingTopicStartExchange() {
		return new TopicExchange(environment.getProperty("mq.bindingtopic.start.exchange"), true, false);
	}
	//为设备绑定主题绑定
	@Bean
	public Binding bindingTopicStartBinding() {
		return BindingBuilder.bind(bindingTopicStartQueue())
				.to(bindingTopicStartExchange())
				.with(environment.getProperty("mq.bindingtopic.start.routing.key"));
	}
	...
	//发送消息到主题队列
	@Bean
	public Queue sendTopicNoticeStartQueue() {
		return new Queue(environment.getProperty("mq.topicnoticemessage.start.queue"), true);
	}
	//发送消息到主题交换机
	@Bean
	public TopicExchange sendTopicNoticeStartExchange() {
		return new TopicExchange(environment.getProperty("mq.topicnoticemessage.start.exchange"), true, false);
	}
	//发送消息到主题绑定
	@Bean
	public Binding sendTopicNoticeStartBinding() {
		return BindingBuilder.bind(sendTopicNoticeStartQueue())
				.to(sendTopicNoticeStartExchange())
				.with(environment.getProperty("mq.topicnoticemessage.start.routing.key"));
	}
