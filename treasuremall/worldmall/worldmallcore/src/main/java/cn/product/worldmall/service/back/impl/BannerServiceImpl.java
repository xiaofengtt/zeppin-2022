...
public class BannerServiceImpl implements BannerService{
	...
	public void add(InputParams params, DataResult<Object> result) {
		...
		Integer sort = paramsMap.get("sort") == null ? 0 : Integer.valueOf(paramsMap.get("sort").toString());
		...
		try {
			...
			banner.setRefreshtime(banner.getCreatetime());
			if(!Utlity.checkStringNull(frontuserlevel)) {
	...
	public void refresh(InputParams params, DataResult<Object> result) {
		...
		List<Banner> list = this.bannerDao.getListByParams(searchMap);
		try {
			if(list != null && list.size() > 0) {
				this.bannerDao.batchUpdate(list);
			}
		} catch (Exception e) {
			e.printStackTrace();
			result.setStatus(ResultStatusType.FAILED);
