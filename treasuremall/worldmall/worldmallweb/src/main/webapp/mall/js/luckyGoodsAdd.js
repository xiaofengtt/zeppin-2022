	...
	$.ajax({
        ...
        async:false
  		...
  		if (r.status == "SUCCESS") {
			...
			if(window.localStorage.getItem("demoFlag") && window.localStorage.getItem("demoFlag")=="true"){
				exchangeRate = 1
			}
			$("input[name='dPrice']").val(Number(window.localStorage.getItem("price"))*exchangeRate);
		}
	})
		        ...
		        data:{
		    		...
		    		if (r.status == "SUCCESS") {
		    			 ...
		    			 if(r.data.totalIssueNum=="-1"){
	    		  		...
	    		  		if(r.data.tabs){
	    		  			tabsArr = r.data.tabs.split(",");
			    			 $("input[name='tabs']").each(function(i,value){
			    				 var value = $(this).val();
...
$(".lay-submit").click(function(){
	...
	if(uuid!=""){
		var data = {
	        	...
	        	'tabs':$("input[name='tabs']:checked").val()
	        }
		if($("input[name='totalIssueNum']").parent().css("display")=="block"){
		...
		$.ajax({
			...
			window.localStorage.removeItem("demoFlag");
    		if (r.status == "SUCCESS") {
    			layer.msg('编辑成功', {
    				time: 1000 
    			}, function(){
