...
var demoFlag = (url('?demoFlag') != null) ? url('?demoFlag') : '';
var tab = url('?tab');
var flagSubmit = true;
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
								...
								'<th width="15%" class="text-center">商品成本价(美元)</th>'+
				...
				for(var i=0;i<r.data.length;i++){
					...
					+(r.data[i].price * exchangeRate).toFixed(2)+'</td><td class="text-center">'+formatDate(r.data[i].createtime)+'</td><td class="text-center" data-id="'+encodeURI(JSON.stringify(r.data[i]))+
					'">'+categoryHtml+'</td></tr>'; 
				}
				...
				$(".chooseBtn").click(function(){
					...
					window.localStorage.setItem("demoFlag",data.demoFlag);
