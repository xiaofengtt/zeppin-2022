		...
		$.ajax({
	        ...
	        data:{
	    		...
	    		if (r.status == "SUCCESS") {
					 ...
					 var len=500-r.data.description.length;				
					 $("#wordsLength").text(len+"/500");
	    			 if(r.data.demoFlag==true){
	    				 $("input[name=demoFlag][value='true']").prop("checked",true);
	    			 }else{
	    				 $("input[name=demoFlag][value='false']").prop("checked",true);
	    			 }
	    			 form.render();
	    			 $('input[name=demoFlag]').attr("disabled",true);
	    			 //把图片分组
...
$(".lay-submit").click(function(){
	...
	var data = {
    	...
    	'demoFlag':$("input[name='demoFlag']:checked").val().replace(/(^\s*)|(\s*$)/g, "")
	}
