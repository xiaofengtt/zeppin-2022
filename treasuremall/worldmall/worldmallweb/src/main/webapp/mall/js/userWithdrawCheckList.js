	...
	$.ajax({
		...
		if (r.status == "SUCCESS") {
			var tableHtml = '<tr>'+
				...
				'<th width="100px" class="text-center">提现金额/美元</th>'+
				'<th width="80px" class="text-center">手续费/美元</th>'+
				'<th width="100px" class="text-center">到账金额/美元</th>'+
			...
			for(var i=0;i<r.data.length;i++){
				...
				tableHtml += '<tr data-showId="'+r.data[i].frontUser+'">'
					...
					+'<td class="text-left">PayPal账户：'+r.data[i].frontUserAccountNumber+'</br></td>'
					...
					+'<p class="'+(r.data[i].totalRecharge<r.data[i].totalWithdraw+r.data[i].frontUserBalance+r.data[i].totalDelivery?"color-red":"")+'">充值量：'+(r.data[i].totalRecharge).toFixed(2)+'美元</p>'
					+'<p class="'+(r.data[i].totalPayment<r.data[i].totalWithdraw*1.5?"color-red":"")+'">提现量：'+(r.data[i].totalWithdraw).toFixed(2)+'美元</p></td>'
