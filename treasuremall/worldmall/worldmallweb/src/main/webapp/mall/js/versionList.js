...
$(function(){
	$(".goPrePage").click(function(){
		window.location.href='versionDetail.html?pagesize='+pageSize+'&pagenum='+pageNum
	});
	getList();
});
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
								...
								'<th width="18%">主包地址</th>'+
								'<th width="17%">下载地址</th>'+
								'<th width="4%" class="text-center">是否主包</th>'+
...
//    					opteration = '<a class="mr-5" href="versionDetail.html?uuid='+r.data[i].uuid+'">编辑</a><a class="deleteBtn">删除</a>';
    					opteration = '<a class="editBtn mr-5">编辑</a><a class="deleteBtn">删除</a>';
    				...
    				tableHtml += '<tr>'+
    					...
    					'<td class="text-center '+classColor+'">'+statusArray[r.data[i].status]+'</td>'+
    					'<td class="text-center">'+flagArray[r.data[i].flagupdate]+'</td>'+
    					'<td class="text-center">'+flagArray[r.data[i].flagcompel]+'</td>'+
    					'<td class="text-center" data-id="'+r.data[i].uuid+'">'+opteration+'</td>'+
    					'</tr>'; 
    			}
    			...
    			$(".editBtn").click(function(){
    				window.location.href = 'versionDetail.html?uuid='+$(this).parent().attr("data-id")+'&pagesize='+pageSize+'&pagenum='+pageNum;
    				return false;
    			});
