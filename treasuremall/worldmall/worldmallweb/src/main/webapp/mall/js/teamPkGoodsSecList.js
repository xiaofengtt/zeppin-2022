...
function getList(){
	$.ajax({
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
								'<th width="20%" class="text-center">期号</th>'+
								'<th width="20%" class="text-center">获奖战队</th>'+
								'<th width="30%" class="text-center">中奖时间</th>'+
								'<th width="" class="text-center">操作</th>'+
							'</tr>';
				for(var i=0;i<r.data.length;i++){
					var categoryHtml = '';
					categoryHtml = '<a href="teamPkGoodsThiList.html?uuid='+r.data[i].uuid+'&pagesize='+pagesize+'&pagenum='+pagenum+'">查看详情</a>';
					tableHtml += '<tr data-url="teamPkGoodsThiList.html?uuid='+r.data[i].uuid+
					'"><td class="text-center">'+r.data[i].issueNum+'</td><td class="text-center">'
					+(r.data[i].status=="finished"?teamGroup[r.data[i].luckyGroup]:'-')+
					'</td><td class="text-center">'+(r.data[i].status=="finished"?formatDate(r.data[i].finishedtime):'-')
