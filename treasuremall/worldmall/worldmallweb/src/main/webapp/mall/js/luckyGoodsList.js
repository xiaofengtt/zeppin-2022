...
$(function(){
	...
	$(".goPrePage").click(function(){
		window.location.href='luckyGoodsAdd.html?pagesize='+pageSize+'&pagenum=1'
	});
});
//list
function getList(size,num){
	$.ajax({
        ...
        data: {
        	...
        	'gameType':'tradition',
        	'demoFlag':$("select[name='demoFlag']").val()
        }
    }).done(function(r) {
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
				...
				for(var i=0;i<r.data.length;i++){
					...
					if(r.data[i].status=="disable"){
						categoryHtml = '<a class="editBtn mr-5">编辑</a><a class="deleteBtn mr-5">删除</a><a href="luckyGoodsSecList.html?uuid='+r.data[i].uuid+'&pagesize='+pageSize+'&pagenum='+pageNum+'">查看详情</a>';
					}else{
						categoryHtml = '<a href="luckyGoodsSecList.html?uuid='+r.data[i].uuid+'&pagesize='+pageSize+'&pagenum='+pageNum+'">查看详情</a>';
					}
					if(r.data[i].status=="disable"){
					...
					tableHtml += '<tr class="robot'+r.data[i].status+'" data-url="luckyGoodsAdd.html?uuid='+r.data[i].uuid+
				...
				$(".editBtn").click(function(){
					window.location.href = 'luckyGoodsAdd.html?uuid='+$(this).parent().attr("data-id")+'&pagesize='+pageSize+'&pagenum='+pageNum;
					return false;
				});
