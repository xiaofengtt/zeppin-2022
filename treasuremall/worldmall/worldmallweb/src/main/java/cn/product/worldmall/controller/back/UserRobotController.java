...
public class UserRobotController extends BaseController{
	...
	@ActionParam(key = "upload", message="资源文件ID", type = DataType.STRING)
	@ActionParam(key = "filename", message="文件名", type = DataType.STRING, required = true)
	@ResponseBody
	public Result upload(String upload, String filename){
		InputParams params = new InputParams("userRobotService", "upload");
		params.addParams("upload", null, upload);
		params.addParams("filename", null, filename);
		return this.execute(params);
	}
}
