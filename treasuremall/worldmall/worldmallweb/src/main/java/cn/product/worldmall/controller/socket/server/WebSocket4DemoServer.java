package cn.product.worldmall.controller.socket.server;
...
import cn.product.worldmall.util.socket.ServerEncoder;
import cn.product.worldmall.api.base.DataResult;
import cn.product.worldmall.api.base.InputParams;

@ServerEndpoint(value = "/weblink", encoders = { ServerEncoder.class })
@Component
public class WebSocket4DemoServer {
	private final static Logger log = LoggerFactory.getLogger(WebSocket4DemoServer.class);
	 
    // 静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
    private static int onlineCount = 0;
 
    // concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。
    private static CopyOnWriteArraySet<WebSocket4DemoServer> webSocketSet = new CopyOnWriteArraySet<WebSocket4DemoServer>();
...
//	}
	...
	public void setTransferService(TransferService transferService) {
		WebSocket4DemoServer.transferService = transferService;
	}
...
//        }
        ...
        if(Utlity.checkStringNull(dataType)) {//返回错误提示
        	...
        	responseMap.put("message", "path error!");//方法路径错误！
...
//        transferService = applicationContext.getBean(TransferService.class);
        if("lotteryList".equals(dataType)) {//获取即将开奖列表
    		...
    		DataResult<Object> data =  (DataResult<Object>) WebSocket4DemoServer.transferService.execute(input);
...
//        this.session.getBasicRemote().sendText(message);
    ...
    public void sendInfo(String message) throws IOException {
        ...
        for (WebSocket4DemoServer item : webSocketSet) {
            try {
    ...
    public static synchronized void addOnlineCount() {
        WebSocket4DemoServer.onlineCount++;
    }
