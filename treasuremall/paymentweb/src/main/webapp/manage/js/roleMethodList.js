	...
	$.ajax({
		...
		if (r.status == "SUCCESS") {
			for(var i=0;i<r.data.length;i++){
				roleList += '<li data-id="'+r.data[i].uuid+'" >'+r.data[i].name+'</li>';
			}
...
function getList(){
	...
	if(role != ''){
		$.get('../system/roleMethod/all',function(r) {
			if(r.status =='SUCCESS') {
				var leftHtml='';
				var rightHtml='';
				var lastHtml = '';
			...
			if(r.totalResultCount > 0){
				$.get('../system/roleMethod/list?role='+role,function(ret) {
