	...
	$.ajax({
		...
		if (r.status == "SUCCESS") {
			var tableHtml = '<tr>'+
				...
				'<th width="200px" class="text-center">商户渠道ID</th>'+
			...
			for(var i=0;i<r.data.length;i++){
				...
				tableHtml += '<tr>'
					...
					+'<td class="text-center">'+(r.data[i].poundageRate == null ? ((r.data[i].poundage/100).toFixed(2) + '元') : (r.data[i].poundageRate*100+'%'))+'</td>'
