...
var typeObj = {
	...
	'company_recharge':'store_recharge',
	'company_withdraw':'store_withdraw'
}
...
function getList(){
	...
	$.ajax({
        url: '../store/companyAccountHistory/list',
        type: 'get',
        async:false,
        data: datas
    }).done(function(r) {
		if (r.status == "SUCCESS") {
			var tableHtml = '<tr>'+
				...
				'<th width="150px" class="text-center">Trade time</th>'+
...
function exportFile(){
	...
	window.open('../store/companyAccountHistory/export?type='+type+'&channel='+channel+
