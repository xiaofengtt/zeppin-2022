...
@Service
public class TJyXtzjmjjfplService extends BaseService implements ITJyXtzjmjjfplService {
	...
	public void insertAll(List<Map<String, Object>> datasList, TSsOperatorVO operator) {
		for(Map<String, Object> datas : datasList){
			...
			check.setDatatype("TJyXtzjmjjfpl");
	...
	@Override
	public void deleteWholeData(TJyXtzjmjjfpl t, TSsOperatorVO operator) {
		TSsLog log = new TSsLog();
		log.setId(UUID.randomUUID().toString());
		log.setDatatype("TJyXtzjmjjfpl");
		...
		this.TCheckInfoDAO.deleteByData("TJyXtzjmjjfpl", t.getId());
		this.delete(t);
	}
}
