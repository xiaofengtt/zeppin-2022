...
@Service
public class TXmXtzjyyhtxxService extends BaseService implements ITXmXtzjyyhtxxService {
	...
	public void insertAll(List<Map<String, Object>> datasList, TSsOperatorVO operator) {
		for(Map<String, Object> datas : datasList){
			...
			check.setDatatype("TXmXtzjyyhtxx");
	...
	@Override
	public void deleteWholeData(TXmXtzjyyhtxx t, TSsOperatorVO operator) {
		TSsLog log = new TSsLog();
		log.setId(UUID.randomUUID().toString());
		log.setDatatype("TXmXtzjyyhtxx");
		...
		this.TCheckInfoDAO.deleteByData("TXmXtzjyyhtxx", t.getId());
		this.delete(t);
	}
}
