...
function getList(){
	if(types==""){
		...
		$.get("../rest/backadmin/input/sync?"+types.substring(1,types.length),function(r){
			layer.closeAll();
	    	if(r.status=="SUCCESS"){
				$(".check").each(function(){
					if ($(this).is(":checked") == true) {
			            $(this).parent().parent().find(".lastTd").html("成功，采集了0条数据");
			        } 
				});	
				
				if(JSON.stringify(r.data)!= "{}"){
					for(var item in r.data){
						$("."+item+"Td").html("成功，采集了"+r.data[item]+"条数据");
					}
				}				
				...
				$(".check").each(function(){
					if ($(this).is(":checked") == true) {
			            $(this).parent().parent().find(".lastTd").html("失败");
			        } 
				});	
	    		layer.msg(r.message);
	    	}
...
$(".editSql").click(function(){
	...
	$.ajax({
	    url: '../rest/backadmin/input/getSql',
	    type: 'get',
	    data: {
	    	"datatype":datatype,
		...
		if (r.status == "SUCCESS") {
			var index = layer.open({
				...
				,yes: function(index, layero){
					...
					$.post('../rest/backadmin/input/editSql',data,function(r){
						if (r.status == "SUCCESS") {
							layer.close(index);
							layer.msg("修改成功");
						}else{
