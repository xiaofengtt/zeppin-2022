...
public class ProjectAdmOptAction extends ActionSupport {
	...
	public void opProjectAdmin() {
		...
		try {
			...
			String createuser = parMap.containsKey("createuser") ? parMap.get("createuser")[0] : ""; // 是否可创建用户
			...
			if (id == null || id.equals("0") || id.equals("")) {
				...
				Organization o = this.iOragnizationService.get(Integer.parseInt(organization));
					...
					if (restrictRightId != null) {
						for (String s : restrictRightId) {
							if (s != null && !s.equals("0") && !s.equals("")) {
								...
								ProjectType pt = this.iProjectTypeService.get(Integer.parseInt(s));
				...
				try {
					...
					// 先删除以前所有
					this.iProjectAdminRightService.deleteByProjectAdmin(pa.getId());
					if (restrictRightId != null) {
						for (String s : restrictRightId) {
							if (s != null && !s.equals("0") && !s.equals("")) {
								ProjectType pt = this.iProjectTypeService.get(Integer.parseInt(s));
	...
	public void getOrganizationLevel() {
		...
		if (id.equals("0")) {
			...
			if (ROLE != ROLEENUM.SUPERADMIN) {
				...
				sb.append("[");
				String sr = "{\"id\":" + ora.getId() + ",\"name\":\"" + ora.getName() + "\",\"haschild\":" + (flag > 0 ? 1 : 0) + "}";
				sb.append(sr);
				sb.append("]");
		...
		if (id != null && !id.equals("")) {
			...
			for (int i = 0; i < li.size(); i++) {
				...
				String sr = "{\"id\":" + hid + ",\"name\":\"" + obj[1].toString() + "\",\"haschild\":" + (flag > 0 ? 1 : 0) + "},";
				sb.append(sr);
			}
			if (li.size() > 0) {
	...
	public void setiProjectAdminRightService(IProjectAdminRightService iProjectAdminRightService) {
