...
public class TeacherTrainingRecordsDaoImpl extends BaseDaoImpl<TeacherTrainingRecords, Integer> implements ITeacherTrainingRecordsDao {
	...
	public Map<Object, Long> getTrainingRecordsTeacherNumber(int projectId, int subjectId, int collegeId, Organization organization) {
		...
		if (organization != null) {
			sb.append(" and o.scode like '").append(organization.getScode()).append("%'");
		}
		...
		result.put(DictionyMap.TEACHER_TRAINING_RECORDS_CHECKNOPASS, checkNoPassNum);
	...
	public int getAduTeacherCount(String teacherName, int projectId, int subjectId, int trainingUnit, int isAdmin, int status, Organization organization, List<ProjectType> lityps) {
		...
		if (teacherName != null && teacherName.trim().length() > 0) {
			sb.append(" and (t.name like'%" + teacherName + "%' or t.idcard like'%" + teacherName + "%') ");
		}
	...
	public TeacherTrainingRecords getTeacherTrainingRecordByName(String teacherName, String projectName, String trainingtName, String subjectName) {
		StringBuilder sb = new StringBuilder();
		sb.append("select ttr from TeacherTrainingRecords ttr, Project p, Teacher t, TrainingSubject ts,TrainingCollege tc").append(" where ttr.teacher=t.id and ttr.project=p.id and ttr.trainingSubject=ts.id and ttr.trainingCollege=tc.id");
		...
		TeacherTrainingRecords ttr = (TeacherTrainingRecords) this.getObjectByHql(sb.toString(), null);
		return ttr;
	}
	...
	public List getAduTeacher(int projectId, int subjectId, int trainingUnit, int status, int trainingStatus, int offset, int length) {
		...
		if (trainingStatus > -1) {
			if (trainingStatus == 2) {
				sb.append(" and r.trainingStatus in (2,3,4)");
			} else {
	...
	public TeacherTrainingRecords getTeacherTrainingRecord(String project, String subject, String trainingCollege, String teacher) {
		...
		sb.append(" from TeacherTrainingRecords where project=").append(project);
	...
	public int getAduTeacherCount(HashMap<String, String> map, Organization organization, List<ProjectType> lityps) {
		...
		sb.append(" select count(*) from TeacherTrainingRecords r, Teacher t, Organization o,Project p where r.teacher=t.id");
		sb.append(" and t.organization = o.id and r.project=p.id ");
		if (map.containsKey("pid") && map.get("pid") != null && !map.get("pid").equals("0")) {
		...
		if (map.containsKey("sid") && map.get("sid") != null && !map.get("sid").equals("0")) {
			sb.append(" and r.trainingSubject=" + map.get("sid"));
		}

		if (map.containsKey("tid") && map.get("tid") != null && !map.get("tid").equals("0")) {
		...
		if (map.containsKey("name") && map.get("name") != null && !map.get("name").equals("")) {
			sb.append(" and (t.name like'%" + map.get("name") + "%' or t.idcard like'%" + map.get("name") + "%')");
		}
	...
	public int getListCount(Integer pid, Short sid, Integer cid, int classes) {
		// TODO Auto-generated method stub
		String hqlString = "select count(*) from TeacherTrainingRecords where finalStatus=2 and project=" + pid + " and trainingSubject=" + sid + " and trainingCollege=" + cid;
		if (classes > 0) {
			hqlString = hqlString + " and classes=" + classes;
		}
		int count = Integer.parseInt(getObjectByHql(hqlString, null).toString());
		return count;
	}
	...
	public int getTeacherTrainingRecordsCount(Integer projectId, Short trainingSubjectId, Integer trainingCollegeId, Organization organization) {
		...
		// sb.append(")");
		// }
		return Integer.valueOf(this.getObjectBySql(sb.toString(), null).toString());
	}
	...
	public TeacherTrainingRecords getTeacherTrainingRecordsByIdCard(int trainingCollegeId, String idCard) {
		...
		List<TeacherTrainingRecords> lstRecords = this.getListByHSQL(sb.toString());
...
//		sb.append(" and r.finalStatus = 2 and p.enrollType=2 and p.traintype=1");
		sb.append(" and p.enrollType=2 and p.traintype=1");
		...
		if(status == 1){//未确认
			...
			sb.append(" and r.isConfirm in (-1,0,1) ");
		}
	...
	public List getListTeacherCount(String teacherName, Integer projectId,
			Integer trainingSubjectId, Integer trainingUnitId, int status, int offset, int length) {
		// TODO Auto-generated method stub
		StringBuilder sb = new StringBuilder();
		sb.append("from TeacherTrainingRecords r, Teacher t, Organization o,Project p where r.teacher=t.id ");
		sb.append(" and t.organization = o.id and r.project=p.id");
...
//		sb.append(" from TeacherTrainingRecords r, Project p, ProjectApply pa where 1=1 and r.project = p.id and pa.project=p.id and r.trainingSubject=pa.trainingSubject and p.enrollType=2");
	...
	public List getTeacherRecordsForSubject(int offset, int length,
		...
		if(param != null && param.size() > 0){
			if(param.containsKey("projectId") && param.get("projectId") != null && !"".equals(param.get("projectId"))){
				String projectId = param.get("projectId").toString();
				sb.append(" and r.project="+projectId);
			}
			
			if(param.containsKey("subjectId") && param.get("subjectId") != null && !"".equals(param.get("subjectId"))){
				String subjectId = param.get("subjectId").toString();
				sb.append(" and r.trainingSubject="+subjectId);
			}
			
			if(param.containsKey("trainCollegeId") && param.get("trainCollegeId") != null && !"".equals(param.get("trainCollegeId"))){
				String trainingCollege = param.get("trainCollegeId").toString();
				sb.append(" and r.trainingCollege="+trainingCollege);
			}
			
			if(param.containsKey("teacherName") && param.get("teacherName") != null && !"".equals(param.get("teacherName"))){
				String teacherName = (String)param.get("teacherName");
				sb.append(" and t.name like'%" + teacherName + "%'");
			}
			
			if(param.containsKey("teacherIdCard") && param.get("teacherIdCard") != null && !"".equals(param.get("teacherIdCard"))){
				String teacherIdCard = (String)param.get("teacherIdCard");
				sb.append(" and t.idcard like'%" + teacherIdCard + "%'");
			}
			
			if(param.containsKey("teacherOrganization") && param.get("teacherOrganization") != null && !"".equals(param.get("teacherOrganization"))){
				String teacherOrganization = param.get("teacherOrganization").toString();
