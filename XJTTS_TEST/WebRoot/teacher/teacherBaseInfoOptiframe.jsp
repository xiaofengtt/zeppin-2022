...
<body>
	<div class="ifrcnt container">
		...
		<div class="bd">
			...
			<form class="form-horizontal" id="teacherManage" action="trg_register.action"
				method="post" name="teahcer">
				<input type="hidden" name="teacherEx.teacher.id" value="<s:property value="teacherEx.teacher.id" />">
				<div class="clearfix">
							...
							<div class="controls">
								<select name="jobDuty">
									...
									<s:iterator value="lstJobDuties" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.jobDuty.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
							...
							<div class="controls">
								<select name="jobTitle">
									...
									<s:iterator value="lstJobTitles" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.jobTitle.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
							...
							<div class="controls">
								<select name="eductionBackground">
									...
									<s:iterator value="lstBackgrounds" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.eductionBackground.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
							...
							<div class="controls">
								<select name="politics">
									...
									<s:iterator value="lstPolitics" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.politics.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
					...
					<div class="span5">
							...
							<div class="controls">
								<select name="ethnic">
									...
									<s:iterator value="lstEthnics" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.ethnic.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
						...
						<div class="control-group">
							...
							<div class="controls">
							<input name="organization" data-val="true" data-val-number="<s:property value="teacherEx.teacher.organization.id"/>" data-name="<s:property value="teacherEx.teacher.organization.name"/>" id="organization" type="text" class="span3"  value='<s:property value="teacherEx.teacher.organization.id"/>'>	
							...
							<div class="controls">
								<select name="chineseLanguageLevel">
									...
									<s:iterator value="lstChineseLanguageLevels" id="e">
										<option value="<s:property value="#e.id" />"
											<s:if test="%{#e.id==teacherEx.teacher.chineseLanguageLevel.id}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
								</select>
							</div>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" readonly autocomplete="off" data-provide="datepicker"
									class="datepicker" id="" name="teachingAge" value="<s:date name="teacherEx.teacher.teachingAge" format="yyyy-MM-dd"/>">
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="" name="email"
									value="<s:property value="teacherEx.teacher.email" />" placeholder="邮箱">
							</div>
						</div>
								...
								<select name="unMainTeachingSubject" id="unMainTeachingSubject"
									...
									<s:iterator value="lstteacTeachingSubjectExs" id="e">
										<option value="<s:property value="#e.id" />"
											search="<s:property
												value="#e.searchString" />"
											<s:if test="%{#e.id==teacherEx.teacher.chineseLanguageLevel}" >selected</s:if>><s:property
												value="#e.name" /></option>
									</s:iterator>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input name="multiLanguage" type="checkbox" data-text-label=""
									...
									data-off-label="否"
									<s:if test="teacherEx.teacher.multiLanguage==1" >checked</s:if>
		...
		$(function() {
			$('#teacherManage').submit(
					function() {
						...
						str=encodeURI(str);
						//alert(str);
						$.post('../teacher/tinfo_edit.action?'+ str, function(data) {
							var Result = data.Result;
							var message = data.Message;
							if (Result == "OK") {
								window.alert("操作成功");
								window.top.location.href='../teacher/review.html';
							} else {
