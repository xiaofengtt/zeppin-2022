	...
	$(".delete").click(function(){
		...
		layer.confirm('您确定要删除吗？', {
			  btn: ['确定','取消'] //按钮
				...
				$.get('../admin/newmail_delete.action?id='+ mids+'&type='+types, function(r) {
					if (r.Result == 'OK') {
						layer.msg('删除成功！', {
		        			  icon: 1,
		        			  time: 2000 
...
function basicInfo(){
	$.ajax({
        type: "GET",
        url: "../admin/newmail_getInfo.action",
        data: {id:id},
        dataType: "text",
        async: true,
...
function attachment(){
	$.ajax({
        type: "GET",
        url: "../admin/newmail_getAttachmentList.action",
        data: {id:id},
        dataType: "text",
        async: true,
        success: function (json) {
...
//转发
$(".forwarded").click(function(){
	if(types=="1"){
		window.location.href="mailListForWriting.jsp?id="+id+"&method=edit";
	}else if(types=="2"){
		window.location.href="mailListForWriting.jsp?id="+id+"&method=edit";
	}	
});
...
//添加回复
function answer(obj){
	var content=$(".answerInputs").val().replace(/(^\s*)|(\s*$)/g, "");
	if(content==""){
		...
		if(isadd==true){
			layer.confirm('您确定要删除吗？', {
				  btn: ['确定','取消'] //按钮
					...
					$.get('../admin/newmail_delete.action?id='+ ids+'&type='+type, function(r) {
						if (r.Result == 'OK') {
							layer.msg('删除成功！', {
			        			  icon: 1,
			        			  time: 2000 
