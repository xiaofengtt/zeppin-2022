...
$(document).ready(function(){
	$(".loadingDiv").css({"width":$(".loadingDiv").parent().parent().width()+30+"px","height":$(".loadingDiv").parent().parent().height()
		+"px"});
...
$(window).scroll(function() {	
    ...
    if(offsetHeight<$(window).scrollTop()&&$(window).scrollTop()<(offsetHeight+$(".tableThead").height()+
    		$(".dv-table").height()+$(".totalTr").height()-2)){
    	$(".tableTheadFix").css({"position":"fixed","top":"0","left":"225px","width":width,"margin-top":"0","display":"block"});
    	$(".totalTr,.dv-table").width(width);
    }else{
    	$(".tableTheadFix").css({"position":"static","width":width,"display":"none"});
    	$(".totalTr,.dv-table").width(width);
    }
  });
...
function getInfo(){
	...
	if(judgeYear("#endTime")){
		...
		$.ajax({
	        type: "POST",
	        url: "../admin/countTraining_getPoorCount.action",
	        ...
	        success: function (data) {
	        	...
	        	if(json.status=="OK"){
	        		...
	        		$(".number span.passengers").html(formatNum(json.totalPoorTrainingTeacher));
	        		$(".number span.proportion").html(json.totalPoorTrainingRate);
	        		$(".number span.coverage").html(json.totalPoorTrainingTeacherRate);
	        		for (var i = 0; i < json.records.length; i++) {
	        			if(json.level=="1"&&json.records[i].name!="null"){
		        			...
		        			'</td><td width="10%">'+formatNum(json.records[i].poorTeacherCount)+'</td><td width="10%">'
		        			+formatNum(json.records[i].trainingCount)+'</td><td width="10%">'+formatNum(json.records[i].poorTrainingCount)+
		        			'</td><td width="10%">'+formatNum(json.records[i].poorTrainingTeacherCount)+'</td><td width="10%">'
		        			+json.records[i].poorTrainingRate+'%</td><td width="10%">'+json.records[i].poorTrainingTeacherRate+'%</td></tr>';		        			
	        			}else{
	        				if(json.records[i].name!="null"){
	        					table+='<tr><td class="extend" width="25%">'+json.records[i].name+'</td><td width="15%">'
			        			+formatNum(json.records[i].teacherCount)+
			        			'</td><td width="10%">'+formatNum(json.records[i].poorTeacherCount)+'</td><td width="10%">'
			        			+formatNum(json.records[i].trainingCount)+'</td><td width="10%">'+formatNum(json.records[i].poorTrainingCount)+
			        			'</td><td width="10%">'+formatNum(json.records[i].poorTrainingTeacherCount)+'</td><td width="10%">'
	        			...
	        			tArrayarea[i]=json.records[i].name;
	        		}
	        		...
	        		tArray[1]=new Array();
        			tArray[0]=new Array();
        			tArray[0][0]="贫困地区教师参训人次";
        			tArray[0][1]=json.totalPoorTrainingRate;
        			tArray[1][0]="非贫困地区教师参训人次";
        			tArray[1][1]=100-Number(json.totalPoorTrainingRate);
        			tArraypie[1]=new Array();
        			tArraypie[0]=new Array();
        			tArraypie[0][0]="贫困地区教师参训人数";
        			tArraypie[0][1]=json.totalPoorTrainingTeacherRate;
        			tArraypie[1][0]="贫困地区教师未参训人数";
        			tArraypie[1][1]=100-Number(json.totalPoorTrainingTeacherRate);
	        	}
	        	$(".dv-table table").html(table);
	        	$(".totalTr").html('<tr class=""><td class="extend" width="25%">总计</td><td width="15%">'+formatNum(json.totalTeacher)+
	        			'</td><td width="10%">'
	        			+formatNum(json.totalPoorTeacher)+'</td><td width="10%">'+formatNum(json.totalTraining)+'</td><td width="10%">'
	        			+formatNum(json.totalPoorTraining)+'</td><td width="10%">'+formatNum(json.totalPoorTrainingTeacher)+
	        			'</td><td width="10%">'+json.totalPoorTrainingRate+'%</td><td width="10%">'+json.totalPoorTrainingTeacherRate+
	        			'%</td></tr>');
	        ...
	        success: function (data) {
	        	...
	        	if(json.status=="OK"){
	        		...
	        		if(json.records.length=="0"){
	        			...
	        			for (var i = 0; i < json.records.length; i++) {
	        				if(json.records[i].name!="null"){
	        					table+='<tr class="exendChild exend'+name+'"><td class="extend" width="25%">'+json.records[i].name+'</td><td width="15%">'
			        			+formatNum(json.records[i].teacherCount)+
			        			'</td><td width="10%">'+formatNum(json.records[i].poorTeacherCount)+'</td><td width="10%">'
			        			+formatNum(json.records[i].trainingCount)+'</td><td width="10%">'+formatNum(json.records[i].poorTrainingCount)+
			        			'</td><td width="10%">'+formatNum(json.records[i].poorTrainingTeacherCount)+'</td><td width="10%">'
