...
function getProjectType(rid) {
	...
	var rpId = $("[name='id']").val();
	cUrl += '&rpId='+rpId;
	
	$.getJSON(cUrl,function(data) {

						if (data.length > 0) {
							var ufc = '<div class="ufc"><span class="usel" onclick="showul($(this))"><strong>请选择...</strong></span><ul class="uul" style="max-height:260px;overflow-x:scroll;">';
							$.each(data,function(i, c) {
...
function addExpert(t) {
	if(addExpertFlag){
		...
		$.getJSON(cUrl, function(ret) {
			if (ret.Result == 'OK') {
				$('#status_' + obj.attr('data-id')).html('<a onclick="deleteExpert(this)" data-id="'+ret.expertid+'"  data-url="../admin/projectApplyOpt_deleteProjectApplyExpert.action?id='+ret.expertid+'&paid='+ret.paid+'&evaluateType='+ret.evaluateType+'"  href="javascript:void(0)">删除评审权限</a>');
			}else if (ret.Result == "ERROR" || ret.Result == 'FAIL') {
				alert('失败,' + ret.Message);
			}
			addExpertFlag = true;
...
var deleteExpertFlag = true;
//删除评审权限
function deleteExpert(t) {
	if(deleteExpertFlag){
		deleteExpertFlag = false;
		var obj = $(t);
		var cUrl = obj.attr('data-url');
		$.getJSON(cUrl, function(ret) {
			if (ret.Result == 'OK') {
				$('#status_' + obj.attr('data-id')).html('<a onclick="addExpert(this)" data-id="'+ret.expertid+'"  data-url="../admin/projectApplyOpt_addProjectApplyExpert.action?id='+ret.expertid+'&paid='+ret.paid+'&evaluateType='+ret.evaluateType+'"  href="javascript:void(0)">添加评审权限</a>');
			}else if (ret.Result == "ERROR" || ret.Result == 'FAIL') {
				alert('失败,' + ret.Message);
			}
			deleteExpertFlag = true;
		})
	}
	return false;
}

var aduRecordFlag = true;
// 单个审核学员
...
var aduRecordallFlag = true;
// 批量审核学员
function aduRecordall(t) {
	if(aduRecordallFlag){
		aduRecordallFlag = false;
		$(t).closest('.popover').hide();
		var obj = $(t);
		var cUrl = obj.attr('data-url');
		var popover=obj.closest('.popover'),reason = popover.find('input[name="allreason"]').val();
		var keyarr = [];
		...
		$.getJSON(cUrl+'&reason='+reason + '&key=' + keyarr.join(","), function(ret) {
			if (ret.Result == 'OK') {
...
function addProjectApplyall(t) {
	...
	if ($('input[name=traininfo]:checked').length < 1) {
		...
		window.location.href=cUrl+ '&id=' + keyarr.join(",");
	}
}

//批量添加评审专家
var addExpertallFlag = true;
function addExpertall(t) {
	if(aduRecordallFlag){
		aduRecordallFlag = false;
		$(t).closest('.popover').hide();
		var obj = $(t);
		var cUrl = obj.attr('data-url');
		var keyarr = [];
		if ($('input[name=expertinfo]:checked').length < 1) {
			alert('至少选择一位评审专家');
			aduRecordallFlag = true;
			return false;
		} else {
			$.each($('input[name=expertinfo]').serializeArray(), function(i, v) {
				keyarr.push(v.value);
			});
			$.getJSON(cUrl+ '&expertid=' + keyarr.join(","),function(ret) {
				if (ret.Result == 'OK') {
					alert('操作成功！');
					window.location.href = "../admin/projectApply_initPage.action";
				} else {
					alert('失败,' + ret.Message);
				}
				aduRecordallFlag = true;
