...
<div class="main">
						...
						<tbody>
							<tr>
								...
								<td width="160px" class="text-center"><span id="status_{{:id}}">
									{{if record_status}}
									<a class="btn-create cboxElement iframeA" href="../admin/ttRecord_addReplaceTeacherInit.action?teacherId={{:teacherId}}&replacedTtrId={{:replacedTtrId}}&projectId={{:projectId}}&subjectId={{:subjectId}}&collegeId={{:tcid}}&gorganizationId={{:gorganizationId}}&porganizationId={{:porganizationId}}" 
									data-fancybox-type="iframe" style="display:none;"></a>
									<a class="a{{:teacherId}}" onclick="addRecords('teacherId={{:teacherId}}&replacedTtrId={{:replacedTtrId}}&projectId={{:projectId}}&subjectId={{:subjectId}}&collegeId={{:tcid}}&gorganizationId={{:gorganizationId}}&porganizationId={{:porganizationId}}',{{:teacherId}})">替换为该学员 </a>
									{{else}}
									不符合报名条件
									{{/if}}
									</span>
	...
	<script>
		...
		function addRecords(t,s) {
			var cUrl="../admin/sizer_signupCheckerSizer.action?"+t;
			$.getJSON(cUrl, function(json) {
		        	...
		        	if(json.Result=="WARNING"){
			        	...
			        	for(i=0;i<json.Records.sizer.multi.length;i++){
			        		...
			        		}else if(json.Records.sizer.multi[i].name=="0"){
		        		...
		        		layer.open({
	        			  ...
	        			  +''+'<a class="btn btn-primary btn-myself" onclick="addRecordNext(\''+t+'\','+s+')">确定</a>'
		...
		function originalAddRecords(obj){
			layer.closeAll();
			$(".a"+obj).parent().find("a.iframeA").click();
		}

		function addRecordNext(t,s){
			var cUrl="../admin/sizer_signupCheckerSizer.action?"+t+"&isNext=1";
			$.getJSON(cUrl, function(json) {
				if(json.Result=="OK"){
	        		originalAddRecords(s);
