...
<body>
	<div class="ifrcnt container">
		<div class="hd">
			<h3>上传培训记录</h3>
			<h3>首先导出培训记录模板，填写学员培训内容后在此处上传，<br/>为了防止上传失败,表格中请不要出现空行。</h3>
		</div>
		<div class="bd">
			...
			<form class="form-horizontal" id="upload" method="post" name="upload" enctype="multipart/form-data" >
				<div class="row clearfix">
					...
					<div class="form-group">
						<label class="align-label control-label" for="">项目类型</label>
						<select class="form-control" name="projectType" id="projectType">
							<option value="0">请选择</option>
							<s:iterator value="searchProjectType" id="ptt">
								<option value='<s:property value="#ptt.getId()" />'><s:property value="#ptt.getName()" /></option>
							</s:iterator>
						</select>
					</div>
					<div class="form-group">
						<label class="align-label control-label" for="">项目名称</label>
						<input type="text" id="projectName" class="form-control" name="projectName"/>
					</div>
					<div class="form-group">
						<label class="align-label control-label" for="">项目简称</label>
						<input type="text" id="shortName" class="form-control" name="shortName"/>
					</div>
					<div class="span5">					
						<div class="control-group" style="width:600px">
							<label class="control-label" for="">上传培训记录</label>
							<div class="controls" >
								<div id="applydoc" class="">&nbsp;上传培训记录&nbsp;&nbsp;</div>
	...
	<script>
		...
		$(function() {
			...
			function getSchedule(){
				...
				str=encodeURI(str);
				$.post('../admin/otherTrainingRecords_getPercent.action?' + str, function(data) {
			...
			$('#upload').submit(function() {
				if(submitFlag){
					...
					$.post('../admin/otherTrainingRecords_upload.action?year=' + year + '&projectType=' + projectType + '&projectName=' + projectName+ '&shortName=' + shortName + '&' + str, function(data) {
						var Result = data.Result;
						var message = data.Message;
						if (Result == "OK") {
