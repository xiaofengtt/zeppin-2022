...
<s:action name="left_init" namespace="/admin" executeResult="true">
	<s:param name="leftName">六五周期学员审核管理</s:param>
</s:action>
		...
		<div class="list-content clearfix">
			...
			<div class="more-filters-container clearfix">
				<div class="more-fliters-btn">
					<a class="apply-filters btn btn-default btn-sm btn-myself"
						onclick="Adufilters();" href="javascript:void(0)">筛选</a> <a
						class="clear-filters btn btn-default btn-sm btn-myself"
			...
			<form id="aduTaskTeacher" class="form-horizontal"
				...
				<input type="hidden" value="2" name="projectCycle" />
...
<!-- 						</div> -->
						...
						<div class="form-group">
							<label class="col-sm-2 control-label" for="">学员所在单位</label>
							<div class="col-sm-9">
								<input class="form-control" name="teacherOrg" data-val="true" data-val-number="" data-name="" id="teacherOrg" type="text" class="span3"  value=''>
							</div>
						</div>
				...
				<div class="row actionbar">
					...
					<div class="text-center" style="padding-bottom:25px;">
						...
						<button class="btn btn-default btn-myself" id="AduTeacherRecord" type="button">批量审核</button>
						</s:if>
					</div>
				</div>
			</form>
			<div class="stateDiv" style="margin-top:0;">
				...
				<a class="" value="2">已审核<span id="span2">(0)</span></a>
				<a class="" value="-2">已通过<span id="span4">(0)</span></a>
						      	...
						      	<div class="popover-content">
									...
									<div style="padding:10px 0 0;border-top:1px solid #e5e5e5">
										...
										<button type="button" onclick="aduRecordall(this)"
											data-url="../admin/ttRecord_aduTeacherReport.action?method=noadu" class="btn btn-primary">不通过</button>
							...
							<table class="table table-bordered">
								<tbody>
									<tr>
										...
										{{if teacherStatus==1}}<font color=red>（被替换学员）</font>{{else teacherStatus==2}}<font color=green>（替换学员）</font>{{else teacherStatus==3}}<font color=red>（替换中）</font>{{else}}{{/if}}
										</td>
										...
										{{if status=='已撤回'}}
										<td width="110px" align="center"><a class="btn-group" href="javascript:void(0)" onclick="audlist(this)"
										...
										<td width="110px" align="center"><a class="btn-group" href="javascript:void(0)" onclick="audlist(this)"
											...
											<div class="btn-group" style="">
												<button type="button"
													style="font-size:13px;background:none;border:0;color:#4365be;"
													class="btn btn-xs btn-default dropdown-toggle"
											...
											<a class="btn-group" href="javascript:void(0)" onclick="replace(this)" data-id="{{:id}}" data-url="../admin/ttRecord_replaceTeacherInit?ttrId={{:id}}">替换学员</a>&nbsp;&nbsp;&nbsp;
											 {{if add}}
											 <a style="vertical-align:middle;" href="javascript:void(0)" onclick="delRecord(this)" data-id="{{:id}}"  data-url="../admin/ttRecord_aduTeacherReport.action?method=del&key={{:id}}-{{:level}}">撤回</a>
											 {{/if}}
										</td>					
										{{/if}}
...
// 			window.location.href = "../admin/ttRecord_searchAduPage.action?status=" + status; 
	...
	var aduRecord2Flag = true;
	function aduRecord2(t) {
		if(aduRecord2Flag){
			aduRecord2Flag = false;
			var obj = $(t),cUrl = obj.attr('data-url'),popover=obj.closest('.popover'),
			reason = encodeURI(popover.find('input[name="reason"]').val().replace(/(^\s*)|(\s*$)/g, ""));
			if(reason!=""){
				...
				//$(obj).closest('.btn-group').find('.popover').hide();
				aduRecord2Flag = true;
				alert("原因不能为空")
			}
		}
	...
	$(function() {
		...
		var aduTeacherRecordFlag = true;
		$('#AduTeacherRecord').click(function(e) {//批量审核
			e.preventDefault();
			if(aduTeacherRecordFlag){
				aduTeacherRecordFlag = false;
...
// 				$('#adu_'+obj.attr('data-id')).remove();
	...
	function getNumber(){
		...
		var enrollType = url();
		$('#list-content').html('').addClass('loading');
		$.getJSON('../admin/ttRecord_getAduPageStatusCount.action?' + str,
				function(r) {
