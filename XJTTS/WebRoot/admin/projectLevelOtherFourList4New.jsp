...
<s:action name="left_init" namespace="/admin" executeResult="true">
	<s:param name="leftName">六五周期查看培训学员</s:param>
</s:action>
...
<!-- 			</a> -->
		...
		<div class="list-content clearfix">
			...
			<form id="aduTaskTeacher" class="form-horizontal"
				style="display:none" action="../admin/ttResult_outputOScore.action">
									...
									<tr>
										...
										<td width="110px" align="center"><a href="javascript:void(0)" onclick="audlist(this)"
											...
											<div class="btn-group" style="margin-top:-2px;">
											 ...
											 {{if add}}
											 <a href="javascript:void(0)" onclick="delRecord(this)" data-id="{{:id}}"  data-url="../admin/ttResult_aduTeacherReport.action?method=del&key={{:id}}-{{:level}}">删除</a>
											 {{/if}}
										</td>								
...
// 					window.location.href = "../admin/ttRecord_searchAduPage.action?status=" + status; 
	...
	function ttrecordAdu(o, pp) {
		...
		$.getJSON('../admin/ttResult_getOAduPageStatusCount.action?' + strs,
				function(r) {
		...
		var enrollType = url();
		$('#list-content').html('').addClass('loading');
		$.getJSON('../admin/ttResult_searchOAduPage.action?' + str,
				function(data) {
					if (data.status == 'OK') {
