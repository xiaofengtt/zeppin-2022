								...
								<tbody>
									<tr>
										...
										{{if status==1 && timeup_status==1}}
											<a href="../admin/ttRecord_initReportPage.action?projectId={{:projectId}}&subjectId={{:subjectId}}&collegeId={{:tcid}}&gorganizationId={{:gorganizationId}}&&porganizationId={{:porganizationId}}" data-status={{:status}} data-timestatus="{{:timeup_status}}" onclick="setPage()">添加报送学员</a>&nbsp;&nbsp;&nbsp;
										{{else}}
											<a href="javascript:void(0)" onclick="showMessage({{:status}},{{:timeup_status}})" onclick="setPage()">添加报送学员</a>&nbsp;&nbsp;&nbsp;
										{{/if}}
										{{if enrollType==2}}
											{{if projectCycle == 1}}
												<a  href="../admin/ttRecord_initReviewPage.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{else}}
												<a  href="../admin/ttRecord_initReview65Page.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{/if}}
										{{else}}
											{{if projectCycle == 1}}
												<a  href="../admin/ttRecord_initAduPage.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{else}}
												<a  href="../admin/ttRecord_initAdu65Page.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{/if}}
