...
<div class="main">
	<div class="listwrap">
		<div class="list_bar">
			...
			<a href="../admin/hoursType2.jsp">学时认定设置 （特殊学科）</a>&nbsp;&nbsp;&nbsp;
			学时认定设置 （学科分年份）&nbsp;&nbsp;&nbsp;
		...
		<div class="iconDiv">
			<a class="btn btn-create btnMyself btn-screen"
				href="../admin/identifyClasshours_initAddYearPage"
		...
		<div id="list-content" class="list-content clearfix"
			...
			<table class="table table-hover">
				<tr>
					<th width="17%" style="text-align: left;">项目类型一级</th>
					<th width="15%" style="text-align: left;">项目类型二级</th>
					<th width="10%" style="text-align: left;">学科</th>
					<th width="6%" style="text-align: left;">年份</th>
					...
					<th width="7%" align="center">培训总学时</th>
					<th width="7%" align="center">培训总学分</th>
					<th width="12%">操作</th>
				</tr>
...
<script type="text/javascript">
	...
	function getTable() {
		...
		$
				.getJSON(
						'../admin/identifyClasshours_getSpecialYearList.action?page='+page,
						function(data) {
							if (data.status == "OK") {
								for (var i = 0; i < data.records.length; i++) {
									...
									urlstr += '<td class="td">'
											...
											+ '</td><td><div class="td9 td6_0">'
											+ year
											+ '</div><div class="td9_0 td5_0"><select class="form-control" name="start">'
											+ yearSelect
											+ '</select></div>'
						...
						function() {
							$(".modify").click(
									function() {
										var this_ = $(this);
										var thisYear = this_.parent().parent().find('div.td9').html();
										$.getJSON('../admin/projectCycle_getYearList.action',function(data){
											if(data.Result == 'OK'){
												var yearStr = '<select class="form-control">';
												for(var i=0;i< data.Options.length;i++){
													if(data.Options[i].DisplayText == thisYear){
														var options = '<option value="'+data.Options[i].Value+'" selected="selected">'+data.Options[i].DisplayText+'</option>'
													}else{
												...
												yearStr += '</select>';
												this_.parent().parent().find('div.td9_0').html(yearStr);
	...
	function modify(obj) {
		...
		var rs7 = tirm3($(obj).parent().parent().find('div.td9_0 option:selected').val());
		if(!rs7){
			layer.confirm('请选择起始年份', {
				btn : [ '确定' ]
			}, function() {
				layer.closeAll();
			});
		...
		if (rs1 && rs2 && rs3 && rs4 && rs5 && rs6) {
			...
			var credit = $(obj).parent().parent().find("div.td8_0 input")
					.val().replace(/(^\s*)|(\s*$)/g, "");
			var year = $(obj).parent().parent().find("div.td9_0 option:selected").val();
			var centralizeCHs;
			var informationCHs;
			var regionalCHs;
			var schoolCHs;
			...
			$.getJSON('../admin/identifyClasshours_specialYearAdd.action?method=edit&id=' + id
					+ '&projectTypeId=' + projectTypeId + '&centralizeCH='
					+ centralizeCH + '&informationCH=' + informationCH
					+ '&regionalCH=' + regionalCH + '&schoolCH=' + schoolCH + '&totalhours=' + totalhours + '&credit=' + credit + '&startyear=' + year,
					function(data) {
						if (data.status == "OK") {
							...
							$(obj).parent().parent().find("div.td6").html(
									creditCHs);
							$(obj).parent().parent().find("div.td9").html(
									year);
	...
	function dele(obj){
		...
		//按钮
			...
			$.getJSON('../admin/identifyClasshours_deleteYear.action?id=' + obj,
					function(data) {
						if (data.status == "OK") {
