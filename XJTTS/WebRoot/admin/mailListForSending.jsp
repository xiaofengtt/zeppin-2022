...
<s:action name="left_init" namespace="/admin" executeResult="true">
	<s:param name="leftName">发送的消息</s:param>
</s:action>
...
<div class="main">
	...
	<div class="list_bar" style="height:58px;">发送的消息&nbsp;&nbsp;&nbsp;<a href="../admin/mailListForInbox.jsp">收到的消息</a></div>
	<div class="iconDiv">
		<a class="btn btnMyself"
...
function getTable(){
	...
	$.ajax({
        type: "GET",
        url: "../admin/newmail_getOutBoxList.action",
        ...
        success: function (json) {
        	...
        	if(data.Result=='OK'){
        		...
        		var type;
        		if(data.Role>2){
        			window.location.href="mailListForInbox.jsp";
        		}
        		...
        		for(var i=0;i<data.Records.length;i++){
        			...
        			if(data.Records[i].attachment.length==0){
	        			...
	        			'</p><p class="applyType" name="'+data.Records[i].id+'">'+data.Records[i].title+
	        			'</p><div class="applyContent clamp">'+data.Records[i].content+
	        			'</div><p class="applyInscription text-right">'+data.Records[i].inscription+
	        			'</p><div class="button"><div><div><div><div><a class="replied" name='+data.Records[i].id+'>查看回复</a>'+''+
	        			'<a class="answer">回复</a><a class="delete" onclick="doDelete(this,'+data.Records[i].id+',2)">删除</a>'+''+
	        			...
	        			'<div class="operation"><a class="sendBtn" onclick="answer(this,'+data.Records[i].id+',\'answerInputs\')">发送</a>'+''+
	        			...
	        			'<div class="operation"><a class="sendBtn" onclick="answer(this,'+data.Records[i].id+',\'answerInput\')">发送</a>'+''+
	        			...
	        			'</p><div class="button"><div><div><div><div><a class="replied" name='+data.Records[i].id+'>查看回复</a>'+''+
	        			'<a class="answer">回复</a>'+''+
	        			'<a class="attachment">查看附件</a><a class="delete" onclick="doDelete(this,'+data.Records[i].id+',2)">删除</a>'+''+
	        			'<a class="showAll">显示全部</a></div></div></div></div></div><div class="dataBox">'+data.Records[i].createtime+'</div>'+''+
	        			'<div class="clear"></div><div class="alreadyReply"><img class="artboard" src="../img/artboard_03.png" />'+''+
	        			'<div class="repliedDiv"><div class="repliedItems"><div class="answerDiv" style="border:none"><input class="answerInputs" value=""/>'+''+
	        			'<div class="operation"><a class="sendBtn" onclick="answer(this,'+data.Records[i].id+',\'answerInputs\')">发送</a>'+''+
		...
		$(".applyType").click(function(){
			window.location.href="mailListForInformate.jsp?id="+$(this).attr("name")+"&page="+page+"&type=2";
		})
	});	
}
//回复列表
