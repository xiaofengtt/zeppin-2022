				...
				<div class="clearfix">
					<div class="span10">
						...
						<div class="control-group">
							...
							<div class="controls">
								<select class="span3" name="project" onchange="changeProject(this)">
									<option value="0">全部</option>
									<s:iterator value="lsProject" id="ya">
										<option <s:if test="%{projectApply.project.id==#ya.getId()}">selected</s:if>  value="<s:property value="#ya.getId()" />" search="<s:property value="#ya.getName()" />" ><s:property
...
<!-- 						<div class="control-group" id="trainingClasshour" <s:if test="%{projectApply.project.traintype==2}"> style="display:none"</s:if>> -->
...
<!-- 							</div> -->
<!-- 						</div> -->
<!-- 						<div class="control-group" id="trainingOnlineHour" <s:if test="%{projectApply.project.traintype==1}"> style="display:none"</s:if>> -->
...
<!-- 								<div class="control-group" id="centralize" -->
<!-- 									<s:if test="%{value<=0}"> style="display:none"</s:if>> -->
<%-- 									<label class="control-label" for=""><s:property value="nameCN"/></label> --%>
<!-- 									<div class="controls"> -->
<%-- 										<input type="text" id="input_<s:property value="name"/>" name="<s:property value="name"/>" class="readonly" readonly="readonly" --%>
...
<%-- 						</s:if> --%>
				...
				<div class="row actionbar">
					<div class="offset7">
						<button class="btn btn-primary" type="submit" onclick="difaction('../admin/projectApplyOpt_passProjectApply.action')">审核通过</button>
						<button class="btn btn-default" type="submit" onclick="difaction('../admin/projectApplyOpt_passNoProjectApply.action')">审核不通过</button>
	...
	function changeProject(t) {
		var vid = $(t).val();
// 		$.getJSON(
// 			'../admin/projectApplyOpt_getProjectTraintype.action?id='
// 					+ vid, function(data) {
// 				if (data.traintype == 1) {
...
// 			});
	...
	function difaction(action) {
		$('#addProjectApply').attr('action',action);
