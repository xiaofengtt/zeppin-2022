										...
										.jtable(
												{
													...
													fields : {
														...
														customedit : {
															...
															display : function(
																	data) {
																var html = '<select onchange="checkin(this)" class="form-control" name="status" id="status"><option value=-1>请选择</option><option value=2>已报到</option><option value=1>未报到</option>'
// 																+'<option value=0>异动</option>'
							...
							$('#batchcheckin').click(function(){//批量审核
								...
								if ($selectedRows.length > 0) {
									...
									$.post('../admin/trainingStudentOpt_changeStudentStatus.action',{ids:id,status:2}, function(data){
										if(data.Result == 'OK') {
							...
							$('#batchclass li a').click(function(){//批量分班
								...
								if ($selectedRows.length > 0) {
									var id = $('input[name="checkinids"]').val();
									$.post('../admin/trainingStudentOpt_editClasses.action',{ids:id,classes:classes}, function(data){
										if(data.Result == 'OK') {
											location.reload();
										}
									})
