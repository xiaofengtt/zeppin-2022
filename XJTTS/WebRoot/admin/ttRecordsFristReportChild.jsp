...
<s:action name="left_init" namespace="/admin" executeResult="true">
	<s:param name="leftName">报送管理</s:param>
</s:action>
								...
								<tbody>
									<tr>
										...
										{{if status==1 && timeup_status==1}}
											<a href="../admin/ttResult_initReportPage.action?leveltype={{:leveltype}}&projectId={{:projectId}}&subjectId={{:subjectId}}&collegeId={{:tcid}}&gorganizationId={{:gorganizationId}}&&porganizationId={{:porganizationId}}" data-status={{:status}} data-timestatus="{{:timeup_status}}" onclick="setPage()">添加报送学员</a>&nbsp;&nbsp;&nbsp;
										{{else}}
											<a href="javascript:void(0)" onclick="showMessage({{:status}},{{:timeup_status}})" onclick="setPage()">添加报送学员</a>&nbsp;&nbsp;&nbsp;
										{{/if}}
											{{if projectCycle == 1}}
												<a  href="../admin/ttResult_initPage.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{else}}
												<a  href="../admin/ttResult_init65Page.action?key={{:key}}" onclick="setPage()">查看已报送学员</a>
											{{/if}}
										</td>
									</tr>
								</tbody>
