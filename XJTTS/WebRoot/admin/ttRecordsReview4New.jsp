...
<s:action name="left_init" namespace="/admin" executeResult="true">
	<s:param name="leftName">六五周期自主报名学员审核管理</s:param>
</s:action>
...
<div class="iconDiv">
	...
	<div class="listwrap">
		<div class="list_bar">审核自主报名学员</div>
...
<!-- 						</div> -->
			...
			<div class="stateDiv" style="margin-top:0px;">
				<label>状态&nbsp;：</label>
				...
				<a class="" value="3">已撤回<span id="span5">(0)</span></a>
			</div>
					...
					<div id="adu_{{:id}}" class="list-item">
						<div class="list-item-hd">
							<table class="table table-bordered">
								<tbody>
									<tr>
										<td width="200px" class="text-primary"><input class="listcheck" onclick="toCheck(this)" type="checkbox" name="traininfo" value="{{:id}}-{{:level}}"> &nbsp;<span>{{:schoolName}}—{{:name}}</span></td>
										...
										{{if status=='已撤回'}}
										<td width="110px" class="text-center"><a href="javascript:void(0)" onclick="audlist(this)"
											...
											<div class="btn-group" style="margin-top:-3px;">
												...
												<ul class="dropdown-menu" role="menu" style="min-width:100px;">
													<li><a href="javascript:void(0)" onclick="aduRecord(this)" data-id="{{:id}}"
															data-url="../admin/ttRecord_reviewTeacherReport.action?method=adu&key={{:id}}-{{:level}}">通过
														</a>
													</li>
												...
												data-url="../admin/ttRecord_reviewTeacherReport.action?method=noadu&key={{:id}}-{{:level}}" class="btn btn-primary">不通过</button>
											 ...
											 {{if add}}
											 <a style="vertical-align:middle;" href="javascript:void(0)" onclick="delRecord(this)" data-id="{{:id}}"  data-url="../admin/ttRecord_reviewTeacherReport.action?method=del&key={{:id}}-{{:level}}">撤回</a>
											 {{/if}}
...
<script>
	...
	function ttrecordAdu(o, pp) {
		...
		$.getJSON('../admin/ttRecord_searchEnrollReviewPage.action?' + str+'&status='+$('.stateDiv a.light').attr("value"),
				function(data) {
					if (data.status == 'OK') {
	...
	function getNumber(){
		...
		$('#list-content').html('').addClass('loading');
		$.getJSON('../admin/ttRecord_searchEnrollReviewStatusCount.action?' + str,
				function(r) {
