...
public class PaperAction extends ActionSupport {
	...
	private List<Psq> summarizeList;
	...
	/**
	 * 问卷列表 分为4类 1.专家评分 2.教评学 3.学评教 4.总结评估
	 * 
	 * @return
	 */
	public String list() {
...
//		this.searchReportTask = this.iProjectService.getProjectByParams(map,null,lityps);
		...
		case 1:
			this.reviewList = this.getiPsqService().getPsqByTypePage(stype, map, offset, length);
			return "listexpert";
		case 2:
			this.teachList = this.getiPsqService().getPsqByTypePage(stype, map, offset, length);
			return "listteacher";
		case 3:
			this.learnList = this.getiPsqService().getPsqByTypePage(stype, map, offset, length);
			return "list";
		case 4:
			this.summarizeList = this.getiPsqService().getPsqByTypePage(stype, map, offset, length);
	...
	/**
	 * 新建问卷 分为3类 1.专家评分 2.教评学 3.学评教 4.总结评估
	 * 
	 * @return
	 */
	public String add() {
	...
	public String view() {
		...
		if (this.uid.equals("fail")) {
			...
			if (submit != null) {
				// 已经提交过问卷
				if (papertype != null && (!papertype.equals(DictionyMap.PSQ_TYPE_TRAINING) || (!papertype.equals(DictionyMap.PSQ_TYPE_SUMMARIZE)))) {
					this.paperView = "edit";
	...
	public String copy() {
		...
		int ipaperId = Integer.parseInt(paperId);
		Psq paper = iPsqService.get(ipaperId);
		Psq npaper = this.iPsqService.copy(paper);
		// 判断问卷是否有人提交文件
		// 如果已经有人提交问卷也不可以修改

		this.paperID = npaper.getId();
		this.psqtype = npaper.getType();
	...
	public void process() {
		...
		try {
			...
			if (type.equals(DictionyMap.PSQ_TYPE_TEACHER)) {
				...
				int ttrid_id = Integer.valueOf(ttrid);
				TeacherTrainingRecords ttr = this.getiTeacherTrainingRecords().get(ttrid_id);
				if(ttr == null){
					response.getWriter().write("1000〒培训记录不存在，请确认后重新操作！");
					return;
				}
//				TeacherTrainingRecords ttr = this.getiTeacherTrainingRecords().getTeacherTrainingRecord(projectId, subjectId, trainingCollege, valuator);
				...
				TrainingAdmin rater = this.iTrainingAdminService.get(us.getId());
				if(rater!=null){
					ttr.setRater(rater);
				}
				...
				ProjectApplyExpert pae = this.iProjectApplyExpertService.getProjectApplyExpertByParams(projectId_id,subjectId_id,trainingCollege_id,us.getId());
				pae.setStatus((short)1);
				this.iProjectApplyExpertService.update(pae);
			} else if (type.equals(DictionyMap.PSQ_TYPE_SUMMARIZE)){
				ProjectApplyEvaluate pae = this.iProjectApplyEvaluateService.getProjectApplyEvaluateByParams(projectId_id,subjectId_id,trainingCollege_id,us.getId());
				pae.setStatus((short)1);
				this.iProjectApplyEvaluateService.update(pae);
			}
		...
		if (status.equals(DictionyMap.PSQ_DEL)) {
			...
			if (status.equals(DictionyMap.PSQ_RELEASE)) {
				...
				if (type.equals(DictionyMap.PSQ_TYPE_TRAINING)) {
					...
					if (paper.getProjectsForProjectSummarizePsq().size() == 0) {
						flag = false;
					}
				}

				if (flag) {
					if (type.equals(DictionyMap.PSQ_TYPE_TEACHER)) {//教评学问卷必须设置总分才可以发布
						
						if(paper.getTotalScore() == null || (paper.getTotalScore() != null && paper.getTotalScore() == 0)){
							sb.append("\"status\":false");
							sb.append(",");
							sb.append("\"message\":\"" + "修改失败，未设置总分！" + "\"");
							sb.append("}");
	...
	public void addTotalScore() {
		...
		String paperId = request.getParameter("id");
		String score = request.getParameter("score") == null ? "0":request.getParameter("score");
		if("".equals(score)){
			score = "0";
		}
		if("0".equals(score)){
			...
			sb.append("\"status\":false");
			sb.append(",");
			sb.append("\"message\":\"" + "总分不能为0，请填写问卷总分！" + "\"");
			sb.append("}");
			Utlity.ResponseWrite(sb.toString(), "application/json", response);
			return;
		}
		...
		if(paper != null){
			...
			this.iPsqService.update(paper);
			sb.append("\"status\":true");
			sb.append(",");
			sb.append("\"message\":\"" + "设置成功！" + "\"");
		}else{
			sb.append("\"status\":false");
			sb.append(",");
			sb.append("\"message\":\"" + "问卷不存在！" + "\"");
		}
	...
	public void getPsqProjects() {
		...
		List<Project> lips = this.getiProjectService().getProjectByTypes(type,us.getOrganization());
		...
		try {
			...
			if (status.equals(DictionyMap.PSQ_PROJECT_ADD)) {
				...
				if (type.equals(DictionyMap.PSQ_TYPE_TRAINING)) {
					...
					p.setPsqByProjectSummarizePsq(pq);
				}
				...
				if (pq.getStatus().toString().equals(DictionyMap.PSQ_RELEASE)) {
					...
					if (count > 0) {
						...
						if (type.equals(DictionyMap.PSQ_TYPE_TRAINING)) {
							...
							p.setPsqByProjectSummarizePsq(null);
	...
	public void getPageJson() {
		...
		int count = this.getiPsqService().getPsqByTypeCount(stype,map);
	...
	public List<Psq> getSummarizeList() {
		return summarizeList;
	}

	public void setSummarizeList(List<Psq> summarizeList) {
