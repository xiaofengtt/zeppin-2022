...
public class SubmitDaoImpl extends BaseDaoImpl<Submit, Integer> implements ISubmitDao {
	...
	public List getExpertSubmitByParams(HashMap<String, String> mapParams) {
		StringBuilder sb = new StringBuilder();
		sb.append("FROM Submit s , Result r WHERE r.submit = s.id and s.psq=").append(mapParams.get("paperid"));
		sb.append(" and r.question=").append(mapParams.get("questionid"));
		if (mapParams.get("pid") != null && !mapParams.get("pid").equals("0")) {
			sb.append(" and s.project=").append(mapParams.get("pid"));
		}
		if (mapParams.get("stid") != null && !mapParams.get("stid").equals("0")) {
		...
		sb.append(" ORDER BY s.project,s.trainingSubject,s.trainingCollege");
