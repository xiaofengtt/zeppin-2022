...
//		sb.append("select t from Teacher t, Organization o where 1=1 ");
//		sb.append(" and t.organization=o.id");
		sb.append("select t from Teacher t where 1=1 ");
		
		String report = paramsMap.get("report") == null ? "" : paramsMap.get("report").toString();
		if(!"".equals(report)){
		...
		if (org != null) {
			sb.append(" and t.organization.scode like '").append(org.getScode()).append("%' ");
		}
...
//		sb.append(" and tg.isprime=1");
		sb.append(" select count(*) from Teacher t where 1=1 ");
...
//		}
    ...
    public List<Teacher> getTeacherListByParams(HashMap<String,String> map){
    	StringBuilder sb = new StringBuilder();
    	sb.append("from Teacher where 1=1");
    	if(map.get("name")!=null && !map.get("name").equals("")){
    		sb.append(" and name like '%").append(map.get("name")).append("%'");
    	}
    	if(map.get("idcard")!=null && !map.get("idcard").equals("")){
    		sb.append(" and idcard = '").append(map.get("idcard")).append("'");
    	}
    	//9月12新加：通过id查询Teacher表记录
	 ...
	 * @return
	 */
	public List<Object[]> getListForPageBySql(Integer projectId,
			Short sbujectId, Integer trainingUnitId, int classes, int start,
			int limit, String sort) {
		// TODO Auto-generated method stub
		StringBuilder sb = new StringBuilder();
		sb.append("select t.NAME,t.IDCARD,t.HEAD_PIC_PATH from teacher_training_records r left join teacher t on r.teacher=t.id where r.final_status=2");

		if (projectId > 0) {
			sb.append(" and r.project=" + projectId);
		}
		if (sbujectId > 0) {
			sb.append(" and r.subject=" + sbujectId);
		}
		if (classes > 0) {
			sb.append(" and r.classes=" + classes);
		}
		if (trainingUnitId > 0) {
			sb.append(" and r.training_college=");
			sb.append(trainingUnitId);
		}

		sb.append(" order by " + sort);

		return this.getListPage(sb.toString(), start, limit, null);
