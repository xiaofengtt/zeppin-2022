...
public class PsqDaoImpl extends BaseDaoImpl<Psq, Integer> implements IPsqDao {
	...
	public List<Psq> getPsqByTypePage(short type, Map<String, Object> map, int offset, int length) {
		String hql = "select distinct t from Psq t left join t.";
		if(type == 1){
			hql += "projectsForProjectJudgePsq";
		}else if(type == 2){
			hql += "projectsForEvaluationTeacherPsq";
		}else{
			hql += "projectsForEvaluationTrainingPsq";
		}
		hql = hql + " as p,ProjectAdmin pa where t.creator=pa.id and t.type=" + type;
		if(map.get("year")!=null && !map.get("year").equals("")){
			hql = hql + " and p.year=" + map.get("year");
		}
		if(map.get("projectId")!=null && !map.get("projectId").equals("")){
			hql = hql + " and p.id=" + map.get("projectId");
		}
		if(map.get("organization")!=null && !map.get("organization").equals("")){
			hql = hql + " and pa.organization=" + map.get("organization");
		}
	...
	public int getPsqByTypeCount(short type, Map<String,Object> map) {
		String hql = "select count(distinct t.id) from Psq t left join Project p on t.id=p.";
		if(type == 1){
			hql += "PROJECT_JUDGE_PSQ";
		}else if(type == 2){
			hql += "EVALUATION_TEACHER_PSQ";
		}else{
			hql += "EVALUATION_TRAINING_PSQ";
		}
		hql +=" left join project_admin as pa on t.creator=pa.id ";
		hql = hql + " where t.type=" + type;
		if(map.get("year")!=null && !map.get("year").equals("")){
			hql = hql + " and p.year=" + map.get("year");
		}
		if(map.get("projectId")!=null && !map.get("projectId").equals("")){
		...
		Object result = this.getObjectBySql(hql, null);
	...
	public HashMap<String, String[]> getPsqExpertPaper(HashMap<String, String> map) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT r.question,r.answer,COUNT(*) AS SUM ");
		sb.append("FROM submit s ,result r ");
		sb.append("WHERE r.submit = s.id ");
	...
	public List getExpertPsqManyiLv(HashMap<String, String> map, String tableName) {
		...
		try {
			...
			// 跟新 planNumber
			if (map.containsKey("type")&&map.get("type").equals("4")) {
				sb.append("UPDATE ").append(tableName).append(" av ").append("SET planNumber=").append("(").append(" SELECT COUNT(*) FROM project_apply_evaluate pae, project_apply pa  WHERE pa.proejct=av.id AND pa.subject=av.tsid AND av.tcid=pa.training_college AND pa.id=pae.project_apply");
			}else{
				sb.append("UPDATE ").append(tableName).append(" av ").append("SET planNumber=").append("(").append(" SELECT COUNT(*) FROM project_apply_expert pae, project_apply pa  WHERE pa.proejct=av.id AND pa.subject=av.tsid AND av.tcid=pa.training_college AND pa.id=pae.project_apply");
			}
			if (map.containsKey("pid")) {
				if (map.get("pid") != null) {
					if (map.get("pid").indexOf(",") > 0) {
	 ...
	 * @return
	 */
	public List searchPsqByParams(HashMap<String, String> map){
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT pe.name ,SUM(r.score) AS score ");
		sb.append("FROM submit s,result r,project p,training_subject ts,training_college tc,question q , project_expert pe ");
		sb.append("WHERE s.id = r.submit AND s.project = p.id AND s.subject = ts.id AND s.training_college = tc.id AND pe.id = s.creater AND r.question=q.id AND q.is_count=1 AND q.type=2 ");
		...
		sb.append(" GROUP BY p.name,ts.name,tc.name , s.id order by score desc");
