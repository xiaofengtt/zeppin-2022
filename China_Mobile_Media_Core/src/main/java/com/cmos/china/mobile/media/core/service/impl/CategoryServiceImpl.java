...
public class CategoryServiceImpl extends BaseServiceImpl implements ICategoryService {
	...
	public void add(InputObject inputObject, OutputObject outputObject) throws Exception {
		...
		if(!status.equals(GerneralStatusType.NORMAL) && !status.equals(GerneralStatusType.STOPPED)){
		...
		if(level == 1){
			Map<String, String> paramMap = new HashMap<String, String>();
			paramMap.put("level", level+"");
			int count = this.getBaseDao().getTotalCount("category_getCountByParams", paramMap);
			count++;
		...
		if(parent==null || parent.equals("")){
			...
			if(category==null){
				...
				while (category.getParent() != null && !"".equals(category.getParent())) {
					category = this.getBaseDao().queryForObject("category_get", parent, Category.class);
					categorylist.add(category);
				}
