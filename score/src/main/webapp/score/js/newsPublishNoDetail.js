	...
	$.ajax({
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			...
    			if(r.data.status=="uncheck"||r.data.status=="nopass"){
    				$(".submit-btn").show();
    			}else{
    				$(".submit-btn").hide();
    			}
...
function changeTeam(obj){
	...
	if($(obj).hasClass("light")){
		...
		$("#team a").unbind("click").click(function(){
			...
			if($(this).parent().parent().find("span").hasClass("light")){
				$(this).parent().parent().prev("span").addClass("light");
				return false;
			}else{
				$(this).parent().parent().prev("span").removeClass("light");
			}
		});
	}
