...
function getList(){
	$.ajax({
        url: '../back/userWithdraw/checkList',
        type: 'get',
        async:false,
        data: {
        	"status":status,
    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				var status = "";//状态
    				var role = "";
    				if(r.data[i].type=="apply"){
    					type = "申请";
    				}else if(r.data[i].type=="confirm"){
    					type = "确认";
    				}else if(r.data[i].type=="rollback"){
    					type = "退回";
    				}else if(r.data[i].type=="delete"){
    					type = "删除";
    				}
    				...
    				+'<td>'+moneyFormat(r.data[i].value.pay) +'</td>'
    				+'<td>'+(r.data[i].creatorName == null ? "" : r.data[i].creatorName) +'</td>'
    				+'<td>'+formatDate(r.data[i].submittime) +'</td>'
    				+'<td>'+status+'</td>'
    				if(r.data[i].status == "normal"){
    					tableHtml = tableHtml +'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userWithdrawCheckDetail.html?type=1&uuid='+r.data[i].uuid+'">审核</a></td>'
    				}else{
    					tableHtml = tableHtml +'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userWithdrawCheckDetail.html?type=0&uuid='+r.data[i].uuid+'">查看</a></td>'
    				}
...
function getStatusNumber(){
	$.ajax({
        url: '../back/userWithdraw/checkStatusList',
        type: 'get',
        async:false,
        data: {
        }
