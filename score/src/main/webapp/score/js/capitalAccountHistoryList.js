        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
								...
								'<th width="15%">交易渠道</th>'+
								'<th width="10%">交易账户</th>'+
								'<th class="text-right" width="10%">交易金额</th>'+
								'<th class="text-right" width="8%">手续费</th>'+
								'<th class="text-center" width="10%">经办人</th>'+
								'<th width="7%">状态</th>'+
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				if(r.data[i].status == "normal" || r.data[i].status == "close" || r.data[i].status == "failed"){
    					tableHtml = tableHtml +'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userRechargeConfirm.html?uuid='+r.data[i].uuid+'">确认到账</a></td>'
    				}else{
