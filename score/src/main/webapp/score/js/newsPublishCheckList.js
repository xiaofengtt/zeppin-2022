...
function getList(){
	$.ajax({
        url: '../back/newsPublish/checkList',
        type: 'get',
        async:false,
        data: {
        	"status":status,
    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				if(r.data[i].status=="uncheck"){
    					...
    					hrefUrl = 'newsPublishCheckDetail.html?uuid='+r.data[i].uuid;
    				}else if(r.data[i].status=="publish"){
    					status = "已发布";
    					opteration = '<a class="removeBtn mr-5">下架</a><a class="previewBtn">预览</a>';
    					hrefUrl = 'newsPublishCheckNoDetail.html?uuid='+r.data[i].uuid;
    				}else if(r.data[i].status=="nopass"){
    					status = "未通过";
    					opteration = '<a class="deleteBtn mr-5">删除</a><a class="previewBtn">预览</a>';
    					hrefUrl = 'newsPublishCheckDetail.html?uuid='+r.data[i].uuid;
    				}
    			...
    			$(".removeBtn").click(function(){
    				removeList($(this).parent().attr("data-id"));
    				return false;
    			});
...
$(".screen-box ul.screen-ul li").click(function(){
	...
	if(status=="publish"||status=="delete"){//已发布状态和已删除
		$(".screen-operate a").hide();
	}else if(status == "nopass"){//未通过
		$(".screen-operate a").hide();
		$(".screen-operate a.screen-delete").show();
	}else{
...
function checkList(status){
	...
	if(checkUuid==""){
		...
		checkUuid = checkUuid.substring(0,checkUuid.length-1);
	}					
...
function removeList(listid){
	...
	if(listid){
		...
		if(removeUuid==""){
			...
			removeUuid = removeUuid.substring(0,removeUuid.length-1);
		}			
	}		
...
function getStatusNumber(){
	$.ajax({
        url: '../back/newsPublish/checkStatusList',
        type: 'get',
        async:false,
        data: {
