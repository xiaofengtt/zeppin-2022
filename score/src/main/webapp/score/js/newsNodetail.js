	...
	$.ajax({
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.imageurlList.length;i++){
    				imageUrl+='<img src="'+r.data.imageurlList[i]+'" />'
    			}
    			$("#revenueFeature").html(r.data.content);
    			$(".imagesBox").html(imageUrl)
    			categoryArr = r.data.categoryList;
    			teamArr = r.data.teamList;
    			if(r.data.status=="publish"||r.data.status=="disable"||r.data.status=="delete"){//已发布状态
    				$(".submit-btn,.disable-btn").hide();
    			}
    		} else {
    			if(r.errorCode=="302"){
    				layer.msg(r.message, {
...
//    			treeCategorys(r.data);
    			...
    			$(".firstUl span.light").each(function(){
    				categoryResult += '<div data-id="'+$(this).parent().attr("data-id")+'"><span>'+$(this).html()+'</span></div>';
    			});
...
$(".preview-btn").click(function(){
	...
	$(".layerContent").append($('#revenueFeature').html());//内容
	$(".layerDiv").show();
});
//获取球队列表
