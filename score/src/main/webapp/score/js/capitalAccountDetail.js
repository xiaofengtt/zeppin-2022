	...
	$.ajax({
        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			...
    			$("#dailyMax").val(moneyFormat(r.data.dailyMax));
    			...
    			if(type == "company_bankcard"){
    				$("#privateKeyDiv").hide();
    			}else if (type == "company_alipay"){
    				$("#accountNameDiv").hide();
    				$("#bankNameDiv").hide();
    				$("#branchBankDiv").hide();
    			}else if (type == "personal_bankcard"){
    				$("#privateKeyDiv").hide();
					$("#branchBankDiv").hide();
    			}else{
    				$("#privateKeyDiv").hide();
		  ...
		  btn: ['确定','取消'] //按钮
			...
			$.ajax({
		        ...
		        data: {
					...
					"dailyMax":$("#dailyMax").val().replace(/[^\d.]/g,""),
