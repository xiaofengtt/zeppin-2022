    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				if(r.data[i].status == "nopass"){
    					tableHtml = tableHtml +'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userWithdrawOperateDetail.html?type=1&uuid='+r.data[i].uuid+'">重新提交</a></td>'
    				}else{
    					tableHtml = tableHtml +'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userWithdrawOperateDetail.html?type=0&uuid='+r.data[i].uuid+'">查看</a></td>'
    				}
...
function getStatusNumber(){
	$.ajax({
        url: '../back/userWithdraw/statusList',
        type: 'get',
        async:false,
        data: {
        }
