	...
	$.ajax({
		...
		if (r.status == "SUCCESS") {
			...
			for(var i=0;i<r.data.length;i++){
				...
				tableHtml += '<tr>'
    				+'<td><b>'+r.data[i].description+'</b></td>'
    				+'<td>'+value+'</td>'
    				+'<td>'+partitional+'</td>'
    				+'<td>'+type+'</td>'
    				+'<td data-id="'+r.data[i].uuid+'" class="text-right"><a class="editBtn" data-key="'
    				+r.data[i].paramKey+'" data-type="'+r.data[i].type+'" data-value="'+encodeURI(valueUuid)+'">修改</a>'
    				+'</tr>';
			}
            $('#pageTool').Paging({
                prevTpl: "<",
