    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				if(r.data[i].status=="uncheck"){
    					status = "未审核";
    					hrefUrl = 'newsPublishDetail.html?uuid='+r.data[i].uuid;
    					...
    					hrefUrl = 'newsPublishNoDetail.html?uuid='+r.data[i].uuid;
    				}else if(r.data[i].status=="nopass"){
    					status = "未通过";
    					opteration = '<a class="previewBtn">预览</a> <a class="resubmit">提交审核</a>';
    					hrefUrl = 'newsPublishDetail.html?uuid='+r.data[i].uuid;
    				}
