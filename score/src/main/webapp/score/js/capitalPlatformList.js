        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				var status = "";//状态
    				var type = "";//类型
    				if(r.data[i].transType=="recharge"){
    					transType = "充值渠道";
    				}else if(r.data[i].transType=="withdraw"){
    					transType = "提现渠道";
    				}
    				...
    				if(r.data[i].type=="company_bankcard"){
    					status = "企业银行卡";
    				}else if(r.data[i].type=="company_alipay"){
    					status = "企业支付宝";
    				}else if(r.data[i].type=="personal_bankcard"){
    					status = "个人银行卡";
    				}else if(r.data[i].type=="personal_alipay"){
    					status = "个人支付宝";
    				}else if(r.data[i].type=="wechat"){
    					status = "微信";
    				}else if(r.data[i].type=="reapal"){
    					status = "融宝支付";
    				}
    				...
    				'<a href="capitalPlatformDetail.html?uuid='+r.data[i].uuid+'">修改</a> <a class="disableBtn ml-5">禁用</a></td></tr>'; 
    			}
