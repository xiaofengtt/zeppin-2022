        ...
        data: {
    		...
    		if (r.status == "SUCCESS") {
    			var tableHtml = '<tr>'+
								'<th width="20%">手机号</th>'+
								'<th width="10%">用户名</th>'+
								'<th width="20%" class="text-right">可提现余额</th>'+
								'<th width="20%" class="text-right">不可提现余额</th>'+
    			...
    			for(var i=0;i<r.data.length;i++){
    				...
    				tableHtml += '<tr>'
    				+'<td><b>'+handlePhoneNumber(r.data[i].mobile)+'</b></td>'
    				+'<td>'+(r.data[i].realname == null ? "" : r.data[i].realname) +'</td>'
    				+'<td class="text-right">'+moneyFormat(r.data[i].balanceFree)+'元</td>'
    				+'<td class="text-right">'+moneyFormat(r.data[i].balanceLock)+'元</td>'
    				+'<td class="text-center">'+status+'</td>'
    				+'<td class="text-right" data-id="'+r.data[i].uuid+'"><a class="" href="userAccountRecharge.html?uuid='+r.data[i].uuid+'">充值</a></td>'
    				+'</tr>';
    			}
                $('#pageTool').Paging({
                    prevTpl: "<",
