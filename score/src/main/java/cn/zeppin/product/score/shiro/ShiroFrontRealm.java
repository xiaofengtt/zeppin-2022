...
public class ShiroFrontRealm extends AuthorizingRealm {
    ...
    public static ShiroFrontRealm getInstance() {
    	DefaultSecurityManager securityManager = (DefaultSecurityManager) SecurityUtils.getSecurityManager();
    	Collection<Realm> realms = securityManager.getRealms();
    	Collection<Realm> typeRealms = new ArrayList<>();
        for (Realm realm : realms) {
            if (realm.getName().contains(LoginType.FRONT.toString()))
                typeRealms.add(realm);
        }
        ShiroFrontRealm realm = (ShiroFrontRealm) typeRealms.iterator().next();
		return realm;
    }
    
    //获取用户加密密码
    public static String encrypt(Object source, ByteSource salt) {
    	HashedCredentialsMatcher credentialsMatcher = (HashedCredentialsMatcher) ShiroFrontRealm.getInstance().getCredentialsMatcher();
    ...
    public void clearUserAuthByUserId(List<Integer> userIds){
        ...
        ShiroFrontRealm realm = (ShiroFrontRealm)securityManager.getRealms().iterator().next();
