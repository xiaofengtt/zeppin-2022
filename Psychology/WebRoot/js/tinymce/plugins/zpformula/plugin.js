		...
		var charMapPanel = {
			...
			onclick: function(e) {
				...
				if(target.tagName == 'A') {
					...
					$('#previewImg').attr('src','../CETV_TEST/latex.action?'+encodeURIComponent($('#editorBlockValue').val())).show();
		...
		f.bind(f.get("editorBlockValue"), "focus", function() {
            f.bind(f.get("editorBlockValue"), "keyup", function() {
                clearTimeout(d), d = setTimeout(function() {
					...
					$('#previewImg').attr('src','/CETV_TEST/latex.action?'+encodeURIComponent(vl)).show();
