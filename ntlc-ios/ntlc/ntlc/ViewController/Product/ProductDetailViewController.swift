...
class ProductDetailViewController: UIViewController, UIScrollViewDelegate{
    ...
    func createHeaderView(){
        headerView = UIView(frame: CGRect(x: paddingLeft * screenScale, y: 10 * screenScale, width: screenWidth - paddingLeft * 2 * screenScale, height: 205 * screenScale))
        ...
        topView.frame.size = CGSize(width: topView.frame.width, height: codeLabel.frame.origin.y + codeLabel.frame.height + 18 * screenScale)
        imageView.frame.origin = CGPoint(x: imageView.frame.origin.x, y: (topView.frame.height - imageView.frame.height)/2)
        let bottomLine = CALayer()
        bottomLine.frame = CGRect(x: 0, y: topView.frame.height - 1, width: topView.frame.width, height: 1)
        bottomLine.backgroundColor = UIColor.backgroundGray().cgColor
        topView.layer.addSublayer(bottomLine)
        headerView.addSubview(topView)
        
        let bodyView = UIView(frame: CGRect(x: 0, y: topView.frame.origin.y + topView.frame.height, width: headerView.frame.width, height: 130 * screenScale))
        ...
        minLabel.font = UIFont.numFont(size: UIFont.smallSize() * screenScale)
        ...
        headerView.frame.size = CGSize(width: headerView.frame.width, height: bodyView.frame.origin.y + bodyView.frame.height)
    ...
    func createTermView(){
        ...
        collectStartValue.font = UIFont.numFont(size: UIFont.smallSize() * screenScale)
        ...
        let collectEndValue = UILabel(frame: CGRect(x: 0, y: collectStartValue.frame.origin.y, width: collectEndView.frame.width, height: collectStartValue.frame.height))
        ...
        let valueDataValue = UILabel(frame: CGRect(x: 0, y: collectStartValue.frame.origin.y, width: valueDataView.frame.width, height: collectStartValue.frame.height))
        ...
        let maturityDateValue = UILabel(frame: CGRect(x: 0, y: collectStartValue.frame.origin.y, width: maturityDateView.frame.width, height: collectStartValue.frame.height))
