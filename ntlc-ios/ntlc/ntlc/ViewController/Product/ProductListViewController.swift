...
class ProductListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    var headerView: BannerScrollView = BannerScrollView()
    ...
    override func viewDidLoad() {
        ...
        let rightItem = UIBarButtonItem(image: rightImage.imageWithScale(18/rightImage.size.height), style: UIBarButtonItemStyle.plain, target: self, action: #selector(toProductSearch(_:)))
    ...
    override func viewDidAppear(_ animated: Bool) {
        headerView.bannerScrollView.setContentOffset(CGPoint.zero, animated: false)
    }
    
    func createHeaderView(){
        if(isIphoneX()){
            let imageUrl = "/resource/product_head_bg_x.png"
            ...
            headerView = BannerScrollView(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: screenWidth, height: 264 * screenScale)), datas: datas)
    ...
    func createSortView(){
        sortView = UIView(frame: CGRect(x: paddingLeft * screenScale, y: headerView.frame.origin.y + headerView.frame.height - 30 * screenScale, width: screenWidth - paddingLeft * 2 * screenScale, height: 60 * screenScale))
        sortView.backgroundColor = UIColor.white
        sortView.layer.cornerRadius = cornerRadius * screenScale
        sortView.addBaseShadow()
    ...
    func createTableView(){
        tableView = UITableView(frame: CGRect(x: 0, y: sortView.frame.origin.y + sortView.frame.height + 2, width: screenWidth, height: screenHeight - self.tabBarController!.tabBar.frame.height - (sortView.frame.origin.y + sortView.frame.height + 2)))
    ...
    func createFilterView(){
        filterView = UIView(frame: CGRect(x: sortView.frame.origin.x, y: sortView.frame.origin.y, width: sortView.frame.width, height: screenHeight - self.tabBarController!.tabBar.frame.height - sortView.frame.origin.y))
        ...
        let topLine = CALayer()
        topLine.frame = CGRect(x: 0, y: 0, width: buttonView.frame.width, height: 1)
        topLine.backgroundColor = UIColor.backgroundGray() .cgColor
        buttonView.layer.addSublayer(topLine)
        let middleLine = CALayer()
        middleLine.frame = CGRect(x: buttonView.frame.width/2, y: 0, width: 1, height: buttonView.frame.height)
    ...
    @objc func filterCancle(_ sender: UIButton){
        let filterButton = self.navigationItem.leftBarButtonItem!.customView!.viewWithTag(TagController.productListTags.leftButton) as! UIButton
    ...
    @objc func getList(){
        ...
        let params = NSDictionary(dictionary: ["sorts": sort, "term": term, "custodian": custodian, "device": EncodingUtil.getBase64(systemType)])
    ...
    func getBankList(){
        HttpController.get("product/bankList", params: NSDictionary(dictionary: ["device": EncodingUtil.getBase64(systemType)]), data: { (data) in
            let dataDictionary = data as! NSDictionary
            let status = dataDictionary.object(forKey: "status") as! String
            if status == "SUCCESS" {
                self.bankList = []
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(dataList.count > 0){
            ...
            if(product.flagBuy && product.stage == "uninvest"){
                ...
                fullLabel.text = "已满"
                fullLabel.backgroundColor = UIColor.lightGray
                fullLabel.textColor = UIColor.white
                fullLabel.font = bottomLabel.font
                fullLabel.textAlignment = NSTextAlignment.center
                fullLabel.sizeToFit()
                fullLabel.frame.size = CGSize(width: fullLabel.frame.width + 10 * screenScale, height: bottomView.frame.height/2)
                fullLabel.frame.origin = CGPoint(x: bottomView.frame.width - fullLabel.frame.width - 5 * screenScale, y: bottomView.frame.height/4)
                fullLabel.layer.cornerRadius = 4 * screenScale
                fullLabel.layer.masksToBounds = true
                bottomView.addSubview(fullLabel)
            }
