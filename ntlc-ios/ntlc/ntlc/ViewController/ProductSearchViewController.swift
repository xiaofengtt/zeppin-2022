...
class ProductSearchViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    func createHeaderView(){
        headerView = UIView(frame: CGRect(x: leftPadding * screenScale, y: 8 * screenScale, width: screenWidth - leftPadding * 2 * screenScale, height: 100 * screenScale))
        ...
        searchInput.text = ""
        searchInput.placeholder = "请输入理财产品代码、名称搜索"
        searchInput.adjustsFontSizeToFitWidth = false
        searchInput.keyboardType = UIKeyboardType.webSearch
    ...
    func createTableView(){
        tableView = UITableView(frame: CGRect(x: 0, y: headerView.frame.origin.y + headerView.frame.height + 7 * screenScale, width: screenWidth, height: mainView.frame.height - (headerView.frame.origin.y + headerView.frame.height + 12 * screenScale)))
    ...
    @objc func getList(){
        ...
        let params = NSDictionary(dictionary: ["sorts": sort, "name": search.text!])
        HttpController.get("product/list", params: params, data: { (data) in
            ...
            if status == "SUCCESS" {
                ...
                self.tableView.frame.size = CGSize(width: screenWidth, height: self.mainView.frame.height - (self.headerView.frame.origin.y + self.headerView.frame.height + 12 * screenScale))
