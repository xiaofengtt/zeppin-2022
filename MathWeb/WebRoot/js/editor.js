...
TextEdit.prototype = {
  ...
  T_plus : function() {
	...
	$(_this.OptionCnt).on('click','.js-plus',function(){
	    ...
	    $('.js-tableoptions tbody').append(template);
		_this.T_sortOptions();
	});
  },
  T_up_down : function() {
  ...
  T_delete : function() {
	...
	$(_this.OptionCnt).on('click','.js-remove',function(){
		if($('.js-tableoptions tbody').find('tr').length == 2) {
			var d = dialog({
			    title: '系统提示',
				width : 320,
			    content: '选项不能少于2个'
  ...
  createMce :  function(){
	  var _this = this;
		tinymce.init({
		    ...
		    plugins: [
		        "autoresize autolink link  lists fullscreen paste advlist table charmap nonbreaking zpformula",

		    ],
		    toolbar: "bold italic underline strikethrough subscript superscript  nonbreaking |  link  charmap | zpformula table  bullist numlist | removeformat | fullscreen",
			menubar : !1,
			statusbar : !1,
...
var QuestionGOpt = {
	...
	G_sortoptions : function(th) {
		...
		m.find('label[name="option"]').each(function(j){
			if(mtype == 1) {
				$(this).prev().attr('name','radio-'+mtype);
			}
	...
	G_createjson : function(type) {
		...
		var material = string2json2(tinyMCE.get('qustionMaterial').getContent());//材料
		var stem = '';
		var datas = '[';
		
		$('.module').each(function(g){//题型循环
			var th = $(this),qtype = th.data('qtype'),mtype = th.data('mtype'),num = parseInt(g)+1;
			...
			$.post('../itemAddJson',{'questionItem':QuestionGOpt.G_createjson('add')},function(r){	
				...
				if(r.Status == 'success') {
					if(e.target.id == 'savenextbutton') {
						...
						location.href = 'http://'+url('hostname')+'/CETV_TEST/admin/paperLoadPaper?id='+_this.Pid;
					}else {
						location.href = 'http://'+url('hostname')+'/CETV_TEST/admin/questionList.html';
					}
				}
			})
		})
	...
	G_editinit: function () {
		...
		$.getJSON('../itemLoadItem?id='+url('?id')+'&split=_', function(r) {
			if(r.Status == 'success') {	
				...
				questionOpt.getgklist('grade','gradeSearchAllGrade');
				questionOpt.getgklist('knowledge','knowledgeSearchAllKnowledge');
				questionOpt.getgklist('textbook','textbookSearchAllTextbook');
...
var questionOpt = {
		        ...
		        if (object.hasOwnProperty(property)) {
		            ...
		            if (typeof object[property] == "object"){
						...
						if(type != 'textbook') //教材json格式不同，没有level 
							divClass = (object['level'] == 1) ? ' data-level="1" class="tSub1"' : ' data-level="'+ object['level'] +'"  class="tSub'+ object['level'] +'"';
						else 
							divClass = (typeof object['level'] == 'undefined') ? ' data-level="1" class="tSub1"' : ' data-level="'+ parseInt(object['level']) + 1 +'" class="tSub'+ (parseInt(object['level']) + 1) +'"';
						
						aClick = ' onclick="questionOpt.setinputid(\'' + type + '\',\'' +object['name'] + '\',\'' + object['id'] + '\',this)"';
						
						if(property == 'name') {
							cHtml += '<div'+ divClass +'><em  '+ emClass + emClick +' class=""></em><a data-level="'+ object['level'] +'" href="javascript:void(0)" '+ aClick +'>'+ object['name'] +'</a></div>';
	...
	depeandbysubject : function(sid,gid) {
		...
		questionOpt.getgklist('textbook','textbookSearchAllTextbook',sid,gid);
		questionOpt.getgklist('grade','gradeSearchAllGrade','',gid);
	},
	...
	queMce : function(id) {
		tinymce.init({
			...
			//theme: "modern",
		    plugins: [
		        "autoresize autolink link gkpatch gkimage lists fullscreen paste advlist table charmap nonbreaking zpformula zpimages",
		    ],
		    toolbar: "bold italic underline strikethrough subscript superscript  nonbreaking | link  gkimage charmap | zpformula  table  bullist numlist | removeformat | fullscreen",
			menubar : !1,
			statusbar : !1,
			toolbar_items_size: "normal",
			strikethrough: {inline: 'del'},
	...
	createjson : function(type) {//简单题型
		...
		if (questionOpt.Mtype == 1) {//单选题
			...
			var rckscore = (rck.length > 0) ? rck.closest('tr').find('input[name="score"]').val() : '';
			var content = '';
			var rckjson = '"results":[{"mce":"1","inx":"'+ optionid +'","score":"'+ rckscore +'","content":"'+ content +'"}]';
	...
	optionshtmledit: function(data) {
		...
		if(questionOpt.Mtype == 1) {//单选题
			...
			$.each(re.data.options,function(i,v) {
				var tt = $('.options-edit').eq(i).closest('tr');
				if(rinx == (parseInt(i)+1)) {
					tt.find('input[name="score"]').val(rscore);
					tt.find('input[name="radio"]').prop('checked','true');
				}
			});
		}else if(questionOpt.Mtype == 3){//判断题
	...
	Addinit : function() {
		if(this.Pid == null) {//单独录入
			...
			questionOpt.getSubject();
			questionOpt.getgklist('grade','gradeSearchAllGrade','',obj.gid);
			questionOpt.getgklist('knowledge','knowledgeSearchAllKnowledge');
			questionOpt.getgklist('textbook','textbookSearchAllTextbook');
		}else {//试卷形式录入试题
			questionOpt.getgklist('knowledge','knowledgeSearchAllKnowledge');
	...
	addsetting : function() {//保存
		$('#saveSetting').click(function(){
			$.post('../paperAdd',{paper:paperQuestion.settingjson()},function(r){
				if(r.Status == 'success') {
					location.href = 'http://'+url('hostname')+'/CETV_TEST/admin/paperLoadPaper?id='+r.Records.id;
				}else {
					alert('服务端出错，稍后请重试');
				}
			}).fail(function(){
