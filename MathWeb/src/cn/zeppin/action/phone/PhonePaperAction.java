...
public class PhonePaperAction extends BaseAction {
	...
	private int paperId;
	private int usertId;
	private short clientType = Dictionary.CLIENT_TYPE_A;;
	...
	public String LoadPaper() {
		...
		if (user != null) {
			...
			searchMap.put("type", Dictionary.PAPER_TYPE_ZT);
			...
			if (lstPapers != null && lstPapers.size() > 0) {
				...
				map.put("paper.type", Dictionary.PAPER_TYPE_ZT);
	...
	public String GroupPaper() {
		...
		if (user != null) {
			...
			if(knowledgeId != 0){
				sgrade = this.getKnowledgeService().getById(knowledgeId).getGrade();
			}
...
//			Grade sgrade = user.getGrade();
			...
			searchMap.put("subject.id", subjectId);
			searchMap.put("status", Dictionary.ITEM_STATUS_AUDITING + "," + Dictionary.ITEM_STATUS_RELEASE);
			...
			if (itemCount > 0) {
				...
				if (itemCount - rnd < 5) {
					rnd = rnd - (5 - (itemCount - rnd));
				}
				if (rnd < 0) {
					rnd = 0;
				}

				List<Item> itemList = this.getItemService().searchItem(searchMap, "createtime asc", rnd, 5);
				int totalScore = 0;
				...
				paper.setStatus(Dictionary.PAPER_STATUS_RELEASE);
	...
	public String RandomPaper() {
		...
		if (user != null) {
			...
			searchMap.put("subject.id", subjectId);
			searchMap.put("grade.scode", user.getGrade().getScode());
			searchMap.put("type", Dictionary.PAPER_TYPE_ZJ);
			searchMap.put("status", Dictionary.PAPER_STATUS_RELEASE);
...
//					searchMap.put("item.id", Integer.valueOf(itemMap.get("item.id").toString()));
	...
	public String ParesePaper() {
		...
		if (user != null) {
			...
			utce.setSubjectId(Dictionary.SUBJECT_ID + "");
			...
			float lowertime = (float) (totalCount - durationCount) / totalCount;
			...
			for (UserTestItem uti : lstUtis) {
				...
				if (!utce.getMapUti().containsKey(itemId)) {
					...
					if (uti.getIsFlag() && uti.getScore() == uti.getItem().getDefaultScore()) {
					...
					if (uti.getIsFlag() && tscore == uti.getItem().getDefaultScore()) {
				...
				if (uti.getItem().getParent() != null) {
					if (uti.getItem().getParent().getKnowledge() != null) {
						...
						knowledgeName = uti.getItem().getParent().getKnowledge().getName();
					} else if (uti.getItem().getKnowledge() != null) {
						knowledgeId = uti.getItem().getKnowledge().getId();
						knowledgeName = uti.getItem().getKnowledge().getName();
					}
				} else {
				...
				if (knowledgeId > 0) {
					if (!utce.getLstUtk().containsKey(knowledgeId)) {
						...
						utkex.setKnowledgeName(knowledgeName);
	...
	public void LoadScore() {
		...
		if (user != null) {
			...
			map.put("paper.subject.id", subjectId);
			...
			if (tlstut != null && tlstut.size() > 0) {
				for (int i = tlstut.size() - 1; i >= 0; i--) {
					UserTest ut = (UserTest) tlstut.get(i);
			...
			result.init(FAIL_STATUS, "登录失效", null);
		}
	 ...
	 */
	public void LoadApi() throws HttpException, IOException {

		HttpClient httpClient = new HttpClient();
		httpClient.getHostConfiguration().setHost("119.90.15.157", 80, "http");
		GetMethod getMethod = new GetMethod("/api/v1/courses_list?name=chen");

		httpClient.executeMethod(getMethod);
		String retVaule = getMethod.getResponseBodyAsString();

		String strRet = "{\"result\":" + retVaule + "}";
