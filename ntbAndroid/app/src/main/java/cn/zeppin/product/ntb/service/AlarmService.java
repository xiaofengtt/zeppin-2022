...
//                Log.d("AlarmService", "间隔：" + (System.currentTimeMillis() - entryTime));
                if (userGesture == null) {
                    try {
                        userGesture = (UserGesture) RealmOperationHelper.getInstance(AppApplication.REALM_INSTANCE).queryByFieldFirst(UserGesture.class, "phone", AppApplication.getInstance().getUser().getPhone());
