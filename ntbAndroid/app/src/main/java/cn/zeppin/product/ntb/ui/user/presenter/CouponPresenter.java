        ...
        mRxManage.add(Api.getTime().subscribe(new RxSubscriber<ResultData2<String>>(AppApplication.getAppContext(), false) {
            ...
            protected void _onNext(ResultData2<String> resultData2) {
                ...
                if (resultData2.success()) {
                    mRxManage.add(mModel.getCouponList(uuid, status, EncryptUtil.getBase64(price), ApiConstants.getToken(uuid, resultData2.getData())).subscribe(new RxSubscriber<ResultData<Coupon>>(mContext, false) {
                        @Override
