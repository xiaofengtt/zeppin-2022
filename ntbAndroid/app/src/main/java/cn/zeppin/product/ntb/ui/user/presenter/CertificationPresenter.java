        ...
        mRxManage.add(Api.getTime().subscribe(new RxSubscriber<ResultData2<String>>(AppApplication.getAppContext(), false) {
            ...
            protected void _onNext(ResultData2<String> resultData2) {
                ...
                if (resultData2.success()) {
                    mRxManage.add(mModel.uploadImage(file, ApiConstants.getToken(AppApplication.getInstance().getUuid(), resultData2.getData()))
    ...
    public void certificationImg(final String uuid, final String imgface, final String imgback) {
        ...
        mRxManage.add(Api.getTime().subscribe(new RxSubscriber<ResultData2<String>>(AppApplication.getAppContext(), false) {
            ...
            protected void _onNext(ResultData2<String> resultData2) {
                //获取服务器系统时间
                if (resultData2.success()) {
                    mRxManage.add(mModel.certificationImg(uuid, imgface, imgback, ApiConstants.getToken(uuid, resultData2.getData()))
                            .subscribe(new RxSubscriber<ResultData2>(mContext, false) {
                                @Override
                                protected void _onNext(ResultData2 resultData2) {
                                    //证件照认证成功
